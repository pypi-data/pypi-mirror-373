...
from ..asyncio import Round, Photometer, Sample, Summary
...
async def get_all_sessions() -> List[datetime]:
...
async def check_summary(meas_session: datetime) -> None:
    if meas_session is not None:
        await check_summary_single(meas_session)
    else:
        meas_session = await get_all_sessions()
        for ses in meas_session:
            await check_summary_single(ses)


async def check_rounds(meas_session: datetime) -> None:
    if meas_session is not None:
        await check_rounds_single(meas_session)
    else:
        meas_session = await get_all_sessions()
        for ses in meas_session:
            await check_rounds_single(ses)


async def check_samples(meas_session: datetime) -> None:
    if meas_session is not None:
        await check_samples_single(meas_session)
    else:
        meas_session = await get_all_sessions()
        for ses in meas_session:
            await check_samples_single(ses)


async def check_all(meas_session: datetime) -> None:
    if meas_session is not None:
        await check_all_single(meas_session)
    else:
        meas_session = await get_all_sessions()
        for ses in meas_session:
            await check_all_single(ses)


async def check_summary_single(meas_session: datetime) -> None:
    async with Session() as session:
        async with session.begin():
            q = (
...
async def check_rounds_single(meas_session: datetime) -> None:
    async with Session() as session:
        async with session.begin():
            q = (
...
async def check_samples_single(meas_session: datetime) -> None:
    async with Session() as session:
        async with session.begin():
            q = (
...
async def check_all_single(meas_session: datetime) -> None:
    await check_summary_single(meas_session)
    await check_rounds_single(meas_session)
    await check_samples_single(meas_session)
...
def add_args(parser):
    ...
    subparser.add_parser("summary", parents=[prs.session()], help="Browse summary data")
    subparser.add_parser("rounds", parents=[prs.session()], help="Browse rounds data")
    subparser.add_parser("samples", parents=[prs.session()], help="Browse samples data")
    subparser.add_parser("all", parents=[prs.session()], help="Browse all data")

    ...
    # kj continuous 
        ...
        if experiment_name.startswith('xi'):
            ...
            event_time_raw = rng.beta(
                ...
                size=max_stage)
            # # Scale to (0, max_stage-1] so that the max(event_times) is exactly max_stage - 1
            # # Why? i want to make sure it's taking up all the space and we still have the range[max_stage-1, max_stage] for very late participants
            # event_times = event_time_raw * ((max_stage-1) / event_time_raw.max())

            # scale to (0, max_stage-1] because we want to make sure there are people who are at the final stage
            event_times = event_time_raw * (max_stage-1)
        ...
        # scale to (0, max_stage) such that max(disease_stages_raw) = max_stage
        # disease_stages = disease_stages_raw * (max_stage / disease_stages_raw.max())

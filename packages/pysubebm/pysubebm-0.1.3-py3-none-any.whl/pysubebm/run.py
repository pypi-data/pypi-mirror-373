...
def run_subebm(
    ...
    if not post_burn_in_likelihoods:
    ...
    if true_order_matrix is not None:
        ...
        for i in range(n):
            for j in range(n):
                dist[i,j]= utils.normalized_kendalls_tau_distance(best_order_matrix[i], true_order_matrix[j])
    ...
    if true_subtype_assignments is not None and mapping is not None:
        remapped_ml_subtypes = [mapping[x] for x in ml_subtypes]

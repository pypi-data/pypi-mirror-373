...
import numpy as np
import argparse
from tpu_mlir.python.tools.deploy_qat import FakeQuantNodelProcessor
from transform.BaseConverter import BaseConverter
from tpu_mlir.python.utils.mlir_shell import *
from tpu_mlir.python.utils.mlir_parser import *
from tpu_mlir.python.utils.misc import *
from tpu_mlir.python.utils.auto_remove import file_mark, file_clean
from tpu_mlir.python.utils.preprocess import get_preprocess_parser, preprocess, supported_yuv_type
from tpu_mlir.python.utils.log_setting import setup_logger
import pymlir
import warnings
from tpu_mlir.python.utils.cache_tool import CommandRecorder
...
class ModelTransformer(object):
    ...
    def model_validate(self, file_list: str, tolerance, excepts, test_result, yuv_type):
        from tpu_mlir.python.tools.model_runner import mlir_inference, free_mlir_module, show_fake_cmd
        import gc
        inputs = dict()
...
class OnnxTransformer(ModelTransformer):
    ...
    def origin_inference(self, inputs: dict):
        from tpu_mlir.python.tools.model_runner import onnx_inference
...
class CaffeTransformer(ModelTransformer):
    ...
    def origin_inference(self, inputs: dict):
        from tpu_mlir.python.tools.model_runner import caffe_inference
...
class TFLiteTransformer(ModelTransformer):
    ...
    def origin_inference(self, inputs: dict):
        from tpu_mlir.python.tools.model_runner import tflite_inference
        is_nchw = True
        tf_layout = 1
...
class TorchTransformer(ModelTransformer):
    ...
    def origin_inference(self, inputs: dict):
        from tpu_mlir.python.tools.model_runner import torch_inference

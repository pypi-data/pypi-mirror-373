...
class TestTokenCounter:
    ...
    def test_count_tokens_accuracy_vs_character_estimation(self):
        ...
        test_cases = [
            ...
            ("This is a longer sentence with more words and complexity.", 11),  # Longer text
        ...
        for text, expected_min_tokens in test_cases:
            ...
            if len(text) > 15:
                assert actual_tokens != char_estimate, f"Token count {actual_tokens} equals char estimate {char_estimate} for longer text '{text}'"
            ...
            assert 0.3 <= ratio <= max_ratio, f"Token count ratio {ratio} for '{text}' is unreasonable"
    ...
    def test_count_message_tokens_with_tool_calls(self):
        ...
        tokens_without_tools = self.token_counter.count_message_tokens(message_without_tools)
    ...
    def test_count_messages_tokens_accumulation(self):
        ...
        individual_sum = sum(self.token_counter.count_message_tokens(msg) for msg in messages)
    ...
    def test_token_counting_edge_cases(self):
        ...
        for text, expected_max in edge_cases:
            tokens = self.token_counter.count_tokens(text)
            assert tokens <= expected_max, f"Token count {tokens} for '{text}' exceeds expected maximum {expected_max}"
    ...
    def test_token_counting_unicode_handling(self):
        ...
        for text in unicode_texts:
            ...
            char_count = len(text)
            assert tokens <= char_count * 2, f"Token count {tokens} for '{text}' is unreasonably high"

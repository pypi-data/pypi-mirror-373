...
class TestInference:
    ...
    def setup_method(self):
        ...
        with patch('todo_agent.infrastructure.inference.Config') as mock_config_class, \
             patch('todo_agent.infrastructure.inference.LLMClientFactory') as mock_factory, \
             patch('todo_agent.infrastructure.inference.Logger') as mock_logger, \
             patch('todo_agent.infrastructure.inference.ConversationManager') as mock_conversation_manager, \
             patch('todo_agent.infrastructure.inference.ToolCallHandler') as mock_tool_handler:
    ...
    def test_process_request_success(self):
        ...
        self.inference.conversation_manager.get_messages = Mock(return_value=[{"role": "user", "content": user_input}])
        ...
        self.inference.llm_client.extract_content = Mock(return_value="Here are your tasks")
    ...
    def test_system_prompt_datetime_interpolation(self):
        ...
        with patch('builtins.open', mock_open(read_data=mock_prompt_content)):
            ...
            assert re.search(datetime_pattern, result), "System prompt should contain properly formatted datetime"
    ...
    def test_process_request_with_tool_calls(self):
        ...
        # Mock LLM responses
        self.inference.llm_client.chat_with_tools = Mock(side_effect=[
            {"choices": [{"message": {"tool_calls": tool_calls}}]},
            {"choices": [{"message": {"content": "Here are your tasks"}}]}
        ])
        self.inference.llm_client.extract_tool_calls = Mock(side_effect=[tool_calls, []])
        self.inference.llm_client.extract_content = Mock(return_value="Here are your tasks")
    ...
    def test_process_request_exception(self):
        ...
        # Mock exception
        self.inference.conversation_manager.add_message = Mock(side_effect=Exception("Test error"))
    ...
    def test_get_conversation_summary(self):
        ...
        self.inference.conversation_manager.get_conversation_summary = Mock(return_value=mock_summary)

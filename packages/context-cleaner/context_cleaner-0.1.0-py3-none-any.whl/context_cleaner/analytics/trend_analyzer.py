        ...
        return {
            ...
            'trends': {
                'productivity': {
                    ...
                    'change_percent': ((self.productivity_trend.end_value - self.productivity_trend.start_value) / self.productivity_trend.start_value * 100) if self.productivity_trend.start_value > 0 else 0
                },
                'focus_time': {
...
class TrendAnalyzer:
    ...
    def analyze_trends(
        ...
        try:
            ...
            if len(filtered_sessions) < self.minimum_data_points:
                logger.warning(f"Insufficient data for trend analysis: {len(filtered_sessions)} sessions")
            ...
            productivity_trend = self._analyze_productivity_trend(filtered_sessions, productivity_history)
            ...
            health_score_trend = self._analyze_health_score_trend(filtered_sessions, health_history)
            ...
            logger.info(f"Trend analysis completed for {len(filtered_sessions)} sessions")
            return analysis
        ...
        try:
            ...
            for session in sessions:
                ...
                if productivity == 0 and productivity_history:
                    ...
                    matching_metrics = [
                        m for m in productivity_history
                        if abs((m.timestamp - session_date).total_seconds()) < 3600  # Within 1 hour
                    ]
                    if matching_metrics:
                ...
                if productivity > 0:
                    productivity_data.append((
                        self._parse_date(session.get('start_time', '')),
                        productivity
                    ))
    ...
    def _calculate_trend(self, data: List[Tuple[datetime, float]], metric_name: str) -> TrendData:
        ...
        try:
            ...
            ss_res = sum((y - (slope * x + (sum_y - slope * sum_x) / n)) ** 2 for x, y in zip(times, values))
            r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
            
            # Determine trend direction and strength
            direction = self._determine_trend_direction(slope, values)
            strength = min(abs(r_squared * 100), 100.0)  # Convert to percentage, cap at 100
            ...
            return TrendData(
                ...
                metadata={'metric': metric_name, 'values': values[-5:]}  # Last 5 values for context
            )
    ...
    def _detect_daily_productivity_pattern(self, sessions: List[Dict[str, Any]]) -> Optional[Pattern]:
        ...
        try:
            ...
            valid_hours = {h: scores for h, scores in hourly_productivity.items() if len(scores) >= 2}
            
            if len(valid_hours) < 3:
                return None
            
            # Calculate average productivity by hour
            hourly_averages = {h: statistics.mean(scores) for h, scores in valid_hours.items()}
            ...
            if len(all_averages) > 1:
                ...
                strength = min(variance / statistics.mean(all_averages) * 100, 100)  # Coefficient of variation as %
            else:
                strength = 0
            ...
            if sorted_hours[0][1] - sorted_hours[-1][1] > 15:  # Significant difference
                if best_hour < 12:
                    recommendations.append("Schedule important work in the morning hours")
                elif best_hour > 17:
                    recommendations.append("Take advantage of evening productivity peaks")
                else:
            ...
            return Pattern(
                ...
                strength=strength,
                confidence=min(len(valid_hours) * 10, 100),  # More hours = higher confidence
                frequency="daily",
    ...
    def _detect_weekly_activity_pattern(self, sessions: List[Dict[str, Any]]) -> Optional[Pattern]:
        ...
        try:
            ...
            strength = min(variance / statistics.mean(all_averages) * 50, 100) if all_averages else 0
    ...
    def _detect_session_length_pattern(self, sessions: List[Dict[str, Any]]) -> Optional[Pattern]:
        ...
        try:
            ...
            return Pattern(
                ...
                strength=max(short_sessions, medium_sessions, long_sessions) / total_sessions * 100,
                ...
                recommendations=self._get_session_length_recommendations(pattern_type, avg_duration)
            )
    ...
    def _detect_focus_time_pattern(self, sessions: List[Dict[str, Any]]) -> Optional[Pattern]:
        """Detect focus time patterns."""
        try:
            focus_times = [s.get('focus_time_minutes', 0) for s in sessions if s.get('focus_time_minutes', 0) > 0]
            
            if len(focus_times) < 5:
                return None
            
            avg_focus = statistics.mean(focus_times)
            focus_ratio = avg_focus / statistics.mean([s.get('duration_minutes', 1) for s in sessions if s.get('duration_minutes', 0) > 0])
            ...
            return Pattern(
                ...
                recommendations=self._get_focus_recommendations(efficiency, focus_ratio)
            )
    ...
    def _generate_key_insights(
        ...
        if productivity_trend.direction == TrendDirection.IMPROVING and productivity_trend.confidence in [TrendConfidence.HIGH, TrendConfidence.VERY_HIGH]:
            change_pct = ((productivity_trend.end_value - productivity_trend.start_value) / productivity_trend.start_value * 100) if productivity_trend.start_value > 0 else 0
            insights.append(f"Productivity has improved by {change_pct:.1f}% over the past {productivity_trend.time_period_days} days")
        
        elif productivity_trend.direction == TrendDirection.DECLINING and productivity_trend.confidence in [TrendConfidence.HIGH, TrendConfidence.VERY_HIGH]:
            change_pct = abs((productivity_trend.end_value - productivity_trend.start_value) / productivity_trend.start_value * 100) if productivity_trend.start_value > 0 else 0
            insights.append(f"Productivity has declined by {change_pct:.1f}% - consider reviewing work habits")
        
        # Health score insights
        if health_trend.direction == TrendDirection.DECLINING and health_trend.confidence in [TrendConfidence.MEDIUM, TrendConfidence.HIGH, TrendConfidence.VERY_HIGH]:
            insights.append("Context health scores are declining - consider optimizing context management")
        
        # Pattern insights
        daily_patterns = [p for p in patterns if p.type == PatternType.DAILY_CYCLE]
        if daily_patterns and daily_patterns[0].strength > 50:
            peak_time = daily_patterns[0].peak_times[0] if daily_patterns[0].peak_times else "unknown"
            insights.append(f"You have a strong daily productivity pattern with peak performance around {peak_time}")
        
        # Session insights
        if len(sessions) > 0:
            avg_session_length = statistics.mean([s.get('duration_minutes', 0) for s in sessions if s.get('duration_minutes', 0) > 0])
            if avg_session_length > 120:
                insights.append("You tend to work in long sessions - consider taking more breaks to maintain focus")
            elif avg_session_length < 30:
                insights.append("Your sessions are quite short - you might benefit from longer focused work periods")
        ...
        try:
            ...
            if len(durations) > 5:
                ...
                for session in sessions:
                    ...
                    if duration > 0:
                        z_score = abs(duration - duration_mean) / duration_std if duration_std > 0 else 0
            ...
            context_sizes = [s.get('context_size', 0) for s in sessions if s.get('context_size', 0) > 0]
            if len(context_sizes) > 5:
                ...
                for session in sessions:
                    ...
                    if size > 0:
                        z_score = abs(size - size_mean) / size_std if size_std > 0 else 0
    ...
    def _generate_predictions(
        ...
        try:
            ...
            if (productivity_trend.direction != TrendDirection.INSUFFICIENT_DATA and 
                productivity_trend.confidence in [TrendConfidence.MEDIUM, TrendConfidence.HIGH, TrendConfidence.VERY_HIGH]):
                
                # Simple linear extrapolation
                future_productivity = productivity_trend.end_value + (productivity_trend.slope * 7)  # 7 days ahead
                future_productivity = max(0, min(100, future_productivity))  # Clamp to valid range
            ...
            if (health_trend.direction != TrendDirection.INSUFFICIENT_DATA and 
                health_trend.confidence in [TrendConfidence.MEDIUM, TrendConfidence.HIGH, TrendConfidence.VERY_HIGH]):
            ...
            daily_patterns = [p for p in patterns if p.type == PatternType.DAILY_CYCLE and p.strength > 60]
            if daily_patterns:
    ...
    def _assess_data_quality(self, sessions: List[Dict[str, Any]]) -> float:
        ...
        for field in required_fields:
            present_count = sum(1 for s in sessions if s.get(field) is not None and s.get(field) != 0)
    ...
    def _determine_confidence(self, r_squared: float, data_points: int) -> TrendConfidence:
    ...
    def _get_session_length_recommendations(self, pattern_type: str, avg_duration: float) -> List[str]:
    ...
    def _get_focus_recommendations(self, efficiency: str, focus_ratio: float) -> List[str]:
    ...
    def _create_insufficient_data_analysis(self, start_date: datetime, end_date: datetime) -> TrendAnalysis:
        ...
        return TrendAnalysis(
            ...
            patterns=[],
            key_insights=["Insufficient data for trend analysis. Need at least 7 sessions over 7 days."],
            anomalies=[],
            predictions={}
        )

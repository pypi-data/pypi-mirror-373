...
class RecommendationEngine:
    ...
    def __init__(self, config: Optional[ContextCleanerConfig] = None):
        ...
        self.min_confidence_threshold = 70.0  # Only show high-confidence recommendations
    ...
    def generate_recommendations(
        ...
        try:
            ...
            recommendations.extend(self._context_size_recommendations(context_data, health_score))
            recommendations.extend(self._structure_recommendations(context_data, health_score))
            recommendations.extend(self._performance_recommendations(context_data, health_score))
            
            if session_history:
                recommendations.extend(self._pattern_based_recommendations(session_history))
                recommendations.extend(self._timing_recommendations(session_history))
            
            if productivity_metrics:
                recommendations.extend(self._productivity_recommendations(productivity_metrics))
    ...
    def _context_size_recommendations(
        ...
        if context_size > 100000:
            recommendations.append(Recommendation(
                ...
                metadata={'current_size': context_size, 'critical_threshold': 100000}
            ))
    ...
    def _pattern_based_recommendations(self, session_history: List[Dict[str, Any]]) -> List[Recommendation]:
        ...
        try:
            ...
            if durations and statistics.mean(durations) > 120:  # 2+ hour sessions
                recommendations.append(Recommendation(
                    ...
                    metadata={'average_session_minutes': statistics.mean(durations)}
                ))
    ...
    def _timing_recommendations(self, session_history: List[Dict[str, Any]]) -> List[Recommendation]:
    ...
    def _productivity_recommendations(self, metrics: ProductivityMetrics) -> List[Recommendation]:
    ...
    def _filter_and_prioritize(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        """Filter and prioritize recommendations."""
    ...
    def get_recommendation_summary(self, recommendations: List[Recommendation]) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_confidence': statistics.mean(r.confidence for r in recommendations),

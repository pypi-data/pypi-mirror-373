...
class ContextHealthScorer:
    ...
    def __init__(self, config: Optional[ContextCleanerConfig] = None):
        ...
        self.model_weights = {
            HealthScoringModel.BASIC: {
                'size': 0.6, 'structure': 0.2, 'freshness': 0.1, 'complexity': 0.1
            },
            HealthScoringModel.ADVANCED: {
                'size': 0.3, 'structure': 0.3, 'freshness': 0.2, 'complexity': 0.2
            },
            HealthScoringModel.ADAPTIVE: {
                'size': 0.25, 'structure': 0.25, 'freshness': 0.25, 'complexity': 0.25
            },
            HealthScoringModel.PRODUCTIVITY_FOCUSED: {
                'size': 0.4, 'structure': 0.4, 'freshness': 0.15, 'complexity': 0.05
            }
        }
    ...
    def calculate_health_score(
        ...
        try:
            ...
            confidence = self._calculate_confidence(component_scores, context_data, model)
            ...
            factors = self._extract_scoring_factors(context_data, component_scores, weights)
            ...
            logger.debug(f"Health score calculated: {overall_score}/100 (confidence: {confidence:.2f})")
            ...
            return HealthScore(
                ...
                model_used=model.value,
                factors={'error': str(e)},
                recommendations=['Health scoring encountered an error - manual review recommended'],
                timestamp=datetime.now().isoformat()
            )
    
    def _calculate_component_scores(self, context_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _score_context_structure(self, context_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if isinstance(context_data, dict):
                ...
                long_strings = sum(
                    1 for v in context_data.values() 
            ...
            if isinstance(context_data, dict):
                good_patterns = sum(1 for key in context_data.keys() if isinstance(key, str) and any(
                    pattern in key.lower() for pattern in 
                    ['timestamp', 'id', 'type', 'status', 'config', 'data', 'metadata']
                ))
                if good_patterns > 0:
    ...
    def _score_content_freshness(self, context_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            timestamp_keys = [
                ...
                'time', 'datetime', 'date', 'last_update'
            ]
                        ...
                        if any(ts_key in key_lower for ts_key in timestamp_keys):
                            try:
                                if isinstance(value, str):
                                    ...
                                    if parsed_time and (not most_recent_timestamp or parsed_time > most_recent_timestamp):
            ...
            if most_recent_timestamp:
                ...
                age_hours = (datetime.now() - most_recent_timestamp).total_seconds() / 3600
                
                if age_hours < 1:        return 100  # Very fresh (< 1 hour)
                elif age_hours < 4:      return 95   # Fresh (< 4 hours)
                elif age_hours < 12:     return 85   # Recent (< 12 hours)
                elif age_hours < 24:     return 75   # Daily (< 1 day)
                elif age_hours < 72:     return 65   # Recent (< 3 days)
                elif age_hours < 168:    return 55   # Weekly (< 1 week)
                elif age_hours < 720:    return 45   # Monthly (< 1 month)
                else:                    return 30   # Old (> 1 month)
    ...
    def _calculate_nesting_depth(self, obj: Any) -> int:
        ...
        def _depth(o, current_depth=0):
            ...
            if isinstance(o, dict):
                return max([_depth(v, current_depth + 1) for v in o.values()], default=current_depth)
            elif isinstance(o, list):
                return max([_depth(item, current_depth + 1) for item in o], default=current_depth)
            else:
                return current_depth
    ...
    def _adapt_weights(self, base_weights: Dict[str, float], context_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            if len(self.scoring_history) > 10:
                ...
                for component, adjustment in correlation_adjustments.items():
                    if component in adapted_weights:
                        adapted_weights[component] *= (1 + adjustment * 0.1)  # Max 10% adjustment
            ...
            if any(keyword in context_str.lower() for keyword in ['code', 'function', 'class', 'import']):
    ...
    def _calculate_confidence(self, component_scores: Dict[str, int], context_data: Dict[str, Any], model: HealthScoringModel) -> float:
        ...
        try:
            ...
            model_confidence = {
                ...
                HealthScoringModel.ADAPTIVE: 0.9 if len(self.scoring_history) > 5 else 0.6,
            ...
            if len(self.scoring_history) > 3:
                ...
                consistency = 1 - abs(statistics.mean(recent_scores) - current_score) / 100
    ...
    def _generate_health_recommendations(self, overall_score: int, component_scores: Dict[str, int], context_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if overall_score < 50:
                recommendations.append("Context health is poor - consider significant cleanup or restart")
            elif overall_score < 70:
                recommendations.append("Context health is fair - optimization recommended")
            elif overall_score > 90:
                recommendations.append("Excellent context health - maintain current practices")
            
            # Component-specific recommendations
            if component_scores.get('size', 100) < 60:
                recommendations.append("Context size is large - consider removing unnecessary data or splitting into smaller contexts")
            
            if component_scores.get('structure', 100) < 60:
                recommendations.append("Context structure could be improved - organize data more clearly")
            
            if component_scores.get('freshness', 100) < 60:
                recommendations.append("Context contains outdated information - refresh with recent data")
            
            if component_scores.get('complexity', 100) < 60:
                recommendations.append("Context is overly complex - simplify data structures where possible")
            
            # Data-specific recommendations
            context_str = json.dumps(context_data, default=str)
            
            if len(context_str) > 100000:
                recommendations.append("Very large context detected - consider chunking or summarization")
            
            if '"error"' in context_str.lower() or '"exception"' in context_str.lower():
                recommendations.append("Error information detected - resolve issues for better context health")
            
            # Ensure we always have at least one recommendation
            if not recommendations:
                if overall_score > 80:
                    recommendations.append("Context health is good - continue current practices")
                else:
                    recommendations.append("Consider general context cleanup and optimization")
            
            return recommendations[:5]  # Limit to top 5 recommendations
            
        except Exception:
            return ["Unable to generate specific recommendations - manual review suggested"]
    
    def _extract_scoring_factors(self, context_data: Dict[str, Any], component_scores: Dict[str, int], weights: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'top_level_keys': len(context_data) if isinstance(context_data, dict) else 0,
    ...
    def _record_scoring_event(self, result: HealthScore, calculation_time: float, context_data: Dict[str, Any]):
    ...
    def get_scoring_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'model_usage': {
                    model: sum(1 for h in self.scoring_history if h['model_used'] == model)
                    for model in ['basic', 'advanced', 'adaptive', 'productivity']
                },
                'trends': {
                    'recent_average': statistics.mean(scores[-10:]) if len(scores) >= 10 else statistics.mean(scores),
                    'trend_direction': 'improving' if len(scores) > 5 and statistics.mean(scores[-5:]) > statistics.mean(scores[:5]) else 'stable'
                }
            }

...
class SafeContextAnalyzer:
    ...
    def _analyze_context_size(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if elapsed > self.SIZE_ANALYSIS_TIMEOUT:
                logger.warning(f"Size analysis took {elapsed:.3f}s (limit: {self.SIZE_ANALYSIS_TIMEOUT}s)")
    ...
    def _calculate_health_score(self, size_info: Dict[str, Any], session_duration: float = 0) -> Dict[str, Any]:
        """Calculate basic health score."""
        try:
            estimated_tokens = size_info.get('estimated_tokens', 0)
            total_chars = size_info.get('total_chars', 0)
            ...
            if estimated_tokens > 0:
                token_penalty = min(50, estimated_tokens // 1000)  # 1 point per 1000 tokens, max 50
            ...
            return {
                ...
                'token_penalty': min(50, estimated_tokens // 1000) if estimated_tokens > 0 else 0
            }
    ...
    async def analyze_context_safely(self, data: Dict[str, Any]) -> Optional[AnalysisResult]:

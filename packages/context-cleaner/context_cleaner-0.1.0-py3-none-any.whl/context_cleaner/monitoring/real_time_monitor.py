...
class RealTimeMonitor:
    ...
    async def start_monitoring(self):
        ...
        self._broadcast_event('monitor_started', {
    ...
    async def stop_monitoring(self):
        ...
        self._broadcast_event('monitor_stopped', {
            'timestamp': datetime.now().isoformat()
        })
    ...
    async def _monitoring_loop(self):
        ...
        try:
            while self.is_monitoring:
                ...
                if current_time - self.last_update_times['activity'] >= self.activity_update_interval:
                    tasks.append(self._update_activity_status())
                
                if current_time - self.last_update_times['health'] >= self.health_update_interval:
                    tasks.append(self._update_context_health())
                
                if current_time - self.last_update_times['session'] >= self.session_update_interval:
        ...
        try:
            ...
            activity_data = {
                ...
                'current_session': {
                    ...
                    'duration_seconds': (current_time - current_session.start_time.timestamp()) 
                                      if current_session else 0,
                    'project_path': current_session.project_path if current_session else None
                },
                'monitor_status': {
                    'is_monitoring': self.is_monitoring,
                    'uptime_seconds': current_time - self.last_update_times.get('monitor_start', current_time)
                }
            }
    ...
    async def _update_context_health(self):
        ...
        try:
            ...
            if (not previous_health or 
                abs(enhanced_health['health_score'] - previous_health.get('health_score', 0)) >= 5 or
                enhanced_health['health_status'] != previous_health.get('health_status')):
    ...
    async def _update_session_metrics(self):
        ...
        try:
            ...
            summary = self.session_tracker.get_productivity_summary(days=1)  # Last 24 hours
            ...
            if current_session:
                current_metrics = {
                    ...
                    'last_event_time': (
                        max(e.timestamp for e in current_session.context_events).isoformat()
                        if current_session.context_events else None
                    )
                }
    ...
    async def trigger_context_event(
        ...
        try:
            ...
            if success:
                ...
                self._broadcast_event('context_event_triggered', {

...
class SessionObserver:
    ...
    async def _activity_monitoring_loop(self):
        ...
        try:
            while self.is_observing:
                ...
                if (not self.auto_session_started and 
                    time_since_activity < 30):  # Recent activity
                    await self._auto_start_session()
                
                # Check if we should pause/end a session due to inactivity
                elif (self.auto_session_started and 
                      time_since_activity > self.activity_threshold):
        ...
        try:
            if self.monitor and self.auto_session_started:
                ...
                if success:
                    ...
                    logger.info(f"Auto-ended session after {duration:.1f}s of inactivity")
    ...
    def get_observer_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'file_monitoring': {
                'observer_running': self.file_observer is not None and self.file_observer.is_alive() if self.file_observer else False,

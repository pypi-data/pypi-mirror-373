...
from .models import SessionModel, ContextEventModel, EventType, MetricsModel, SessionStatus
...
class SessionTracker:
    ...
    def end_session(
        ...
        try:
            ...
            if success:
                logger.info(f"Ended session: {session.session_id} ({session.duration_seconds:.1f}s)")
    ...
    def track_context_event(
        ...
        try:
            ...
            if event_saved and session_saved:
                ...
                logger.warning(f"Failed to save event or session: event={event_saved}, session={session_saved}")
                return False
    ...
    def _update_session_metrics(self, event: ContextEventModel):
        ...
        if event.duration_ms is not None:
            metrics.hook_execution_time_ms = max(metrics.hook_execution_time_ms, event.duration_ms)
        ...
        metrics.completion_rate = min(1.0, metrics.tools_used / 10.0)  # Rough approximation
    ...
    def get_recent_sessions(self, limit: int = 10, days: int = 30) -> List[SessionModel]:
    ...
    def get_productivity_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            total_optimizations = sum(
                s.metrics.optimizations_applied if s.metrics else 0 
                for s in sessions
            )
            total_tools = sum(
                s.metrics.tools_used if s.metrics else 0
                for s in sessions
            )
            total_errors = sum(
                s.metrics.errors_encountered if s.metrics else 0
                for s in sessions
            )
            ...
            return {
                ...
                'average_session_duration_minutes': round(total_time / len(sessions) / 60, 1),
    ...
    def _generate_recommendations(self, sessions: List[SessionModel]) -> List[str]:
        ...
        avg_productivity = sum(s.calculate_productivity_score() for s in sessions) / len(sessions)
        ...
        if avg_productivity < 60:
            recommendations.append("Consider using context optimization more frequently to improve productivity")
        
        if optimization_rate < 1:
            recommendations.append("Try using 'context-cleaner optimize --dashboard' to monitor context health")
        
        if error_rate > 2:
            recommendations.append("Focus on error reduction - consider shorter context windows and clearer prompts")
        
        if avg_duration < 300:  # Less than 5 minutes
            recommendations.append("Longer sessions tend to be more productive - consider batching related tasks")
        
        if avg_duration > 7200:  # More than 2 hours
            recommendations.append("Consider taking breaks or using context optimization during very long sessions")
        
        # Always include a positive note
        if avg_productivity > 70:
            recommendations.append("Great work! Your productivity is above average - keep using the optimization tools")

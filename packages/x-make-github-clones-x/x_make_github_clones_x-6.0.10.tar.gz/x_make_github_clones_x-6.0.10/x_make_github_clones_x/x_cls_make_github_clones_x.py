...
class x_cls_make_github_clones_x:
    ...
    def _reclone_cleanup(self, dest: str, clone_url: str) -> str:
        ...
        try:
            ...
            # Use onexc if available (newer Python), otherwise fallback to onerror.
            try:
                import inspect
    ...
    def _write_standard_configs(self, name: str, dest: str) -> None:
        ...
        if write_pyproject:
            # Provide a minimal pyproject.toml that includes a placeholder [project]
            # section so new code in the repo can be packaged immediately. Tools
            # configuration (black/ruff/mypy) is included so editors and CI pick
            # up the conventions automatically.
            pyproject_content = (
                f"[project]\n"
                f'name = "{name}"\n'
                f'version = "0.0.0"\n'
        ...
        # Add simple bootstrap scripts so developers can quickly enable the
        # dev tooling in their environment. These scripts are intentionally
        # simple and safe: they install dev requirements (if desired) and
        ...
        try:
            ...
            pass
        # Make the shell script executable where possible
        try:
            ...
            pass
        # Add a short README explaining how to use the template and bootstrap

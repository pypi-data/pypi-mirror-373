...
class x_cls_make_github_clones_x:
    ...
    def fetch_repos(
        self, username: str, token: Optional[str], include_forks: bool
    ) -> List[Dict[str, Any]]:
            ...
            if not os.path.exists(dest):
                ...
                try:
                    result = subprocess.run(
                        ["git", "-C", dest, "pull"], check=False, capture_output=True, text=True
                    )
                ...
                try:
                    ...
                    if pre_exec:
                        ...
                        try:
                            ...
                            subprocess.run(
                                ["pre-commit", "run", "--all-files"], cwd=dest, check=False
                            )
                    else:
                        print(

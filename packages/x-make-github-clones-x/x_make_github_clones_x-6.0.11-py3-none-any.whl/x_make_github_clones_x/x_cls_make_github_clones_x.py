...
class x_cls_make_github_clones_x:
    ...
    def _write_standard_configs(self, name: str, dest: str) -> None:
        """Top-level writer that delegates to smaller helpers to keep complexity low."""
        os.makedirs(os.path.join(dest, ".github", "workflows"), exist_ok=True)
        self._write_precommit_config(name, dest)
        self._maybe_write_pyproject(name, dest)
        self._write_ci_yaml(dest)
        self._write_gitignore_and_requirements(name, dest)
        self._write_bootstrap_scripts(dest)
        self._write_readme(name, dest)

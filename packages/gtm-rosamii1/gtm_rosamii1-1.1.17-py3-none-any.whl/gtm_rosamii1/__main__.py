...
@mcp.tool()
async def continue_email_generation(res: FileResource, row_number: int):
    row_number += 1
    return await generate_email(res, row_number)

@mcp.tool()
async def start_email_generation(res: FileResource):
    return await generate_email(res, 0)

async def generate_email(res: FileResource, row_number: int) -> str:
    email = ""
    try:
        with open(res.name, mode="r", encoding="latin-1") as file:
            ...
            for i, row in enumerate(csv_reader, start=1):
                try:
                    ...
                    errors += 1
                    return f"Row {row_number}: Error processing row: {e}"
                    continue

            return email

    except Exception as e:
        return f"Error in process_emails: {e}"

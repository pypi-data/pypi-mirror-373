...
async def continue_email_generation(res: FileResource, row_number: int):
    row_number += 1
    await generate_email(res, row_number)
...
async def generate_email(res: FileResource, row_number: int):
    ...
    try:
        with open(res.path, mode="r", encoding="latin-1") as file:
            ...
            csv_row = csv_reader[row_number]

            processed = 0
            errors = 0

            for column in enumerate(csv_row, start=0):
                try:
                    if len(column) < 6:  # Check if row has enough columns
                        print(f"Row {row_number}: Skipping - insufficient data")
                        continue

                    result = await gtm_agent.generate_mail(column, row_number)

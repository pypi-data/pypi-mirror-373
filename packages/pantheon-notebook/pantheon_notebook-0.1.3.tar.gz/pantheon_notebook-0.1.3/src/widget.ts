...
Ready for your queries!</span>`;
   ...
   */
  private _createContextPanel(rightPanel: HTMLDivElement): void {
      ...
      if (notebookContext.cells) {
        for (const cell of notebookContext.cells) {
          ...
          if (cell.outputs && cell.outputs.length > 0) {
            const outputText = cell.outputs.map((o: any) => JSON.stringify(o)).join('');
...
⛶ Free space: ${(freeSpace/1000).toFixed(1)}k (${freePercentage}%)`;
    ...
    if (context.cells && context.cells.length > 0) {
      ...
      // Show cell previews
      context.cells.forEach((cell: any, index: number) => {
        ...
        let changeIndicator = '';
        if (changeType === 'new') changeIndicator = '🆕';
        else if (changeType === 'modified') changeIndicator = '✏️';
...
Please analyze the error and the user's guidance, then provide a corrected version of the code that addresses both the error and incorporates the user's suggestions. Respond with only the fixed Python code block.`;
   ...
   */
  private _estimateLineTokens(line: string): number {
    if (!line.trim()) return 1; // Empty lines still count as 1 token
   ...
   */
  private _estimateCodeTokens(text: string): number {
   ...
   */
  private _estimateNaturalLanguageTokens(text: string): number {
   ...
   */
  private _calculateCurrentTokenUsage(query?: string): void {
    const notebook = this._notebookTracker.currentWidget;
    if (!notebook) return;
  ...
  private _contextPanel!: HTMLDivElement;
  private _contextContent!: HTMLDivElement;
  private _contextStats!: HTMLDivElement;
    ...
    try {
      ...
      if (notebookContext) {
        ...
        if (notebookContext.cells && notebookContext.cells.length > 0) {
          ...
          notebookContext.cells.slice(0, 3).forEach((cell: any, i: number) => {
...
• /api-key anthropic sk-ant-xxx --local - Set Anthropic key locally`, 'error');
   ...
   * Fetch current model info on startup
   */
  private async _fetchCurrentModel(): Promise<void> {
    try {
      const response = await this._callModelAPI('GET', { action: 'current' });

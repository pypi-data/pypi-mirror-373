...
Type `/help` for more information."""
        ...
        try:
            ...
            if notebook_context:
                ...
                # Debug: Check if "Aristotle" appears in enhanced query
                if "Aristotle" in enhanced_query:
                    logger.info(f"[DEBUG] ✅ 'Aristotle' found in ENHANCED QUERY sent to agent")
                    # Find position of Aristotle in the query
                    pos = enhanced_query.find("Aristotle")
                    context_around = enhanced_query[max(0, pos-50):pos+100]
                    logger.info(f"[DEBUG] 📍 Context around 'Aristotle': ...{context_around}...")
                else:
                    logger.info(f"[DEBUG] ❌ 'Aristotle' NOT found in ENHANCED QUERY sent to agent")
    ...
    def _format_notebook_context(self, context: Dict[str, Any]) -> str:
        """Format raw notebook context for agent consumption with incremental update support"""
        if not context:
            return ""
        ...
        update_type = context.get('updateType', 'unknown')
        is_first_conversation = context.get('isFirstConversation', False)
        
        # Handle different update types
        if update_type == 'full':
            formatted_parts.append("## 📓 Full Notebook Context (Initial Analysis)")
            ...
            changed_count = context.get('changedCellsCount', 0)
            new_count = context.get('newCellsCount', 0)
            formatted_parts.append("## 🔄 Notebook Context Update (Incremental)")
            formatted_parts.append(f"**Status**: {total_cells} cells total, showing {processed_cells} updated cells")
            formatted_parts.append(f"**Changes**: {changed_count} modified, {new_count} new cells")
        elif update_type == 'none':
            formatted_parts.append("## ✅ Notebook Context (No Changes)")
            formatted_parts.append(f"**Status**: {total_cells} cells total, no changes since last interaction")
            formatted_parts.append("**Note**: Continuing with previous context from agent memory")
        ...
        # Only show cells if there are any to show
        if not context.get('cells'):
            return "\n".join(formatted_parts)
        ...
        for i, cell in enumerate(cells):
            ...
            change_type = cell.get('changeType', None)
            
            # Cell header with change indicators for incremental updates
            status_indicator = "🟢" if is_active else "⚪"
            change_indicator = ""
            if update_type == 'incremental' and change_type:
                if change_type == 'new':
                    change_indicator = "🆕"
                elif change_type == 'modified':
                    change_indicator = "✏️"
            
            formatted_parts.append(f"{status_indicator}{change_indicator} **Cell {cell_idx + 1}** [{cell_type}]:")
        ...
        # Add appropriate explanation based on update type
        if update_type == 'full':
            formatted_parts.append("*Above is the complete notebook content. Use this to understand existing variables, imports, and context when providing assistance.*")
        elif update_type == 'incremental':
            formatted_parts.append("*Above shows only the changed cells since last interaction. Previous context is available in your memory. 🆕 = new cell, ✏️ = modified cell.*")
        else:
        ...
        # Debug: Log formatted context length and first few lines
        logger.info(f"[DEBUG] 📝 FORMATTED CONTEXT LENGTH: {len(formatted_context)} characters")
        lines = formatted_context.split('\n')
        logger.info(f"[DEBUG] 📝 FORMATTED CONTEXT PREVIEW (first 10 lines):")
        for i, line in enumerate(lines[:10]):
            logger.info(f"[DEBUG] Line {i+1}: {line}")
        
        # Debug: Check if "Aristotle" appears in formatted context
        if "Aristotle" in formatted_context:
            logger.info(f"[DEBUG] ✅ 'Aristotle' found in formatted context")
            aristotle_lines = [i for i, line in enumerate(lines) if "Aristotle" in line]
            logger.info(f"[DEBUG] 📍 'Aristotle' appears on lines: {aristotle_lines}")
        else:
            logger.info(f"[DEBUG] ❌ 'Aristotle' NOT found in formatted context")

"use strict";
(self["webpackChunkpantheon_notebook"] = self["webpackChunkpantheon_notebook"] || []).push([["lib_index_js"],{
...
const plugin = {
    ...
    activate: (app, notebookTracker) => {
        ...
        notebookTracker.widgetAdded.connect((sender, notebook) => {
            ...
            const floatingWidget = new _widget__WEBPACK_IMPORTED_MODULE_1__.FloatingInputWidget(app, notebookTracker);
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   FloatingInputWidget: () => (/* binding */ FloatingInputWidget)
/* harmony export */ });
/* harmony import */ var _lumino_widgets__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @lumino/widgets */ "webpack/sharing/consume/default/@lumino/widgets");
/* harmony import */ var _lumino_widgets__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_lumino_widgets__WEBPACK_IMPORTED_MODULE_0__);

// Token calculation using improved estimation algorithms
/**
 * A floating input widget for Pantheon commands
 */
class FloatingInputWidget extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_0__.Widget {
    constructor(app, notebookTracker) {
        super();
        this._isExpanded = false;
        this._isStatusExpanded = false;
        this._history = [];
        this._historyIndex = -1;
        this._commandHistory = [];
        this._parentLayout = null; // Store reference to parent layout
        this._currentQueryHandler = null;
        // Context management for incremental updates
        this._lastContextHash = '';
        this._lastCellHashes = new Map();
        this._conversationCount = 0;
        this._lastSentCells = new Set();
        ...
        // Set initial styling for split layout (increased height and width)
     ...
     */
    _createContent() {
        console.log('Creating Pantheon widget content with split layout...');
        // Create main split container
        const mainContainer = document.createElement('div');
        mainContainer.className = 'pantheon-main-container';
      ...
      box-sizing: border-box;
    `;
        // Left panel - Input/Output (60% width)
        const leftPanel = document.createElement('div');
        leftPanel.className = 'pantheon-left-panel';
    ...
    `;
        // Right panel - Context Display (40% width)
        const rightPanel = document.createElement('div');
        rightPanel.className = 'pantheon-right-panel';
      ...
      height: 100%;
      padding: 8px;
      background: var(--jp-layout-color0, #fafafa);
      box-sizing: border-box;
      overflow-y: auto;
    `;
        // Create the original container structure for left panel
        const container = leftPanel;
        // Create header
    ...
    `;
        ...
        // Create output area (above input) - optimized for split layout
...
Ready for your queries!</span>`;
        ...
        // Create status output area - compressed for split layout
    ...
    `;
        ...
        // Create context display panel for right side
        this._createContextPanel(rightPanel);
        ...
        // Fetch current model info on startup
        this._fetchCurrentModel();
        console.log('Pantheon widget content created with split layout and added to DOM');
    }
    /**
     * Create context display panel for the right side
     */
    _createContextPanel(rightPanel) {
        // Context panel header
        const contextHeader = document.createElement('div');
        contextHeader.className = 'pantheon-context-header';
      ...
      border-radius: 4px 4px 0 0;
    `;
        contextHeader.innerHTML = '📊 Notebook Context';
        // Context statistics
        this._contextStats = document.createElement('div');
        this._contextStats.className = 'pantheon-context-stats';
        this._contextStats.style.cssText = `
      padding: 8px 12px;
      font-size: 11px;
      color: var(--jp-ui-font-color2, #666);
      background: var(--jp-layout-color1, #ffffff);
    ...
    `;
        ...
        this._contextContent.className = 'pantheon-context-content';
        this._contextContent.style.cssText = `
      flex: 1;
      padding: 8px 12px;
      font-size: 11px;
      font-family: var(--jp-code-font-family, monospace);
    ...
    `;
        ...
        rightPanel.appendChild(contextHeader);
        rightPanel.appendChild(this._contextStats);
        rightPanel.appendChild(this._contextContent);
        this._contextPanel = rightPanel;
    }
    /**
     * Update the context display with current notebook state
     */
    _updateContextDisplay() {
        const notebook = this._notebookTracker.currentWidget;
        if (!notebook) {
            this._contextStats.innerHTML = `
        📝 Total Cells: 0
        🎯 Current Cell: None
        💾 Context Usage: No notebook open
      `;
            this._contextContent.innerHTML = 'No notebook currently active.\n\nOpen a notebook to see its context here.';
            return;
        }
        // Get current context info
        const context = this._collectNotebookContext(notebook, 50);
        const totalCells = context.totalCells || 0;
        const currentCell = context.currentCellIndex >= 0 ? context.currentCellIndex + 1 : 'None';
        const hasOutputs = context.hasOutputs ? 'Yes' : 'No';
        const updateType = context.updateType || 'unknown';
        // Build context window visualization showing actual components
        let tokenDisplay = '';
        // Get the current model name - will be set by _fetchCurrentModel() on startup
        const model = this._tokenUsage.model || 'claude-sonnet-4-20250514';
        // Get dynamic context window components from actual calculations
        // notebook is already declared above, reuse it
        let systemPromptTokens = 0;
        let systemToolsTokens = 0;
        let messagesTokens = 0;
        if (notebook) {
            // Calculate actual system prompt tokens - this is the REAL system prompt from server.py
            const realSystemPrompt = `I am Pantheon Notebook Assistant, a specialized AI agent for Jupyter notebook environments with ADVANCED TOOLSET INTEGRATION.
...
8. **SEQUENTIAL DESIGN**: Split complex workflows into logical, sequential blocks that build on each other`;
            systemPromptTokens = this._estimateTokens(realSystemPrompt);
            // System tools tokens (estimated based on typical tool descriptions)
            systemToolsTokens = 8500;
            // Calculate actual messages tokens - this includes:
            // 1. Notebook context (cells as conversation context)
            // 2. Historical conversations (user queries + AI responses)
            // 3. Current conversation context
            // 1. Notebook context tokens
            const notebookContext = this._collectNotebookContext(notebook, 50);
            let notebookContextTokens = 0;
            if (notebookContext.cells) {
                for (const cell of notebookContext.cells) {
                    const cellText = cell.source || '';
                    notebookContextTokens += this._estimateTokens(cellText);
                    // Add output tokens if present
                    if (cell.outputs && cell.outputs.length > 0) {
                        const outputText = cell.outputs.map((o) => JSON.stringify(o)).join('');
                        notebookContextTokens += this._estimateTokens(outputText);
                    }
                }
            }
            // 2. Historical conversation tokens (user inputs from history)
            let conversationHistoryTokens = 0;
            for (const query of this._history) {
                conversationHistoryTokens += this._estimateTokens(query);
            }
            // 3. Current session output tokens (cumulative AI responses)
            const currentSessionOutputTokens = this._tokenUsage.cumulativeOutputTokens || 0;
            // Total messages = notebook context + conversation history + AI outputs
            messagesTokens = notebookContextTokens + conversationHistoryTokens + currentSessionOutputTokens;
        }
        else {
            // Fallback values when no notebook is available - use same system prompt
            const realSystemPrompt = `I am Pantheon Notebook Assistant, a specialized AI agent for Jupyter notebook environments with ADVANCED TOOLSET INTEGRATION.
...
8. **SEQUENTIAL DESIGN**: Split complex workflows into logical, sequential blocks that build on each other`;
            systemPromptTokens = this._estimateTokens(realSystemPrompt);
            systemToolsTokens = 8500;
            // Even without notebook, still count conversation history + AI outputs
            let conversationHistoryTokens = 0;
            for (const query of this._history) {
                conversationHistoryTokens += this._estimateTokens(query);
            }
            const currentSessionOutputTokens = this._tokenUsage.cumulativeOutputTokens || 0;
            messagesTokens = conversationHistoryTokens + currentSessionOutputTokens;
        }
        const currentTokens = systemPromptTokens + systemToolsTokens + messagesTokens;
        const maxTokens = 200000;
        const usagePercentage = Math.round((currentTokens / maxTokens) * 100);
        const freeSpace = maxTokens - currentTokens;
        const freePercentage = Math.round((freeSpace / maxTokens) * 100);
        const systemPromptPercentage = ((systemPromptTokens / maxTokens) * 100).toFixed(1);
        const systemToolsPercentage = ((systemToolsTokens / maxTokens) * 100).toFixed(1);
        const messagesPercentage = ((messagesTokens / maxTokens) * 100).toFixed(1);
        // Create bar visualization - 20 chars total based on context usage
        const barLength = 20;
        const filledBars = Math.round((currentTokens / maxTokens) * barLength);
        const emptyBars = barLength - filledBars;
        const filledChar = '⛁';
        const emptyChar = '⛶';
        // Create bar with spaces between characters like: ⛁ ⛁ ⛁ ⛀ ⛶ ⛶
        const filledPart = filledChar.repeat(Math.min(filledBars, barLength)).split('').join(' ');
        const emptyPart = emptyChar.repeat(Math.max(emptyBars, 0)).split('').join(' ');
        const separator = filledPart && emptyPart ? ' ' : '';
        const bar = filledPart + separator + emptyPart;
        // Calculate detailed breakdown for Messages
        const notebookContextTokens = notebook ?
            (messagesTokens - (this._tokenUsage.cumulativeOutputTokens || 0) -
                this._history.reduce((total, query) => total + this._estimateTokens(query), 0)) : 0;
        const conversationHistoryTokens = this._history.reduce((total, query) => total + this._estimateTokens(query), 0);
        const aiOutputTokens = this._tokenUsage.cumulativeOutputTokens || 0;
        // Display context window usage with detailed breakdown
        tokenDisplay = `⎿ ${bar}

Context Window Usage
─────────────────────────────
${model} • ${(currentTokens / 1000).toFixed(1)}k/200k tokens (${usagePercentage}%)

⛁ System prompt: ${(systemPromptTokens / 1000).toFixed(1)}k tokens (${systemPromptPercentage}%)
⛁ System tools: ${(systemToolsTokens / 1000).toFixed(1)}k tokens (${systemToolsPercentage}%)
⛁ Messages: ${(messagesTokens / 1000).toFixed(1)}k tokens (${messagesPercentage}%)
  └ Notebook context: ${(notebookContextTokens / 1000).toFixed(1)}k
  └ User inputs (${this._history.length}): ${(conversationHistoryTokens / 1000).toFixed(1)}k
  └ AI outputs: ${(aiOutputTokens / 1000).toFixed(1)}k
⛶ Free space: ${(freeSpace / 1000).toFixed(1)}k (${freePercentage}%)`;
        // Update stats with token usage display
        this._contextStats.innerHTML = tokenDisplay;
        // Update content display
        if (context.cells && context.cells.length > 0) {
            let contentDisplay = '';
            if (updateType === 'full') {
                contentDisplay += '🟦 FULL CONTEXT\n';
                contentDisplay += `Showing all ${context.cells.length} cells:\n\n`;
            }
            else if (updateType === 'incremental') {
                contentDisplay += '🟨 INCREMENTAL UPDATE\n';
                contentDisplay += `Showing ${context.cells.length} changed cells:\n\n`;
            }
            else if (updateType === 'none') {
                contentDisplay += '🟩 NO CHANGES\n';
                contentDisplay += 'Using previous context from memory.\n\n';
                this._contextContent.innerHTML = contentDisplay;
                return;
            }
            // Show cell previews
            context.cells.forEach((cell, index) => {
                const cellNum = cell.index + 1;
                const cellType = cell.type || 'unknown';
                const isActive = cell.isActive ? '🟢' : '⚪';
                const changeType = cell.changeType;
                let changeIndicator = '';
                if (changeType === 'new')
                    changeIndicator = '🆕';
                else if (changeType === 'modified')
                    changeIndicator = '✏️';
                contentDisplay += `${isActive}${changeIndicator} Cell ${cellNum} [${cellType}]\n`;
                // Show source preview (first 100 chars)
                const source = cell.source || '';
                if (source.trim()) {
                    const preview = source.length > 100 ? source.substring(0, 100) + '...' : source;
                    contentDisplay += `   ${preview.split('\n').join('\n   ')}\n`;
                }
                else {
                    contentDisplay += '   (empty cell)\n';
                }
                // Show output info
                if (cell.hasOutputs) {
                    contentDisplay += `   📤 ${cell.outputCount || 0} output(s)\n`;
                }
                contentDisplay += '\n';
            });
            this._contextContent.innerHTML = contentDisplay;
            ...
            this._contextContent.innerHTML = 'No cells to display.';
        }
    }
    /**
    ...
    async _sendQuery() {
        ...
        try {
            ...
            // Calculate current token usage before sending query
            this._calculateCurrentTokenUsage(query);
            // Update context display before sending query
            this._updateContextDisplay();
            // Call Pantheon backend API
            const response = await this._callPantheonAPI(query, notebookPath);
            if (response.success) {
                // Increment conversation counter after successful API call
                this._conversationCount++;
                console.log(`✅ Conversation ${this._conversationCount} completed successfully`);
                // Update context display with new conversation count
                this._updateContextDisplay();
                // Show response info
     ...
     */
    async _notebookExecutionLoop(originalQuery, initialResponse, notebook) {
        var _a;
        try {
        ...
        const insertedCellIndexes = []; // Track inserted cells
        while (iterationCount < maxIterations) {
            ...
            for (let i = 0; i < currentResponse.code_blocks.length; i++) {
                ...
                if (!targetCell) {
                    ...
                    (_a = notebook.model) === null || _a === void 0 ? void 0 : _a.sharedModel.insertCell(cellIndex, {
                        cell_type: 'code',
                        source: code.trim()
                    });
     ...
     */
    async _intelligentExecutionLoop(originalQuery, initialResponse) {
                ...
                switch (msgType) {
                    ...
                    case 'error':
                        ...
                        if (msg.content.traceback && msg.content.traceback.length > 0) {
                            ...
                            const cleanTraceback = msg.content.traceback.map((line) => line.replace(/\u001b\[[0-9;]*m/g, ''));
     ...
     */
    async _handleCellError(notebook, cellIndex, originalCode, errorOutput, blockNumber) {
        try {
...
Please analyze the error and provide a corrected version of the code. If the error is due to missing dependencies or data files, please provide installation instructions or alternative approaches. Respond with only the fixed code block.`;
     ...
     */
    async _askUserForFixGuidance(notebook, cellIndex, originalCode, errorOutput) {
        return new Promise((resolve) => {
            ...
            this._currentQueryHandler = async (input) => {
     ...
     */
    async _tryUserGuidedFix(notebook, cellIndex, originalCode, errorOutput, userGuidance) {
        try {
...
Please analyze the error and the user's guidance, then provide a corrected version of the code that addresses both the error and incorporates the user's suggestions. Respond with only the fixed Python code block.`;
     ...
     */
    _updateCellContent(notebook, cellIndex, newCode) {
    ...
    /**
     * Accurate token estimation for different text types
     */
    _estimateTokens(text) {
        ...
        let tokens = 0;
        // Split text into different components for more accurate counting
        const lines = text.split('\n');
        for (const line of lines) {
            tokens += this._estimateLineTokens(line);
        }
        return Math.max(1, Math.round(tokens));
    }
    /**
     * Estimate tokens for a single line of text
     */
    _estimateLineTokens(line) {
        if (!line.trim())
            return 1; // Empty lines still count as 1 token
        let tokens = 0;
        // Check if line looks like code
        const isCode = this._looksLikeCode(line);
        if (isCode) {
            tokens = this._estimateCodeTokens(line);
        }
        else {
            tokens = this._estimateNaturalLanguageTokens(line);
        }
        return tokens;
    }
    /**
    ...
    _looksLikeCode(text) {
        const codeIndicators = [
            /^(import|from|def|class|function|const|let|var|if|for|while)\s/,
            /[{}()\[\]]/,
            /[=<>!]+/,
            /\.(py|js|ts|java|cpp|c|go|rs|rb|php|html|css|sql)$/,
    ...
    /**
     * Estimate tokens for code text
     */
    _estimateCodeTokens(text) {
        // Code has more symbols and identifiers, typically denser
        ...
        for (const part of parts) {
            if (part.length === 0)
                continue;
            // Keywords and operators typically 1 token each
            if (part.length <= 3) {
                tokens += 1;
            }
            else if (part.length <= 8) {
                tokens += Math.ceil(part.length / 4); // Variable names, function names
            }
            else {
                tokens += Math.ceil(part.length / 3); // Long identifiers
            }
        }
        // Add tokens for operators and punctuation (roughly estimate)
        const operators = text.match(/[=<>!+\-*/&|^%~(){}[\],;:]/g) || [];
        tokens += Math.ceil(operators.length / 2); // Some operators are multi-char
        return Math.max(1, tokens);
    }
    /**
     * Estimate tokens for natural language text
     */
    _estimateNaturalLanguageTokens(text) {
        // Handle mixed Chinese and English text
        const chineseChars = text.match(/[\u4e00-\u9fff]/g) || [];
        const chineseTokens = chineseChars.length; // Chinese: 1 char ≈ 1 token
        ...
        for (const word of englishWords) {
            ...
            if (cleanWord.length === 0) {
                englishTokens += 0.5; // Punctuation
            }
            else if (cleanWord.length <= 3) {
                englishTokens += 1; // Short words: 1 token
            }
            else if (cleanWord.length <= 7) {
                englishTokens += 1.3; // Medium words: ~1.3 tokens
            }
            else {
                englishTokens += Math.ceil(cleanWord.length / 4); // Long words: split into subwords
            }
        }
        // Add tokens for punctuation
        const punctuation = text.match(/[.,!?;:"'()[\]{}-]/g) || [];
        const punctTokens = Math.ceil(punctuation.length / 2);
        const totalTokens = chineseTokens + englishTokens + punctTokens;
        return Math.max(1, Math.round(totalTokens));
    }
    /**
     * Calculate current context size based on notebook content and query
     */
    _calculateCurrentTokenUsage(query) {
        const notebook = this._notebookTracker.currentWidget;
        if (!notebook)
            return;
        // Get notebook context
        const context = this._collectNotebookContext(notebook, 50);
        // Calculate tokens for notebook cells
        let notebookTokens = 0;
        if (context.cells) {
            for (const cell of context.cells) {
                const cellText = cell.source || '';
                notebookTokens += this._estimateTokens(cellText);
                // Add output tokens if present
                if (cell.outputs && cell.outputs.length > 0) {
                    const outputText = cell.outputs.map((o) => JSON.stringify(o)).join('');
                    notebookTokens += this._estimateTokens(outputText);
                }
            }
        }
        // System components estimation (more accurate based on typical prompts)
        const systemPromptTokens = this._estimateTokens("You are Pantheon, an advanced AI assistant specialized in data analysis and scientific computing. You have access to comprehensive tools for analyzing notebooks, executing code, and helping users with their data science workflows.");
        const systemToolsTokens = 8500; // Estimated based on typical tool descriptions
        // Current query tokens
        const queryTokens = query ? this._estimateTokens(query) : 0;
        // Total input tokens for this interaction (only for current call, not cumulative)
        const currentInputTokens = systemPromptTokens + systemToolsTokens + notebookTokens + queryTokens;
        // Store current interaction tokens (these will be reset each call)
        this._tokenUsage.inputTokens = currentInputTokens;
        this._tokenUsage.outputTokens = 0; // Reset output tokens for new query
        this._tokenUsage.totalTokens = currentInputTokens; // Current interaction total
        console.log('Current interaction token usage:', {
            systemPrompt: systemPromptTokens,
            systemTools: systemToolsTokens,
            notebook: notebookTokens,
            query: queryTokens,
            currentInputTokens: currentInputTokens,
            sessionCumulativeInput: this._tokenUsage.cumulativeInputTokens,
            sessionCumulativeOutput: this._tokenUsage.cumulativeOutputTokens,
            sessionCumulativeTotal: this._tokenUsage.cumulativeTotalTokens
        });
    }
    /**
     * Calculate hash for a cell based on source and outputs
     */
    _calculateCellHash(cellInfo) {
     ...
     */
    _detectContextChanges(context) {
        const cells = context.cells || [];
        const changedCells = [];
        const newCells = [];
        const currentCellHashes = new Map();
        ...
        for (const cell of cells) {
            ...
            if (isFirstConversation || !this._lastSentCells.has(cellIndex)) {
                ...
                console.log(`✏️ Modified cell ${cellIndex} (${previousHash === null || previousHash === void 0 ? void 0 : previousHash.slice(0, 8)} -> ${cellHash.slice(0, 8)})`);
            }
            else {
        ...
        this._lastSentCells = new Set(cells.map((c) => c.index));
        return {
            isFirstConversation,
     ...
     */
    _collectNotebookContext(notebook, maxCells = 50) {
        ...
        try {
            ...
            try {
                context.currentCellIndex = ((_a = notebook.content) === null || _a === void 0 ? void 0 : _a.activeCellIndex) || -1;
            }
            catch (e) {
            ...
            try {
                if ((_c = (_b = notebook.content) === null || _b === void 0 ? void 0 : _b.model) === null || _c === void 0 ? void 0 : _c.cells) {
            ...
            if (!cells) {
                try {
                    if ((_d = notebook.model) === null || _d === void 0 ? void 0 : _d.cells) {
            ...
            for (let i = 0; i < cellsToProcess; i++) {
                try {
                    ...
                    else if ((_f = (_e = cell.model) === null || _e === void 0 ? void 0 : _e.source) === null || _f === void 0 ? void 0 : _f.text) {
                        source = cell.model.source.text;
                    }
                    else if ((_h = (_g = cell.model) === null || _g === void 0 ? void 0 : _g.value) === null || _h === void 0 ? void 0 : _h.text) {
                        source = cell.model.value.text;
                    }
                    else if ((_j = cell.sharedModel) === null || _j === void 0 ? void 0 : _j.source) {
                        source = cell.sharedModel.source;
                    }
                    else if ((_l = (_k = cell.model) === null || _k === void 0 ? void 0 : _k.sharedModel) === null || _l === void 0 ? void 0 : _l.source) {
                    ...
                    if (cell.type === 'code') {
                        // Execution count
                        const executionCount = cell.executionCount || ((_m = cell.model) === null || _m === void 0 ? void 0 : _m.executionCount);
                        ...
                        try {
                            const outputs = cell.outputs || ((_o = cell.model) === null || _o === void 0 ? void 0 : _o.outputs);
                            ...
                            console.log(`Error processing outputs for cell ${i}:`, outputError.message);
                        }
                    }
                    context.cells.push(cellInfo);
                }
                catch (cellError) {
                    console.log(`Error processing cell ${i}:`, cellError.message);
            ...
            // Note: Don't increment conversation counter here - do it after successful API call
            return optimizedContext;
        }
        catch (error) {
            console.error('❌ Error in context collection:', error.message);
            return {
                totalCells: 0,
                currentCellIndex: -1,
                cells: [],
                message: `Error collecting context: ${error.message}`
            };
        }
    }
    /**
     ...
     */
    _analyzeNotebookContext(context) {
        const analysis = {
            dataScience: false,
        ...
        if (importText.includes('pandas') || importText.includes('numpy') || importText.includes('scipy')) {
            ...
            analysis.hasDataFrames = variables.some((v) => v.includes('df') || v.includes('data'));
        }
        ...
        const allCellSources = cells.map((c) => { var _a; return ((_a = c.source) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || ''; }).join(' ');
        analysis.dataLoaded = allCellSources.includes('read_csv') ||
            allCellSources.includes('load_data') ||
            allCellSources.includes('pd.read');
        // Check for errors
        analysis.hasErrors = cells.some((c) => c.outputType === 'error');
        ...
        if (variables.some((v) => v.toLowerCase().includes('model'))) {
            analysis.codePatterns.push('ML model development');
        }
        if (functions.some((f) => f.toLowerCase().includes('plot'))) {
            analysis.codePatterns.push('Custom plotting functions');
        }
        if (variables.some((v) => v.toLowerCase().includes('test'))) {
     ...
     */
    async _callPantheonAPI(query, notebookPath) {
        ...
        try {
            ...
            const notebook = this._notebookTracker.currentWidget;
            const workspacePath = (notebook === null || notebook === void 0 ? void 0 : notebook.context.path) ?
            ...
            if (notebookContext) {
                ...
                console.log(`🔄 Context type: ${notebookContext.updateType}, First conversation: ${notebookContext.isFirstConversation}`);
                // Debug: Show first few cells being sent
                if (notebookContext.cells && notebookContext.cells.length > 0) {
                    console.log(`📋 First 3 cells being sent:`);
                    notebookContext.cells.slice(0, 3).forEach((cell, i) => {
                        console.log(`  Cell ${cell.index}: ${cell.type} - ${(cell.source || '').substring(0, 50)}...`);
                    });
                }
            }
            else {
                console.log(`⚠️ No notebook context available`);
            }
            ...
            if (isComplexQuery) {
                ...
                try {
                    ...
                    // Try different possible formats from the API response
                    let currentOutputTokens = 0;
                    if (result.token_usage) {
                        currentOutputTokens = result.token_usage.output_tokens || result.token_usage.output || 0;
                        // Update current session tokens (not cumulative)
                        this._tokenUsage.model = result.model || this._tokenUsage.model;
                        this._tokenUsage.inputTokens = result.token_usage.input_tokens || result.token_usage.input || 0;
                        this._tokenUsage.outputTokens = currentOutputTokens;
                        this._tokenUsage.totalTokens = result.token_usage.total_tokens || result.token_usage.total ||
                            ((result.token_usage.input_tokens || 0) + currentOutputTokens);
                        console.log('Token usage updated from token_usage:', this._tokenUsage);
                    }
                    else if (result.usage) {
                        currentOutputTokens = result.usage.completion_tokens || result.usage.output_tokens || 0;
                        // Alternative format: usage field
                        this._tokenUsage.model = result.model || this._tokenUsage.model;
                        this._tokenUsage.inputTokens = result.usage.prompt_tokens || result.usage.input_tokens || 0;
                        this._tokenUsage.outputTokens = currentOutputTokens;
                        this._tokenUsage.totalTokens = result.usage.total_tokens ||
                            ((result.usage.prompt_tokens || 0) + currentOutputTokens);
                        console.log('Token usage updated from usage:', this._tokenUsage);
                    }
                    else {
                        // No token usage in response - estimate output tokens from response
                        console.log('No token usage found in response. Available keys:', Object.keys(result));
                        ...
                        currentOutputTokens = this._estimateTokens(responseText);
                        // Update current session token usage
                        this._tokenUsage.outputTokens = currentOutputTokens;
                        this._tokenUsage.totalTokens = (this._tokenUsage.inputTokens || 0) + currentOutputTokens;
                        console.log('Estimated output tokens:', currentOutputTokens);
                        console.log('Current session total tokens:', this._tokenUsage.totalTokens);
                    }
                    // Accumulate tokens for cumulative tracking after successful API call
                    this._tokenUsage.cumulativeInputTokens = (this._tokenUsage.cumulativeInputTokens || 0) + (this._tokenUsage.inputTokens || 0);
                    this._tokenUsage.cumulativeOutputTokens = (this._tokenUsage.cumulativeOutputTokens || 0) + currentOutputTokens;
                    this._tokenUsage.cumulativeTotalTokens = this._tokenUsage.cumulativeInputTokens + this._tokenUsage.cumulativeOutputTokens;
                    // Update conversation count to show activity
                    this._conversationCount++;
                    console.log('Updated conversation count to:', this._conversationCount);
                    console.log('Tokens accumulated after API call:', {
                        currentInput: this._tokenUsage.inputTokens,
                        currentOutput: currentOutputTokens,
                        cumulativeInput: this._tokenUsage.cumulativeInputTokens,
                        cumulativeOutput: this._tokenUsage.cumulativeOutputTokens,
                        cumulativeTotal: this._tokenUsage.cumulativeTotalTokens
                    });
                    // Always update context display after a query to show latest info
                    this._updateContextDisplay();
     ...
     */
    async _insertPantheonResponse(notebook, query, response) {
        var _a, _b, _c;
        const currentIndex = notebook.content.activeCellIndex;
        let insertIndex = currentIndex + 1;
        // Add explanation if available
        if (response.explanation && response.explanation.trim()) {
            (_a = notebook.model) === null || _a === void 0 ? void 0 : _a.sharedModel.insertCell(insertIndex, {
                cell_type: 'markdown',
        ...
        if (response.code_blocks && response.code_blocks.length > 0) {
            for (const codeBlock of response.code_blocks) {
                if (codeBlock.trim()) {
                    // Insert code cell
                    (_b = notebook.model) === null || _b === void 0 ? void 0 : _b.sharedModel.insertCell(insertIndex, {
                        cell_type: 'code',
            ...
            (_c = notebook.model) === null || _c === void 0 ? void 0 : _c.sharedModel.insertCell(insertIndex, {
                cell_type: 'markdown',
     ...
     */
    async _executeCellAtIndex(notebook, index) {
        var _a;
        try {
            ...
            const kernel = (_a = notebook.sessionContext.session) === null || _a === void 0 ? void 0 : _a.kernel;
            if (!kernel) {
                ...
                switch (msgType) {
                    ...
                    case 'error':
                        ...
                        if (msg.content.traceback && msg.content.traceback.length > 0) {
                            ...
                            const cleanTraceback = msg.content.traceback.map((line) => typeof line === 'string' ? line.replace(/\u001b\[[0-9;]*m/g, '') : String(line));
                            output += '\n' + cleanTraceback.join('\n');
                        }
                        break;
                }
     ...
     */
    async _ensureKernelReady(notebook) {
        var _a, _b;
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                const finalKernel = (_b = sessionContext.session) === null || _b === void 0 ? void 0 : _b.kernel;
                if (finalKernel && !finalKernel.isDisposed) {
                    try {
                        ...
                        await Promise.race([
                            testFuture.done,
                            new Promise((_, reject) => setTimeout(() => reject(new Error('Kernel test timeout')), 30000) // 30 seconds for kernel test
                            )
                        ]);
    ...
    _initializeCollapsedState() {
        ...
        let floatingHeader = document.querySelector('.pantheon-floating-header');
        if (!floatingHeader) {
    ...
    `;
        ...
        const floatingToggleBtn = floatingHeader.querySelector('.floating-toggle-btn');
    ...
    _toggleExpanded() {
        ...
        const container = this.node.querySelector('.pantheon-container');
        const header = this.node.querySelector('.pantheon-header');
        const inputContainer = this.node.querySelector('.pantheon-input-container');
        const outputContainer = this.node.querySelector('.pantheon-output-container');
        const statusSection = this.node.querySelector('.pantheon-status-section');
        const statusBar = this.node.querySelector('.pantheon-status');
        if (this._isExpanded) {
     ...
     */
    _toggleStatusExpanded() {
        this._isStatusExpanded = !this._isStatusExpanded;
        const statusOutputContainer = this.node.querySelector('.pantheon-status-output-container');
     ...
     */
    _appendToOutput(text, type = 'system') {
     ...
     */
    _appendToStatusOutput(text, type = 'status') {
        const timestamp = new Date().toLocaleTimeString();
        const colorMap = {
     ...
     */
    async _handleSlashCommand(command) {
...
• Time: ${new Date().toLocaleString()}`;
    ...
    async _handleApiKeyCommand(args) {
        try {
            if (args.length === 0 || args[0] === 'list') {
                // Get API key status
...
• /api-key anthropic sk-ant-xxx --local - Set Anthropic key locally`, 'error');
     ...
     */
    async _callModelAPI(method, data) {
        const baseUrl = window.location.origin + window.location.pathname.replace(/\/lab.*/, '');
        const url = method === 'GET' ?
            `${baseUrl}/pantheon/model${(data === null || data === void 0 ? void 0 : data.action) ? `?action=${data.action}` : ''}` :
    ...
    async _fetchCurrentModel() {
        try {
            ...
            if (response.success && response.result) {
                console.log('Raw model response:', response.result);
                // Try to extract model name between "Current Model:" and the first parenthesis
                // Example: "📱 Current Model: GLM-4.5 Air (Zhipu AI - Latest) (zhipu/glm-4.5-air)"
                const modelMatch = response.result.match(/Current Model:\s*([^(]+)/);
                if (modelMatch && modelMatch[1]) {
                    // Clean up the extracted model name
                    let modelName = modelMatch[1].trim();
                    // Remove any trailing whitespace
                    modelName = modelName.replace(/\s+$/, '');
                    this._tokenUsage.model = modelName;
                    console.log('Extracted model name:', modelName);
                }
                else {
                    // Fallback: try to get the model ID from parentheses
                    const idMatch = response.result.match(/\(([^)]+)\)/g);
                    if (idMatch && idMatch.length > 0) {
                        // Get the last match which is usually the model ID
                        const lastMatch = idMatch[idMatch.length - 1];
                        const modelId = lastMatch.replace(/[()]/g, '');
                        this._tokenUsage.model = modelId;
                        console.log('Extracted model ID (fallback):', modelId);
                    }
                    else {
                        this._tokenUsage.model = 'Unknown';
                        console.log('Could not parse model from:', response.result);
                    }
                }
                // Update display to show the model
                this._updateContextDisplay();
            }
        }
        catch (error) {
     ...
     */
    async _callApiKeyAPI(method, data) {
        const baseUrl = window.location.origin + window.location.pathname.replace(/\/lab.*/, '');
        const url = method === 'GET' ?
            `${baseUrl}/pantheon/api-key${(data === null || data === void 0 ? void 0 : data.action) ? `?action=${data.action}` : ''}` :
     ...
     */
    async _executeCodeBlocksSimply(codeBlocks, notebook) {
     ...
     */
    _parseResponseIntoBlocks(response) {
        var _a, _b;
        const blocks = [];
            ...
            if (rawContent && !rawContent.includes('(response truncated)')) {
                ...
                while ((match = codeBlockRegex.exec(rawContent)) !== null) {
                    ...
                    if (codeBlockIndex < response.code_blocks.length) {
                        ...
                        const codeContent = typeof codeBlock === 'string' ? codeBlock.trim() : (_a = codeBlock.code) === null || _a === void 0 ? void 0 : _a.trim();
                        if (codeContent) {
                ...
                for (const codeBlock of response.code_blocks) {
                    const codeContent = typeof codeBlock === 'string' ? codeBlock.trim() : (_b = codeBlock.code) === null || _b === void 0 ? void 0 : _b.trim();
                    if (codeContent) {
                        blocks.push({ type: 'code', content: codeContent });
                    }
                }
     ...
     */
    _insertMarkdownIntoNotebook(notebook, markdown) {
     ...
     */
    _insertCodeIntoNotebook(notebook, code) {
        const notebookPanel = notebook.content;
        const model = notebookPanel.model;
        const currentIndex = notebookPanel.activeCellIndex;
        if (!model) {
     ...
     */
    async _executeCellInNotebook(notebook, cellIndex) {
        try {
            ...
            const { NotebookActions } = await Promise.resolve(/*! import() */).then(__webpack_require__.t.bind(__webpack_require__, /*! @jupyterlab/notebook */ "webpack/sharing/consume/default/@jupyterlab/notebook", 23));
            ...
            await Promise.race([
                promise,
                new Promise((_, reject) => setTimeout(() => reject(new Error('Cell execution timeout')), 180000) // 3 minutes for heavy imports/processing
                )
            ]);
            ...
            for (let i = 0; i < outputs.length; i++) {
                ...
                if (outputData.type === 'stream') {
                    ...
                    if (outputData.traceback && outputData.traceback.length > 0) {
                        const cleanTraceback = outputData.traceback.map((line) => typeof line === 'string' ? line.replace(/\u001b\[[0-9;]*m/g, '') : String(line));
                        output += '\n' + cleanTraceback.join('\n');
                    }
                }
            }
...
//# sourceMappingURL=lib_index_js.1a522e82a7d283d1b02c.js.map
...
class MySQLToMySQLTransfer(AbstractDatabaseTransfer):
    ...
    def transfer_table_data(self, table_name: str):
        ...
        # 确保 max_id 始终为整数，这是关键的第一步
        try:
            max_id_raw = self.get_max_id(table_name)
            if isinstance(max_id_raw, str):
        ...
        # 确保断点恢复的 last_id 也是整数，并进行初始化
        try:
            if self.config.nowLastId > 0:
                self.config.nowLastId = int(self.config.nowLastId)
                print(f"  - 从上次断点恢复, last ID: {self.config.nowLastId}")
            else:
                self.config.nowLastId = 0
        except (TypeError, ValueError) as e:
            print(f"\n[致命错误] 断点恢复ID '{self.config.nowLastId}' 无法转换为整数。错误: {e}")
            raise e

        while True:
            data_batch = None
            # 统一使用参数化查询，防止类型错误和SQL注入
            ...
            # 确保 last_id_in_batch 也是整数，防止后续错误
            last_id_in_batch = int(data_batch[-1]['id'])

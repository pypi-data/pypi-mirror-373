...
class TestGetSchemaNames:
    ...
    def test_get_schema_names(self):
        ...
        assert "md.candle.closed@v1" in names
        assert "strategy.signal@v1" in names
        assert "risk.signal.allowed@v1" in names
        assert "risk.signal.denied@v1" in names
        assert "risk.limit.breached@v1" in names
        assert "exec.order.placed@v1" in names
        assert "exec.order.rejected@v1" in names
        assert "exec.order.filled@v1" in names
        assert "pf.position.updated@v1" in names
        assert "pf.balance.updated@v1" in names
        assert "pf.pnl.updated@v1" in names
        assert "runs.started@v1" in names
        assert "runs.status.changed@v1" in names
        assert "runs.finished@v1" in names

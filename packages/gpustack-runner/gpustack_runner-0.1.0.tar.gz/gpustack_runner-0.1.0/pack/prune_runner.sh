...
# - If backend is specified, but not backend_version, prune all runners with the backend.
# - If backend and backend_version are specified, prune all runners with the backend and backend_version.
# - If service is specified, but not service_version, prune all runners with the service.
# - If service and service_version are specified, prune all runners with the service and service_version
# - If both backend and service are specified, prune runners that match all criteria.

                ...
                with transaction.atomic():
                    ...
                    if all_instances:
                        ...
                        if update_fields:
                            ...
                            print(f"async_upsert_task - Auto-determined update_fields: {fields_to_update}", file=sys.stderr)

                        # Perform true upsert with unique constraint
                        if fields_to_update:
                            print(f"async_upsert_task - Executing bulk_create with update_conflicts", file=sys.stderr)
                            print(f"async_upsert_task - unique_fields: {unique_fields}", file=sys.stderr)
                            print(f"async_upsert_task - update_fields: {fields_to_update}", file=sys.stderr)
                            ...
                            try:
                                ...
                                if created_instances:
                                    ...
                                    print(f"async_upsert_task - bulk_create returned no instances", file=sys.stderr)
                                    result.add_error(0, "Bulk create operation returned no instances")
                            except Exception as bulk_error:
                                print(f"async_upsert_task - bulk_create failed: {str(bulk_error)}", file=sys.stderr)
                                result.add_error(0, f"Bulk upsert failed: {str(bulk_error)}")
                                created_instances = []
                        else:
                            print(f"async_upsert_task - No update fields, using separate operations", file=sys.stderr)
                            ...
                            try:
                                ...
                                print(f"async_upsert_task - Fallback operations failed: {str(fallback_error)}", file=sys.stderr)
                                result.add_error(0, f"Fallback upsert operations failed: {str(fallback_error)}")
                                created_instances = []

            except Exception as e:
                print(f"async_upsert_task - ERROR during bulk operation: {str(e)}", file=sys.stderr)
                # Handle unique constraint errors specifically
                error_msg = str(e)
                if "unique constraint" in error_msg.lower() or "unique_fields" in error_msg.lower():
                    result.add_error(0, f"Unique constraint error: {error_msg}. "
                                      f"Ensure fields {unique_fields} form a unique constraint in your database.")
                else:
                    result.add_error(0, f"Upsert failed: {error_msg}")

        # Store final result in cache
        OperationCache.set_task_result(task_id, result.to_dict())

        print(f"async_upsert_task - COMPLETED: success={result.success_count}, errors={result.error_count}", file=sys.stderr)
        print(f"async_upsert_task - Final result: {result.to_dict()}", file=sys.stderr)

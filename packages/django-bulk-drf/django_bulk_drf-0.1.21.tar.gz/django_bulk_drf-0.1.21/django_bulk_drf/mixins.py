...
class BulkOperationsMixin:
    ...
    def create(self, request, *args, **kwargs):
        ...
        if isinstance(request.data, list):
            ...
            if unique_fields_param:
                ...
                print(f"BulkOperationsMixin.create() - Routing to bulk create (no unique_fields)", file=sys.stderr)
                return self._handle_bulk_create(request)

        print(f"BulkOperationsMixin.create() - Using standard single create behavior", file=sys.stderr)
    ...
    def _sync_upsert(self, request, unique_fields_param, preparsed_data=None):
        ...
        if isinstance(data_payload, dict):
            print(f"BulkOperationsMixin._sync_upsert() - Routing to single upsert", file=sys.stderr)
            ...
            print(f"BulkOperationsMixin._sync_upsert() - Routing to array upsert (Celery)", file=sys.stderr)
    ...
    def _handle_single_upsert(self, request, unique_fields, update_fields, data_dict):
        ...
        if not unique_fields:
            print(f"BulkOperationsMixin._handle_single_upsert() - ERROR: No unique_fields provided", file=sys.stderr)
            return Response(
        ...
        try:
            ...
            if existing_instance:
                ...
                print(f"BulkOperationsMixin._handle_single_upsert() - Creating new instance", file=sys.stderr)
                # Create new instance
    ...
    def _handle_bulk_create(self, request):
        ...
        print(f"BulkOperationsMixin._handle_bulk_create() - Dispatching bulk create to Celery", file=sys.stderr)
        ...
        try:
            print(f"BulkOperationsMixin._handle_bulk_create() - Waiting for task completion (300s timeout)...", file=sys.stderr)
            # Wait for the task to complete (synchronous behavior)
            task_result = task.get(timeout=300)  # 5 minute timeout for bulk create operations
            print(f"BulkOperationsMixin._handle_bulk_create() - Task completed successfully", file=sys.stderr)
            ...
            return Response(
                {
                    "task_id": task.id,
                    "operation_type": "sync_bulk_create_via_worker",
    ...
    def _handle_array_upsert(self, request, unique_fields, update_fields, data_list):
        ...
        if not unique_fields:
            print(f"BulkOperationsMixin._handle_array_upsert() - ERROR: No unique_fields provided", file=sys.stderr)
            return Response(
                {"error": "unique_fields parameter is required for upsert operations"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        print(f"BulkOperationsMixin._handle_array_upsert() - Dispatching Celery task...", file=sys.stderr)
        ...
        # Wait for task completion with fixed timeout
        try:
            print(f"BulkOperationsMixin._handle_array_upsert() - Waiting for task completion (300s timeout)...", file=sys.stderr)
            # Wait for the task to complete (synchronous behavior)
            task_result = task.get(timeout=300)  # 5 minute timeout for upsert operations
            print(f"BulkOperationsMixin._handle_array_upsert() - Task completed successfully", file=sys.stderr)

...
class TestDetectModelType:
    ...
    def test_standard_model_detection(self):
        ...
        assert detect_model_type("gpt-3.5-turbo") == "standard"
        assert detect_model_type("llama-2-70b") == "standard"
        assert detect_model_type("mistral-7b") == "standard"
...
class TestFormatMessagesWithThinking:
    ...
    def test_standard_model_warning(self, capsys):
        """Test standard models show warning about think tokens."""
        prompt = "Test <think>Thinking"
        messages = format_messages_with_thinking(prompt, "gpt-3.5-turbo", verbose=True)
        
        captured = capsys.readouterr()
        assert "may not support thinking tokens" in captured.out
...
class TestCreateTestPrompts:
    ...
    def test_standard_model_prompts(self):
        """Test prompts for standard models."""
        prompts = create_test_prompts("gpt-3.5-turbo")
        
        assert len(prompts) == 2
        assert all("<think>" not in p[1] for p in prompts)

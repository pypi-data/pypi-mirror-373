...
class PostProcessingIntegration:
    ...
    def process_dataframe(self, df: pd.DataFrame, verbose: bool = False) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        try:
            ...
            self.post_processing_stats["faulty_rows_filtered"] = filtered_count
            if filtered_count > 0:
    ...
    def _filter_faulty_time_rows(self, df: pd.DataFrame, verbose: bool = False) -> Tuple[pd.DataFrame, int]:
        """
        Filter out rows with faulty time values (time=0, NaN, out-of-order timestamps).
        ...
        # Filter out rows where any time column equals 0 or is NaN
        ...
        for time_col in time_columns:
            if time_col in df.columns:
                # Check for time=0 values
                time_zero_mask = df[time_col] == 0
                # Check for NaN values
                time_nan_mask = df[time_col].isna()
                # Combine both conditions
                time_faulty_mask = time_zero_mask | time_nan_mask
                mask = mask & ~time_faulty_mask
                
                if verbose:
                    if time_zero_mask.any():
                        zero_count = time_zero_mask.sum()
                        print(f"      Found {zero_count} rows with time=0 in column '{time_col}'")
                        zero_nan_filtered += zero_count
                    if time_nan_mask.any():
                        nan_count = time_nan_mask.sum()
                        print(f"      Found {nan_count} rows with NaN time in column '{time_col}'")
                        zero_nan_filtered += nan_count
        
        # Apply initial filter
        df_after_basic_filter = df[mask].copy()
        
        # Check for out-of-order timestamps in first 5 rows (simple fix)
        if len(df_after_basic_filter) >= 2:
            time_col = time_columns[0]  # Use first time column
            check_rows = min(5, len(df_after_basic_filter))
            
            # Check if first few rows are chronologically ordered
            out_of_order_indices = []
            for i in range(1, check_rows):
                if df_after_basic_filter.iloc[i][time_col] < df_after_basic_filter.iloc[i-1][time_col]:
                    # Current row is earlier than previous - previous row is out of order
                    out_of_order_indices.append(i-1)
            
            # Also check if first row is much later than second row (common case)
            if (len(df_after_basic_filter) >= 2 and 
                df_after_basic_filter.iloc[0][time_col] > df_after_basic_filter.iloc[1][time_col]):
                # Check if first row should be much later in the sequence
                first_row_time = df_after_basic_filter.iloc[0][time_col]
                # Sample a few more rows to see the pattern
                sample_size = min(10, len(df_after_basic_filter))
                sample_times = df_after_basic_filter.iloc[1:sample_size][time_col]
                
                # If first row is significantly later than the pattern, it's likely misplaced
                if first_row_time > sample_times.max():
                    out_of_order_indices.append(0)
            
            # Remove out-of-order rows
            if out_of_order_indices:
                out_of_order_indices = list(set(out_of_order_indices))  # Remove duplicates
                ordering_filtered = len(out_of_order_indices)
                df_after_basic_filter = df_after_basic_filter.drop(df_after_basic_filter.index[out_of_order_indices])
                
                if verbose:
                    print(f"      Found {ordering_filtered} out-of-order rows in first {check_rows} rows")
                
                # Report this in post-processing stats
                warning_msg = f"Detected and removed {ordering_filtered} chronologically out-of-order rows from beginning of dataset"
                if 'warnings' not in self.post_processing_stats:
                    self.post_processing_stats['warnings'] = []
                self.post_processing_stats['warnings'].append(warning_msg)
        
        total_filtered = zero_nan_filtered + ordering_filtered
        
        if verbose:
            if total_filtered > 0:
                print(f"      ✓ Filtered out {total_filtered} rows total ({zero_nan_filtered} faulty values, {ordering_filtered} out-of-order)")
                print(f"      Remaining: {len(df_after_basic_filter)} rows")
            else:
                print("      ✓ No faulty time rows found")
        
        return df_after_basic_filter, total_filtered

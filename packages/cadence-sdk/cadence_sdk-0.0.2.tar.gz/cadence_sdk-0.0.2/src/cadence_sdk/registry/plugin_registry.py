    ...
    def register(self, plugin_class: Type[BasePlugin]) -> None:
        ...
        try:
            ...
            if plugin_name in self._plugins:
                ...
                if existing_version == new_version and getattr(existing_class, "__module__", None) == getattr(
                    plugin_class, "__module__", None
                ):
                    self.logger.debug(
                        f"Plugin '{plugin_name}' v{new_version} from {plugin_class.__module__} already registered. Skipping."
                    )
                    return
                try:
                    if new_v <= existing_v:
                        self.logger.info(
                            f"Ignoring registration of '{plugin_name}' v{new_version} because existing version is v{existing_version}."
                        )
                        return
                except TypeError:

...
def compile_circuit_to_device_with_config_validation(
    ...
    ``config`` as they are expected to arrive to device (latency or distortion
    ...
    -------
    :
        The modified ``schedule`` with pulse information added to all gates, or the
        unmodified schedule if circuit to device compilation is not necessary.
...
def _compile_circuit_to_device(  # noqa: PLR0911
    ...
    if isinstance(operation, ScheduleBase):
        ...
        # it is a two device element operation if the operation not in the
        # device element config
        elif (
        ...
        # we only support 2 device element operations and single device element
        # operations. some single device element operations (reset, measure) can
        # be expressed as acting on multiple device elements simultaneously.
        # That is covered through this for-loop.
        else:
...
def _set_pulse_and_acquisition_clock(
    ...
    """
    if isinstance(operation, ScheduleBase):
        # verify that required clocks are present; print warning if they are
        # inconsistent
        verified_clocks = []
        for inner_op_key in operation.operations:
        ...
        for device_element in device_elements:
            if (
                ...
                if pulse_compensation_element.factory_func is not None:
                    raise ValueError(
                        f"'factory_func' in the device configuration for pulse "
                        f"compensation for device element '{device_element}' "
                        f"is not 'None'. Only 'None' is allowed for 'factory_func' "
                        f"for pulse compensation."
                    )
                ...
                if (sampling_rate != current_sampling_rate) and (
                    ...
                    raise ValueError(
                        ...
                        f"'{current_sampling_rate}', for others "
...
def _get_device_repr_from_cfg_multiplexed(
    ...
    if operation_cfg.gate_info_factory_kwargs is not None:
        for key in operation_cfg.gate_info_factory_kwargs:
            ...
            # acquisition protocols for device element during multiplexed readout), they
            # are assumed to NOT be a list or tuple. If this spoils the correct
            # behaviour of your program in future: sorry :(
...
class MultipleKeysError(KeyError):
    ...
    def __init__(self, operation: str, matches: list[str]) -> None:
        """Initialize a MultipleKeysError."""
        self.value = (

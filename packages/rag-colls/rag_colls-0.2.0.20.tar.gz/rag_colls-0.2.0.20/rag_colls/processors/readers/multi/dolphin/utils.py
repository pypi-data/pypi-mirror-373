...
def convert_pdf_to_images(pdf_path: str | Path, target_size=896):
    ...
    try:
        ...
        doc.close()
        logger.info(f"Successfully converted {len(images)} pages from {str(pdf_path)}.")
        return images
...
def get_output_str(recognition_results):

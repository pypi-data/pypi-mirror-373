...
class MetatraderPosition(TypedDict, total=False):
    ...
    """Optional position comment. The sum of the line lengths of the comment and the clientId must be less than or
    equal to 30 if both clientId and comment are specified, or 31 otherwise. Note, that MT4 accepts only latin1
...
class MetatraderOrder(TypedDict, total=False):
    ...
    comment: Optional[str]
    """Order comment. The sum of the line lengths of the comment and the clientId must be less than or
    equal to 30 if both clientId and comment are specified, or 31 otherwise. Note, that MT4 accepts only latin1
    symbols, while MT5 accepts UTF-16. For more information see https://metaapi.cloud/docs/client/clientIdUsage/"""
    brokerComment: Optional[str]
    """Current comment value on broker side (possibly overriden by the broker)."""
...
class MetatraderDeal(TypedDict, total=False):
    ...
    comment: Optional[str]
    """Deal comment. The sum of the line lengths of the comment and the clientId must be less than or
    equal to 30 if both clientId and comment are specified, or 31 otherwise. Note, that MT4 accepts only latin1
    symbols, while MT5 accepts UTF-16. For more information see https://metaapi.cloud/docs/client/clientIdUsage/"""
    brokerComment: Optional[str]
    """Current comment value on broker side (possibly overriden by the broker)."""
...
class MetatraderSymbolSpecification(TypedDict, total=False):
    ...
    """List of allowed order filling modes. Can contain SYMBOL_FILLING_FOK, SYMBOL_FILLING_IOC, SYMBOL_FILLING_BOC,
    SYMBOL_FILLING_RETURN or their combination. See
    https://www.mql5.com/en/docs/constants/environment_state/marketinfoconstants#symbol_filling_mode for more
    details."""
    executionMode: str
...
class MetatraderTrade(TypedDict):
    """MetaTrader trade"""
    ...
    takeProfit: Optional[float]
    """Take profit price"""
    stopLossUnits: Optional[str]
    """Stop loss units. ABSOLUTE_PRICE means the that the value of stopLoss field is a final stop loss value.
    RELATIVE_* means that the stopLoss field value contains relative stop loss expressed either in price, points,

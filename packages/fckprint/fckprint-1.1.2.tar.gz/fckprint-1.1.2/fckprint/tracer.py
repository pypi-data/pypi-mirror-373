...
DISABLED = bool(os.getenv('fckprint_DISABLED', ''))
...
class Tracer:
    ...
    When you decorate a function with `@fckprint.snoop()`
    or wrap a block of code in `with fckprint.snoop():`, you'll get a log of
    ...
    a file::
        ...
        @fckprint.snoop('/my/log/file.log')

    See values of some expressions that aren't local variables::

        @fckprint.snoop(watch=('foo.bar', 'self.x["whatever"]'))

    Expand values to see all their attributes or items of lists/dictionaries:

        @fckprint.snoop(watch_explode=('foo', 'self'))

    (see Advanced Usage in the README for more control)

    Show snoop lines for functions that your function calls::

        @fckprint.snoop(depth=2)

    Start all snoop lines with a prefix, to grep for them easily::

        @fckprint.snoop(prefix='ZZZ ')

...
class HiguchiEstimator(BaseEstimator):
    ...
    def _calculate_higuchi_dimension_jax(self, data: jnp.ndarray, k: int) -> jnp.ndarray:
        ...
        def calculate_for_m(m):
            # Calculate sum for this m
            j_range = jnp.arange(1, int((n - m) / k))
            
            def calculate_for_j(j):
                idx1 = m + (j - 1) * k
                idx2 = m + j * k
                return jnp.where(idx2 < n, jnp.abs(data[idx2] - data[idx1]), 0.0)
            
            sum_val = jnp.sum(jax.vmap(calculate_for_j)(j_range))
            return sum_val

        # Calculate for all m values
        m_range = jnp.arange(k)
        l_values = jax.vmap(calculate_for_m)(m_range)
        
        # Filter positive values and compute mean
        positive_mask = l_values > 0
        if jnp.any(positive_mask):
            return jnp.mean(l_values[positive_mask])
        else:
            return jnp.array(jnp.nan)

"""
🚀 这个B班值不值得上
一个帮助你评估工作真实价值的工具，综合考虑薪资、工时、通勤、工作环境等多个因素
"""
...
# 基本信息
PACKAGE_NAME = "b-class-worth"
TOOL_NAME = "这个B班值不值得上"
VERSION = "0.1.0"
AUTHOR = "Alvin"
AUTHOR_EMAIL = "alvin@example.com"
DESCRIPTION = "一个帮助你评估工作真实价值的工具，综合考虑薪资、工时、通勤、工作环境等多个因素"
...
def calculate_job_worth(
    ...
    try:
        ...
        # 评估结论 - 使用更口语化的表达
        if total_score >= 80:
            conclusion = "这班值得上！钱给到位了，继续保持！" 
        elif total_score >= 60:
            conclusion = "这班还能忍，有提升空间，骑驴找马吧。"
        elif total_score >= 40:
            conclusion = "这班有点坑，但暂时没更好的选择，先苟着。"
        else:
            conclusion = "这B班不值得上！赶紧跑路找更好的机会！"
...
def compare_jobs(job1: dict, job2: dict) -> dict:
    ...
    try:
        ...
        comparison = {
            "job1": result1,
            "job2": result2,
            "recommendation": "第一份班更值得上" if result1["total_score"] > result2["total_score"] else "第二份班更值得上"
        }
...
def main():
    ...
    # 列出所有可用的工具函数名称
    print("\n可用的工具函数:")
    print("- calculate_job_worth: 计算这个B班值不值得上")
    print("- compare_jobs: 比较两份工作哪个更值得上")

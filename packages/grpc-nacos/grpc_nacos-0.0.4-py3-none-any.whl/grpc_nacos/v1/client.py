...
from .internals import NacosClient as CoreNacosClient
from .internals.listener import SubscribeListener
...
class NacosClient:
    def __init__(
        ...
        self._naming_service: Optional[CoreNacosClient] = None
    ...
    @property
    def naming_service(self) -> CoreNacosClient:
    ...
    def init(self) -> None:
        if self._naming_service is not None:
            return
        self._naming_service = CoreNacosClient(
        ...
        if self._is_setup:
            self.naming_service.subscribe(
                listener_fn=SubscribeListener(
                    fn=lambda event, instance: self._service_info_cache.clear_service(
                        service_name=service_name,
                        group_name=group_name,
                        clusters=clusters_str,
                    ),

...
class NacosChannel:
    def __init__(
        ...
        *,
        ...
        self._channels: OrderedDict[tuple[str, int], grpc.Channel] = OrderedDict()
        self._closed = False
    ...
    def _get_or_create_channel(
        ...
        target = f"{host}:{port}"
        if creds is None:
            new_channel = grpc.insecure_channel(target, options=self._options)
        else:
            new_channel = grpc.secure_channel(target, creds, options=self._options)
        ...
        if self._max_idle_channels > 0 and len(self._channels) >= self._max_idle_channels:
            ...
            try:
                threading.Thread(target=old_channel.close, daemon=True).start()
            except Exception:
                pass
        self._channels[key] = new_channel
        return new_channel

    def _pick_channel(self) -> grpc.Channel:
        if self._closed:
            raise RuntimeError("NacosChannel is closed")
        endpoint = self._nacos.lb_select_one_healthy_instance(
...
class _BaseCallable:
    ...
    def _get_underlying(self, method_name: str):
        ch = self._parent._pick_channel()
...
class _UnaryUnaryCallable(_BaseCallable):
    def __call__(
        ...
        *,
        ...
        mc = self._get_underlying("unary_unary")
        return mc(
            request,
            timeout=timeout,
            metadata=metadata,
...
class _StreamUnaryCallable(_BaseCallable):
    def __call__(
        ...
        *,
        ...
        mc = self._get_underlying("stream_unary")
        return mc(
            request_iterator,
            timeout=timeout,
            metadata=metadata,

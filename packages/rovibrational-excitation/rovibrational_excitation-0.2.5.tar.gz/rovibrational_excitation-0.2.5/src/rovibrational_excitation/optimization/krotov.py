...
def run_krotov_optimization(*, basis, hamiltonian, dipole, states: dict[str, Any], time_cfg: dict, params: dict) -> RunResult:
    ...
    lambda_a = float(params.get("lambda_a", 1e-20))
    ...
    for it in range(max_iter):
        ...
        for i in range(n_traj):
            ...
            psi_i = psi_traj[i]
            chi_i = chi_traj[i]
            grad_x = 2.0 * float(np.imag(np.vdot(chi_i, (mu_x_prime @ psi_i))))
            grad_y = 2.0 * float(np.imag(np.vdot(chi_i, (mu_y_prime @ psi_i))))
            S = float(S_t[jf])

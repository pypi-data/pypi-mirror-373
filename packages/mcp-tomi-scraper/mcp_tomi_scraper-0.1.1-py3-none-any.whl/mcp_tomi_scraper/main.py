...
class TransparentLabelScraper:
    def __init__(self, headless=True, state_file_path=None):
        self.base_url = "https://tid.51tomi.com/newView/NewproductSearch"
        self.headless = headless
        # 如果没有指定路径，则使用默认的 state_file.json
        # 这个文件应该和 main.py 在同一个包目录下
        self.package_state_file = Path(__file__).parent / "tomi_login_state.json"
        
        # 关键：使用 /tmp 目录作为运行时状态文件
        self.runtime_state_file = Path(state_file_path or "/tmp/tomi_login_state.json")

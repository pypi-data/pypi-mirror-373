...
class MCPServerManager:
    ...
    async def ensure_servers_installed(self):
        """Ensure all required MCP servers are installed."""
        logger.info("Checking MCP server installations...")
    ...
    async def _ensure_server_installed(self, server_id: str, server_info: Dict):
        """Ensure a specific MCP server is installed."""
        ...
        try:
            subprocess.run(["uvx", "install", package], check=True, timeout=60)
            logger.info(f"✅ {description} installed successfully")
        except subprocess.CalledProcessError as e:
            logger.warning(f"Failed to install {description}: {e}")
        except subprocess.TimeoutExpired:
            logger.warning(f"Timeout installing {description}")

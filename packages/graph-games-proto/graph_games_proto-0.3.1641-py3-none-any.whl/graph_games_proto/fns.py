...
class LegalAction(PClass):
    ...
    def get_default_action(self, state, created_at):
        if self.keep:
            return ActionKeep.get_default_action(self, state, created_at)
        return None
    def get_public(self, state):
        return PublicLegalAction(
            player_idx=self.player_idx,
    ...
    def __fromdict__(d):
        return LegalAction(
            ...
            allotted_seconds=d.get("allotted_seconds", 120),  # Handle None case
...
class ActionKeep(PClass):
    ...
    @staticmethod
    def get_default_action(legal_action, state, created_at):
...
def get_allotted_times(s):
    return [
        get_player_allotted_time(s, player_idx) 
        for player_idx in range(s.game_config.num_players)
    ]


def get_player_allotted_time(s, player_idx):
    max_alotted_seconds = 0
    for legal_action in s.legal_actions_2:
        if legal_action.player_idx == player_idx:
            max_alotted_seconds = max(max_alotted_seconds, legal_action.allotted_seconds)
...
def getpublicstate(s):
    return PublicState(
        allotted_times=get_allotted_times(s),

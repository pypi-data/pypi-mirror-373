...
source "qemu" "simple-bios" {
  accelerator  = "kvm"
  # `auto` must come first as it serves as a boot option and not as a parameter.
  # See https://www.debian.org/releases/bullseye/amd64/apbs02.en.html for the kernel arguments and what they mean.
  boot_command = [
    "<wait><esc><wait>auto preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg ",
    "hostname={{ .Name }}<enter>"
  ]
  ...
  memory            = 1024
  machine_type      = "q35"
  output_directory  = "dist/simple-bios"
...
build {
  sources = ["source.qemu.simple-bios"]
  provisioner "shell" {
    execute_command = "echo 'virtomate' | sudo -S sh -c '{{ .Vars }} {{ .Path }}'"
    script          = "simple-bios/compact.sh"
  }
}

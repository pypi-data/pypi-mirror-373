...
async def simulate_session_workflow(mcp_client: MCPTestClient, 
                                  project_path: str = "/tmp/test-project") -> Dict[str, Any]:
    ...
    try:
        ...
        init_result = await mcp_client.call_tool('init', {
...
def create_mock_reflection_database():
    ...
    mock_db.get_reflection_stats = AsyncMock(return_value={
        'total_reflections': 0,
        'projects': 0,
        'date_range': None
    })
...
class MockMCPServer:
    ...
    async def _mock_store_reflection(self, args):
        ...
        return {
            'success': True,
            'reflection_id': len(self.state['reflections'])
        }

# Test assertion helpers

def assert_tool_call_made(mcp_client: MCPTestClient, tool_name: str, 
...
def assert_tool_call_count(mcp_client: MCPTestClient, tool_name: str, expected_count: int):

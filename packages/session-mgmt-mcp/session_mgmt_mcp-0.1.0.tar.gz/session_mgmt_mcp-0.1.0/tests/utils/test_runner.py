...
class TestRunner:
    ...
    def run_test_suite(self, 
        ...
        try:
            ...
            test_details = self.parse_junit_xml(junit_file) if junit_file.exists() else {}
        ...
        try:
            ...
            for testcase in root.findall('.//testcase'):
                ...
                if testcase.find('failure') is not None:
                    ...
                    test_info['failure_message'] = testcase.find('failure').get('message')
        ...
        try:
            ...
            if coverage_json.exists():
                ...
                coverage_data['summary'] = {
                    ...
                    'branch_coverage_percent': summary.get('percent_covered_branches', 0)
                }
    ...
    def calculate_quality_metrics(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for suite_name, suite_results in test_results.get('test_results', {}).items():
                if 'test_details' in suite_results:
                    details = suite_results['test_details']
                    total_tests += details.get('total_tests', 0)
                    failed_tests += details.get('failures', 0) + details.get('errors', 0)
                    passed_tests += details.get('total_tests', 0) - details.get('failures', 0) - details.get('errors', 0) - details.get('skipped', 0)
            ...
            # Performance metrics
            performance_results = test_results.get('test_results', {}).get('performance', {})
            metrics['performance_metrics'] = {
                'performance_tests_passed': performance_results.get('success', False),
                'performance_execution_time': performance_results.get('execution_time', 0),
                'performance_issues': []
            }
            ...
            score_components = {
                ...
                'performance_score': (100 if metrics['performance_metrics']['performance_tests_passed'] else 0) * 0.2,
                'security_score': (100 if test_results.get('test_results', {}).get('security', {}).get('success', False) else 0) * 0.1
            }
        ...
        try:
            ...
            for suite_name, suite_results in results.get('test_results', {}).items():
                suite_summary = {
                    ...
                    'test_count': suite_results.get('test_details', {}).get('total_tests', 0),
                    'failure_count': suite_results.get('test_details', {}).get('failures', 0) + suite_results.get('test_details', {}).get('errors', 0)
                }
            ...
            # Quality score
            summary['quality_score'] = results.get('quality_metrics', {}).get('overall_score', 0)
    ...
    def generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if line_coverage < 85:
                recommendations.append(f"Increase line coverage from {line_coverage:.1f}% to at least 85%")
            
            if coverage.get('branch_coverage_percent', 0) < 80:
                recommendations.append(f"Improve branch coverage to at least 80%")
            
            # Test failure recommendations
            for suite_name, suite_results in results.get('test_results', {}).items():
                if not suite_results.get('success', False):
                    failure_count = suite_results.get('test_details', {}).get('failures', 0)
                    error_count = suite_results.get('test_details', {}).get('errors', 0)
                    
                    if failure_count > 0:
                        recommendations.append(f"Fix {failure_count} failing tests in {suite_name} suite")
                    
                    if error_count > 0:
                        recommendations.append(f"Resolve {error_count} test errors in {suite_name} suite")
            ...
            if quality_score < 80:
                recommendations.append(f"Improve overall quality score from {quality_score:.1f} to at least 80")
    ...
    def run_quick_tests(self) -> Dict[str, Any]:
        ...
        return self.run_test_suite('smoke', coverage=False, parallel=True, verbose=False)
    
    def run_regression_tests(self) -> Dict[str, Any]:
        """Run regression tests"""
        return self.run_test_suite('regression', coverage=True, parallel=True, verbose=True)
...
def run_test_harness():
    ...
    for suite_name, suite_summary in summary.get('test_suites', {}).items():
        ...
        print(f"{status_emoji} {suite_name}: {suite_summary['test_count']} tests, {suite_summary['execution_time']:.2f}s")

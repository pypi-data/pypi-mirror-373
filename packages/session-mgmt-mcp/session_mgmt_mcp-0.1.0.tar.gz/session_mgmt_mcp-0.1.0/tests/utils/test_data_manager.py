...
class TestDataManager:
    ...
    @contextmanager
    def temp_directory(self, prefix: str = "session_mgmt_test_") -> Generator[Path, None, None]:
    ...
    @contextmanager
    def temp_file(self, suffix: str = ".tmp", prefix: str = "test_") -> Generator[Path, None, None]:
        """Create and manage temporary file"""
        temp_file = Path(tempfile.mktemp(suffix=suffix, prefix=prefix, dir=self.base_temp_dir))
...
""")
    ...
    def generate_test_dataset(self, dataset_type: str, size: str = "small") -> List[Dict]:
        ...
        try:
            ...
            for table in expected_tables:
                if table not in table_names:
                    ...
                    count_result = await db._execute_query(f"SELECT COUNT(*) FROM {table}")
                    integrity_status['table_counts'][table] = count_result[0][0] if count_result else 0
...
class PerformanceTestDataManager:
    ...
    async def generate_large_dataset_for_testing(self, 

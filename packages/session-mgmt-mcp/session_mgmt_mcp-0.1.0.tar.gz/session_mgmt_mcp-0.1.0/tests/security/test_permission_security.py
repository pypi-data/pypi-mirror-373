...
class TestSessionPermissionSecurity:
        ...
        for malicious_input in malicious_inputs:
            try:
                if malicious_input is None:
                    ...
                    assert permissions_manager.is_trusted_operation(malicious_input) is True
    ...
    def test_permission_state_tampering_protection(self, permissions_manager):
        ...
        original_operations = permissions_manager.trusted_operations.copy()
        ...
        assert permissions_manager.is_trusted_operation('properly_trusted_operation') is True
...
class TestDatabaseSecurity:
    ...
    async def test_sql_injection_prevention(self, secure_database):
        ...
        for payload in sql_injection_payloads:
            ...
            assert result is True, f"Failed to handle SQL injection in project: {payload}"
            ...
            assert isinstance(search_results, list), f"SQL injection in search: {payload}"
    ...
    async def test_input_sanitization(self, secure_database):
        ...
        for dangerous_input in dangerous_inputs:
            ...
            assert result is True, f"Failed to store dangerous input: {dangerous_input[:50]}"
            ...
            assert found_content, f"Dangerous input was corrupted during storage: {dangerous_input[:50]}"
    ...
    async def test_concurrent_access_security(self, secure_database):
        ...
        malicious_count = sum(1 for r in all_results if "malicious" in r.get('project', ''))
        legitimate_count = sum(1 for r in all_results if "legitimate" in r.get('project', ''))
    ...
    async def test_embedding_vector_security(self, secure_database):
        ...
        for embedding in malformed_embeddings:
            try:
                ...
                assert isinstance(result, bool), f"Unexpected result type for embedding: {type(embedding)}"
...
class TestRateLimitingSecurity:
    ...
    def test_basic_rate_limiting(self, rate_limiter):
        ...
        for i in range(rate_limiter.max_requests):
            assert rate_limiter.is_allowed() is True, f"Request {i+1} should be allowed"
        
        # Should block additional requests
        assert rate_limiter.is_allowed() is False, "Request beyond limit should be blocked"
    ...
    async def test_concurrent_rate_limiting(self, rate_limiter):
        ...
        # Flatten results
        flat_results = [result for task_results in all_results for result in task_results]

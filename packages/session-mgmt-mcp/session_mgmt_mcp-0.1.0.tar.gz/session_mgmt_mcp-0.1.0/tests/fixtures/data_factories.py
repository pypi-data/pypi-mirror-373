...
class SessionDataFactory(factory.Factory):
    ...
    # Health metrics
    health_checks = factory.LazyFunction(lambda: {
...
class ReflectionDataFactory(factory.Factory):
    ...
    tags = factory.LazyFunction(
        lambda: random.sample([
            ...
            'testing', 'deployment', 'security', 'performance', 'bug-fix',
            'feature', 'refactoring', 'documentation', 'monitoring'
    ...
    conversation_length = factory.LazyAttribute(
        lambda obj: random.randint(100, 2000)
    )
    tool_calls_count = factory.LazyAttribute(
        lambda obj: random.randint(0, 20)
    )
    error_count = factory.LazyAttribute(
        lambda obj: random.randint(0, 3)
    )
...
class UserDataFactory(factory.Factory):
    ...
    # User preferences
    preferred_language = factory.Iterator(['python', 'javascript', 'typescript', 'go', 'rust'])
...
class ProjectDataFactory(factory.Factory):
    ...
    path = factory.LazyAttribute(
        lambda obj: f"/Users/test/Projects/{obj.name}"
    )
    type = factory.Iterator(['python', 'javascript', 'typescript', 'go', 'rust', 'java'])
    framework = factory.LazyAttribute(lambda obj: {
...
class ErrorDataFactory(factory.Factory):
    ...
    error_type = factory.Iterator([
        ...
        'PermissionError', 'FileNotFoundError', 'DatabaseError'
    ])
...
class PerformanceDataFactory(factory.Factory):
    ...
    operation_name = factory.Iterator([
        'database_query', 'api_call', 'file_read', 'network_request',
        'computation', 'memory_allocation'
    ])
...
class LargeDatasetFactory(factory.Factory):
    ...
    def generate_conversation_history(cls, days: int = 30) -> List[Dict]:
        ...
        for day in range(days):
            day_reflections = ReflectionDataFactory.build_batch(
                random.randint(1, 10)
            )
...
class SecurityTestDataFactory(factory.Factory):
    ...
    operation = factory.Iterator([
        'read_reflections', 'write_reflections', 'delete_reflections',
        'session_init', 'session_checkpoint', 'session_end',
        'database_access', 'file_system_access'
    ])

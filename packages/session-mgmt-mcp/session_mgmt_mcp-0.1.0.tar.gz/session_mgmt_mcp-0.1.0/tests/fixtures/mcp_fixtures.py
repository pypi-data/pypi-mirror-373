...
def mock_workspace_validation():
    """Mock global workspace validation"""
    with patch('session_mgmt_mcp.server.validate_global_workspace') as mock:
        mock.return_value = {
            'exists': True,
...
@pytest.fixture
async def populated_test_database(isolated_database, sample_conversation_data, sample_reflection_data):
...
def performance_metrics_collector():
    ...
    def record_execution_time(operation: str, duration: float):

...
class TestReflectionDatabase:
    ...
    async def test_concurrent_operations(self, temp_db):
        ...
        search_results = await temp_db.search_reflections(
            query="concurrent",
            limit=20
        )
    ...
    async def test_special_characters_handling(self, temp_db):
        ...
        special_content = "Test with special chars: 'quotes', \"double quotes\", <tags>, & ampersand"
    ...
    async def test_performance_large_dataset(self, temp_db, performance_monitor):
        ...
        # Perform searches
        for i in range(10):
            await temp_db.search_reflections(
                query=f"test {i}",
                limit=5
            )
...
class TestReflectionDatabaseWithEmbeddings:
    ...
    async def temp_db_with_embeddings(self):
        ...
        with patch('session_mgmt_mcp.reflection_tools.ReflectionDatabase._load_embedding_model') as mock_load:
            mock_model = Mock()
            mock_tokenizer = Mock()

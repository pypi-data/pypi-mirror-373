...
from session_mgmt_mcp.server import store_reflection, reflect_on_past, quick_search
...
class TestReflectionSystemProperties:
    ...
    @settings(max_examples=50, deadline=30000)  # 30 second deadline for async operations
    @pytest.mark.asyncio
    ...
    @given(
        project_names=st.lists(project_name_strategy, min_size=1, max_size=10, unique=True)
    )
...
class TestMCPToolProperties:
    ...
    async def test_concurrent_mcp_operations_properties(self, batch_size):
        ...
        with patch('session_mgmt_mcp.reflection_tools.ReflectionDatabase') as mock_db:
            ...
            search_tasks = [
                quick_search(query=f"search query {i}")
                for i in range(batch_size)
            ]

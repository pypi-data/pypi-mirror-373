...
def sample_conversations():
    ...
    return [
        ...
        {
            'id': 'conv2', 
            'content': 'This is a much longer conversation that discusses various aspects of software development, including database design, API architecture, testing strategies, and deployment practices. ' * 10,
...
class TestTokenOptimizer:
    ...
    def test_chunk_large_response(self, token_optimizer):
        ...
        for i in range(10):
            large_conversations.append({
                'id': f'conv{i}',
                'content': 'This is a long conversation that contains lots of text. ' * 20,
                'timestamp': datetime.now().isoformat()
            })
        ...
        if info['action'] == 'chunked':
            assert len(optimized) < len(large_conversations)  # Should return first chunk
    ...
    def test_create_quick_summary(self, token_optimizer):
        ...
        long_text = (
            ...
            "Here are many sentences in between that contain various details. " * 5 +
    ...
    def test_usage_stats(self, token_optimizer):
        ...
        token_optimizer.usage_history = [
            TokenUsageMetrics(100, 200, 300, (now - timedelta(hours=1)).isoformat(), 'op1', 'truncate_old'),
            TokenUsageMetrics(150, 250, 400, now.isoformat(), 'op2', None),
            TokenUsageMetrics(80, 120, 200, (now - timedelta(days=2)).isoformat(), 'op3', 'summarize')
        ]

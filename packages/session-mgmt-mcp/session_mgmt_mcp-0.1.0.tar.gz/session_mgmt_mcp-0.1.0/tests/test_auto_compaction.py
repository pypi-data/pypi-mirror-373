...
class TestAutoCompaction:
    ...
    async def test_token_analysis_recommends_compaction(self):
        ...
        assert result['recommend_compact'] == True, "Token analysis should recommend compaction"
        assert result['needs_attention'] == True, "Token analysis should flag attention needed"
        assert result['status'] == 'needs optimization', "Status should indicate optimization needed"
    ...
    async def test_strategic_compaction_includes_conversation(self):
        ...
        assert 'conversation compaction' in result_text, "Should include conversation compaction section"
    ...
    async def test_enhanced_session_insights(self):
        ...
        result_text = ' '.join(results).lower()
        assert any(keyword in result_text for keyword in ['topics', 'decisions', 'next steps']), \
...
class TestCompactionWorkflow:
    ...
    async def test_compaction_recommendation_flow(self):
        ...
        assert 'conversation compaction' in compaction_text, "Step 2: Should prepare conversation compaction"
        
        # Step 3: Conversation summary should be generated
        summary = await summarize_current_conversation()
        assert len(summary['key_topics']) > 0, "Step 3: Should generate conversation summary"
    ...
    async def test_checkpoint_integration(self):
        ...
        assert token_analysis['recommend_compact'], "Token analysis should trigger compaction"
        assert len(conversation_summary['key_topics']) > 0, "Conversation summary should have content"
        
        compaction_text = ' '.join(strategic_compaction).lower()
        assert 'compaction' in compaction_text, "Strategic compaction should handle compaction"

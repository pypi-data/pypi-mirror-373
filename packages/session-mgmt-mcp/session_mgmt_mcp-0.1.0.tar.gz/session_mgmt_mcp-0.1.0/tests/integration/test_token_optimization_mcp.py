...
def mock_reflection_db():
    ...
    sample_conversations = [
        ...
        {
            'id': 'conv2',
            'content': 'This is a much longer conversation about software architecture. ' * 20,
...
class TestReflectOnPastOptimization:
    ...
    async def test_reflect_on_past_with_optimization(self, mock_reflection_db):
        ...
        with patch('session_mgmt_mcp.server.get_reflection_database') as mock_get_db, \
            ...
            mock_optimize.return_value = (
                mock_reflection_db.search_conversations.return_value[:2],  # Optimized results
                {
    ...
    @pytest.mark.asyncio 
    async def test_reflect_on_past_optimization_error_handling(self, mock_reflection_db):
    ...
    @pytest.mark.asyncio
    async def test_reflect_on_past_token_optimizer_unavailable(self, mock_reflection_db):
...
class TestCachedChunkRetrieval:
    ...
    async def test_get_cached_chunk_success(self):
        ...
        with patch('session_mgmt_mcp.server.TOKEN_OPTIMIZER_AVAILABLE', True), \
...
class TestOptimizeMemoryUsage:
    ...
    async def test_optimize_memory_usage_dry_run(self):
        ...
        with patch('session_mgmt_mcp.server.TOKEN_OPTIMIZER_AVAILABLE', True), \
            ...
            with patch('session_mgmt_mcp.server.MemoryOptimizer') as mock_optimizer_class:
    ...
    async def test_optimize_memory_usage_dependencies_unavailable(self):
        ...
        with patch('session_mgmt_mcp.server.TOKEN_OPTIMIZER_AVAILABLE', False):
            ...
            assert "❌ Memory optimization requires both token optimizer and reflection tools" in result

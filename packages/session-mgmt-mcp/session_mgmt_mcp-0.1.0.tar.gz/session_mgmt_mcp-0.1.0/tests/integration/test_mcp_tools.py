...
class TestMCPSessionTools:
    ...
    async def mcp_test_environment(self):
        ...
        env.add_mock_patch('os.environ.get', side_effect=lambda key, default=None: {
    ...
    async def test_session_init_tool(self, mcp_client, mcp_test_environment):
        ...
        # Call init tool
        result = await mcp_client.call_tool('init', {
        ...
        assert_tool_call_made(mcp_client, 'init', {
            'working_directory': str(test_project)
        })
    ...
    async def test_reflection_storage_integration(self, mcp_client):
        ...
        store_result = await mcp_client.call_tool('store_reflection', {
        ...
        search_result = await mcp_client.call_tool('reflect_on_past', {
    ...
    async def test_concurrent_tool_calls(self, mcp_client):
        ...
        for i in range(3):
            task = mcp_client.call_tool('store_reflection', {
...
class TestMCPReflectionTools:
    ...
    async def test_reflection_search_by_content(self, mcp_client_with_reflections):
        ...
        result = await client.call_tool('reflect_on_past', {
        ...
        assert 'oauth2' in auth_result['content'].lower() or \
               'authentication' in auth_result['content'].lower()
    ...
    async def test_reflection_storage_with_tags(self, mcp_client_with_reflections):
        ...
        result = await client.call_tool('store_reflection', {
        ...
        search_result = await client.call_tool('reflect_on_past', {
            'query': 'websocket real-time'
        })
        
        assert len(search_result['results']) >= 1
        websocket_result = next(
            (r for r in search_result['results'] if 'websocket' in r['content'].lower()),
            None
        )
    ...
    @pytest.mark.asyncio
    async def test_reflection_search_case_insensitive(self, mcp_client_with_reflections):
...
class TestMCPPerformanceIntegration:
    ...
    async def test_high_volume_reflections(self, performance_monitor):
        ...
        tasks = []
        for i in range(100):
            task = client.call_tool('store_reflection', {
        ...
        search_tasks = []
        for i in range(20):
            task = client.call_tool('reflect_on_past', {
                'query': f'performance-{i % 10}',
                'limit': 5
            })
    ...
    async def test_concurrent_session_operations(self):
        ...
        async def session_workflow(client, client_id):
            ...
            # Initialize
            init_result = await client.call_tool('init', {
            ...
            for i in range(3):
                ...
                operations.append(client.call_tool('store_reflection', {
        ...
        for batch in range(10):
            ...
            for i in range(20):
                if i % 4 == 0:
                    ...
                    batch_tasks.append(client.call_tool('store_reflection', {
                        'content': f'Batch {batch} reflection {i}',
                        'project': 'memory-test'
                    }))
                elif i % 4 == 2:
                    batch_tasks.append(client.call_tool('reflect_on_past', {
                        'query': f'batch {batch % 3}',
                        'limit': 3
                    }))
                else:

...
class TestReflectionDatabasePerformance:
    ...
    @pytest.mark.asyncio
    async def test_single_reflection_storage_performance(self, perf_database, performance_tracker):
        ...
        assert max(durations) < 0.5, f"Maximum storage time too slow: {max(durations):.4f}s"
        assert metrics['duration'] < 15.0, f"Total time too slow: {metrics['duration']:.2f}s"
        assert metrics['memory_delta'] < 50, f"Memory usage too high: {metrics['memory_delta']:.2f}MB"
    
    @pytest.mark.asyncio
    async def test_bulk_reflection_storage_performance(self, perf_database, performance_tracker):
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            assert all(results), f"Some storage operations failed in batch {i//batch_size}"
        ...
        assert metrics['memory_delta'] < 100, f"Memory delta too high: {metrics['memory_delta']:.2f}MB"
    
    @pytest.mark.asyncio
    async def test_search_performance_small_dataset(self, perf_database, performance_tracker):
        ...
        assert avg_search_time < 0.05, f"Average search too slow: {avg_search_time:.4f}s"
        assert max(durations) < 0.2, f"Maximum search too slow: {max(durations):.4f}s"
    
    @pytest.mark.asyncio
    async def test_search_performance_large_dataset(self, perf_database, performance_tracker):
        """Test search performance on large dataset"""
        # Populate with large dataset
        reflection_count = 5000
        reflections = LargeDatasetFactory.generate_large_reflection_dataset(reflection_count)
        ...
        search_queries = [
            "authentication", "database", "api", "testing", "performance",
            "security", "optimization", "implementation", "feature", "bug"
        ]
        ...
        assert avg_search_time < 0.5, f"Large dataset search too slow: {avg_search_time:.4f}s"
        assert max(search_durations) < 2.0, f"Max search time too slow: {max(search_durations):.4f}s"
    
    @pytest.mark.asyncio
    async def test_concurrent_read_performance(self, perf_database, performance_tracker):
        ...
        async def perform_searches(search_id):
            durations = []
            queries = [f"search {search_id} test", f"concurrent {search_id}", f"performance {search_id}"]
        ...
        all_durations = [duration for worker_durations in worker_results for duration in worker_durations]
        avg_concurrent_duration = statistics.mean(all_durations)
        
        assert avg_concurrent_duration < 0.1, f"Concurrent search too slow: {avg_concurrent_duration:.4f}s"
        assert metrics['duration'] < 30, f"Total concurrent test too slow: {metrics['duration']:.2f}s"
    
    @pytest.mark.asyncio
    async def test_mixed_read_write_performance(self, perf_database, performance_tracker):
        ...
        read_durations = [duration for read_result in results[1:] for duration in read_result]
        ...
        assert avg_write_time < 0.1, f"Write performance degraded: {avg_write_time:.4f}s"
        assert avg_read_time < 0.1, f"Read performance degraded: {avg_read_time:.4f}s"
        assert metrics['duration'] < 45, f"Mixed workload took too long: {metrics['duration']:.2f}s"
    ...
    async def test_memory_usage_growth(self, perf_database):
        ...
        for count in reflection_counts:
            ...
            for i in range(10):
                await perf_database.search_reflections(
                    query=f"test {i}",
                    limit=5
                )
        ...
        assert avg_growth_rate < 0.1, f"Memory growth rate too high: {avg_growth_rate:.4f} MB per reflection"
    ...
    async def test_database_file_size_growth(self, perf_database):
        ...
        for i, (count, size) in enumerate(file_sizes):
            ...
            assert size_per_reflection < 10, f"File size per reflection too large: {size_per_reflection:.2f} KB"
...
class TestDatabaseQueryOptimization:
    ...
    @pytest.mark.asyncio
    async def test_indexed_vs_unindexed_performance(self, indexed_database, performance_tracker):
        ...
        query_patterns = [
            ...
            ('content_search', 'SELECT * FROM reflections WHERE content LIKE ? LIMIT 10')
        ]

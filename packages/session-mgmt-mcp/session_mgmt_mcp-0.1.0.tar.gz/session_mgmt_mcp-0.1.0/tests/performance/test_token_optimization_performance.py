...
def large_conversations_dataset():
    ...
    content_templates = [
        ...
        "This is a medium conversation discussing various aspects of software development including testing strategies and API design. " * 5,
        "This is a very long conversation that covers many topics in software engineering including database design, scalability patterns, microservices architecture, and deployment strategies. " * 15,
...
class TestTokenOptimizerPerformance:
    ...
    def test_token_counting_performance(self):
        ...
        for size in text_sizes:
            ...
            assert duration < 0.1, f"Token counting too slow for {size} chars: {duration:.3f}s"
    ...
    def test_deduplication_performance(self):
        ...
        for i in range(500):
            conversations.append({
                'id': f'conv_{i}',
                'content': base_content + f" variation {i % 10}",  # Only 10 unique variations
                'timestamp': datetime.now().isoformat()
            })
        
        start_time = time.time()
        optimized, info = optimizer._filter_duplicate_content(conversations, max_tokens=10000)
        end_time = time.time()
...
class TestCachePerformance:
    ...
    def test_chunk_cache_creation_performance(self):
        ...
        assert retrieval_duration < 0.1, f"Cache retrieval too slow: {retrieval_duration:.3f}s"
    ...
    def test_cache_cleanup_performance(self):
        ...
        for i in range(100):
            chunks = [[{'id': f'conv_{i}', 'content': f'Content {i}'}]]
            cache_key = f'key_{i}'
            ...
            chunk_result = ChunkResult(
                ...
                metadata={
                    ...
                    'expires': (datetime.now() - timedelta(minutes=i)).isoformat()  # Varying expiration
                }
            )
...
class TestMemoryUsageOptimization:
    ...
    def test_memory_efficiency_large_dataset(self, large_conversations_dataset):
        ...
        for strategy in ['truncate_old', 'prioritize_recent', 'filter_duplicates']:
            ...
            optimized, info = strategy_func(large_conversations_dataset, max_tokens=3000)
            
            current_memory = process.memory_info().rss / 1024 / 1024  # MB
            memory_increase = current_memory - initial_memory
            
            # Memory increase should be reasonable (less than 50MB for this dataset)
            assert memory_increase < 50, f"Memory usage too high for {strategy}: {memory_increase:.1f}MB"
    ...
    def test_token_counting_memory_efficiency(self):
        ...
        assert memory_increase < 10, f"Token counting memory inefficient: {memory_increase:.1f}MB"
...
class TestScalabilityBenchmarks:
    ...
    def test_optimization_scalability(self, dataset_size):
        ...
        start_time = time.time()
        optimized, info = optimizer._truncate_old_conversations(conversations, max_tokens=2000)
        duration = time.time() - start_time
        
        # Performance should scale reasonably (not exponentially)
        max_expected_time = dataset_size * 0.005  # 5ms per conversation maximum
        assert duration < max_expected_time, f"Poor scaling for {dataset_size} items: {duration:.3f}s"
    ...
    def test_concurrent_optimization_performance(self, large_conversations_dataset):
        ...
        assert total_duration < 5.0, f"Concurrent optimization too slow: {total_duration:.3f}s"

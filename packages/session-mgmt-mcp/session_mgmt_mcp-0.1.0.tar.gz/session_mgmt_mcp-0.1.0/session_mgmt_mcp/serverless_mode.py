...
class SessionStorage(ABC):
    ...
    @abstractmethod
    async def store_session(self, session_state: SessionState, ttl_seconds: Optional[int] = None) -> bool:
        """Store session state with optional TTL"""
        pass
    
    @abstractmethod
    async def retrieve_session(self, session_id: str) -> Optional[SessionState]:
...
class RedisStorage(SessionStorage):
    ...
    async def store_session(self, session_state: SessionState, ttl_seconds: Optional[int] = None) -> bool:
        ...
        try:
            ...
            project_index_key = self._get_index_key(f"project:{session_state.project_id}")
    ...
    async def list_sessions(self, user_id: Optional[str] = None, 
        ...
        try:
            ...
            return [key.decode('utf-8').split(':')[-1] if isinstance(key, bytes) else key.split(':')[-1] for key in keys]
...
class LocalFileStorage(SessionStorage):
    ...
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.storage_dir = Path(config.get('storage_dir', Path.home() / '.claude' / 'data' / 'sessions'))
...
class ServerlessSessionManager:
    ...
    async def create_session(self, user_id: str, project_id: str, 
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'storage_config': {k: v for k, v in self.storage.config.items() if 'key' not in k.lower()}
        }

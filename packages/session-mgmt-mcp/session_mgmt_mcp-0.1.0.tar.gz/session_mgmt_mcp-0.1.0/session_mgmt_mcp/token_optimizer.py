...
class TokenOptimizer:
    ...
    def optimize_search_results(self, results: List[Dict[str, Any]], 
        ...
        if strategy in self.strategies:
            optimized_results, optimization_info = self.strategies[strategy](results, max_tokens)
        else:
        ...
        optimization_info['token_savings'] = self._calculate_token_savings(results, optimized_results)
        
        return optimized_results, optimization_info
    
    def _truncate_old_conversations(self, results: List[Dict[str, Any]], 
        ...
        for result in sorted_results:
            ...
            if current_tokens + content_tokens > max_tokens:
                ...
                if len(optimized_results) < 3:  # Always keep at least 3 recent results
                    truncated_content = self._truncate_content(content, max_tokens - current_tokens)
                    if truncated_content:
                        result_copy = result.copy()
                        result_copy['content'] = truncated_content + "... [truncated for token limit]"
    ...
    def _summarize_long_content(self, results: List[Dict[str, Any]], 
                               max_tokens: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    def _chunk_large_response(self, results: List[Dict[str, Any]], 
                             max_tokens: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        total_tokens = sum(self.count_tokens(str(result.get('content', ''))) for result in results)
        
        if total_tokens <= max_tokens:
            return results, {'strategy': 'chunk_response', 'action': 'no_chunking_needed'}
    ...
    def _filter_duplicate_content(self, results: List[Dict[str, Any]], 
                                 max_tokens: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    def _prioritize_recent_content(self, results: List[Dict[str, Any]], 
                                  max_tokens: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        for result in results:
            ...
            if any(keyword in content.lower() for keyword in ['def ', 'class ', 'function', 'error', 'exception']):
                score += 0.2
    ...
    def get_chunk(self, cache_key: str, chunk_index: int) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_token_savings(self, original: List[Dict[str, Any]], 
                                optimized: List[Dict[str, Any]]) -> Dict[str, int]:
        """Calculate token savings from optimization"""
        original_tokens = sum(self.count_tokens(str(item.get('content', ''))) for item in original)
        optimized_tokens = sum(self.count_tokens(str(item.get('content', ''))) for item in optimized)
        
        return {
            'original_tokens': original_tokens,
            'optimized_tokens': optimized_tokens,
            'tokens_saved': original_tokens - optimized_tokens,
            'savings_percentage': round(((original_tokens - optimized_tokens) / original_tokens) * 100, 1) if original_tokens > 0 else 0
        }
    
    def track_usage(self, operation: str, request_tokens: int, response_tokens: int, 
                   optimization_applied: Optional[str] = None):
    ...
    def get_usage_stats(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for metric in recent_usage:
            if metric.optimization_applied:
                optimizations[metric.optimization_applied] = optimizations.get(metric.optimization_applied, 0) + 1
    ...
    def _estimate_cost_savings(self, usage_metrics: List[TokenUsageMetrics]) -> Dict[str, float]:
...
async def optimize_search_response(results: List[Dict[str, Any]], 
                                 strategy: str = 'prioritize_recent',
                                 max_tokens: int = 4000) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """Async wrapper for search result optimization"""
    optimizer = get_token_optimizer()
    return optimizer.optimize_search_results(results, strategy, max_tokens)


async def get_cached_chunk(cache_key: str, chunk_index: int) -> Optional[Dict[str, Any]]:
    """Async wrapper for chunk retrieval"""
    optimizer = get_token_optimizer()
    return optimizer.get_chunk(cache_key, chunk_index)


async def track_token_usage(operation: str, request_tokens: int, response_tokens: int,
                           optimization_applied: Optional[str] = None):
    """Async wrapper for usage tracking"""
    optimizer = get_token_optimizer()
    optimizer.track_usage(operation, request_tokens, response_tokens, optimization_applied)

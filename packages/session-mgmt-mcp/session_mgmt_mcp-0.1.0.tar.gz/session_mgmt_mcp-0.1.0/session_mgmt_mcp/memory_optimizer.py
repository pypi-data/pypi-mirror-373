...
class ConversationSummarizer:
    ...
    def _extractive_summarization(self, content: str, max_sentences: int = 3) -> str:
        ...
        for sentence in sentences[:20]:  # Limit to prevent performance issues
            ...
            tech_keywords = [
                'function', 'class', 'error', 'exception', 'import', 'def',
                'async', 'await', 'return', 'variable', 'method', 'api',
                'database', 'query', 'test', 'debug', 'fix', 'implement'
            ]
    ...
    def _template_based_summarization(self, content: str, max_length: int = 300) -> str:
        ...
        if code_blocks:
            summary_parts.append(f"Code discussion involving {len(code_blocks)} code block(s)")
    ...
    def _keyword_based_summarization(self, content: str, max_keywords: int = 10) -> str:
        ...
        stop_words = {
            ...
            'did', 'way', 'use', 'man', 'say', 'she', 'too', 'any', 'here', 'much',
            'where', 'your', 'them', 'well', 'were', 'been', 'have', 'there', 'what',
            'would', 'make', 'like', 'into', 'time', 'will', 'about', 'think', 'never',
            'after', 'should', 'could', 'also', 'just', 'first', 'over', 'back', 'other'
        }
    ...
    def summarize_conversation(self, content: str, strategy: str = 'template_based') -> str:
...
class ConversationClusterer:
    ...
    def cluster_conversations(self, conversations: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    ...
    def _calculate_similarity(self, conv1: Dict[str, Any], conv2: Dict[str, Any]) -> float:
...
class RetentionPolicyManager:
    ...
    def calculate_importance_score(self, conversation: Dict[str, Any]) -> float:
        ...
        score += self.importance_factors['project_relevance'] * 0.5  # Default middle score
        
        return min(score, 1.0)
    
    def get_conversations_for_retention(self, conversations: List[Dict[str, Any]], 
                                      policy: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
class MemoryOptimizer:
    ...
    async def compress_memory(self, policy: Optional[Dict[str, Any]] = None,
        ...
        keep_conversations, consolidate_conversations = self.retention_manager.get_conversations_for_retention(
        ...
        # Process clusters
        total_original_size = sum(conv['original_size'] for conv in consolidate_conversations)
        ...
        for i, cluster in enumerate(clusters):
            ...
            summary = self.summarizer.summarize_conversation(combined_content, 'template_based')
            
            # Create consolidated metadata
            projects = list(set([conv['project'] for conv in cluster if conv['project']]))
        ...
        self.compression_stats.update({
            ...
            'conversations_consolidated': sum(len(cluster) for cluster in clusters if len(cluster) > 1),
    ...
    async def _create_consolidated_conversation(self, consolidated_conv: Dict[str, Any], 
        ...
        metadata = {
            ...
            'compression_ratio': consolidated_conv['compressed_size'] / consolidated_conv['original_size']
        }
        ...
        self.reflection_db.conn.execute(
            ...
            (
                ...
                ', '.join(consolidated_conv['projects']) if consolidated_conv['projects'] else 'multiple',

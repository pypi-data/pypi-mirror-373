...
class CrackerjackOutputParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'pytest_result': re.compile(r'(\w+\.py)::\s*(\w+)\s*(PASSED|FAILED|SKIPPED|ERROR|XFAIL|XPASS)\s*(?:\[(\d+%)\])?\s*(?:\((.+)\))?'),
            ...
            'pyright_error': re.compile(r'(\S+):(\d+):(\d+)\s*-\s*(error|warning|info):\s*(.+)'),
    ...
    def parse_output(self, command: str, stdout: str, stderr: str) -> Tuple[Dict[str, Any], List[str]]:
    ...
    def _parse_progress_output(self, output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if completion_match:
                task = completion_match.group(1) or completion_match.group(2) or completion_match.group(3)
                if task:
            ...
            if failure_match:
                task = failure_match.group(1) or failure_match.group(2) or failure_match.group(3)
                if task:
    ...
    def _extract_test_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if test_results:
            ...
            if total > 0:
                ...
                insights.append(f"Test suite: {passed}/{total} tests passed ({pass_rate:.1f}% pass rate)")
                
                if failed > 0:
                    failed_files = set(t['file'] for t in test_results if t['status'] == 'failed')
                    insights.append(f"Test failures found in {len(failed_files)} files: {', '.join(failed_files)}")
                
                if pass_rate == 100:
                    insights.append("All tests passing - code quality is stable")
                elif pass_rate < 80:
                    insights.append("Test pass rate below 80% - investigate failing tests")
    ...
    def _extract_lint_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if lint_issues:
            ...
            if top_files and top_files[0][1] > 5:
                insights.append(f"Files needing attention: {top_files[0][0]} ({top_files[0][1]} issues)")
        else:
    ...
    def _extract_security_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if security_issues:
            ...
            insights.append(f"Security scan: {total_issues} potential security issues found")
            
            if high_severity > 0:
                insights.append(f"⚠️ {high_severity} high-severity security issues require immediate attention")
            else:
                insights.append("No high-severity security issues detected")
        else:
            insights.append("Security scan: No security issues detected - code appears secure")
    ...
    def _extract_coverage_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if 'total_coverage' in coverage_summary:
            ...
            if coverage >= 90:
                ...
                insights.append("Moderate test coverage - significant testing gaps exist")
            else:
                insights.append("Low test coverage - critical testing gaps need attention")
    ...
    def _extract_complexity_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if complexity_summary:
            ...
            if total_files > 0:
                ...
                insights.append(f"Code complexity: {high_complexity}/{total_files} files have high complexity ({complexity_rate:.1f}%)")
                
                if complexity_rate == 0:
                    insights.append("Code complexity is well managed")
                elif complexity_rate > 20:
                    insights.append("Consider refactoring high-complexity files for maintainability")
    ...
    def _extract_progress_insights(self, parsed_data: Dict[str, Any]) -> List[str]:
        ...
        if failed_tasks:
            insights.append(f"⚠️ {len(failed_tasks)} tasks failed: {', '.join(failed_tasks[:3])}")
...
class CrackerjackIntegration:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.db_path = db_path or str(Path.home() / ".claude" / "data" / "crackerjack_integration.db")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indices
            conn.execute("CREATE INDEX IF NOT EXISTS idx_results_timestamp ON crackerjack_results(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_results_command ON crackerjack_results(command)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_test_results_status ON test_results(status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_progress_project ON progress_snapshots(project_path)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_type ON quality_metrics_history(metric_type)")
    
    async def execute_crackerjack_command(self, command: str, args: List[str] = None,
                                        working_directory: str = ".", 
                                        timeout: int = 300) -> CrackerjackResult:
        ...
        try:
            ...
            parsed_data, memory_insights = self.parser.parse_output(command, stdout_text, stderr_text)
            ...
            error_result = CrackerjackResult(
                ...
                quality_metrics={},
                test_results=[],
                memory_insights=[f"Command '{command}' timed out - consider optimizing or increasing timeout"]
            )
    ...
    async def get_recent_results(self, hours: int = 24, command: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                result['memory_insights'] = json.loads(result['memory_insights'] or '[]')
                results.append(result)
            
            return results
    
    async def get_quality_metrics_history(self, project_path: str, metric_type: Optional[str] = None,
    ...
    def _calculate_quality_metrics(self, parsed_data: Dict[str, Any], exit_code: int) -> Dict[str, float]:
        ...
        if 'total_issues' in lint_summary:
            ...
            metrics['lint_score'] = max(0, 100 - total_issues) if total_issues < 100 else 0
        ...
        if 'total_issues' in security_summary:
            ...
            metrics['security_score'] = max(0, 100 - (total_issues * 10)) if total_issues < 10 else 0
    ...
    async def _store_result(self, result_id: str, result: CrackerjackResult):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                result_id, result.command, result.exit_code, result.stdout, result.stderr,
                result.execution_time, result.timestamp, result.working_directory,
                json.dumps(result.parsed_data), json.dumps(result.quality_metrics),
            ...
            for test_result in result.test_results:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    test_id, result_id, test_result.get('test', ''),
                    test_result.get('status', ''), test_result.get('duration', 0),
                    test_result.get('file', ''), result.timestamp
                ))
            ...
            for metric_name, metric_value in result.quality_metrics.items():
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    metric_id, result.working_directory, metric_name, 
                    metric_value, result.timestamp, result_id
                ))
    
    async def _store_progress_snapshot(self, result_id: str, result: CrackerjackResult, project_path: str):
        ...
        if progress_info:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    snapshot_id, project_path, result.command, 
...
async def execute_crackerjack_command(command: str, args: List[str] = None, 
                                    working_directory: str = ".", timeout: int = 300) -> Dict[str, Any]:
    """Execute Crackerjack command and return structured results"""
    integration = get_crackerjack_integration()
    result = await integration.execute_crackerjack_command(command, args, working_directory, timeout)
    return asdict(result)


async def get_recent_crackerjack_results(hours: int = 24, command: Optional[str] = None) -> List[Dict[str, Any]]:
    """Get recent Crackerjack execution results"""
    integration = get_crackerjack_integration()
    return await integration.get_recent_results(hours, command)


async def get_quality_metrics_history(project_path: str, metric_type: Optional[str] = None, 
                                    days: int = 30) -> List[Dict[str, Any]]:
    """Get quality metrics history for trend analysis"""
    integration = get_crackerjack_integration()
    return await integration.get_quality_metrics_history(project_path, metric_type, days)

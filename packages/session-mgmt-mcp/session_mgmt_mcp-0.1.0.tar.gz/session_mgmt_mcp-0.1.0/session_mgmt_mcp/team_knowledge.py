...
class TeamKnowledgeManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.db_path = db_path or str(Path.home() / ".claude" / "data" / "team_knowledge.db")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indices
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reflections_team ON team_reflections(team_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reflections_project ON team_reflections(project_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reflections_author ON team_reflections(author_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reflections_access ON team_reflections(access_level)")
    ...
    async def create_user(self, user_id: str, username: str, email: Optional[str] = None, 
                         role: UserRole = UserRole.CONTRIBUTOR) -> TeamUser:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps(user.teams), user.created_at, user.last_active,
                json.dumps(user.permissions)
            ))
        
        await self._log_access(user_id, "user_created", user_id, "user", {"role": role.value})
        return user
    
    async def create_team(self, team_id: str, name: str, description: str, 
                         owner_id: str) -> Team:
        """Create a new team"""
        team = Team(
            team_id=team_id,
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps(list(team.members)), json.dumps(list(team.projects)),
                team.created_at, json.dumps(team.settings)
            ))
        
        # Add owner to team
        await self._add_user_to_team(owner_id, team_id)
        await self._log_access(owner_id, "team_created", team_id, "team", {"name": name})
        return team
    
    async def add_team_reflection(self, content: str, author_id: str, tags: Optional[List[str]] = None,
                                 access_level: AccessLevel = AccessLevel.TEAM,
                                 team_id: Optional[str] = None, project_id: Optional[str] = None) -> str:
        """Add reflection to team knowledge base"""
        reflection_id = hashlib.sha256(f"{content}{author_id}{time.time()}".encode()).hexdigest()[:16]
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                reflection.id, reflection.content, json.dumps(reflection.tags),
                reflection.access_level.value, reflection.team_id, reflection.project_id,
                reflection.author_id, reflection.created_at, reflection.updated_at,
                reflection.votes, json.dumps(list(reflection.viewers)),
                json.dumps(list(reflection.editors))
            ))
        
        await self._log_access(author_id, "reflection_created", reflection_id, "reflection", 
                             {"team_id": team_id, "access_level": access_level.value})
        return reflection_id
    
    async def search_team_reflections(self, query: str, user_id: str, team_id: Optional[str] = None,
        ...
        await self._log_access(user_id, "reflections_searched", None, "search", 
                             {"query": query, "results_count": len(results)})
        return results
    
    async def vote_reflection(self, reflection_id: str, user_id: str, vote_delta: int = 1) -> bool:
        ...
        await self._log_access(user_id, "reflection_voted", reflection_id, "reflection", 
                             {"vote_delta": vote_delta})
        return True
    
    async def join_team(self, user_id: str, team_id: str, requester_id: Optional[str] = None) -> bool:
        ...
        await self._log_access(user_id, "team_joined", team_id, "team", 
                             {"requester_id": requester_id})
        return True
    
    async def get_team_stats(self, team_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Get team info
            team_row = conn.execute("SELECT * FROM teams WHERE team_id = ?", (team_id,)).fetchone()
            if not team_row:
                return None
            ...
            """, (team_id, datetime.now() - timedelta(days=7))).fetchone()
    ...
    async def get_user_permissions(self, user_id: str) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if user_data['teams']:
                ...
                team_rows = conn.execute(f"SELECT team_id, name, description FROM teams WHERE team_id IN ({placeholders})", 
                                       user_data['teams']).fetchall()
    ...
    async def _add_user_to_team(self, user_id: str, team_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            # Update team members
            team_row = conn.execute("SELECT members FROM teams WHERE team_id = ?", (team_id,)).fetchone()
            if team_row:
                members = set(json.loads(team_row[0] or '[]'))
                members.add(user_id)
                conn.execute("UPDATE teams SET members = ? WHERE team_id = ?", 
                           (json.dumps(list(members)), team_id))
            
            # Update user teams
            user_row = conn.execute("SELECT teams FROM users WHERE user_id = ?", (user_id,)).fetchone()
            if user_row:
                teams = json.loads(user_row[0] or '[]')
                if team_id not in teams:
                    teams.append(team_id)
                    conn.execute("UPDATE users SET teams = ?, last_active = ? WHERE user_id = ?", 
                               (json.dumps(teams), datetime.now(), user_id))
    
    async def _get_user_teams(self, user_id: str) -> List[str]:
        """Get teams user belongs to"""
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT teams FROM users WHERE user_id = ?", (user_id,)).fetchone()
            return json.loads(row[0] or '[]') if row else []
    
    async def _get_team(self, team_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _can_manage_team(self, user_id: str, team_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT permissions FROM users WHERE user_id = ?", (user_id,)).fetchone()
            if row:
                permissions = json.loads(row[0] or '{}')
                return permissions.get('admin_access', False) or permissions.get('moderate_content', False)
        
        return False
    
    async def _log_access(self, user_id: str, action: str, resource_id: Optional[str], 
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (user_id, action, resource_id, resource_type, datetime.now(), json.dumps(details)))
...
async def create_team_user(user_id: str, username: str, email: Optional[str] = None, 
...
async def create_team(team_id: str, name: str, description: str, owner_id: str) -> Dict[str, Any]:
    """Create a new team"""
    manager = get_team_knowledge_manager()
...
async def add_team_reflection(content: str, author_id: str, tags: Optional[List[str]] = None,
                             access_level: str = "team", team_id: Optional[str] = None,
                             project_id: Optional[str] = None) -> str:
    """Add reflection to team knowledge base"""
    manager = get_team_knowledge_manager()
    level = AccessLevel(access_level.lower())
    return await manager.add_team_reflection(content, author_id, tags, level, team_id, project_id)


async def search_team_knowledge(query: str, user_id: str, team_id: Optional[str] = None,
                               project_id: Optional[str] = None, tags: Optional[List[str]] = None,
                               limit: int = 20) -> List[Dict[str, Any]]:
    """Search team reflections with access control"""
    manager = get_team_knowledge_manager()
    return await manager.search_team_reflections(query, user_id, team_id, project_id, tags, limit)


async def join_team(user_id: str, team_id: str, requester_id: Optional[str] = None) -> bool:
    """Join a team or add user to team"""
    manager = get_team_knowledge_manager()
    return await manager.join_team(user_id, team_id, requester_id)


async def get_team_statistics(team_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    """Get team statistics and activity"""
    manager = get_team_knowledge_manager()
...
async def vote_on_reflection(reflection_id: str, user_id: str, vote_delta: int = 1) -> bool:
    """Vote on a team reflection"""
    manager = get_team_knowledge_manager()

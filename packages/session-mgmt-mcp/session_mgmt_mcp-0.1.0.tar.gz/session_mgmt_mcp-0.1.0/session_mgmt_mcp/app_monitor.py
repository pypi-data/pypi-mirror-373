...
class IDEActivityMonitor:
    ...
    def __init__(self, project_paths: List[str]):
        ...
        self.ide_extensions = {
            ...
            '.html', '.css', '.scss', '.vue', '.svelte', '.json', '.yaml',
            '.yml', '.toml', '.ini', '.md', '.txt', '.sql', '.sh', '.bat'
        }
...
class BrowserDocumentationMonitor:
    ...
    def __init__(self):
        self.doc_domains = {
            'docs.python.org', 'developer.mozilla.org', 'docs.rs',
            'docs.oracle.com', 'docs.microsoft.com', 'docs.aws.amazon.com',
            'cloud.google.com', 'docs.github.com', 'docs.gitlab.com',
            'stackoverflow.com', 'github.com', 'fastapi.tiangolo.com',
            'pydantic-docs.helpmanual.io', 'django-documentation',
            'flask.palletsprojects.com', 'nodejs.org', 'reactjs.org',
    ...
    def extract_documentation_context(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            elif 'javascript' in path or 'js' in path or domain in ['developer.mozilla.org', 'nodejs.org']:
                context['technology'] = 'javascript'
                context['relevance'] = 0.8
            elif 'rust' in domain or 'docs.rs' in domain:
                context['technology'] = 'rust'
                context['relevance'] = 0.8
            elif any(framework in domain for framework in ['django', 'flask', 'fastapi']):
                context['technology'] = 'python-web'
                context['relevance'] = 0.9
            elif any(framework in domain for framework in ['react', 'vue', 'angular', 'svelte']):
            ...
            if path_parts:
                context['topic'] = path_parts[-1] if path_parts[-1] != 'index.html' else path_parts[-2] if len(path_parts) > 1 else ''
...
class ApplicationFocusMonitor:
    ...
    def __init__(self):
        ...
        self.app_categories = {
            'ide': {'code', 'pycharm', 'vscode', 'sublime', 'atom', 'vim', 'emacs', 'intellij'},
            'browser': {'chrome', 'firefox', 'safari', 'edge', 'brave'},
            'terminal': {'terminal', 'iterm', 'console', 'cmd', 'powershell', 'zsh', 'bash'},
    ...
    def _categorize_app(self, app_name: str) -> Optional[str]:
...
class ApplicationMonitor:
    ...
    def __init__(self, data_dir: str, project_paths: Optional[List[str]] = None):

        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    ...
                    name = proc.info['name']
                    if any(gui_hint in name.lower() for gui_hint in ['code', 'browser', 'terminal', 'editor', 'ide']):
                        return name
...
class InterruptionManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.db_path = db_path or str(Path.home() / ".claude" / "data" / "interruption_manager.db")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indices
            conn.execute("CREATE INDEX IF NOT EXISTS idx_interruptions_timestamp ON interruption_events(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_interruptions_user ON interruption_events(user_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_contexts_user ON session_contexts(user_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_contexts_state ON session_contexts(state)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_session ON context_snapshots(session_id)")
    ...
    async def create_session_context(self, user_id: str, project_id: Optional[str] = None,
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                session_id, user_id, project_id, json.dumps(asdict(context)),
                ContextState.ACTIVE.value, datetime.now(), datetime.now()
            ))
        
        return session_id
    
    async def preserve_context(self, session_id: Optional[str] = None, force: bool = False) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    snapshot_id, session_id, 'preservation', datetime.now(),
                    compressed_data, json.dumps({'compressed': COMPRESSION_AVAILABLE, 'size': len(compressed_data)})
                ))
                ...
                conn.execute("""
                    ...
                    WHERE session_id = ?
                """, (ContextState.PRESERVED.value, datetime.now(), datetime.now(), session_id))
    ...
    async def restore_context(self, session_id: str) -> Optional[SessionContext]:
    ...
    async def get_interruption_history(self, user_id: str, hours: int = 24) -> List[Dict[str, Any]]:
    ...
    async def get_context_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return {
                ...
                'interruptions': {
                    'total': dict(interruption_stats[0])['total_interruptions'] if interruption_stats else 0,
                    'by_type': [dict(row) for row in interruption_stats] if interruption_stats else []
                },
    ...
    def _handle_interruption(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            if (self.auto_save_enabled and 
                ...
                interruption_type in [InterruptionType.APP_SWITCH, InterruptionType.FOCUS_LOST]):
            ...
            interruption = InterruptionEvent(
                ...
                project_id=self.current_context.project_id if self.current_context else None
            )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    interruption.id, interruption.event_type.value, interruption.timestamp,
                    json.dumps(interruption.source_context), json.dumps(interruption.target_context),
                    interruption.duration, json.dumps(interruption.recovery_data or {}),
                    interruption.auto_saved, interruption.user_id, interruption.project_id
                ))
        except Exception as e:
        ...
        if PSUTIL_AVAILABLE:
            try:
                for proc in psutil.process_iter(['pid', 'name']):
                    try:
                        name = proc.info['name']
                        if any(keyword in name.lower() for keyword in ['code', 'python', 'node', 'git']):
...
async def start_interruption_monitoring(working_directory: str = ".", watch_files: bool = True):
    """Start interruption monitoring"""
...
async def create_session_context(user_id: str, project_id: Optional[str] = None, working_directory: str = ".") -> str:
...
async def preserve_current_context(session_id: Optional[str] = None, force: bool = False) -> bool:
    """Preserve current session context"""
    manager = get_interruption_manager()
    return await manager.preserve_context(session_id, force)


async def restore_session_context(session_id: str) -> Optional[Dict[str, Any]]:
    """Restore session context from snapshot"""
    manager = get_interruption_manager()
...
async def get_interruption_history(user_id: str, hours: int = 24) -> List[Dict[str, Any]]:

        ...
        for context_name, regex in self.context_patterns.items():
            if re.search(regex, content, re.IGNORECASE):
                patterns.append({
                    ...
                    'content': content,
                    'relevance': 'high' if context_name in ['debugging', 'error_handling'] else 'medium'
                })
...
class TemporalSearchParser:
    ...
    def __init__(self):
        self.relative_patterns = {
            ...
            'this month': relativedelta(months=1) if DATEUTIL_AVAILABLE else timedelta(days=30),
            'last month': relativedelta(months=2) if DATEUTIL_AVAILABLE else timedelta(days=60),
            'this year': relativedelta(years=1) if DATEUTIL_AVAILABLE else timedelta(days=365),
        }
    ...
    def parse_time_expression(self, expression: str) -> Tuple[Optional[datetime], Optional[datetime]]:
        ...
        # Handle "X time units ago" pattern
        match = re.search(r'(\d+)\s+(minute|hour|day|week|month|year)s?\s+ago', expression)
        if match:
            ...
            if unit == 'minute':
                ...
                delta = relativedelta(months=amount) if DATEUTIL_AVAILABLE else timedelta(days=amount * 30)
            elif unit == 'year':
                delta = relativedelta(years=amount) if DATEUTIL_AVAILABLE else timedelta(days=amount * 365)
            
            end_time = now - delta
            return end_time, now
        
        # Handle "in the last X units" pattern
        match = re.search(r'in\s+the\s+last\s+(\d+)\s+(minute|hour|day|week|month|year)s?', expression)
        if match:
            amount = int(match.group(1))
            unit = match.group(2)
...
class EnhancedSearchEngine:
    ...
    async def search_code_patterns(self, query: str, pattern_type: Optional[str] = None, 
    ...
    async def search_error_patterns(self, query: str, error_type: Optional[str] = None,
                                  limit: int = 10) -> List[Dict[str, Any]]:
    ...
    async def search_temporal(self, time_expression: str, query: Optional[str] = None,
                            limit: int = 10) -> List[Dict[str, Any]]:
        """Search conversations within a time range"""
        start_time, end_time = self.temporal_parser.parse_time_expression(time_expression)

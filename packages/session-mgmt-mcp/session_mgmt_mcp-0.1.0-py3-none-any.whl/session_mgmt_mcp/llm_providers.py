        ...
        for msg in messages:
            if msg.role == 'system':
                ...
                if converted and converted[-1]['role'] == 'user':
                    converted[-1]['parts'] = [f"System: {msg.content}\n\nUser: {converted[-1]['parts'][0]}"]
                else:
                    converted.append({
                        'role': 'user',
...
class OllamaProvider(LLMProvider):
    ...
    def get_models(self) -> List[str]:
        ...
        return self._available_models if self._available_models else [
            'llama2', 'llama2:13b', 'llama2:70b', 'codellama', 'mistral', 'mixtral'
        ]
...
class LLMManager:
    ...
    async def stream_generate(self, 
        ...
        if target_provider in self.providers:
            try:
                provider_instance = self.providers[target_provider]
                if await provider_instance.is_available():
                    async for chunk in provider_instance.stream_generate(messages, model, **kwargs):
                        yield chunk
                    return
            except Exception as e:
                self.logger.warning(f"Provider {target_provider} failed: {e}")
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            info['providers'][name] = {
                ...
                'config': {k: v for k, v in provider.config.items() if 'key' not in k.lower()}
            }
    ...
    async def test_providers(self) -> Dict[str, Any]:
        ...
        test_message = [LLMMessage(role='user', content='Hello, respond with just "OK"')]
        results = {}

...
class NaturalLanguageParser:
    ...
    def __init__(self):
        ...
        self.time_patterns = {
            ...
            r'in (\d+) (minute|min|minutes|mins)': lambda m: timedelta(minutes=int(m.group(1))),
            ...
            r'in (\d+) (month|months)': lambda m: relativedelta(months=int(m.group(1))) if DATEUTIL_AVAILABLE else timedelta(days=int(m.group(1)) * 30),
            ...
            # Session-relative
            r'end of (session|work)': lambda m: timedelta(hours=2),  # Default session length
    ...
    def parse_time_expression(self, expression: str, base_time: Optional[datetime] = None) -> Optional[datetime]:
    ...
    def parse_recurrence(self, expression: str) -> Optional[str]:
...
class ReminderScheduler:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.db_path = db_path or str(Path.home() / ".claude" / "data" / "natural_scheduler.db")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Create indices
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reminders_scheduled ON reminders(scheduled_for)")
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_reminders_project ON reminders(project_id)")
    
    async def create_reminder(self, title: str, time_expression: str, description: str = "",
                            user_id: str = "default", project_id: Optional[str] = None,
                            notification_method: str = "session", context_triggers: Optional[List[str]] = None,
        ...
        reminder_type = ReminderType.RECURRING if recurrence_rule else ReminderType.ONE_TIME
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                reminder.id, reminder.title, reminder.description, reminder.reminder_type.value,
                reminder.status.value, reminder.created_at, reminder.scheduled_for, reminder.executed_at,
                reminder.user_id, reminder.project_id, json.dumps(reminder.context_triggers),
                reminder.recurrence_rule, reminder.notification_method, json.dumps(reminder.metadata)
            ))
        
        # Log creation
        await self._log_reminder_action(reminder_id, "created", "success", {
    ...
    async def get_pending_reminders(self, user_id: Optional[str] = None, 
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                result = dict(row)
                result['context_triggers'] = json.loads(result['context_triggers'] or '[]')
                result['metadata'] = json.loads(result['metadata'] or '{}')
                results.append(result)
            
            return results
    
    async def get_due_reminders(self, check_time: Optional[datetime] = None) -> List[Dict[str, Any]]:
    ...
    async def execute_reminder(self, reminder_id: str) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                reminder_data['context_triggers'] = json.loads(reminder_data['context_triggers'] or '[]')
                reminder_data['metadata'] = json.loads(reminder_data['metadata'] or '{}')
            ...
            if reminder_data['recurrence_rule']:
                ...
                if next_time:
                    ...
                    await self._log_reminder_action(reminder_id, "rescheduled", "success", {
            ...
            await self._log_reminder_action(reminder_id, "executed", "success", {
            ...
            await self._log_reminder_action(reminder_id, "executed", "failed", {
                "error": str(e)
            })
            return False
    ...
    async def cancel_reminder(self, reminder_id: str) -> bool:
        ...
        try:
            ...
            await self._log_reminder_action(reminder_id, "cancelled", "failed", {"error": str(e)})
            return False
    ...
    def _calculate_next_occurrence(self, last_time: datetime, recurrence_rule: str) -> Optional[datetime]:
    ...
    async def _log_reminder_action(self, reminder_id: str, action: str, result: str, details: Dict[str, Any]):
...
async def create_natural_reminder(title: str, time_expression: str, description: str = "",
                                user_id: str = "default", project_id: Optional[str] = None,
                                notification_method: str = "session") -> Optional[str]:
    """Create reminder from natural language time expression"""
    scheduler = get_reminder_scheduler()
    return await scheduler.create_reminder(title, time_expression, description, user_id, project_id, notification_method)


async def list_user_reminders(user_id: str = "default", project_id: Optional[str] = None) -> List[Dict[str, Any]]:
...
def register_session_notifications():
    ...
    async def session_notification(reminder_data: Dict[str, Any]):
        ...
        logger.info(f"Reminder: {reminder_data['title']} - {reminder_data['description']}")

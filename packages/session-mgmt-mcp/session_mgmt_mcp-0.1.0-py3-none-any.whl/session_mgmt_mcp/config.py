...
class SessionConfig:
    ...
    default_trusted_operations: List[str] = field(default_factory=lambda: [
        "git_commit", "uv_sync", "file_operations"
    ])
...
class SecurityConfig:
    ...
    exclude_sensitive_patterns: List[str] = field(default_factory=lambda: [
        r"password\s*=\s*['\"][^'\"]+['\"]",
        r"api[_-]?key\s*=\s*['\"][^'\"]+['\"]",
        r"secret\s*=\s*['\"][^'\"]+['\"]",
        r"token\s*=\s*['\"][^'\"]+['\"]"
...
class ConfigLoader:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        self._config_cache: Optional[SessionMgmtConfig] = None
                ...
                try:
                    with open(parent / "pyproject.toml", "rb") as f:
                        if tomllib:
                            ...
                            if toml_data.get("project", {}).get("name") == "session-mgmt-mcp":
                                return parent
                except Exception:
                    pass
    ...
    def _apply_toml_config(self, config: SessionMgmtConfig, toml_data: Dict[str, Any]):
        ...
        # Look for tool.session-mgmt-mcp section
        ...
        if tool_config:
            ...
            if "token_optimization" in tool_config:
                token_config = tool_config["token_optimization"]
                for key, value in token_config.items():
                    if hasattr(config.token_optimization, key):
                        setattr(config.token_optimization, key, value)
            
            # Session config
            if "session" in tool_config:
                session_config = tool_config["session"]
                for key, value in session_config.items():
                    if hasattr(config.session, key):
                        setattr(config.session, key, value)
            
            # Integration config
            if "integration" in tool_config:
                integration_config = tool_config["integration"]
            ...
            if "logging" in tool_config:
                logging_config = tool_config["logging"]
            ...
            if "security" in tool_config:
                security_config = tool_config["security"]
                for key, value in security_config.items():
                    if hasattr(config.security, key):
                        setattr(config.security, key, value)
            
            # Server-level config
            server_keys = ["server_host", "server_port", "enable_websockets", "debug", "enable_hot_reload"]
    ...
    def _apply_env_config(self, config: SessionMgmtConfig):
        ...
        env_mappings = {
            ...
            "SESSION_MGMT_ENABLE_MULTI_PROJECT": ("database", "enable_multi_project", bool),
            
            # Search
            "SESSION_MGMT_ENABLE_SEMANTIC_SEARCH": ("search", "enable_semantic_search", bool),
            "SESSION_MGMT_EMBEDDING_MODEL": ("search", "embedding_model"),
            
            # Token optimization
            "SESSION_MGMT_ENABLE_OPTIMIZATION": ("token_optimization", "enable_optimization", bool),
            "SESSION_MGMT_MAX_TOKENS": ("token_optimization", "default_max_tokens", int),
            
            # Session
            "SESSION_MGMT_AUTO_CHECKPOINT": ("session", "auto_checkpoint_interval", int),
            "SESSION_MGMT_ENABLE_AUTO_COMMIT": ("session", "enable_auto_commit", bool),
            
            # Logging
            "SESSION_MGMT_LOG_LEVEL": ("logging", "level"),
            "SESSION_MGMT_ENABLE_FILE_LOGGING": ("logging", "enable_file_logging", bool),
        ...
        for env_var, mapping in env_mappings.items():
            ...
            if value is not None:
                try:
                    ...
                    if len(mapping) == 2:
                        # Nested attribute (e.g., database.path)
    ...
    def _expand_paths(self, config: SessionMgmtConfig):
        ...
        config.integration.global_workspace_path = os.path.expanduser(config.integration.global_workspace_path)
    ...
    def _validate_config(self, config: SessionMgmtConfig):
        ...
        if not (1024 <= config.server_port <= 65535):
            print(f"Warning: Invalid server port {config.server_port}, using default 3000")

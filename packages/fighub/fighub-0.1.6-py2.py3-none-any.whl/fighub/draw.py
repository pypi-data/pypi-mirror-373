...
from fighub import colors, marker, paras_latex
...
class ParasPlot:
    def __init__(
        ...
        self.fig_label = defaultdict(lambda: defaultdict(list))

        mpl.rcParams["font.family"] = (
            "Times New Roman"  # Set all fonts to Times New Roman
        )
        mpl.rcParams["mathtext.fontset"] = "stix"  # Set math font to Times-like
        mpl.rcParams["mathtext.rm"] = "Times New Roman"  # Set math roman font to Times
        mpl.rcParams["axes.unicode_minus"] = (
            False  # Ensure minus sign is shown correctly
        )
    ...
    def _extract_info(self, pkl_path):
        ...
        batch_size_tuple = parts[model_name_id + 4].split("_")
        model_name = parts[model_name_id]
        sample_str_list = parts[model_name_id + 3].split("_")
        sample_number = (
            int(sample_str_list[1]),
            int(sample_str_list[3]),
            int(sample_str_list[5]),
        ...
        batch_size_str = (
            f"{batch_size_tuple[0]}{batch_size_tuple[1]} = {batch_size_tuple[2]}"
        )

        # --------- get_optimization_paras ----------
        filename = Path(pkl_path).stem
        param_latex_map = paras_latex.param_latex_map()
        
        params = []
        """
        ADAM_alpha_0.0005_epsilon_1e-08_beta1_0.9_beta2_0.999.pkl
        example:
            filename.split(para_name) = [ADAM, 0.0005_epsilon_1e-08_beta1_0.9_beta2_0.999]
        """
        for para_name in list(param_latex_map.keys()):
            if para_name in filename:
                value = filename.split(para_name)[1].split("_")[0]
                params.append(f"{param_latex_map[para_name]} = {value}")
        
        if params:
            label_name = f"{optimizer_name} ({', '.join(params)})"
        ...
        for pkl_path in pkl_paths:
            if os.path.exists(pkl_path):
                ...
                with open(pkl_path, "rb") as file:
                    ...
                    self.fig_label[self.info["data_name"]][
                        self.info["optimizer_name"]
                    ].append(self.info["label_name"])
            else:
                print(f"{pkl_path} ---> error")

    def draw_one(self, solid_methods=None, ncol=1, save_name="", grid_tune =True):
        ...
        for i, (data_name, data) in enumerate(self.fig_value.items()):
            for optimizer_name, curves_value in data.items():
                ...
                if self.marker_schedule:
                    ...
                    plt.plot(
                        x[self.marker_point[data_name]],
                        data[self.marker_point[data_name]],
                        linestyle="",
                        color=self.colors_dict[optimizer_name],
                        marker=self.marker_dict[optimizer_name],
                    )

            plt.yscale("log")
            plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=ncol)
            plt.xlabel("epochs")
            plt.grid(grid_tune)
            plt.ylabel(self.ylabel[self.category_dict[data_name]])
            plt.title(f"{data_name}, {self.info['batch_size_str']}")
    ...
    def subplot(self, solid_methods=None, ncols=3, save_name="", grid_tune =True):
    ...
    def draw_one_with_paras(self, solid_methods=None, ncol=1, save_name="", grid_tune =True):
        plt.figure(figsize=(9, 6))
        # data: dict , for example: data["GSD"]
        for i, (data_name, data) in enumerate(self.fig_value.items()):
            for optimizer_name, curves_value in data.items():
                ...
                if self.colors_schedule != None:
                    plt.plot(
                        ...
                        linestyle,
                        color=self.colors_dict[optimizer_name],
                        label=self.fig_label[data_name][optimizer_name],
                    )
                    plt.fill_between(
                        x,
                        mean_curve - std_curve,
                    ...
                    plt.plot(x, mean_curve, linestyle, label=self.fig_label[data_name][optimizer_name])
            ...
            # Saving as PDF
            if save_name == "":
                save_name = f"Figs/{self.model_name}/{self.py_name}_OptParas.pdf"
            plt.savefig(save_name, bbox_inches="tight")
            print(f"Saved figure as {save_name}")
            plt.close()
    
    # ---------------------------------------------------------
    def oneplot(self, solid_methods=None, ncol=1, save_name="", grid_tune =True, figsize=(9, 6), with_opt_paras = True, title_name = ""):
        ...
        for i, (data_name, data) in enumerate(self.fig_value.items()):
            for optimizer_name, curves_value in data.items():
                ...
                if with_opt_paras:
                    label_name = self.fig_label[data_name][optimizer_name]
                else:
                    label_name = optimizer_name
                ...
                if self.colors_schedule != None:
                    ...
                    plt.plot(x, mean_curve, linestyle, label=label_name)
            ...
            if save_name == "":
                if with_opt_paras:
                    ...
                    save_name = f"Figs/{self.model_name}/{self.py_name}_NoneParas.pdf"
                    
            
            if title_name == "":
                if with_opt_paras:
                    title_name = f"{self.info["model_name"]}, {data_name}, {self.info['batch_size_str']}, {self.info["sample_number"]}"
                
                else:
                    title_name = f"{data_name}"

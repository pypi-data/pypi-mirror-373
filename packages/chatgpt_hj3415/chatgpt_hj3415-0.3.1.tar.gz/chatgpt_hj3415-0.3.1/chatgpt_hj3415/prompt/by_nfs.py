...
async def get_prompt(ticker:str) -> list[dict]:
    ...
    return [
        {"role": "system",
         "content": "너의 역할은 CSV 재무분석 데이터를 바탕으로 데이터를 분석하고, 과거/현재 요약뿐 아니라"
                    " 향후 주가의 흐름에 대한 확률적 예측을 제공하는 한국어 사용 기술적 애널리스트다. "
                    "모든 예측은 교육적·정보 제공 목적의 확률적 시나리오로 제시해라(투자 조언 아님)."
                    "숫자 근거(지표 값, 기준일, 구간)를 반드시 명시하고, 불확실성/무효화 조건도 함께 적어라."},
        {"role": "user",
         "content": content},
    ]
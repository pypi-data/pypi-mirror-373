...
def proxy_tester():
    ...
    class SimpleProxyTester:
        ...
        def get_cidr_ranges(self):
            """Prompt user for CIDR ranges"""
            print(f"{Fore.CYAN}\nEnter CIDR ranges (one per line). Press Enter twice to start scanning:{Style.RESET_ALL}")
            cidr_ranges = []
            while True:
                cidr = input().strip()
                if not cidr:
        ...
        def save_results(self):
            ...
            filename = "proxy_results.txt"
            with open(filename, 'w') as f:
                f.write("Proxy,Domain,Status Code,Server\n")
                for result in self.valid_proxies:
                    f.write(f"{result['proxy']},{result['domain']},{result['status_code']},{result['server']}\n")
            
            print(f"{Fore.GREEN}Results saved to {filename}{Style.RESET_ALL}")
            print(f"{Fore.GREEN}Total working proxies found: {len(self.valid_proxies)}{Style.RESET_ALL}")
    ...
    def check_websocket(host, port, path="/", use_ssl=False):
        ...
        try:
            ...
            if status_code == "101" and "upgrade: websocket" in response.lower():
                ...
                print(f"{YELLOW}[-] {result} — Status: {status_code}{RESET}")
                if output_file:
                    try:
                        with open(output_file, "a", encoding="utf-8") as f:
                            f.write(result + f" (Status: {status_code})\n")
                ...
                print(f"{YELLOW}[-] — WebSocket NOT supported (Status: {status_code}){RESET}")
...
def banner():
    ...
    PURPLE + "script_version@ 1.1.6 ®" + ENDC,

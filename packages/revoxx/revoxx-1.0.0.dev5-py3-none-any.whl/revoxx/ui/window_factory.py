...
class WindowFactory:
    ...
    def _set_window_icon(cls, window: WindowBase) -> None:
        ...
        """
        # Try to find the icon file - handle both development and installed cases
        # First try relative to this file (development)
        icon_path = Path(__file__).parent.parent / "resources" / "microphone.png"
        
        # If not found, try package resources (installed)
                ...
                if sys.version_info >= (3, 9):
                    ...
                    try:
                        ...
                        if hasattr(pkg_resources, 'files'):
                            # Python 3.9+ new API
                            resource = pkg_resources.files('revoxx.resources') / 'microphone.png'
                            if hasattr(resource, 'read_bytes'):
                                # Create temp file for the icon
                                import tempfile
                                with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp:
                                    tmp.write(resource.read_bytes())
                                    icon_path = Path(tmp.name)
                            else:
                                icon_path = Path(str(resource))
                        else:
                            # Fallback for older importlib_resources
                            with pkg_resources.path('revoxx.resources', 'microphone.png') as p:
                                icon_path = Path(p)
                    except Exception:
                        # Try older pkg_resources
                        import pkg_resources as old_pkg
                        icon_path = Path(old_pkg.resource_filename('revoxx', 'resources/microphone.png'))
            except Exception:
                icon_path = None

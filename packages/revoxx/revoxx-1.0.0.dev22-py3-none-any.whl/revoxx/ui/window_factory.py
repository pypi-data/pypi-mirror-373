...
class WindowFactory:
    ...
    def _configure_window(cls, window: WindowBase, template: dict) -> None:
        ...
        # Set WM_CLASS for better Linux desktop integration
        if sys.platform.startswith('linux'):
            try:
                # Set the window class name for desktop integration
                window.window.wm_attributes('-type', 'normal')
                # Use tk.call to set the WM_CLASS property
                window.window.tk.call('wm', 'class', window.window._w, 'Revoxx')
    ...
    def _set_window_icon(cls, window: WindowBase) -> None:
        ...
        """
        # Get absolute path to current file location
        current_file = Path(__file__).resolve()
        
        # window_factory.py is in revoxx/ui/, icon is in revoxx/resources/
        # So we go up to revoxx/ and then into resources/
        icon_path = current_file.parent.parent / "resources" / "microphone.png"
        ...
        if icon:
            try:
                # Use False for first parameter (don't use as default for all windows)
                # This seems to work better across platforms
                window.window.iconphoto(False, icon)
            except tk.TclError:
                # Fallback for platforms that don't support iconphoto
                try:
                    window.window.tk.call('wm', 'iconphoto', window.window._w, icon)
                except tk.TclError:
                    # Icon setting not supported on this platform
                    pass

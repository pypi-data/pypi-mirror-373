    ...
    async def get_apartment_trades(
        ...
        try:
            ...
            if "OpenAPI_ServiceResponse" in data:
                service_response = data["OpenAPI_ServiceResponse"]
                if "cmmMsgHeader" in service_response:
                    header = service_response["cmmMsgHeader"]
                    err_msg = header.get("errMsg", "Unknown error")
                    auth_msg = header.get("returnAuthMsg", "")
                    reason_code = header.get("returnReasonCode", "")
                    
                    # 에러 메시지 상세 정보
                    error_details = f"API Error: {err_msg}"
                    if auth_msg:
                        error_details += f" - {auth_msg}"
                    if reason_code:
                        error_details += f" (Code: {reason_code})"
                    
                    raise ValueError(error_details)
            
            # Check for normal API response
            if "response" in data:

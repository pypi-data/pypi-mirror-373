...
class WebSocketClient:
    ...
    负责与服务端的WebSocket通信，支持消息发送、接收和自动重连功能。
    
    Attributes:
        ws: WebSocket连接对象
        url (str): WebSocket服务器URL
        message_handler (Optional[Callable]): 消息处理回调函数
        is_connected (bool): 连接状态
        reconnect_interval (int): 重连间隔（秒）
        max_reconnect_attempts (int): 最大重连尝试次数
        reconnect_count (int): 当前重连次数
    """
    
    def __init__(self, url: Optional[str] = None, message_handler: Optional[Callable[[Dict[str, Any]], None]] = None) -> None:
        """初始化WebSocket客户端
        
        Args:
            url: WebSocket服务器URL，默认为ws://127.0.0.1:8765
    ...
    def send_result(self, key_id: str, rpa_type: str, rpa_state: int, rpa_note: str) -> None:
        """发送RPA任务执行结果
    ...
    def send_message(self, action: str, message: str, data: Optional[Dict[str, Any]] = None) -> None:
        """发送消息到WebSocket服务器

...
class PDLossWeighter(LossWeighter):
    ...
    def get_balance_param(
        ...
        **target_input_kwargs,
        ...
        if self.arithmetic_errorr:  # do arithmetic update if using arithmetic error
            ...
            mult = 1 + ((self.kp * error) + (self.kd * derivative))
            mult = np.clip(mult, 1 - self.update_max, 1 + self.update_max)

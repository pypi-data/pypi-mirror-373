...
#### PDLossWeighter
PD controller with proportional and derivative terms. Applies correction proportional to the error in actual loss value and target loss value, and dampens with a derivative term.
```python
from PD_loss_balancing import PDLossWeighter, ConstantTarget
...
- Start with kd = 0, you should see oscillations. If there are no oscillations and it works, no need to add kd. If it doesn't converge increase kp.
- add in Kd and ensure oscillations stop. If it is dampened too much reduce kd. If it still oscilates, increase kp. 

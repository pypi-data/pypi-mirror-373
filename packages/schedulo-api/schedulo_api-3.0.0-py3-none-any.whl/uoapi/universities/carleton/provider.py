...
from uoapi.carleton.models import Course as OldCourse, CourseSection as OldSection, MeetingTime as OldMeetingTime
...
class CarletonProvider(BaseUniversityProvider):
    ...
    def __init__(self, max_workers: int = 4, cookie_file: Optional[str] = None):
        super().__init__()
        self._discovery = CarletonDiscovery(max_workers=max_workers, cookie_file=cookie_file)
    ...
    def get_subjects(self) -> List[Subject]:
        ...
        try:
            ...
            raise DataSourceError(f"Failed to retrieve subjects from Carleton: {str(e)}")
    ...
    def _load_catalog_courses(self) -> List[Course]:
        ...
        courses = []
        for course_code, course_data in self._catalog_data.items():
            try:
                course = Course(
                    ...
                    prerequisite_courses=[],  # Would need parsing
    ...
    def get_available_terms(self) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            raise DataSourceError(f"Failed to get available terms from Carleton: {str(e)}")
    ...
    def discover_courses(
        ...
        try:
            ...
            if course_codes:
                ...
                filtered_courses = [
                    course for course in converted_courses
            ...
            error_courses = [c for c in converted_courses if any('error' in str(s.notes).lower() for s in c.sections)]
            ...
            return DiscoveryResult(
                ...
                offering_rate=len(offered_courses) / max(1, len(converted_courses)) * 100,

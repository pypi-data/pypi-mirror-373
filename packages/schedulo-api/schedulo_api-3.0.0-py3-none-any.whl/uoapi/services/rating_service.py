...
class DefaultRatingService(RatingService):
    ...
    def get_instructor_rating(
        ...
        try:
            ...
            if rating_data:
                logger.info(f"Found rating for {instructor_name}: {rating_data.get('rating', 'N/A')}")
            else:
            ...
            for course in courses:
                ...
                for section in course.sections:
                    ...
                    if section.instructor and section.instructor.strip():
                        first_name, last_name = self._parse_instructor_name(section.instructor)
                        if first_name and last_name:
                            full_name = f"{first_name} {last_name}"
                            if full_name in ratings:
                                logger.debug(f"Rating available for {section.instructor}")
    ...
    def get_rating_statistics(self, university: University) -> Dict[str, Any]:
        ...
        try:
            ...
            university_cache_keys = [k for k in self._rating_cache.keys() if k.startswith(f"{university.value}:")]
            cached_ratings = [v for k, v in self._rating_cache.items() if k.startswith(f"{university.value}:") and v]
            
            if cached_ratings:
                ratings_values = [r.get('rating', 0) for r in cached_ratings if r.get('rating')]
                ...
                stats = {
                    ...
                    'rating_coverage': len(cached_ratings) / max(1, len(university_cache_keys)) * 100
                }
            else:
                stats = {
                    'university': university.value,

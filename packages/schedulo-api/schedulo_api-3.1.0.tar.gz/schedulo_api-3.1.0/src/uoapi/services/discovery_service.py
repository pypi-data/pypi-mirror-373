...
class DefaultDiscoveryService(DiscoveryService):
    ...
    def get_subjects_list(self, university: str) -> List[str]:
        ...
        try:
            ...
            raise ServiceError(f"Failed to get subjects list for {university}: {str(e)}")
    ...
    def normalize_university_name(self, university_input: str) -> Optional[str]:
        ...
        """
        # Normalize the input
        normalized = university_input.lower().replace(' ', '').replace('university', '').replace('of', '')
        ...
        try:
            ...
            for uni in available:
                if normalized == uni.lower().replace(' ', '').replace('university', '').replace('of', ''):
                    return uni

...
class DefaultTimetableService(TimetableService):
    ...
    def get_available_terms(self, university: University) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(terms)} available terms for {university.value}")
            return terms
    ...
    def get_live_courses(
        ...
        try:
            ...
            if term_code not in available_codes:
                ...
                raise TermNotAvailableError(term_code, university.value, available_names)
        ...
        try:
            ...
            logger.info(f"Discovered {result.total_courses} courses for {university.value} {term_code}")
            logger.info(f"Offering rate: {result.offering_rate:.1f}%")
            
            return result
            
        except Exception as e:
            logger.error(f"Failed to discover courses for {university.value} {term_code}: {e}")
    ...
    def get_supported_universities(self) -> List[University]:
        ...
        for university in self._course_service.get_all_universities():
            try:
                ...
                logger.warning(f"Error checking live data support for {university}: {e}")

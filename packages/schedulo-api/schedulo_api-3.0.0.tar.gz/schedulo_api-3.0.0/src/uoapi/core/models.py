...
class Subject(BaseModel):
    ...
    university: University = Field(..., description="University this subject belongs to")
...
class CourseSection(BaseModel):
    ...
    meeting_times: List[MeetingTime] = Field(default_factory=list, description="Meeting times")
...
class Course(BaseModel):
    ...
    prerequisite_courses: List[str] = Field(default_factory=list, description="Required course codes")
    
    # Live timetable information
    sections: List[CourseSection] = Field(default_factory=list, description="Course sections")
...
class Prerequisite(BaseModel):
    ...
    parsed_courses: List[str] = Field(default_factory=list, description="Extracted course codes")
    conditions: Dict[str, Any] = Field(default_factory=dict, description="Structured conditions")
    ...
    def try_parse(cls, text: str) -> Optional["Prerequisite"]:
        ...
        """
        if any(keyword in text for keyword in ["Prerequisite", "Préalable", "prereq", "Prereq"]):
...
class Component(BaseModel):
    ...
    def try_parse(cls, text: str) -> Optional["Component"]:
        ...
        """
        if any(keyword in text for keyword in ["Course Component", "Volet", "component"]):
...
class DiscoveryResult(BaseModel):
    ...
    subjects_queried: List[str] = Field(default_factory=list, description="Subject codes queried")
    ...
    offering_rate: float = Field(default=0.0, description="Offering rate percentage", ge=0, le=100)
    processing_time: Optional[float] = Field(None, description="Processing time in seconds", ge=0)
    courses: List[Course] = Field(default_factory=list, description="Discovered courses")

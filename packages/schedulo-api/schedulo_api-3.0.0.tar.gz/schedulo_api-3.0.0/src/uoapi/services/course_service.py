...
class DefaultCourseService(CourseService):
    ...
    def get_subjects(self, university: University) -> List[Subject]:
        ...
        try:
            ...
            raise ServiceError(f"Failed to get subjects for {university.value}: {str(e)}")
    ...
    def get_subject_by_code(self, university: University, subject_code: str) -> Subject:
        ...
        try:
            ...
            logger.error(f"Failed to get subject {subject_code} for {university.value}: {e}")
    ...
    def get_course_by_code(self, university: University, course_code: str) -> Course:
        ...
        try:
            ...
            logger.error(f"Failed to get course {course_code} for {university.value}: {e}")
    ...
    def search_courses(
        ...
        try:
            ...
            logger.info(f"Search for '{query}' in {university.value} found {result.total_found} courses")
            return result
        except Exception as e:
    ...
    def get_course_statistics(self, university: University) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                'subjects_with_courses': len(set(course.subject_code for course in courses)),
            ...
            subject_dist = {}
            for course in courses:
                subject_dist[course.subject_code] = subject_dist.get(course.subject_code, 0) + 1
            stats['subject_distribution'] = dict(sorted(subject_dist.items(), key=lambda x: x[1], reverse=True)[:10])
    ...
    def validate_university_string(self, university_str: str) -> University:
        ...
        """
        # Normalize the input
        normalized = university_str.lower().replace(' ', '').replace('university', '').replace('of', '')

...
3. Create API token for automated uploads:
   ...
   - Create token with "Entire account" scope (for first-time publishing)
   - Copy the token (format: `pypi-...`)

### Method 1: Configure ~/.pypirc (Recommended)
```ini
[distutils]
...
# Increment version in octavia_loxilb_driver/version.py
# Rebuild and upload
python -m build
twine upload dist/*
```

**3. Interactive Prompt Issues (EOFError)**
```bash
# Use environment variables or command line auth
...
# Visit: https://pypi.org/project/octavia-loxilb-driver/
```

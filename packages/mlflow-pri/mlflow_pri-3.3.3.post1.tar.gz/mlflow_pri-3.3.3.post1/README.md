...
This is a custom version of MLflow with enhanced support for johnsnowlabs models.

## What's Different

This package includes improvements to the johnsnowlabs integration in MLflow:

- **Fixed Model Loading**: Resolved issues with `python_model` key preservation in MLmodel configuration
- **Enhanced Java Support**: Improved Java environment setup for johnsnowlabs models in Docker containers
- **Better Error Handling**: More robust error handling for johnsnowlabs model serialization

## Installation

```bash
pip install mlflow-pri
```

## Usage

Use exactly like regular MLflow:

```python
import mlflow
import mlflow.johnsnowlabs

# Log your johnsnowlabs model
model_info = mlflow.johnsnowlabs.log_model(
    spark_model=your_model,
    artifact_path="model",
    signature=signature,
...
## Changes from Original MLflow

This package is based on MLflow 3.3.3.dev0 with the following modifications:

1. **Fixed python_model preservation**: When overriding the loader_module for johnsnowlabs support, all existing keys in the python_function flavor are now preserved, including the critical `python_model` key.

2. **Optional JAR saving**: JAR file saving can be disabled to avoid installation issues while maintaining Java support in containers.

## Original MLflow README

...
def wrap_module(module):
    if socksocket.default_proxy:
        module.socket = __import__("socket")
...
class socksocket(_BaseSocket):
    ...
    def recvfrom(self, bufsize, flags=0, *args, **kwargs):
        if self.type != socket.SOCK_DGRAM:
            return super(socksocket, self).recvfrom(bufsize, flags, *args, **kwargs)
        if not self._proxyconn:
            self.bind(("", 0))
    ...
    def recv(self, bufsize, *args, **kwargs):
        if bufsize == 0:
            return b""
        data, _ = self.recvfrom(bufsize, *args, **kwargs)
        return data

...
# Show the buffered output in a scrollable text area at the top for visibility
st.subheader("Console Output Buffer (scrollable, oldest at top)")
st.text_area(
    "Output",
    st.session_state["output_buffer"],
    height=600,
    key="main_console_output",
    help="Scroll to see all output. Oldest output is at the top.",
)
...
def process_with_output(options, models, output_area, plot_area):
    ...
    st.session_state["output_buffer"] += buf.getvalue()
...
def process_cache_with_output(options, models, output_area, plot_area):
    ...
    with redirect_stdout(buf):
        Process_cache(
            options, models=models, output_area=output_area, plot_area=plot_area
        )
    st.session_state["output_buffer"] += buf.getvalue()
...
# Show the buffered output in a text area
st.subheader("Console Output Buffer")
st.text_area("Output", st.session_state["output_buffer"], height=300)

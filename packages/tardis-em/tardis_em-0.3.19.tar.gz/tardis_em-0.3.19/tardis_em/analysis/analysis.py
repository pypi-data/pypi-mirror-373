...
def analyse_filaments(
    ...
    for id_, d_ in enumerate(data):
        ...
        if image is not None:
            intensity_mean, intensity_sum = intensity_list(d_, image[id_], thickness, normalize_image)
...
def analyse_filaments_list(
    ...
    analysis = analyse_filaments(data, images, thicknesses, px=px, normalize_image=normalize_image)
    correlations, correlations_px = calculate_spline_correlations(image_corr, data[0],
                                                                  frame_id=frame_id,
                                                                  thickness=[1, 1])

...
def _load_session_image(session_id: str) -> tuple[str | None, str | None]:
    ...
    candidates = sorted(
        [d for d in out_path.iterdir() if d.is_dir() and d.name.endswith(f"_{session_id}")],
...
def _resize_image_smart(
    ...
    pix_limit_scale = sqrt(max_pixels / float(width * height)) if (width * height) > 0 else 1.0

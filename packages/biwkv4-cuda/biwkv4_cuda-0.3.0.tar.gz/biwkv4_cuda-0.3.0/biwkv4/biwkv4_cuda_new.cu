...
__global__ void biwkv4_cuda_forward_kernel(
    ...
    int i = _t;
    for (; i <= _t + _tokenLength - UNROLL_FACTOR; i += UNROLL_FACTOR) {
        #pragma unroll
        for (int j = 0; j < UNROLL_FACTOR; j++) {
            const int current_i = i + j;
            const int ii = current_i * C;
            scalar_t no = max(o1, k[ii] - w * (current_i - _t));
            scalar_t e1 = exp(o1 - no);
            scalar_t e3 = exp(k[ii] - w * (current_i - _t) - no);
            c = e1 * c + e3 * v[ii];
            d = e1 * d + e3;
            o1 = no;
            const int ni = 2 * _t + _tokenLength - 1 - current_i;
            const int nini = ni * C;

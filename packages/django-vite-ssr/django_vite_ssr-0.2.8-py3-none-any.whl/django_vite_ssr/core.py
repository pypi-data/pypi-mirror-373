...
from .helper import get_config, configure_app
...
class ViteLoader:
    ...
    def render(
        ...
        base_dir = get_config().get("dir")
        print('Default basedir: ', base_dir)

        if vite_path:
            vite_index_path = Path(os.path.join(base_dir, "index.html"))
        else:
            vite_index_path = Path(os.path.join(base_dir, "static", "web", "index.html"))
        ...
        for css in assets["css"]:
            asset_injection += f'<link rel="stylesheet" href="/static/web{css}">\n'
        for js in assets["js"]:
            asset_injection += f'<script type="module" src="/static/web{js}"></script>\n'
        ...
        react_root += f'\n<script id="django-vite-ssr" type="application/json">{json.dumps(context)}</script>\n'

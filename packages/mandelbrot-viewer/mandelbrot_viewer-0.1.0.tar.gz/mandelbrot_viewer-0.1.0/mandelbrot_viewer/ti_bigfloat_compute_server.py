...
from server.shared_memory_handler import connect_to_shared_memory, safe_cleanup
import sympy as s
import numpy as np
import gc
import taichi as ti
...
def compute(x0: float_t,
    ...
    for k, j in ti.ndrange(row_count, n):
        ...
        while zx*zx + zy*zy < i32_to_float(4) and iterations < iter_depth:
            zx_old, zy_old = zx, zy
    ...
    while True:
        ...
        match command:
            ...
            case 'deallocate':
                ...
                print('Server {N}: deallocated img and freed shm')
            ...
            case 'compute':
                try:
                    ...
                    num_chunks = int(time_estimate/0.5+1)
                    row_count = m//num_chunks
                    start_row = 0

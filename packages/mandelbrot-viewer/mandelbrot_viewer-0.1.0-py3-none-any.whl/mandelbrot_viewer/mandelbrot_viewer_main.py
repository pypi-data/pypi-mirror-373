...
import kivy
from server.shared_memory_handler import create_shared_memory, safe_cleanup
...
from mandelbrot_calculator import compute as compute64
...
def start_server_subprocess(i):
    p = subprocess.run(['python', 'ti_bigfloat_compute_server.py', str(i)], capture_output=True)
...
def start_servers():
    for i in range(ranges[0], ranges[1]):
        p = subprocess.Popen(['python', '-u', 'ti_bigfloat_compute_server.py', str(i)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)#, creationflags=subprocess.CREATE_NEW_CONSOLE)#
    ...
    def on_transform_work(self, instance, value):
        ...
        if f != compute64:
            ...
            if not do_sync[i]:
                ...
                p = subprocess.Popen(['python', '-u', 'ti_bigfloat_compute_server.py', str(curr_max_n[0])], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)#, creationflags=subprocess.CREATE_NEW_CONSOLE)#
                subprocesses.append(p)

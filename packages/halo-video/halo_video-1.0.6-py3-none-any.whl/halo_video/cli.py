...
def extract_frames_from_url(youtube_url: str, output_dir: str) -> List[str]:
    ...
    try:
        ...
        console.print(f"[yellow]⚠️  Frame extraction failed: {e}[/yellow]")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if frames:
            try:
                ...
                console.print(f"[green]✅ Analyzed {len(frame_descriptions)} visual frames[/green]")
                        
            except Exception as e:
                console.print(f"[bold red]❌ Error with vision analysis: {e}[/bold red]")
                console.print("[yellow]Continuing with audio transcript only...[/yellow]")
                
        # Combine transcript with visual context
        if frame_descriptions:
            frames_text = "\n".join(frame_descriptions)
            transcript_with_images = transcript + "\n\n[VISUAL CONTEXT - Key frames every 15 seconds]\n" + frames_text
            console.print(f"[bold green]🎯 Ready for Q&A! Video content analyzed with audio transcript + {len(frame_descriptions)} visual frames[/bold green]")
        else:
            transcript_with_images = transcript
            console.print("[bold green]🎯 Ready for Q&A! Video content analyzed with audio transcript[/bold green]")

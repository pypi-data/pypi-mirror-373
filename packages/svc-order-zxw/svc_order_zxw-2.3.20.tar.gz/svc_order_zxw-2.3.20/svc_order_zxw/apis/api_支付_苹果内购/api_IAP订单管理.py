...
async def step1_验证收据_并更新订单(
    ...
    if request.transactionReceipt and request.transactionIdentifier:
        # 首先尝试生产环境验证，如果失败则尝试沙盒环境
        try:
            验证结果 = await apple支付服务_生产环境.验证特定交易(request.transactionIdentifier)
            logger.info(f"苹果支付收据验证结果: {验证结果.model_dump()}")
        except HTTPException_AppToolsSZXW as e:
            logger.exception(f"生产环境 - 苹果支付收据验证失败: {e}")
            验证结果 = await apple支付服务_沙盒环境.验证特定交易(request.transactionIdentifier)
            logger.info(f"沙盒环境 - 苹果支付收据验证结果: {验证结果.model_dump()}")
...
async def step2_恢复购买(request: RequestModel_IPA_恢复购买, db: AsyncSession = Depends(get_db)):
    ...
    if product_type in [ProductType.AUTO_RENEWABLE.value, ProductType.NON_RENEWABLE.value]:
        # 1 订阅型商品
        try:    
            订阅状态: SubscriptionStatus = await apple支付服务_生产环境.获取订阅状态(request.transactionIdentifier)
        except HTTPException_AppToolsSZXW as e:
            logger.exception(f"生产环境 - 获取订阅状态失败: {e}")
            订阅状态: SubscriptionStatus = await apple支付服务_沙盒环境.获取订阅状态(request.transactionIdentifier)
            logger.info(f"沙盒环境 - 获取订阅状态结果: {订阅状态.model_dump()}")
        ...
        # 2. 验证非订阅型商品
        try:
            最新交易 = await apple支付服务_生产环境.验证特定交易(request.transactionIdentifier)
        except HTTPException_AppToolsSZXW as e:
            logger.exception(f"生产环境 - 验证特定交易失败: {e}")
            最新交易 = await apple支付服务_沙盒环境.验证特定交易(request.transactionIdentifier)
            logger.info(f"沙盒环境 - 验证特定交易结果: {最新交易.model_dump()}")

...
class MTerminalCodeController(BaseController):

    """A Controller to access Endpoints in the uexcorpapi API."""
    def __init__(self, config):
        super(MTerminalCodeController, self).__init__(config)

    def retrieve_a_list_of_prices_for_all_commodities(self,
                                                      terminal_code,
                                                      accept):
        """Does a GET request to /commodities_prices/terminal_code/{terminal_code}.

        Args:
            terminal_code (str): (Required)
        ...
        return super().new_api_call_builder.request(
            ...
            .path('/commodities_prices/terminal_code/{terminal_code}')
    ...
    def retrieve_a_list_of_all_fuel_prices(self,
                                           terminal_code,
                                           accept):
        """Does a GET request to /fuel_prices/terminal_code/{terminal_code}.
        ...
        return super().new_api_call_builder.request(
            RequestBuilder().server(Server.SERVER_1)
            .path('/fuel_prices/terminal_code/{terminal_code}')
            .http_method(HttpMethodEnum.GET)
            .template_param(Parameter()
                            .key('terminal_code')
                            .value(terminal_code)

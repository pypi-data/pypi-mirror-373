...
class MTerminalNameController(BaseController):

    """A Controller to access Endpoints in the uexcorpapi API."""
    def __init__(self, config):
        super(MTerminalNameController, self).__init__(config)

    def retrieve_a_list_of_prices_for_all_commodities(self,
                                                      terminal_name,
                                                      accept):
        """Does a GET request to /commodities_prices/terminal_name/{terminal_name}.

        Args:
            terminal_name (str): (Required)
        ...
        return super().new_api_call_builder.request(
            ...
            .path('/commodities_prices/terminal_name/{terminal_name}')
    ...
    def retrieve_a_list_of_all_fuel_prices(self,
                                           terminal_name,
                                           accept):
        """Does a GET request to /fuel_prices/terminal_name/{terminal_name}.
        ...
        return super().new_api_call_builder.request(
            RequestBuilder().server(Server.SERVER_1)
            .path('/fuel_prices/terminal_name/{terminal_name}')
            .http_method(HttpMethodEnum.GET)
            .template_param(Parameter()
                            .key('terminal_name')
                            .value(terminal_name)

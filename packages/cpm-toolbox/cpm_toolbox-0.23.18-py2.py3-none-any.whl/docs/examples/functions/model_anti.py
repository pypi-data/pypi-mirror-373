def model_anticorrelated(data):
    ...
    def model_anticorrelated_generator(parameters, trial):
        ...
        if numpy.isnan(choice_rule.policies).any():
            ...
            choice_rule.policies /= numpy.sum(choice_rule.policies)  # normalize to sum to 1
        ...
        numpy.clip(values, 0, 1, out=values)  # ensure values are between 0 and 1
        # compile output
        output = {
    ...
    def model_anticorrelated_fit(parameters, trial):
        # pull out the parameters
        alpha = parameters.alpha
        temperature = parameters.temperature
        values = numpy.asarray(parameters.values)
    ...
    model_anticorrelated_generative = cpm.generators.Wrapper(
        model=model_anticorrelated_generator,
        parameters=parameters,
        data=data[data.ppt == 1],
    )

    model_anticorrelated_fitting = cpm.generators.Wrapper(
        model=model_anticorrelated_fit,
        parameters=parameters,
        data=data[data.ppt == 1],
    )

    return model_anticorrelated_generative, model_anticorrelated_fitting, parameters
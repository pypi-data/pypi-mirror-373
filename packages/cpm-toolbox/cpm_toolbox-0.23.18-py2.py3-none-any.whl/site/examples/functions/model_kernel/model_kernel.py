def model_kernel(data):
    
    import numpy
    import pandas
    import cpm

    parameters_two = cpm.generators.Parameters(
        ...
        kernels = numpy.array([0.0, 0.0, 0.0, 0.0])
        )

    def model_kernel_generator(parameters, trial):
        # pull out the parameters
        alpha = parameters.alpha
        temperature = parameters.temperature
        kernels = numpy.asarray(parameters.kernels)
        ...
        expected_rewards = kernels[stimulus - 1]
        ...
        # Equation 4.
        error = (teacher - alpha) - expected_rewards[model_choices]
        update = what_to_update * alpha * error
        # Equation 5.
        kernels += numpy.array(update)
        # kernels += np.float64(update)
        # compile output
        output = {
            ...
            "error"    : update,          # prediction error
            "kernels"   : kernels,                  # updated values
    ...
    def model_kernel_fitting(parameters, trial):
        # pull out the parameters
        alpha = parameters.alpha
        temperature = parameters.temperature
        kernels = numpy.asarray(parameters.kernels)
        ...
        # Equation 4.
        error = (teacher - alpha) - expected_rewards[human_choice]
        update = what_to_update * alpha * error
        # Equation 5.
        kernels += numpy.array(update)
        # kernels += np.float64(update)
    ...
    model_kernel_generative = cpm.generators.Wrapper(
        model=model_kernel_generator,
        parameters=parameters_two,
        data=data[data.ppt == 1],
    )

    model_kernel_fit = cpm.generators.Wrapper(
        model=model_kernel_fitting,
        parameters=parameters_two,
        data=data[data.ppt == 1],
    )

    return model_kernel_generative, model_kernel_fit, parameters_two
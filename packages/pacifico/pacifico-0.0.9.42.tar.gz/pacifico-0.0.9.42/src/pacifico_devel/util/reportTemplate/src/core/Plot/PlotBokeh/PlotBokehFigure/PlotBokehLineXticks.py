...
class PlotBokehLineXticks(pltb.PlotBokeh):
    def __init__(self,
                 df: pd.DataFrame,
                 ...
                 title: str = None,
                 xTicks: str = None):
        super(PlotBokehLineXticks, self).__init__(df=df, shape=shape, languageInput=languageInput,
        ...
        for colName, color in zip(self.df.columns,
            ...
            if pd.api.types.is_numeric_dtype(self.df[colName].astype('float')):
                ...
                chart.line(x=data.index,
                           ...
                           # source=data,
                           line_width=2)
                chart.circle(x=data.index,
                             y=data,
                             size=4,
                             fill_color=color.getCodeString(),
                             line_color=color.getCodeString(),
                             # source=data,
                             line_width=1)
        self.style(chart)
    ...
    def style(self, plot):
        ...
        plot.xaxis.major_label_orientation = 'vertical' #pi/4
        plot.yaxis[0].formatter = NumeralTickFormatter(format="y{}")
        plot.xaxis[0].ticker = FixedTicker(ticks=self.df.index)
        plot.xaxis.major_label_overrides = dict(zip(self.df.index, self.xTicks))
        # BACKGROUND
        plot.xgrid.grid_line_color = None
...
if __name__ == '__main__':
    ...
    p = PlotBokehLineXticks(df=dataPlot,
                            shape=(1, 1 / 4),

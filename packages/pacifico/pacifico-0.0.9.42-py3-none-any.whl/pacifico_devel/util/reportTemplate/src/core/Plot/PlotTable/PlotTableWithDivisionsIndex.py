...
class PlotTableWithDivisionsIndex(bplt.Plot):
    def __init__(self,
                 df: pd.DataFrame,
                 ...
                 columnsSubset: list = []):
        super(PlotTableWithDivisionsIndex, self).__init__(df=df,
                                                     shape=shape,
    ...
    def _makeChart(self) -> pdst.Styler:
        ...
        for subset in self.columnsSubset:
            cols.append({"selector": ".col{}".format(subset), "props": [('border-right', '5px solid #ffffff')]})
        dfst = self.df.style.set_table_styles([{"selector": "th.blank", "props": thProps},
                                               {"selector": "td", "props": tdProps},
                                               index_col, index_row] + cols)

    ...
    def __init__(self, value, valueType=Enumerations.ValueType.ValueType_Double):
        if not isinstance(value, valueType.getType()):
            if isinstance(value, str):
                if valueType == Enumerations.ValueType.ValueType_Bool:
                    value = bool(value)
                elif valueType == Enumerations.ValueType.ValueType_Date:
                    value = Dates.julianToDate(float(value))
                elif valueType == Enumerations.ValueType.ValueType_DateTime:
                    value = Dates.julianToDateTime(float(value))
                elif valueType == Enumerations.ValueType.ValueType_Integer:
                    value = int(value)
                elif valueType == Enumerations.ValueType.ValueType_Double:
                    value = float(value)
                elif 'list' in valueType.getString().lower():
                    value = valueType.forceType(value)
                else:

...
def generate_video(prompt: str = "", first_frame_image: str = None, last_frame_image: str = None,
                  reference_images: list = None, duration: int = 5, resolution: str = "720p",
                  fps: int = 24, seed: int = -1, **kwargs) -> Dict[str, Any]:
    """Generate a video using Seedance model with first/last frame images or reference images.
    ...
    Args:
        ...
        reference_images: List of reference image URLs (1-4 images supported)
    ...
    try:
        ...
        # Add reference images if provided (1-4 images supported)
        if reference_images and isinstance(reference_images, list):
            if len(reference_images) > 4:
                logger.warning(f"Too many reference images ({len(reference_images)}), using first 4")
                reference_images = reference_images[:4]
            
            for ref_img_url in reference_images:
                content.append({
...
if __name__ == '__main__':
    # Example usage with first/last frame images
    ...
    # Example usage with reference images
    # video_url = generate_video(
    #     prompt="[Image 1] a beautiful woman, [Image 2] elegant dress, create a fashion video",
    #     reference_images=[

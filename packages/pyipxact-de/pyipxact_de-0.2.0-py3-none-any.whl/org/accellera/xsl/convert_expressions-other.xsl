...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xalan="http://xml.apache.org/xalan" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="xalan exslt msxsl" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
			...
			<xsl:choose>
				...
				<xsl:when test="contains($tokens/token[1], ':pow')">
					<xsl:variable name="closing-bracket-location">
						<xsl:call-template name="find-closing-bracket">
							<xsl:with-param name="tokens">
								<xsl:for-each select="$tokens/token[position() > 2]">
									<xsl:copy-of select="."/>
								</xsl:for-each>
					...
					<xsl:call-template name="replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="$tokens/token[position() > 1 and position() &lt;= $closing-bracket-location]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
					...
					<xsl:call-template name="replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="$tokens/token[position() > $closing-bracket-location]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
	...
	<!-- Finds a token, making sure the token is located at the same level (identified by '(' and ')' brackets) as where the search started, i.e. outside any internal brackets. -->
	<xsl:template name="find-closing-bracket">
		<xsl:param name="tokens"/>
		<xsl:param name="index" select="1"/>
		<xsl:param name="level" select="1"/>
		
		<xsl:if test="debug">
			<xsl:message>find-closing-bracket(<xsl:value-of select="$tokens"/>, <xsl:value-of select="$index"/>, <xsl:value-of select="$level"/>)</xsl:message>
		</xsl:if>
		
		<xsl:if test="$level = 0 or count($tokens/token) &lt; $index">
			<xsl:value-of select="$index"/>
		</xsl:if>
		
		<xsl:if test="$level > 0 and count($tokens/token) >= $index">
			<xsl:choose>
				<xsl:when test="$tokens/token[$index] = '('">
					<xsl:call-template name="find-closing-bracket">
						<xsl:with-param name="tokens" select="$tokens"/>

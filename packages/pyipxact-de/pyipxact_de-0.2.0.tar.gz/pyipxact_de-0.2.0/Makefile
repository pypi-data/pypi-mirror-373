...
test:
	$(run) pytest -q
#	$(run) pytest -v tests/test_versions.py
#	$(run) pytest -v tests/test_xml.py
#	$(run) pytest -v tests/test_xml_validator.py
#	$(run) pytest -v tests/test_xml_transformer.py
...
.PHONY: run-app
run-app:
	@$(run) uvicorn --host ${HOSTNAME} --app-dir src amal.eda.ipxact_webapp.main:app --reload
...
.PHONY: run-app-gen
run-app-gen:
	@$(run) uvicorn --host ${HOSTNAME} --app-dir .temp/app main:app --reload
...
cli:
	...
	@echo "========================================================================================================================"
	@$(run) -m amal.eda.ipxact_de.cli --help
...
# ----------------------------------------------------------------------------------------------------------------------
# view-docs: ## View HTML documentation
# ----------------------------------------------------------------------------------------------------------------------
.PHONY: view-docs
...
# ----------------------------------------------------------------------------------------------------------------------
# changelog-draft: ## Draft changelog
# ----------------------------------------------------------------------------------------------------------------------
.PHONY: changelog-draft
changelog-draft:
	@echo "Drafting Unreleased notes from git commits since last tag..."
	@$(run) scripts/changelog.py draft --select --write


# ----------------------------------------------------------------------------------------------------------------------
# changelog-finalize: ## Finalize changelog
# ----------------------------------------------------------------------------------------------------------------------
.PHONY: changelog-finalize
changelog-finalize:
	@if [ -z "$(VERSION)" ]; then echo "Usage: make changelog-finalize VERSION=X.Y.Z" >&2; exit 1; fi
	@echo "Finalizing CHANGELOG for version $(VERSION) ..."
	@$(run) scripts/changelog.py finalize --version $(VERSION)

...
keyword = on_message(rule=Rule(_dynamic_fullmatch_rule), priority=10, block=True)
...
async def handle_function(msg = EventPlainText()):
    for cmd in cmds_config:
        if msg in cmd:
            ...
            if res:
                await keyword.finish(MessageSegment.video(res.content))
            else:
                # 轮询所有接口
                for url_dic in urls:
                    url = url_dic.get('url')
                    ...
                    if res:
                        await keyword.finish(MessageSegment.video(res.content))
                        return
                await keyword.finish(f"{msg}接口已全部失效，请稍后再试或更换新的接口")
            return
...
@add_api.got("video_api", prompt="请输入视频接口")

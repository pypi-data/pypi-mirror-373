...
from starlette.responses import Response, StreamingResponse, ContentStream
...
async def _make_aiter(content: bytes, chunk_size: int):
    length = len(content)
    seek = 0
    while seek < length:
...
class StreamResponse(StreamingResponse):
    ...
    def __init__(
            ...
            chunk_size: int = 1048576 * 1048576,
    ) -> None:
        super().__init__(
        ...
        if isinstance(content, bytes):
            ...
            self.body_iterator = _make_aiter(content, chunk_size)

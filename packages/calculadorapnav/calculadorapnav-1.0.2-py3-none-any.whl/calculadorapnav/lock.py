"""
Ensures that only one instance of calculadorapnav is running at a time.
It creates a lock file in /tmp/.calculadorapnav.lock and removes it on exit.
...
LOCKFILE = os.path.join(tempfile.gettempdir(), ".calculadorapnav.lock")


class Lock:
    """A lock to ensure only one instance of calculadorapnav is running."""
    ...
    def acquire(self):
        ...
        with self._thread_lock:
            ...
            try:
                ...
                logger.debug(
                    f"Lock file {LOCKFILE} already exists. This usually means another instance of calculadorapnav is running. You can safely delete it if you want or use allow_multiple_runs parameter to always bypass it."
                )
                raise

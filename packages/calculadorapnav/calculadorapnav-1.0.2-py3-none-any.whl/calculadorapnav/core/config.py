...
def clean_env_key(k: str) -> str:
    ...
    calculadorapnav_ and to lower case.

    eg: "calculadorapnav_PROJECT_NAME" -> "project_name"
    ...
    return k.lower().replace("calculadorapnav_", "", 1)


def parse_env_config() -> dict:
    """
    Get all environment variables starting with "calculadorapnav_" (also in lower caps)
    mapped to their values in a dict

    eg:
        * "calculadorapnav_PROJECT_NAME=DeepLearning" -> {"project_name": "DeepLearning"}
        * "calculadorapnav_project_name=DeepLearning" -> {"project_name": "DeepLearning"}
    ...
    return {
        "calculadorapnav": {
            ...
            if k.lower().startswith("calculadorapnav_")
        }
    }
...
def get_hierarchical_config():
    """
    Get the user-defined calculadorapnav configuration ConfigParser dictionnary
    ...
    It looks for, and reads, a config file .calculadorapnav.config in the user's $HOME.
    It then looks for, reads, and updates the previous configuration from a config
    file .calculadorapnav.config in the current working directory (Path.cwd()).
    Finally it updates the resulting config from any environment variable starting
    with `calculadorapnav_` (for instance if `calculadorapnav_PROJECT_NAME` is `your-project`
    ...
    cwd = Path.cwd()
    home = Path.home()
    global_path = str((home / ".calculadorapnav.config").expanduser().resolve())
    local_path = str((cwd / ".calculadorapnav.config").expanduser().resolve())
    if Path(global_path).exists():
        logger.info(
            f"calculadorapnav is taking the configuration from global file: {global_path}"
        )
        if Path(local_path).exists():
            logger.info(f"Some variables are overriden by the local file: {local_path}")
    elif Path(local_path).exists():
        logger.info(
            f"calculadorapnav is taking the configuration from the local file {local_path}"
        )

    config.read([global_path, local_path])
    config.read_dict(parse_env_config())

    return dict(config["calculadorapnav"])

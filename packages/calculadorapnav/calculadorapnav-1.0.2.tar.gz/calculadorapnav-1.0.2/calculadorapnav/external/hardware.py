...
from calculadorapnav.core.cpu import IntelPowerGadget, IntelRAPL
from calculadorapnav.core.gpu import AllGPUDevices
from calculadorapnav.core.powermetrics import ApplePowermetrics
from calculadorapnav.core.units import Energy, Power, Time
from calculadorapnav.core.util import SLURM_JOB_ID, detect_cpu_model
...
class RAM(BaseHardware):
    ...
    def __init__(
        ...
        self.set_memory_type(type_ram)

    def set_memory_type(self, memory_type: str):
        """
        Set the power consumption per GB based on the memory type.

        Args:
            memory_type (str): Type of memory (e.g., DDR3, DDR4)
        """
        if memory_type == "DDR3":
            self.power_per_GB = 3 / 8  # W/GB
        elif memory_type == "DDR4":
            self.power_per_GB = 2 / 8  # W/GB
        elif memory_type == "DDR5":
            self.power_per_GB = 1.5 / 8  # W/GB
        else:
            logger.warning(f"Unknown memory type {memory_type}, defaulting to DDR4")
            self.power_per_GB = 2 / 8  # W/GB
    ...
    def _parse_scontrol(self, scontrol_str):
        ...
        if len(mem_matches) == 0:
            # Try with TRES, see https://github.com/mlco2/calculadorapnav/issues/569#issuecomment-2167706145

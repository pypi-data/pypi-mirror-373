...
from .core import ListeningSocket, find_pids_by_port, list_open_ports, print_table, is_dev_port, _is_port_free
from .docker_utils import enrich_with_docker_info, _docker_container_by_host_port
from .process_utils import kill_pid, parse_signal, _kill_project_processes, _get_current_project_path
...
def cmd_clean(args: argparse.Namespace) -> int:
    ...
    if not project_path:
        print("No project detected (no package.json, Cargo.toml, etc. found)")
        return 1
    
    project_name = project_path.split("/")[-1] if "/" in project_path else project_path.split("\\")[-1]
    print(f"Finding processes for project: {project_name}")
    ...
    killed_count = _kill_project_processes(project_path, force)
    
    if killed_count == 0:
        print(f"No processes found for project {project_name}")
        return 0
    ...
    if not getattr(args, 'yes', False):
        try:
            response = input(f"\nKill {killed_count} processes? [y/N]: ").strip().lower()
    ...
    print(f"\nCleaned {killed_count} processes for project {project_name}")
    return 0
...
def cmd_free(args: argparse.Namespace) -> int:
    ...
    container_name = _docker_container_by_host_port(port, protocol)
    if container_name and shutil.which("docker"):
        ...
        try:
            subprocess.run(["docker", "stop", container_name], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            print(f"docker stop {container_name}")
        except subprocess.CalledProcessError as e:
            print(f"docker stop failed: {e.stderr.strip() or e.stdout.strip()}")
    ...
    for pid, process_name in pids:
        ...
        try:
            if force and not dry_run:
                ...
                ok, msg = kill_pid(pid, parse_signal("KILL"), dry_run=False)
            else:
                # Graceful termination
                ok, msg = kill_pid(pid, sig, dry_run=dry_run)
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    all_sockets = list_open_ports(protocol)
    sockets = [s for s in all_sockets if is_dev_port(s.local_port, dev_mode)]
...
def cmd_kill(args: argparse.Namespace) -> int:
    ...
    if args.pid:
        ...
        ok, msg = kill_pid(args.pid, sig, dry_run=getattr(args, 'dry_run', False))
        if ok:
        ...
        for pid, name in selected_pids:
            if pid is None:
                continue
            ok, msg = kill_pid(pid, sig, dry_run=getattr(args, 'dry_run', False))
            if ok:
...
def cmd_pick(args: argparse.Namespace) -> int:
    ...
    killable = [s for s in killable if is_dev_port(s.local_port, dev_mode)]

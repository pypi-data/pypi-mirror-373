...
class Commands:
    ...
    def cmd_genius(self, args):
        "Execute a Genius Mode command"
        
        if not self.genius_mode:
            self.io.tool_error("Genius Mode is not enabled. Use --genius to enable it.")
            return
            
        try:
            result = self.genius_mode.handle_request(args.strip())
            return result
        except Exception as err:
            self.io.tool_error(f"Genius Mode error: {err}")
            return
    ...
    def completions_genius(self):
        # Return empty list for now - can be extended with specific genius commands
        return []
    ...
    def is_genius_command(self, inp):
        return inp.startswith("/genius")
    ...
    def run(self, inp):
        ...
        # Handle genius commands first
        if inp.startswith("/genius"):
            self.coder.event("command_genius")
            return self.do_run("genius", inp[8:])  # Remove "/genius " prefix

...
MODEL_REGISTRY = {
    ...
    # Anthropic Models
    "claude-sonnet-4": {
        "full_name": "anthropic/claude-sonnet-4-20250514",
        "provider": "anthropic",
        "max_tokens": 8192,
    ...
    "deepseek-reasoner": {
        "full_name": "deepseek/deepseek-reasoner",
        "provider": "deepseek",
        "max_tokens": 8192,
    ...
    # Google Models
    "gemini-2.5-pro": {
        "full_name": "gemini/gemini-2.5-pro",
        "provider": "google",
        "max_tokens": 8192,
        ...
        "supports_functions": True,
        "supports_streaming": True,
        "pricing": {"input": 0.001250, "output": 0.005},
        "capabilities": ["chat", "vision", "function_calling"],
        "category": "premium"
    },
    "gemini-2.5-flash": {
...
class ModelRegistry:
    """Centralized model registry for managing model metadata and capabilities."""
    
    def __init__(self):
        self._registry = MODEL_REGISTRY.copy()
        self._aliases = MODEL_ALIASES.copy()
    
    def register_model(self, alias: str, metadata: Dict[str, Any]) -> None:
        """Register a new model with metadata."""
        self._registry[alias] = metadata
    
    def get_model_info(self, model_name: str) -> Dict[str, Any]:
        """Get comprehensive model information by name or alias."""
        # Resolve alias first
        resolved_name = self._aliases.get(model_name, model_name)
        
        # Look for direct match in registry
        if resolved_name in self._registry:
            return self._registry[resolved_name]
        
        # Look for partial matches
        for key, info in self._registry.items():
            if resolved_name in info.get("full_name", ""):
                return info
        
        return {}
    
    def list_supported_models(self, category: Optional[str] = None, 
                            capability: Optional[str] = None) -> List[str]:
        """List all supported models, optionally filtered by category or capability."""
        models = []
        for alias, info in self._registry.items():
    ...
    def is_model_supported(self, model_name: str) -> bool:
        ...
        return bool(self.get_model_info(model_name))
    
    def get_models_by_provider(self, provider: str) -> List[str]:
        """Get all models for a specific provider."""
        return [alias for alias, info in self._registry.items() 
    ...
    def get_optimal_model(self, criteria: Dict[str, Any]) -> Optional[str]:
        """Find the optimal model based on criteria like max_tokens, price, etc."""
        candidates = []
        ...
        for alias, info in self._registry.items():
            ...
            if "capabilities" in criteria:
                ...
                if not all(cap in info.get("capabilities", []) for cap in required_caps):
                    continue
            
            # Filter by max tokens requirement
            if "min_max_tokens" in criteria:
                if info.get("max_tokens", 0) < criteria["min_max_tokens"]:
                    continue
            
            # Filter by context length requirement  
            if "min_context_length" in criteria:
                if info.get("context_length", 0) < criteria["min_context_length"]:
                    continue
            
            # Filter by category
            if "category" in criteria:
                if info.get("category") != criteria["category"]:
                    continue
        ...
        if "optimize_for" in criteria:
            if criteria["optimize_for"] == "cost":
                # Sort by input price (ascending)
                candidates.sort(key=lambda x: x[1].get("pricing", {}).get("input", float('inf')))
            elif criteria["optimize_for"] == "speed":
                # Prefer models with streaming support and smaller context
                candidates.sort(key=lambda x: (
                    not x[1].get("supports_streaming", False),
                    x[1].get("context_length", 0)
                ))
            elif criteria["optimize_for"] == "capability":
                # Sort by number of capabilities (descending)
                candidates.sort(key=lambda x: -len(x[1].get("capabilities", [])))
...
# Helper functions for backward compatibility and external integration
def get_model_info(model_name: str) -> Dict[str, Any]:
    """Get model information - main integration point for llm.py and sendchat.py."""
    return model_registry.get_model_info(model_name)

def list_supported_models(category: Optional[str] = None) -> List[str]:
    """List all supported models - main integration point."""
    return model_registry.list_supported_models(category=category)

def is_model_supported(model_name: str) -> bool:
    """Check if model is supported - main integration point."""
    return model_registry.is_model_supported(model_name)

def get_optimal_model_for_task(task_type: str, **kwargs) -> Optional[str]:
    ...
    return model_registry.get_optimal_model(criteria)
...
class ModelInfoManager:
    ...
    def get_model_info(self, model):
        # First check our enhanced registry
        registry_info = model_registry.get_model_info(model)
        if registry_info:
            return {
                "max_tokens": registry_info.get("max_tokens"),
                "max_input_tokens": registry_info.get("context_length"),
                "max_output_tokens": registry_info.get("max_tokens"),
                "input_cost_per_token": registry_info.get("pricing", {}).get("input"),
                "output_cost_per_token": registry_info.get("pricing", {}).get("output"),
                "litellm_provider": registry_info.get("provider"),
                "supports_vision": registry_info.get("supports_vision", False),
                "supports_function_calling": registry_info.get("supports_functions", False),
                "mode": "chat"
            }
...
class Model(ModelSettings):
    ...
    def get_enhanced_info(self):
        """Get enhanced model information from the registry."""
        return model_registry.get_model_info(self.name)

    def supports_capability(self, capability: str) -> bool:
        """Check if this model supports a specific capability."""
        return model_registry.supports_capability(self.name, capability)

    def estimate_cost(self, input_tokens: int, output_tokens: int) -> Optional[float]:
        """Estimate cost for this model with given token counts."""
        return model_registry.estimate_cost(self.name, input_tokens, output_tokens)
...
# Integration helper functions for llm.py and sendchat.py
...
def get_model_for_task_and_budget(task: str, max_cost_per_1k_tokens: float = None) -> Optional[str]:
    """Get the best model for a task within budget constraints."""
    criteria = {}
    ...
    if max_cost_per_1k_tokens:
        ...
        for alias, info in MODEL_REGISTRY.items():
            pricing = info.get("pricing", {})
            if pricing.get("input", 0) <= max_cost_per_1k_tokens:
                affordable_models.append(alias)
        
        # If we have affordable models, limit search to them
...
def validate_model_for_use_case(model_name: str, use_case: Dict[str, Any]) -> Dict[str, Any]:
    """Validate if a model meets specific use case requirements."""
    ...
    if not info:
        ...
        validation_result["errors"].append(f"Model {model_name} not found in registry")
        return validation_result
    
    # Check required capabilities
    required_caps = use_case.get("required_capabilities", [])
    model_caps = info.get("capabilities", [])
    
    for cap in required_caps:
        if cap not in model_caps:
            validation_result["valid"] = False
            validation_result["errors"].append(f"Model does not support required capability: {cap}")
    
    # Check token requirements
    min_tokens = use_case.get("min_max_tokens")
    if min_tokens and info.get("max_tokens", 0) < min_tokens:
        validation_result["valid"] = False
        validation_result["errors"].append(f"Model max_tokens ({info.get('max_tokens')}) < required ({min_tokens})")
    
    # Check context length requirements
    min_context = use_case.get("min_context_length")
    if min_context and info.get("context_length", 0) < min_context:
        validation_result["warnings"].append(f"Model context length ({info.get('context_length')}) < preferred ({min_context})")
    
    # Check budget constraints
    max_cost = use_case.get("max_cost_per_1k_tokens")
    if max_cost:
        input_cost = info.get("pricing", {}).get("input", 0)
        if input_cost > max_cost:
            validation_result["warnings"].append(f"Model cost ({input_cost}) exceeds budget ({max_cost})")
    
    return validation_result


def get_fallback_models(primary_model: str, fallback_criteria: Dict[str, Any] = None) -> List[str]:
    ...
    # Default fallback criteria based on primary model
    criteria = fallback_criteria or {}
    
    # Try to match capabilities of primary model
    if "capabilities" not in criteria:
        criteria["capabilities"] = primary_info.get("capabilities", ["chat"])
    
    # Prefer same provider first, then others
    fallbacks = []
    primary_provider = primary_info.get("provider")
    
    # Same provider models first
    if primary_provider:
        same_provider_models = model_registry.get_models_by_provider(primary_provider)
        for model in same_provider_models:
            if model != primary_model:
                fallbacks.append(model)
    
    # Then other providers
    all_suitable = model_registry.list_supported_models()
    for model in all_suitable:
        if model not in fallbacks and model != primary_model:
...
def main():
    if len(sys.argv) < 2:
        ...
        print("       python models.py --list [category]")
        print("       python models.py --info <model_name>")
        print("       python models.py --optimal <task_type>")
        sys.exit(1)
    ...
    if command == "--yaml":
        ...
        models = list_supported_models(category)
        if category:
        ...
        if len(sys.argv) < 3:
            print("Usage: python models.py --info <model_name>")
            sys.exit(1)
        ...
        if info:
            ...
            if 'pricing' in info:
                ...
                print(f"  Pricing: ${pricing.get('input', 0):.6f} input, ${pricing.get('output', 0):.6f} output per token")
        else:
            print(f"Model '{model_name}' not found in registry")
    elif command == "--optimal":
        if len(sys.argv) < 3:
            print("Usage: python models.py --optimal <task_type>")
            print("Task types: coding, reasoning, vision, chat, budget")
            sys.exit(1)
        task_type = sys.argv[2]
        optimal = get_optimal_model_for_task(task_type)
        if optimal:
            print(f"Optimal model for '{task_type}': {optimal}")
            info = get_model_info(optimal)
            if info and 'pricing' in info:
                pricing = info['pricing']
                print(f"  Cost: ${pricing.get('input', 0):.6f} input, ${pricing.get('output', 0):.6f} output per token")
        else:
            print(f"No suitable model found for task '{task_type}'")
    else:
        # Original fuzzy matching behavior
        model_name = command

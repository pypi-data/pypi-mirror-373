...
from spreadlinks.models import get_library_or_404, get_library_set


def render_with_template(
    default_template_name, default_base_template_name="spreadlinks/base.html"
):
        ...
        def wrapped_handler(
            ...
            **kwargs,
            ...
            if "style_links" not in result:
                result["style_links"] = (
                    style_links if style_links is not None else ["style/base.css"]
                )
            if "style_includes" not in result:
                result["style_includes"] = (
                    style_includes if style_links is not None else []
                )
...
def library_detail(request, root_dir, library_name, urlencoded_keywords="", page=1):
    ...
    for facet_name, keywords in facet_keywords.items():
        ...
        facet_drillupdowns[facet_name] = {
            "drillups": drillups,
            "drilldowns": drilldowns,
        }

    facet_drillupdowns = sorted(
        facet_drillupdowns.items(),
        key=lambda facet_name_facet: (
            facet_name_facet[0] != "main",
            facet_name_facet[0],
        ),
    )
    ...
    return {
        "library": lib,
        "base_template_name": (
            lib.base_template if hasattr(lib, "base_template") else None
        ),
        "links": links_page,

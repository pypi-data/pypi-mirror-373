...
import httpx
import yaml
from superai_langchain.rag.extract_info import extract_title
from superai_langchain.rag.init_retrievers import (BaselineVectorRetriever, KeywordRetriever,
                                                   MixRetriever, SmallerChunksVectorRetriever)
from superai_langchain.rag.utils import import_by_type, import_class
from superai_langchain.retrievers import EnsembleRetriever
from superai_langchain.utils.util import clear_query
from superai_langchain.vectorstores import ElasticKeywordsSearch, Milvus
...
class SuperaiRAGTool:
    ...
    def __init__(self,
        ...
        if keyword_store:
            ...
            if self.params['elasticsearch'].get('extract_key_by_llm', False):
                extract_key_prompt = import_class(
                    'superai_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
        ...
        if QA_PROMPT:
            ...
            if 'prompt_type' in self.params['generate']:
                ...
                prompt = import_class(f'superai_langchain.rag.prompts.{prompt_type}')
            else:
                prompt = None
    ...
    def retrieval_and_rerank(self, query):
        ...
        for doc in docs:
            ...
            if file_id:
                doc.page_content = page_content.replace('{<file_name>', f'{{<file_id>{file_id}</file_id><file_name>', 1)
        ...
        if self.params['post_retrieval'].get('sort_by_source_and_index', False):
            ...
            docs = sorted(docs, key=lambda x: (x.metadata.get('source',''), x.metadata.get('chunk_index',0)))
        return docs
    ...
    def run(self,
        ...
        #引入qwen330B后，用户可能输入/think或者/no_think，以便控制模型的思考开关，为了不污染召回，把/think和/no_think删掉
        cquery = clear_query(query)
        docs = self.retrieval_and_rerank(cquery)
        try:
            kwargs = {}
            if run_manager:
...
if __name__ == '__main__':
    # rag_tool = SuperaiRAGTool(collection_name='rag_finance_report_0_test')
    ...
    tool = SuperaiRAGTool.get_rag_tool(name='rag_knowledge_retrieve',
    ...
    # tool = SuperaiRAGTool.get_rag_tool(

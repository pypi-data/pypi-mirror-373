...
def jobs():
    ...
    Tip: Use '--stream' (or '--follow') to watch long-running jobs live.
    """
    pass
...
@click.option('--stream', is_flag=True, help='Alias for --follow (stream live updates)')
@click.pass_context
def status(ctx, job_id, follow, interval, logs, logs_tail, format, stream):
    ...
    USAGE:
      ...
      ivybloom jobs status <job_id> --stream      # Alias for --follow
        ...
        with IvyBloomAPIClient(config, auth_manager) as client:
            if follow or stream:
                ...
                with Live(console=console, refresh_per_second=2) as live:
                    while True:
                        job_data = client.get_job_status(job_id, include_logs=logs, logs_tail=logs_tail if logs else None)

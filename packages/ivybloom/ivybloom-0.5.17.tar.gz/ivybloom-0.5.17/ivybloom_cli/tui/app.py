...
class IvyBloomTUI(App):
    ...
    /* Column widths: responsive defaults */
    #left_column {{
        width: 1fr;
        min-width: 40;
    }}
    ...
    def compose(self) -> ComposeResult:
        if self.show_header:
            yield Header()
        with Horizontal():
            with Vertical(id="left_column") as _left_col:
                self.left_column = _left_col  # type: ignore[attr-defined]
                yield Static("Jobs", classes="panel-title")
                self.jobs_table = DataTable(zebra_stripes=True)
                yield self.jobs_table
            with Vertical(id="right_column") as _right_col:
                self.right_column = _right_col  # type: ignore[attr-defined]
                ...
                # Dedicated parameters pane (separate from summary)
                self.details_params = Static("", classes="muted details_params")
                self.details_structure = Static("No structure loaded", classes="muted")
                # Streaming console for long-running subprocess output
                self.details_console = TextLog(highlight=False, markup=False, wrap=True)
                ...
                try:
                    ...
                    self.details_params.styles.overflow_y = "auto"
                ...
                with TabbedContent():
                    ...
                    with TabPane("Console", id="console"):
                        yield self.details_console
                    # Optional hidden Debug tab, shown only when debug is enabled
    ...
    def _compute_visualization_size(self) -> Tuple[int, int]:
        ...
        try:
            total_w = max(0, int(self.size.width))  # type: ignore[attr-defined]
            total_h = max(0, int(self.size.height))  # type: ignore[attr-defined]
        except Exception:
            total_w, total_h = 120, 30
        # Heuristic: details panel is roughly right side; aim for ~45% to 60% of total width
        # Subtract some padding for borders and margins
        width = max(40, min(120, int(total_w * 0.55) - 6))
        # Height: use available screen height minus header/footer/input/status (~8-12 rows)
        height = max(18, min(40, total_h - 12))
        return width, height

    def _apply_responsive_layout(self, width: int, height: int) -> None:
        """Adjust column ratios and any density settings based on terminal size."""
        try:
    ...
    def on_resize(self, event) -> None:  # type: ignore[override]
        ...
        try:
            if getattr(self, "_last_viz_kind", None) == "protein" and getattr(self, "_last_viz_payload", None):
                payload = self._last_viz_payload or {}
                pdb_text = payload.get("pdb_text", "")
                filename = payload.get("filename", "protein.pdb")
                if pdb_text and self.details_visualization:
                    pv = ProteinVisualizer()
                    vw, vh = self._compute_visualization_size()
                    ascii_art = pv.render_pdb_as_text(pdb_text, width=vw, height=vh, filename_hint=filename)
                    self.details_visualization.update(f"[green]Protein Structure (ASCII)[/green]\n\n{ascii_art}")
    ...
    def _stream_to_console(self, args: List[str], timeout: int = 600, env_overrides: Optional[Dict[str, str]] = None) -> None:
        """Stream CLI output lines into the console TextLog widget.

        This improves perceived performance and avoids blocking on long subprocess runs.
        """
        try:
            if not hasattr(self, "details_console") or self.details_console is None:
                return
            self.details_console.clear()
            # Ensure console tab is visible
            try:
                # Best effort: rely on TabbedContent default selection or user to switch
                pass
            except Exception:
                pass
            for line in self._runner.run_cli_stream(args, env_overrides=env_overrides):
                try:
                    self.details_console.write(line)
                except Exception:
                    # As a fallback, append to summary
                    if self.details_summary:
                        self.details_summary.update(line)
            # Final newline for aesthetics
            try:
                self.details_console.write("")
            except Exception:
                pass
        except Exception as e:
            if self.details_summary:
                self.details_summary.update(f"[red]Stream failed:[/red] {self._format_error(e)}")
    ...
    def _follow_job_to_console(self, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> None:
        ...
        try:
            ...
            self.details_console.clear()
            terminal_hit = False
            terminals = {"COMPLETED", "SUCCESS", "FAILURE", "FAILED", "CANCELLED", "ARCHIVED"}
            for raw in self._runner.run_cli_stream(args, env_overrides=env_overrides):
                line = str(raw or "")
                try:
                    self.details_console.write(line)
                except Exception:
                    if self.details_summary:
                        self.details_summary.update(line)
                up = line.upper()
                if any(s in up for s in terminals):
                    terminal_hit = True
            try:
                self.details_console.write("")
            except Exception:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Follow stream failed:[/red] {self._format_error(e)}")
    ...
    def _cmd_auth_status(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Auth status failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_auth_whoami(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Auth whoami failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_auth_logout(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Logout failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_auth_link(self, wait: bool) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Auth link failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_show(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config show failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_list(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config list failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_get(self, key: Optional[str]) -> None:
        ...
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config get failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_set(self, key: str, value: Optional[str]) -> None:
        ...
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config set failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_reset(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config reset failed:[/red]\n{self._format_error(e)}")
    ...
    def _cmd_config_path(self) -> None:
        try:
            ...
            if self.details_summary:
                self.details_summary.update(f"[red]Config path failed:[/red]\n{self._format_error(e)}")

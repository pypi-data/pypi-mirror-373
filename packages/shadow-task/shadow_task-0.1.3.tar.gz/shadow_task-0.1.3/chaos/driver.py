...
from shadows.execution import Operator
...
async def main(
    ...
    async with (
        ...
        environment = {
            **os.environ,
            "SHADOW_NAME": shadows.name,
            "SHADOW_URL": redis_url,
        }
        ...
        for _ in range(100):
            ...
            await shadows.strike("rando", parameter, operator, value)
        ...
        while True:
            try:
                async with shadows.redis() as r:
                    ...
                    stream_length = await r.xlen(shadows.stream_key)
                    pending = await r.xpending(
                        shadows.stream_key, shadows.worker_group_name
                    )

...
shadow tasks include a sophisticated dependency injection system that provides access to context, configuration, and custom resources. this system is similar to fastapi's dependency injection but tailored specifically for background task patterns and distributed execution scenarios.

## built-in context dependencies

### accessing the current shadow

tasks often need to schedule more work. the `currentshadow` dependency gives you access to the same shadows the worker is processing:

```python
from pathlib import Path
from datetime import datetime, timedelta, timezone
from shadows import Shadow, CurrentShadow
...
```python
from shadows import CurrentShadow, TaskKey
...
```python
from shadows import CurrentWorker, CurrentExecution, Worker, Execution
...
dependencies can depend on other dependencies, and shadow resolves them in the correct order:
...
dependencies can access shadow's built-in context dependencies:
...
```python
from shadows import TaskArgument

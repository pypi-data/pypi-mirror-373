...
async def test_task_timeout_respects_redelivery_timeout(shadows: Shadow):
    ...
    async with Worker(
        ...
        task_started = False
        task_completed = False
        await shadows.add(long_running_task)(customer_id=2, test_mode="complete")
        await worker.run_until_finished()
        ...
        task_started = False
        task_completed = False
        await shadows.add(long_running_task)(customer_id=3, test_mode="long_complete")
        await worker.run_until_finished()
        assert task_started, "Third task should have started"
        assert task_completed, "Third task should have completed"


async def test_task_timeout_with_concurrent_tasks(shadows: Shadow):
    ...
    async with Worker(
        ...
        for _ in range(3):  # 3 tasks, but max_concurrent=2
            await shadows.add(task_within_timeout)(customer_id=1)
...
async def test_redelivery_timeout_limits_long_tasks(shadows: Shadow):
    ...
    async with Worker(
        ...
        await shadows.add(long_task)(customer_id=1)
        ...
        task_completed = False
        await shadows.add(long_task)(customer_id=2, test_mode="complete")
        await worker.run_until_finished()
        assert task_completed, "Second task should have completed"

        # Test long completion path for coverage
        task_completed = False
        await shadows.add(long_task)(customer_id=3, test_mode="long_complete")
        await worker.run_until_finished()
        assert task_completed, "Third task should have completed"


async def test_short_tasks_complete_within_timeout(shadows: Shadow):
    ...
    async with Worker(
        ...
        for _ in range(5):
            await shadows.add(short_task)(customer_id=1)

...
from shadows.dependencies import Retry
from shadows.instrumentation import (
    healthcheck_server,
    message_getter,
    message_setter,
    metrics_server,
...
async def test_executing_a_task_is_wrapped_in_a_span(shadows: Shadow, worker: Worker):
    captured: list[Span] = []
    ...
    execution = await shadows.add(the_task)()
    ...
    assert task_span.attributes["shadows.name"] == shadows.name
    assert task_span.attributes["shadows.task"] == "the_task"
    assert task_span.attributes["shadows.key"] == execution.key
    assert task_span.attributes["shadows.when"] == execution.when.isoformat()
    assert task_span.attributes["shadows.attempt"] == 1
...
async def test_failed_task_span_has_error_status(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(the_failing_task)()
    await worker.run_until_finished()
...
async def test_retried_task_spans_have_error_status(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(the_retrying_task)()
    await worker.run_until_finished()
...
async def test_infinitely_retrying_task_spans_have_error_status(
    ...
    execution = await shadows.add(the_infinite_retry_task)()
...
@pytest.fixture
def task_labels(shadows: Shadow, the_task: AsyncMock) -> dict[str, str]:
    """Create labels dictionary for the task-side metrics."""
    return {"shadows.name": shadows.name, "shadows.task": the_task.__name__}
...
def TASKS_ADDED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    monkeypatch.setattr("shadows.instrumentation.TASKS_ADDED.add", mock)
    return mock
...
def TASKS_REPLACED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_REPLACED.add", mock)
    return mock
...
def TASKS_SCHEDULED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_SCHEDULED.add", mock)
    return mock
...
async def test_replacing_a_task_increments_counter(
    ...
    await shadows.replace(the_task, when, key)()
...
def TASKS_CANCELLED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_CANCELLED.add", mock)
    return mock
...
async def test_cancelling_a_task_increments_counter(
    ...
    when = datetime.now(timezone.utc) + timedelta(minutes=5)
    key = "test-cancel-key"
    await shadows.add(the_task, when=when, key=key)()

    await shadows.cancel(key)

    TASKS_CANCELLED.assert_called_once_with(1, {"shadows.name": shadows.name})
...
def worker_labels(
    ...
    return {
        "shadows.name": shadows.name,
        "shadows.worker": worker.name,
        "shadows.task": the_task.__name__,
    }
...
def TASKS_STARTED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_STARTED.add", mock)
    return mock
...
def TASKS_COMPLETED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_COMPLETED.add", mock)
    return mock
...
def TASKS_SUCCEEDED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_SUCCEEDED.add", mock)
    return mock
...
def TASKS_FAILED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_FAILED.add", mock)
    return mock
...
def TASKS_RETRIED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_RETRIED.add", mock)
    return mock
...
def TASKS_REDELIVERED(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Counter.add)
    monkeypatch.setattr("shadows.instrumentation.TASKS_REDELIVERED.add", mock)
    return mock
...
async def test_redelivered_tasks_increment_redelivered_counter(
    ...
    worker = Worker(shadows, redelivery_timeout=timedelta(milliseconds=50))
    ...
    worker2 = Worker(shadows, redelivery_timeout=timedelta(milliseconds=100))
    async with worker2:
...
def TASK_DURATION(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    monkeypatch.setattr("shadows.instrumentation.TASK_DURATION.record", mock)
    return mock


async def test_task_duration_is_measured(
    shadows: Shadow, worker: Worker, worker_labels: dict[str, str], TASK_DURATION: Mock
):
...
def TASK_PUNCTUALITY(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Histogram.record)
    monkeypatch.setattr("shadows.instrumentation.TASK_PUNCTUALITY.record", mock)
    return mock
...
async def test_task_punctuality_is_measured(
    ...
    await shadows.add(the_task, when=when)()
...
def TASKS_RUNNING(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    monkeypatch.setattr("shadows.instrumentation.TASKS_RUNNING.add", mock)
    return mock


async def test_task_running_gauge_is_incremented(
    shadows: Shadow, worker: Worker, worker_labels: dict[str, str], TASKS_RUNNING: Mock
):
...
async def test_exports_metrics_as_prometheus_metrics(
    ...
    with metrics_server(port=metrics_port):
        ...
        assert "shadows_tasks_added" in body
        assert "shadows_tasks_completed" in body

        assert f'shadows_name="{shadows.name}"' in body
        assert 'shadows_task="the_task"' in body
        assert f'shadows_worker="{worker.name}"' in body
...
def QUEUE_DEPTH(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    monkeypatch.setattr("shadows.instrumentation.QUEUE_DEPTH.set", mock)
    return mock
...
def SCHEDULE_DEPTH(monkeypatch: pytest.MonkeyPatch) -> Mock:
    ...
    mock = Mock(spec=Gauge.set)
    monkeypatch.setattr("shadows.instrumentation.SCHEDULE_DEPTH.set", mock)
    return mock


@pytest.fixture
def shadows_labels(shadows: Shadow) -> dict[str, str]:
    """Create labels dictionary for the Shadow client-side metrics."""
    return {"shadows.name": shadows.name}


async def test_worker_publishes_depth_gauges(
    shadows: Shadow,
    shadows_labels: dict[str, str],
    the_task: AsyncMock,
    QUEUE_DEPTH: Mock,
    SCHEDULE_DEPTH: Mock,
):
    ...
    await shadows.add(the_task, when=future)()
    await shadows.add(the_task, when=future)()
    await shadows.add(the_task, when=future)()

    shadows.heartbeat_interval = timedelta(seconds=0.1)
    async with Worker(shadows):
        await asyncio.sleep(0.2)  # enough for a heartbeat to be published

    QUEUE_DEPTH.assert_called_once_with(2, shadows_labels)
    SCHEDULE_DEPTH.assert_called_once_with(3, shadows_labels)

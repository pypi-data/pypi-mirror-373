...
async def test_concurrency_limit_single_argument(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(slow_task)(customer_id=1)
    await shadows.add(slow_task)(customer_id=1)
    await shadows.add(slow_task)(customer_id=1)
    await shadows.add(slow_task)(customer_id=1)
    await shadows.add(slow_task)(customer_id=1)
...
async def test_concurrency_limit_different_arguments(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(slow_task)(customer_id=1)
    await shadows.add(slow_task)(customer_id=2)
    await shadows.add(slow_task)(customer_id=3)
...
async def test_concurrency_limit_max_concurrent(shadows: Shadow, worker: Worker):
    ...
    for i in range(5):
        await shadows.add(slow_task)(task_id=i, db_name="postgres")
...
async def test_concurrency_limit_missing_argument_error(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(task_with_missing_arg)(customer_id=123)

    # This should cause the task to fail but not crash the worker
    await worker.run_until_finished()


async def test_concurrency_limit_with_custom_scope(shadows: Shadow, worker: Worker):
    ...
    await shadows.add(task_with_scope)(customer_id=1)
    await shadows.add(task_with_scope)(customer_id=1)
...
async def test_concurrency_limit_single_dependency_validation(shadows: Shadow):
    ...
    with pytest.raises(
        ...
        await shadows.add(invalid_task)(customer_id=1)
...
async def test_concurrency_limit_without_concurrency_dependency(
    ...
    for i in range(5):
        await shadows.add(normal_task)(customer_id=i)

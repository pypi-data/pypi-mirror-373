...
from shadows.cli import app, relative_time
from shadows.shadows import Shadow
from shadows.worker import Worker


@pytest.fixture(autouse=True)
async def empty_shadows(shadows: Shadow, aiolib: str):
    """Ensure that the shadows has been created"""
    future = datetime.now(timezone.utc) + timedelta(seconds=60)
    await shadows.add(tasks.trace, key="initial", when=future)("hi")
    await shadows.cancel("initial")


async def test_snapshot_empty_shadows(shadows: Shadow, runner: CliRunner):
...
async def test_snapshot_with_scheduled_tasks(shadows: Shadow, runner: CliRunner):
    """Should show scheduled tasks in the snapshot"""
    when = datetime.now(timezone.utc) + timedelta(seconds=5)
    await shadows.add(tasks.trace, when=when, key="future-task")("hiya!")
...
async def test_snapshot_with_mixed_tasks(shadows: Shadow, runner: CliRunner):
    """Should show both running and scheduled tasks in the snapshot"""
    heartbeat = timedelta(milliseconds=20)
    shadows.heartbeat_interval = heartbeat

    future = datetime.now(timezone.utc) + timedelta(seconds=5)
    await shadows.add(tasks.trace, when=future)("hi!")
    for _ in range(5):  # more than the concurrency allows
        await shadows.add(tasks.sleep)(2)

    async with Worker(shadows, name="test-worker", concurrency=2) as worker:
        worker_running = asyncio.create_task(worker.run_until_finished())
...
def test_relative_time(
    ...
    monkeypatch.setattr("shadows.cli.local_time", consistent_format)

    assert relative_time(now, when) == expected


async def test_snapshot_with_stats_flag_empty(shadows: Shadow, runner: CliRunner):
    ...
    assert "0 workers, 0/0 running" in result.output
    # With empty shadows, stats table shouldn't appear since there are no tasks


async def test_snapshot_with_stats_flag_mixed_tasks(shadows: Shadow, runner: CliRunner):
    ...
    future = datetime.now(timezone.utc) + timedelta(seconds=5)
    await shadows.add(tasks.trace, when=future)("hi!")
    await shadows.add(tasks.trace, when=future)("hello!")
    for _ in range(3):
        await shadows.add(tasks.sleep)(2)
...
async def test_snapshot_with_stats_shows_timestamp_columns(
    shadows: Shadow, runner: CliRunner
):
    ...
    await shadows.add(tasks.trace, when=early_time)("early task")
    await shadows.add(tasks.trace, when=late_time)("late task")
    await shadows.add(tasks.sleep, when=early_time)(1)
...
async def test_snapshot_stats_with_running_tasks_only(
    ...
    await shadows.add(tasks.sleep)(0.5)
    await shadows.add(tasks.sleep)(0.5)

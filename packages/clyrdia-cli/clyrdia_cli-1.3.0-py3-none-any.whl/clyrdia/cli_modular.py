...
def login():
    ...
    try:
        ...
        try:
            ...
            console.print(f"\n[bold yellow]🚀 Next step: Initialize your environment[/bold yellow]")
            console.print(f"Run: [cyan]clyrdia-cli init[/cyan] to set up your benchmarking environment")
            console.print(f"This will configure your local setup and check your API keys.")
...
@require_auth
def init():
    """Initialize your environment for benchmarking after login"""
    try:
        licensing_manager = LicensingManager()
        
        # Get user status to confirm authentication
        user_status = asyncio.run(licensing_manager.get_status())
        
        console.print(Panel.fit(
            f"[bold green]🚀 Welcome to Clyrdia, {user_status.user_name}![/bold green]\n\n"
            f"Initializing your environment for AI benchmarking...",
            border_style="green",
            padding=(1, 2),
        ...
        # Check if user has API keys for AI providers
        from .core.env_loader import get_api_keys, has_api_keys
        
        api_keys = get_api_keys()
        
        if not has_api_keys():
            console.print("[yellow]⚠️  No AI provider API keys found.[/yellow]")
            console.print("\n[bold]To run benchmarks, you need API keys for:[/bold]")
            console.print("• OpenAI (OPENAI_API_KEY)")
            console.print("• Anthropic (ANTHROPIC_API_KEY)")
            console.print("• Or other supported providers")
            
            console.print("\n[bold]Setup options:[/bold]")
            console.print("1. Create a .env file in your project directory")
            console.print("2. Set environment variables in your shell")
            console.print("3. Use 'clyrdia-cli env' to check current status")
            
            # Create sample .env file
            sample_env_content = """# Clyrdia Configuration
...
# Optional: Clyrdia API key (if you have one)
CLYRIDIA_API_KEY=your_clyrdia_api_key_here
"""
            ...
            if not env_file.exists():
                ...
                console.print(f"\n[green]✅ Created sample .env file: {env_file.absolute()}[/green]")
                console.print("[yellow]Please edit this file with your actual API keys.[/yellow]")
            else:
                console.print(f"\n[yellow]⚠️  .env file already exists at: {env_file.absolute()}[/yellow]")
                console.print("[yellow]Please ensure it contains your API keys.[/yellow]")
        else:
            console.print(f"[green]✅ Found API keys for: {', '.join(api_keys.keys())}[/green]")
        ...
        try:
            ...
            user_id = licensing_manager._get_user_id_from_api_key()
            if user_id:
                user_data = db.get_user(user_id)
                if not user_data:
                    db.create_user(
                        user_id=user_id,
                        username=user_status.user_name,
                        email=f"{user_status.user_name}@example.com",
                        plan_tier=user_status.plan.value,
                        credits_remaining=user_status.credits_remaining,
                        monthly_credits=user_status.credits_monthly_limit
                    )
                    console.print(f"[green]✅ Created local user profile[/green]")
                else:
                    console.print(f"[green]✅ Local user profile ready[/green]")
        except Exception as e:
            console.print(f"[yellow]⚠️  Could not initialize local database: {str(e)}[/yellow]")
        
        # Initialize cache directory
        try:
            from .caching.manager import CacheManager
            cache_manager = CacheManager()
            console.print(f"[green]✅ Cache system initialized[/green]")
        except Exception as e:
            console.print(f"[yellow]⚠️  Could not initialize cache: {str(e)}[/yellow]")
        
        # Show next steps
        console.print(Panel.fit(
            "[bold green]🎉 Environment initialization complete![/bold green]\n\n"
            "[bold]Next steps:[/bold]\n"
            "1. Ensure your .env file contains API keys\n"
            "2. Run 'clyrdia-cli env' to verify setup\n"
            "3. Create a benchmark config file\n"
            "4. Run 'clyrdia-cli run --config your_config.yaml'\n\n"
            "[bold]Need help?[/bold]\n"
            "• Visit: https://clyrdia.com/docs\n"
            "• Run: clyrdia-cli --help",
            border_style="green",
            padding=(1, 2),

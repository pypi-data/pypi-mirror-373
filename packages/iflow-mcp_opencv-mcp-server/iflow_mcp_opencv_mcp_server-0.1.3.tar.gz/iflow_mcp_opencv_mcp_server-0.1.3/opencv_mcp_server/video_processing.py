        ...
        while time.time() - start_time < duration:
            ...
            if len(indices) > 0:
                ...
                for i in indices:
                    # Extract bounding box coordinates
                    # In some versions this may be i[0] instead of just i
                    # Handle both scalar and array indices for OpenCV compatibility
                    try:
                        if isinstance(i, (list, tuple, np.ndarray)) and len(i) > 0:
                            idx = int(i[0])
                        else:
                            idx = int(i)
                    except (IndexError, TypeError, ValueError) as e:
                        logger.warning(f"Error processing detection index {i}: {str(e)}")
                        continue

...
class ExcelWorker:
    ...
    def _apply_conditional_formatting(self, cell: xw.Range, cf_data: Dict[str, Any]):
        """Apply conditional formatting to the cell."""
        try:
            cf_type = cf_data.get('type', 'cell_value')
            operator = cf_data.get('operator', 'greater')
            value1 = cf_data.get('value1')
            value2 = cf_data.get('value2')
            background_color = cf_data.get('background_color')
            font_color = cf_data.get('font_color')
            ...
            # Get the worksheet to add conditional formatting
            ws = cell.sheet
            
            # Create conditional formatting rule
            if cf_type == 'cell_value':
                # Map operator strings to constants
                op_map = {
                    'equal': xlEqual,
                    'not_equal': xlNotEqual,
                    'greater': xlGreater,
                    'less': xlLess,
                    'greater_equal': xlGreaterEqual,
                    'less_equal': xlLessEqual,
                    'between': xlBetween,
                    'not_between': xlNotBetween
                }
                
                excel_operator = op_map.get(operator, xlGreater)
                
                # Add conditional formatting rule
                cf_rule = ws.api.Range(cell.address).FormatConditions.Add(
                    Type=xlCellValue,
                    Operator=excel_operator,
                    Formula1=value1,
                    Formula2=value2 if value2 is not None else None
                )
                
                # Set formatting
                if background_color and background_color.startswith('#'):
                    rgb = tuple(int(background_color[i:i+2], 16) for i in (1, 3, 5))
                    rgb_int = rgb[0] + (rgb[1] << 8) + (rgb[2] << 16)
                    cf_rule.Interior.Color = rgb_int
                    
                if font_color and font_color.startswith('#'):
                    rgb = tuple(int(font_color[i:i+2], 16) for i in (1, 3, 5))
                    rgb_int = rgb[0] + (rgb[1] << 8) + (rgb[2] << 16)
                    cf_rule.Font.Color = rgb_int
            
            elif cf_type == 'color_scale':
                # Add color scale conditional formatting
                cf_rule = ws.api.Range(cell.address).FormatConditions.AddColorScale(ColorScaleType=3)
                # Default color scale (red-yellow-green)
                
            elif cf_type == 'data_bars':
                # Add data bars conditional formatting
                cf_rule = ws.api.Range(cell.address).FormatConditions.AddDatabar()
                
            elif cf_type == 'icon_set':
                # Add icon set conditional formatting
                cf_rule = ws.api.Range(cell.address).FormatConditions.AddIconSetCondition()
                cf_rule.IconSet = ws.api.Application.WorksheetFunction.IconSets[0]  # 3Arrows
                
            logger.debug(f"Applied conditional formatting to cell {cell.address}")
            
        except Exception as e:
            cell_ref = getattr(cell, 'address', 'unknown')
            logger.warning(f"Warning: Could not set conditional formatting for cell {cell_ref}: {e}")
    ...
    def _update_chart(self, chart: xw.Chart, chart_data: Dict[str, Any]):
        ...
        try:
            ...
            # Apply comprehensive chart styling (including data labels)
            # Get the Chart object from the tuple returned by chart.api
            chart_obj = None
            if hasattr(chart, 'api') and isinstance(chart.api, tuple) and len(chart.api) > 1:
                chart_obj = chart.api[1]  # Get the Chart object (second element)
                logger.debug(f"Got chart object for styling: {type(chart_obj)}")
            
            if chart_obj:
                self._apply_chart_styling(chart_obj, chart_data, chart.name)
                logger.debug(f"Applied comprehensive styling to chart {chart.name}")
            else:
                logger.warning(f"Could not get chart object for styling chart {chart.name}")
    ...
    def _apply_chart_styling(self, chart_obj, chart_data: Dict[str, Any], chart_name: str):
        ...
        try:
            ...
            for i in range(1, series_collection.Count + 1):
                ...
                # Apply data labels for this series
                data_labels_key = f's{i}_data_labels'
                logger.debug(f"Checking data labels for series {i}: key={data_labels_key}, exists={data_labels_key in chart_data}, global_exists={'data_labels' in chart_data}")
                
                if data_labels_key in chart_data or 'data_labels' in chart_data:
                    try:
                        # Use series-specific setting if available, otherwise use global setting
                        show_labels = chart_data.get(data_labels_key, chart_data.get('data_labels', False))
                        logger.debug(f"Data labels setting for series {i}: show_labels={show_labels} (type: {type(show_labels)})")
                        
                        # Convert string boolean values
                        if isinstance(show_labels, str):
                            show_labels = show_labels.lower() in ['true', '1', 'yes', 'on']
                            logger.debug(f"Converted string to boolean for series {i}: {show_labels}")
                        
                        if show_labels:
                            logger.info(f"🏷️  Attempting to enable data labels for series {i}...")
                            
                            # Check if series object is valid
                            logger.debug(f"Series object type: {type(series)}")
                            logger.debug(f"Series object attributes: {[attr for attr in dir(series) if not attr.startswith('_')][:10]}")
                            
                            series.HasDataLabels = True
                            logger.debug(f"Set HasDataLabels=True for series {i}")
                            
                            data_labels = series.DataLabels
                            logger.debug(f"Got DataLabels object: {type(data_labels)}")
                            
                            # Configure data label properties
                            data_labels.ShowValue = True
                            logger.debug(f"Set ShowValue=True for series {i}")
                            
                            # Apply data label font if specified
                            if 'data_label_font' in chart_data:
                                logger.debug(f"Applying data label font for series {i}: {chart_data['data_label_font']}")
                                self._apply_font_to_object(data_labels.Font, chart_data['data_label_font'], f"data labels for series {i} in chart {chart_name}")
                            
                            # Apply data label position if specified
                            if 'data_label_position' in chart_data:
                                position = chart_data['data_label_position'].lower()
                                logger.debug(f"Applying data label position for series {i}: {position}")
                                position_map = {
                                    ...
                                    'left': -4131,        # xlLeft
                                    'right': -4152,       # xlRight
                                    'inside_end': -4117,  # xlInsideEnd
                                    'outside_end': -4114, # xlOutsideEnd
                                    'inside_base': -4112, # xlInsideBase
                                    'best_fit': -4100     # xlBestFit
                                }
                                if position in position_map:
                                    data_labels.Position = position_map[position]
                                    logger.info(f"✅ Set data labels position for series {i}: {position}")
                                else:
                                    logger.warning(f"Unknown data label position: {position}")
                            
                            # Apply data label color if specified
                            if 'data_label_color' in chart_data:
                                color = chart_data['data_label_color']
                                logger.debug(f"Applying data label color for series {i}: {color}")
                                if color.startswith('#'):
                                    rgb = tuple(int(color[j:j+2], 16) for j in (1, 3, 5))
                                    rgb_int = rgb[0] + (rgb[1] << 8) + (rgb[2] << 16)
                                    data_labels.Font.Color = rgb_int
                                    logger.info(f"✅ Set data labels color for series {i}: {color} (RGB: {rgb})")
                            
                            logger.info(f"✅ Successfully enabled data labels for series {i} in chart {chart_name}")
                        else:
                            logger.info(f"❌ Disabling data labels for series {i} (show_labels={show_labels})")
                            series.HasDataLabels = False
                            
                    except Exception as e:
                        logger.error(f"❌ Failed to set data labels for series {i} in chart {chart_name}: {e}")
                        logger.debug(f"Data labels error details:", exc_info=True)
                else:
                    logger.debug(f"No data labels settings found for series {i} (checked {data_labels_key} and data_labels)")
                        
        except Exception as e:
            logger.warning(f"Could not apply series colors and data labels to chart {chart_name}: {e}")
...
import atexit
atexit.register(ExcelWriter.cleanup)
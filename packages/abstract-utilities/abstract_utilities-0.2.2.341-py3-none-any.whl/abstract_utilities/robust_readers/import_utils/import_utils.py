...
def is_import_or_init(sysroot):
    ...
    return nuroot
def _unique_module_name(base: str, path: Path) -> str:
...
def get_imports(files: List[str],self=None) -> Dict[str, Dict[str, Any]]:
    ...
    for file in files:
        ...
        top_pkg_dir = _find_top_package_dir(sysroot_guess) or _find_top_package_dir(file_p)
        if not top_pkg_dir:

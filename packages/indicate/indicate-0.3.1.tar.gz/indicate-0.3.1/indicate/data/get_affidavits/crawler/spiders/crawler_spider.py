...
class CrawlerSpider(Spider):
    ...
    def parse(self, response):
        ...
        for tr in questions.css("table#data-tab>tbody>tr"):
            ...
            # print ('=======================', detail_link)
            item["name_english"] = tr.xpath(
                'td[2]/div[@class="details-name"]/h4/text()'
            ).extract()
    ...
    def parse_product(self, response):
        ...
        can_details = can_details.xpath(
            'div/div/div/div/div[@class="card-body"]/div/div'
        )
        ...
        item["party_name"] = (
            can_details[0].xpath("div[2]/text()").extract()[0].strip("\n").strip()
        )
        item["name_english"] = (
            can_details[2]
            .xpath("div[2]/div/p/b/text()")
            .extract()[0]
            .strip("\n")
            .strip()
        )
        ...
        item["assembly_constituency"] = (
            can_details[4].xpath("div[2]/div/p/text()").extract()[0].strip("\n").strip()
        )
        item["state"] = (
            can_details[5].xpath("div[2]/div/p/text()").extract()[0].strip("\n").strip()
        )
        item["application_uploaded"] = (
            can_details[6].xpath("div[2]/div/p/text()").extract()
        )
        item["current_status"] = can_details[7].xpath("div[2]/div/p/text()").extract()

        can_per_details = questions.css("section")[1]
        can_per_details = can_per_details.xpath(
            'div/div[@class="row"]/div/div/div/div/div/form/div/div[1]/div'
        )

        item["fathers_or_husbands_name_english"] = (
            can_per_details[0].xpath("div/p/text()").extract()
        )
        item["fathers_or_husbands_name_hindi"] = (
            can_per_details[1].xpath("div/p/text()").extract()
        )

...
async def save_geometry(
    ...
    """
    try:
        geometry = json.dumps(payload.cords)
        query = (
            "UPDATE geometries "
...
async def get_geometries(
    ...
    try:
        ...
        result = await conn.fetch_rows(
            geom_query, [payload.entity_type, payload.id_entity]
        )
        geometries = []
        for geom in result:
            geometry = GeometryResponse(**{"id": geom["suuid"], **dict(geom)})
            geometry.cords = json.loads(geom["st_asgeojson"])
...
def format_response(result: Record) -> List[GeometryResponse]:
    ...
    for geom in result:
        ...
        geometry.cords = json.loads(dict(geom)["st_asgeojson"])
        geometries.append(geometry)
    return geometries
...
async def load_geometry(
    ...
    try:
        ...
        geom = SGLFeature(**{
            "cords": geojson, "geomType": geom_type
        })
        return geom

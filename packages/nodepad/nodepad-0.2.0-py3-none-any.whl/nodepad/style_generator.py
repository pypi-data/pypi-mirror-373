...
This module provides functionality to extract Style node information from Blender files
...
def extract_style_nodes(
    filepath: Union[str, Path], node_prefix: str = "Style "
) -> Dict[str, StyleNodeInfo]:
    """
    Extract information about all nodes with the specified prefix from a blend file.

    Parameters
    ----------
    filepath : Union[str, Path]
        Path to the blend file to extract from
    node_prefix : str, optional
        Prefix to filter nodes by, by default "Style "
    ...
    # Find all nodes that start with the specified prefix
    for node_group_name in bpy.data.node_groups.keys():
        if not node_group_name.startswith(node_prefix):
            continue
        ...
        for input_socket in input_sockets:
            if hasattr(input_socket, "default_closed"):
                continue  # skip if panel
...
def save_style_data_to_json(
    output_path: Path, filepath: Union[str, Path], node_prefix: str = "Style "
) -> None:
    """
    ...
    node_prefix : str, optional
        Prefix to filter nodes by, by default "Style "
    """
    style_nodes = extract_style_nodes(filepath, node_prefix)
...
def extract_enum_values_from_error(
    ...
    try:
        ...
        if "in (" in error_message:
            ...
            possible_values = [v.strip().strip("'\"") for v in values_part.split(",")]
...
def generate_python_class(style_info: StyleNodeInfo) -> str:
    ...
    ----------
    style_info : StyleNodeInfo
        StyleNodeInfo object containing node data
...
def generate_style_classes_file(
    output_path: Path, filepath: Union[str, Path], node_prefix: str = "Style "
) -> None:
    """
    ...
    ----------
    output_path : Path
        Path where to save the Python file
    filepath : Union[str, Path]
        Path to the blend file to extract from
    node_prefix : str, optional
        Prefix to filter nodes by, by default "Style "

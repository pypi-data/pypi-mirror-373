...
class FastBHB :
    ...
    def get_full_waveform(self, template=None, beta=None, lam=None,
                          chi1=None, chi2=None, m1=None, m2=None, Deltat=None,
                          phi=None, psi=None, inc=None, dist=None):
        """ Return the waveform full signal including:
        freq, amp, phase, transferL1 -> L3, tf for all modes.
        """
    ...
    def get_waveform(self, **kwargs):
        ...
        sig = self.get_full_waveform(**kwargs)
    ...
    def get_full_tdi(self, template=None, freqs=None, tdi2=False, with_wftdi=False):
        """ Return the waveform full signal including:
        """
        try:
            import lisabeta.lisa.lisa as lisa
        ...
        if self.bbh_type.lower() == "mbhb" :
            if with_wftdi:
                ...
                sig = {'tdi': lisa.GetCAmpPhaseTDI(lisa.GenerateLISATDI_SMBH(params,
                                                                             **self.wvf_pars))}
        if self.bbh_type.lower() in ["sobhb", "sbbh"]:
            if with_wftdi:
                sig = lisa.GenerateLISATDISignal_SOBH(params, **self.wvf_pars)
            else:
                sig = {'tdi': lisa.GetCAmpPhaseTDI(lisa.GenerateLISATDI_SOBH(params,
                                                                             **self.wvf_pars))}
    ...
    def get_fd_tdiaet(self, template=None, freqs=None, tdi2=False):
        ...
        sig = self.get_full_tdi(template=template, freqs=freqs, tdi2=tdi2)

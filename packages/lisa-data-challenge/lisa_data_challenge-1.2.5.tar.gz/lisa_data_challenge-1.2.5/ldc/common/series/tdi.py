...
class TDI(xr.Dataset):
    ...
    @staticmethod
    def load(filename, name='data', compound=True, keepaxis=False):
        """ Load tdi from file
        ...
        if compound:
            ...
            if attrs['coord']=='t':
                if keepaxis:
                    ds = dict([(k,TimeSeries(arr[k], ts=arr['t'], units=attrs["units"]))
                ...
                df = arr['f'][1] - arr['f'][0]
                if keepaxis:
                    ds = dict([(k,FrequencySeries(arr[k], fs=arr['f'], units=attrs["units"]))

...
def CaselessPhrase(text: str):
    """
    Create a case-insensitive parser that matches an entire phrase,
    which may contain spaces, and ensures a word boundary at the end.

    Parameters
    ----------
    text : str
        The phrase to match (can contain spaces).

    Returns
    -------
    pyparsing.ParserElement
        A parser that matches the given phrase, case-insensitive,
        and only when followed by a word boundary.
    """
    pattern = r"(?<!\w)" + re.escape(text) + r"(?=\W|$)"
    return pp.Regex(pattern, flags=re.IGNORECASE)

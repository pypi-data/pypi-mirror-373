...
from .grammar import generate_grammar, CaselessPhrase
...
class BaseDefinition:
  ...
  class Repeated(Enum):
      ...
      # Repeated sections has no header, and their repetition is solved by themselves
   ...
   def _grammar_of_name(self):
        ...
        if self.name_in_grammar:
            if self.name_regex:
                ...
                names = [ CaselessPhrase(i) for i in self.all_names_in_grammar() ]
                if self.do_not_skip_whitespaces_before_name:
                    for i in names:
                        i.leaveWhitespace()
            if len(names) > 1:

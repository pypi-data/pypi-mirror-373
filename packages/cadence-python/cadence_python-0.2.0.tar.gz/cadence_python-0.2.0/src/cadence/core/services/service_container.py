...
class ServiceContainer(Loggable):
    """Enhanced service container with layered architecture and dependency injection.

    Manages the complete service hierarchy:
    - Infrastructure Layer: Repositories, LLM Factory, Plugin Manager
    - Application Layer: ConversationService, OrchestratorService
    - Domain Layer: Models and business rules (injected via repositories)
    """
    ...
    async def initialize(
        ...
        Args:
            ...
            thread_repository: Optional thread repository (defaults to in-memory)
            conversation_repository: Optional conversation repository (defaults to in-memory)
        """

"""Cadence Framework LLM Model Factory - Multi-Provider Model Management with Caching.

This module provides the core LLM model factory implementation for the Cadence multi-agent
AI framework. It handles model creation, caching, provider management, and tool binding
with support for multiple LLM providers and intelligent optimization strategies.

Factory Architecture:
    The LLM factory implements several key design patterns:
    - Factory Pattern: Centralized model creation and configuration
    - Strategy Pattern: Pluggable provider implementations
    - Cache Pattern: Intelligent model instance caching for performance
    - Registry Pattern: Dynamic provider discovery and management

Key Components:
    ModelCacheManager: Intelligent caching of model instances to prevent overhead
    ProviderRegistry: Dynamic registration and management of LLM providers
    LLMModelFactory: Main factory class with provider selection and optimization

Performance Features:
    - Model instance caching based on provider, model, and temperature
    - Lazy loading of provider credentials and configurations
    - Connection pooling and rate limiting for provider APIs
    - Automatic retry and failover mechanisms

Provider Support:
    - OpenAI: GPT models with function calling and streaming
    - Anthropic: Claude models with large context windows
    - Google: Gemini models with multi-modal capabilities
    - Azure OpenAI: Enterprise OpenAI with regional deployment

Example Usage:
    Basic model factory operations:
    ...
    gpt4 = await factory.get_model("gpt-4")
    claude = await factory.get_model("claude-3-opus")

    gpt4_cached = await factory.get_model("gpt-4")

    response = await gpt4.ainvoke("Explain quantum computing")
    ```
    ...
    result = await agent_model.ainvoke("Calculate 15 * 23 + 7")
    ```
    ...
    if factory.validate_provider("openai"):
        model = await factory.get_model("gpt-4")
    ```

Caching Strategy:
    The factory implements intelligent caching to optimize performance:
    - Cache key combines provider, model name, and temperature
    - Cached instances are reused for identical configurations
    - Cache invalidation on provider credential changes

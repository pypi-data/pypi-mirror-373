...
class Organization(BaseModel):
    ...
    Attributes:
        ...
        settings: Free-form settings map (e.g., model defaults, rate limits).
        total_tokens_used: Accumulated tokens for the current accounting period.
        monthly_token_limit: Optional monthly quota; when set, usage is
    ...
    def can_create_threads(self) -> bool:
        """Return whether the org can create new threads.

        The organization must be active, and if a monthly quota is configured,
        current usage must be below that limit.

        Returns:
            True if thread creation is allowed, otherwise False.
        """
    ...
    def add_token_usage(self, tokens: int) -> None:
        ...
        Args:
            tokens: Number of tokens to add. Must be non-negative.
    ...
    def get_token_usage_percentage(self) -> Optional[float]:
        """Return usage as a percentage of the monthly limit.

        Returns:
            Percentage in the range [0, 100] if a limit is set, otherwise
            None when `monthly_token_limit` is not configured.
        """
    ...
    def is_approaching_limit(self, threshold: float = 80.0) -> bool:
        ...
        Args:
            threshold: Percentage threshold to compare against, default 80.0.

        Returns:
            True if usage percentage is computed and >= threshold, else False.
        """

...
def split_openapi_by_tag(spec):
    ...
    Also removes the path prefix up to and including the tag from each path.
    """
    tag_specs = {}
...
def convert_api_spec():
    ...
    if args.split_by_tag:
        save_split_specs(modified_spec, output_path)
        print(f"OpenAPI spec split by tag and saved to directory {output_path}")
    else:

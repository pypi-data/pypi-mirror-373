...
class AbstractAsyncFile(ABC, Generic[T]):
    ...
    async def load_bytes(self, mode: str, **kwargs) -> Optional[Any]:
        async with aiofiles.open(self.local_path, mode=mode, **kwargs) as f:
            return await f.read()

    async def save_bytes(self, data: Any, mode: str, **kwargs):
        async with aiofiles.open(self.local_path, mode=mode, **kwargs) as f:
            await f.write(data)

...
def get_any(string, default=None, logger=False, reraise=False):
    ...
    :param logger: The logger to log errors to (False = No logging,
                   True/None = module default: batutils.tpu.strimp)
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'ImportError in batutils.tpu.strimp.get_any("%s"): %r', string, ex)
        if reraise:
            raise
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'AttributeError in batutils.tpu.strimp.get_any("%s"): %r', string, ex)
        if reraise:
            raise
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_any("%s"): %r', string, ex)
        if reraise:
            raise
...
def get_module(string, default=None, logger=False, reraise=False):
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_module("%s"): %r', string, ex)
        if reraise:
            raise
    except Exception:
        if reraise:
...
def get_class(string, default=None, logger=False, reraise=False):
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_class("%s"): %r', string, ex)
        if reraise:
            raise
    except Exception:
        if reraise:
...
def get_callable(string, default=None, logger=False, reraise=False):
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_callable("%s"): %r', string, ex)
        if reraise:
            raise
    except Exception:
        if reraise:
...
def get_methfunc(string, default=None, logger=False, reraise=False):
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_methfunc("%s"): %r', string, ex)
        if reraise:
            raise
    except Exception:
        if reraise:
...
def get_function(string, default=None, logger=False, reraise=False):
    ...
    try:
        ...
        if logger is not False:
            if logger is True or logger is None:
                logger = log
            logger.exception(u'Exception in batutils.tpu.strimp.get_function("%s"): %r', string, ex)
        if reraise:
            raise
    except Exception:
        if reraise:

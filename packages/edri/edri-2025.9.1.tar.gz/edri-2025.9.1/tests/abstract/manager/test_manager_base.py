...
class TestManagerBaseMeta(unittest.TestCase):
    ...
    # def test_multiple_events_in_union_type(self):
    #     from typing import Union
    #
    #     class MultiEvent(Event): pass
    #     class AnotherEvent(Event): pass
    #
    #     class MultiHandlerManager(ManagerBase):
    #         @request
    #         def multi_event_handler(self, event: Union[MultiEvent, AnotherEvent]):
    #             event._handled = True
    #
    #     mgr = MultiHandlerManager()
    #     mgr._prepare_resolvers()
    #
    #     self.assertIn(MultiEvent, mgr._requests)
    #     self.assertIn(AnotherEvent, mgr._requests)
    #
    #     evt1 = MultiEvent()
    #     evt2 = AnotherEvent()
    #     mgr._requests[MultiEvent](evt1)
    #     mgr._requests[AnotherEvent](evt2)
    #
    #     self.assertTrue(evt1._handled)
    #     self.assertTrue(evt2._handled)
...
class TestManagerBase(unittest.TestCase):
    ...
    def test_run_resolver(self):
        ...
        with (patch("edri.abstract.manager.manager_base.wait",
                    side_effect=[[self.manager.router_pipe, pipe_non_worker], [pipe_non_worker], [pipe_non_worker], [self.worker_pipe],
                                 [self.worker_pipe]]),
              patch.object(self.manager, "resolve") as mock_resolve,
              ...
              patch("edri.abstract.manager.manager_base.sleep") as mock_sleep,
            ...
            mock_get_pipes.side_effect = [[], [self.manager.router_pipe, pipe_non_worker], [self.manager.router_pipe, pipe_non_worker],
                                          ...
                                          [self.manager.router_pipe, pipe_non_worker]]
    ...
    def test_start_worker(self):
        ...
        resolver = MagicMock(return_value=process, spec=WorkerProcess)
        pipe = MagicMock(spec=Connection)

        with patch("edri.abstract.manager.manager_base.Pipe", return_value=(pipe, pipe)), \
                patch("edri.abstract.manager.manager_base.randint", return_value=12345):
            self.manager._workers = MagicMock(spec=Storage)
            worker = self.manager.start_worker(event, resolver)
            resolver.assert_called_once_with(pipe, event, f"{self.manager.name}-Worker_12345")

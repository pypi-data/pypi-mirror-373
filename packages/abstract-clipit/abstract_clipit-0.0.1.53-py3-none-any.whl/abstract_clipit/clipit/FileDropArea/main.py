from .imports import *
from abstract_utilities import get_media_exts, eatOuter,ifFunctionsInFiles
...
class FileDropArea(QtWidgets.QWidget):
    ...
    def __init__(self, log_widget=None, view_widget=None, parent=None):
        ...
        self.view_tabs.addTab(list_tab, "Py List View")


        # List View Tab
        all_list_tab = QtWidgets.QWidget()
        all_list_layout = get_layout(parent=list_tab)
        self.all_file_list = QtWidgets.QListWidget()
        self.all_file_list.setVisible(False)
        self.all_file_list.setAcceptDrops(False)
        self.all_file_list.itemClicked.connect(self.on_file_get_sections)
        add_widgets(all_list_layout, {"widget": self.all_file_list}, {"widget": self.all_function_list})
        self.view_tabs.addTab(all_list_tab, "All List View")
...
ifFunctionsInFiles(root=os.path.dirname(os.path.abspath(__file__)), self=FileDropArea)

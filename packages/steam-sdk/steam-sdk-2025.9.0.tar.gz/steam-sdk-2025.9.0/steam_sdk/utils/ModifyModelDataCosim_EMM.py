...
if __name__ == "__main__":  # pragma: no cover
    ...
    #     "C:\\Users\emm\cernbox\SWAN_projects\steam_models\cosims",
    # )
    # path_to_models = Path.joinpath(
    #     Path(__file__).parent.parent.parent, r"E:\Python311\steam_models\cosims")

    path_to_models = Path.joinpath(
        Path(__file__).parent.parent.parent, "tests/builders/model_library/cosims"
    )
    model_names = [x.parts[-1] for x in Path(path_to_models).iterdir() if x.is_dir()]
    model_names = ['DUMMY_PSPICE_LEDET']
    for model_name in model_names:
        ...
        if os.path.isfile(yaml_file):
            ...
            model_data = yaml_to_data(yaml_file, DataModelCosim)
            ...
            model_data_to_yaml(
                model_data,
                yaml_file_output,
                list_exceptions=['files_to_copy_after_iteration', 'variables_to_copy_after_iteration', 'files_to_copy_after_time_window', 'variables_to_copy_after_time_window', 'variables_to_modify_for_each_time_window', 'convergence'],

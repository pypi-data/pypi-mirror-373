...
class GatewayClientSync:
    def __init__(self, *args, **kwargs):
        ...
        self._loop_thread.start_loop()

        self._client = asyncio.run_coroutine_threadsafe(
            GatewayClient(*args, **kwargs).__aenter__(), self._loop_thread.loop
        ).result()

    def __del__(self):
        if hasattr(self, "_client") and self._client:
            asyncio.run_coroutine_threadsafe(
                self._client.__aexit__(None, None, None), self._loop_thread.loop
            ).result()
        if hasattr(self, "_loop_thread"):
            self._loop_thread.stop_loop()

    def _run_async(self, coro):
        """Run a coroutine on the background loop and return result."""

...
class StandardMAPPOTrainer:
    """
    Enhanced MAPPO trainer for standard MAPPO training (no reward shaping)
    """
    ...
    def __init__(self, config: Dict):
        ...
        self.agent = StandardMAPPOAgent(
            ...
            cooperation_bonus=0.0  # No reward shaping for standard MAPPO
        )
    ...
    def _setup_environment(self):
        ...
        # Create standard environment wrapper
        self.env = StandardEnvironmentWrapper(
            ...
            use_reward_shaping=False  # Disabled for standard MAPPO
        )
        ...
        with tqdm(total=self.rollout_length, desc=f"Rollout", 
            ...
            for step in range(self.rollout_length):
                ...
                self.agent.store_experience(
                    ...
                    actions=actions,
                    rewards=rewards,  # Base rewards (no reward shaping)
                    values=values,
                    log_probs=log_probs,
                    dones=dones,
    ...
    def train(self):
        ...
        # Save final model
        final_save_path = os.path.join(self.save_dir, 'standard_mappo_model_final.pt')
        ...
        logging.info(f"Training completed. Best cooperation rate: {best_cooperation_rate:.4f}")
    ...
    def save_training_metrics(self):
        ...
        metrics_path = os.path.join(self.save_dir, 'standard_mappo_training_metrics.npz')
        ...
        detailed_metrics_path = os.path.join(self.save_dir, 'standard_mappo_detailed_metrics.npz')
        ...
        json_path = os.path.join(self.save_dir, 'standard_mappo_detailed_metrics.json')
    ...
    def plot_training_curves(self, save_path: Optional[str] = None):
        ...
        if save_path is None:
            save_path = os.path.join(self.save_dir, 'standard_mappo_comprehensive_training_curves.png')
        
        # Create figure with 2x4 subplots to match value norm trainer
        fig, axes = plt.subplots(2, 4, figsize=(20, 12))
        fig.suptitle('Standard MAPPO Training Curves', fontsize=16, fontweight='bold')
        ...
        # Empty subplot for consistency with value norm trainer
        ...
        if save_path:
            ...
            logging.info(f"Standard MAPPO training curves saved to {save_path}")
        else:
            plt.show()
...
def create_standard_training_config() -> Dict:
    ...
    return {
        ...
        'num_epochs': 4,           # Low to prevent overfitting
        ...
        # Save directory
        'save_dir': os.path.join(os.path.dirname(os.path.abspath(__file__)), 'standard_mappo_models')
    }
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = StandardMAPPOTrainer(config)
    
    # Start training
    trainer.train()
    
    logging.info("Standard MAPPO training completed successfully!")
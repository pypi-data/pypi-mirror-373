...
from .csv_utils import read_csv_robust, CsvValidationError
from . import _assets
        ...
        def _finalize_df_from_path(file_path: str, filename: str, size: int, mtime: int) -> Dict[str, Any]:
            ...
            try:
                ...
                if opts.allow_excel and lower.endswith(".xlsx"):
                    ...
                    try:
                        df, _ = read_csv_robust(file_path, write_report=True)
                    except CsvValidationError as e:
                        cls._set_html("file-meta",
                                    f"<div style='color:#b00020;font:600 13px ui-sans-serif'>Validation failed:<br>{str(e).replace(chr(10), '<br>')}</div>")
                        return {"ok": False, "error": str(e)}
            except Exception as e:
                last_err = e
        ...
        def read_csv_callback(file_text: Optional[str] = None,
            ...
            try:
                ...
                if UIOptions().allow_excel and lower.endswith(".xlsx"):
                    ...
                    try:
                        df, _ = read_csv_robust(tmp, write_report=True)
                    except CsvValidationError as e:
                        cls._set_html("file-meta",
                                    f"<div style='color:#b00020;font:600 13px ui-sans-serif'>Validation failed:<br>{str(e).replace(chr(10), '<br>')}</div>")
                        return {"ok": False, "error": str(e)}

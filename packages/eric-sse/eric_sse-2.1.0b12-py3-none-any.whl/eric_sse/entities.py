...
from eric_sse.queues import Queue, InMemoryQueue, QueuesFactory, InMemoryQueuesFactory
...
class AbstractChannel(ABC):
    ...
    **Important** When using persistence layer you have to call to **load_persisted_data()** method just after object creation.
    ...
    def __init__(
        ...
        self.__queues_factory: QueuesFactory = queues_factory if queues_factory is not None else InMemoryQueuesFactory()
    ...
    def add_listener(self) -> MessageQueueListener:
        ...
        self.register_connection(Connection(listener=listener, queue=self.__queues_factory.create()))
        return listener

...
def get_project_info() -> str:
    ...
    # 统计文件
    file_count = {
        "Python": len(list(cwd.glob('**/*.py'))),
        "JavaScript": len(list(cwd.glob('**/*.js'))),
        "JSON": len(list(cwd.glob('**/*.json'))),
        "Markdown": len(list(cwd.glob('**/*.md')))
    }
    ...
    info = {
        ...
        "项目名称": cwd.name,
        "文件统计": file_count,
        "配置文件": [f.name for f in cwd.iterdir() 
                    if f.name in ['package.json', 'pyproject.toml', 'requirements.txt']]
    }

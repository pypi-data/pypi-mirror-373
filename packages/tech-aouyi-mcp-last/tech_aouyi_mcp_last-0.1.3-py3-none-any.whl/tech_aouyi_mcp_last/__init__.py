...
def load_stats():
    ...
    stats_file = Path.cwd() / '.analysis_stats.json'
    ...
    return {
        ...
        'risk_distribution': {'低': 0, '中': 0, '高': 0, '严重': 0}
    }
...
def analyze_code(file_path: str) -> dict:
    ...
    try:
        ...
        security_result = security_check(content)
        analysis_time = time.time() - start_analysis
        
        analysis["安全检查"] = security_result
        
        # 更新统计数据
        analysis_stats['total_files'] += 1
        analysis_stats['total_analysis_time'] += analysis_time
        
        # 统计安全问题
        if security_result['发现问题'] != ['未发现安全问题']:
            analysis_stats['security_issues_found'] += len(security_result['发现问题'])
        
        # 统计风险分布
        risk_level = security_result['风险等级']
        analysis_stats['risk_distribution'][risk_level] += 1
...
def get_system_stats() -> dict:
    ...
    # 计算平均分析时间
    avg_time = (analysis_stats['total_analysis_time'] / analysis_stats['total_files'] 
                if analysis_stats['total_files'] > 0 else 0.0)
    ...
    # 检查是否有数据
    has_data = analysis_stats['total_files'] > 0
    
    result = {
        "分析统计": {
            "总分析文件数": analysis_stats['total_files'],
            "发现安全问题数": analysis_stats['security_issues_found'],
            "平均分析时间": f"{avg_time:.2f}s",
            "风险分布": analysis_stats['risk_distribution']
        },
        "系统状态": {
            "支持文件类型": list(ALLOWED_EXTENSIONS),
            "最大文件大小": f"{MAX_FILE_SIZE:,} bytes",
            "分析超时限制": "30s",
            "统计文件状态": "存在" if stats_file_exists else "不存在"
        }
    }
    ...
    if not has_data:
        result["提示"] = {
            "状态": "⚠️ 尚未分析任何文件",
            "如何开始": [
                "使用 analyze_code('文件名') 分析文件",
                "例如: analyze_code('main.py')",
                "例如: analyze_code('test_files/test_python.py')"
            ],
            "可分析文件": [
        ...
        result["提示"] = {
            "状态": "✅ 统计数据正常",
            "管理操作": [
                "使用 reset_system_stats() 重置统计",
                "继续使用 analyze_code() 增加统计"
            ]
        }

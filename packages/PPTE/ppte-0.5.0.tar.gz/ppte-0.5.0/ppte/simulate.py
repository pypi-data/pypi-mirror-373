            ...
            for idx in range(self.num_genomes):
                ...
                if self.diverse:
                    ...
                    if self.diverse_config:
                        ...
                        diverse_chunks = [SeqDiverse(chunk, **divConfig) if use else chunk for chunk, use in zip(chunks, mask)]
                    else:
                        diverse_chunks = [SeqDiverse(chunk) if use else chunk for chunk, use in zip(chunks, mask)]

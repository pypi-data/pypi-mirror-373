...
from adam.utils import log, log2, log_to_file
...
class Idp:
    ...
    def login(app_host: str, username: str = None, idp_uri: str = None, forced = False, use_token_from_env = True, use_cached_creds = True) -> IdpLogin:
        ...
        try:
            ...
            def gp(default_pass: str = None):
                # if default_pass and not frag:
                #     return default_pass
                ...
                try:
                    ...
                    return getpass.getpass(f'Password(default ********): ') or default_pass
                finally:
                    termios.tcsetattr(fd, termios.TCSADRAIN, old)
                # return line
                # frag = getpass.getpass(f'Password(default ********): ')
                # if default_pass and not frag:
                #     return default_pass
                ...
                # log2(f'frag({len(frag)}): {frag}')
                # password = frag
                # while len(frag) == 1024:
                #     frag = getpass.getpass('')
                #     log2(f'frag({len(frag)}): {frag}')
                #     password = password + frag
                #     log2(f'password: {len(password)}')
                # if frag:
                #     log2(f'frag({len(frag)}): {frag}')
                #     password = password + frag
                #     log2(f'password: {len(password)}')
            ...
            while password == None or Idp.ctrl_c_entered: # exit the while loop even if password is empty string
                ...
                if default_pass:
                    if forced:
                        ...
                        password = gp(default_pass=default_pass)
            ...
            if username and password:
                try:
                    # log2(password)
                    kubeconfig_string = base64.b64decode(password.encode('ascii') + b'==').decode('utf-8')

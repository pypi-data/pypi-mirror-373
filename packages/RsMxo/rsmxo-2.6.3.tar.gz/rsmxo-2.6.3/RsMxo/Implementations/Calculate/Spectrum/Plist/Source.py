...
class SourceCls:
	...
	def set(self, source: enums.Source, spectrum=repcap.Spectrum.Default) -> None:
		"""CALCulate:SPECtrum<*>:PLISt:SOURce \n
		Snippet: driver.calculate.spectrum.plist.source.set(source = enums.Source.C1, spectrum = repcap.Spectrum.Default) \n
	...
	def get(self, spectrum=repcap.Spectrum.Default) -> enums.Source:
		"""CALCulate:SPECtrum<*>:PLISt:SOURce \n
		Snippet: value: enums.Source = driver.calculate.spectrum.plist.source.get(spectrum = repcap.Spectrum.Default) \n
		Selects the source of the peak table. You can select one of the traces that is enabled with 'Traces'. \n
			:param spectrum: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Spectrum')

...
class CurrentCls:
	...
	def set(self, source_current: enums.Source, power=repcap.Power.Default) -> None:
		...
		Snippet: driver.power.harmonics.source.current.set(source_current = enums.Source.C1, power = repcap.Power.Default) \n
	...
	def get(self, power=repcap.Power.Default) -> enums.Source:
		"""POWer<*>:HARMonics:SOURce:CURRent \n
		Snippet: value: enums.Source = driver.power.harmonics.source.current.get(power = repcap.Power.Default) \n
		Sets the channel for the current source input of the power harmonics analysis. \n
			:param power: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')

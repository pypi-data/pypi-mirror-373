# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 18:10:35 2022

Based on CWatM functions
"""

import configparser
import re
# import xml.dom.minidom
# It is necessary to add (...)\CWatM-version1.06 or (...)\CWATM_MODFLOW to the PYTHONPATH
...
def parse_configuration(settingsFileName):
    ...
    if not(os.path.isfile(settingsFileName)):
        ...
        raise Exception(settingsFileName,msg)
    config = ExtParser()
    ...
    for sec in config.sections():
        ...
        check_section = False
        # option = dict.fromkeys(options) # Useless initialization of option
        for opt in options:
            if sec=="OPTIONS":
                try:
    ...
    return(config, option, binding, outputDir)
...
class StablePositioner {
  constructor(reference, floating, config) {
    ...
    this.lockUntil = 0;
    this.lastZoom = window.devicePixelRatio || 1;
  }
  async position() {
  ...
  detectOscillation() {
    ...
    return movement > THRESHOLDS.MOVEMENT_THRESHOLD && now - recent[0].timestamp < THRESHOLDS.RAPID_MOVEMENT_WINDOW;
  }
  findBestPlacement() {
    ...
    if (scores[base] > 0.8) return this.config.placement;
    const best = Object.entries(scores).reduce((a, b) => b[1] > a[1] ? b : a);
    return alignment ? `${best[0]}-${alignment}` : best[0];
  }
  lock(placement) {
    const zoom = window.devicePixelRatio || 1;
...
const extractPlacement = (value) => {
  ...
  return valid.includes(str) ? str : "bottom";
};

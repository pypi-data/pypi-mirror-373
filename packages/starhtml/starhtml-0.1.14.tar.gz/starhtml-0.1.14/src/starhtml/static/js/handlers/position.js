...
const extractPlacementValue = (value) => {
...
const parseConfig = (el, value, mods) => {
  ...
  return {
    ...
    placement: extractPlacementValue(mods.get("placement")),
...
const positionAttributePlugin = {
  ...
  onLoad(ctx) {
    ...
    function initializeWithAnchor(anchorElement) {
      ...
      const middleware = [offset(config.offsetValue)];
      ...
      const waitForBounds = (element) => new Promise((resolve) => {
        let attempts = 0;
        const check = () => {
          const bounds = element.getBoundingClientRect();
          if ((bounds.width > 0 || bounds.height > 0) && typeof bounds.x === "number" && typeof bounds.y === "number") {
            resolve(bounds);
          } else if (++attempts >= 3) {
            resolve(null);
          } else {
      ...
      const updatePosition = async () => {
        ...
        try {
          ...
          const changed = Math.abs(result.x - lastPosition.x) > 0.1 || Math.abs(result.y - lastPosition.y) > 0.1 || result.placement !== lastPosition.placement;
          if (changed) {
...
var position_default = positionAttributePlugin;
export {

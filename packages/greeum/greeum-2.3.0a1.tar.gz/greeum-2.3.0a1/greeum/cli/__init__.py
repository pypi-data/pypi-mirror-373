...
@click.group()
@click.version_option(version="2.3.0a1")
def main():
...
def serve(transport: str, port: int):
    ...
    if transport == 'stdio':
        try:
            # FastMCP 기반 서버 사용 (Windows/WSL 호환성 개선)
            from ..mcp.legacy.fastmcp_hotfix_server import main as mcp_main
            import asyncio
            asyncio.run(mcp_main())
        except ImportError as e:
            click.echo(f"❌ FastMCP server import failed: {e}")

...
def serve(transport: str, port: int):
    ...
    # 시작 메시지를 stderr로 출력하여 JSON-RPC 파싱 충돌 방지
    import os
    if not os.getenv('GREEUM_QUIET'):
        click.echo(f"🚀 Starting Greeum FastMCP server ({transport})...", err=True)
    
    if transport == 'stdio':
        # ✅ 통합 MCP 서버로 교체 (환경 자동 감지)
        from ..mcp.unified_mcp_server import main as unified_main
        import asyncio
        try:
            asyncio.run(unified_main())
        except KeyboardInterrupt:
            click.echo("\n👋 Unified MCP server stopped")
    else:
...
def serve(port: int, host: str):
    ...
    # API 서버 시작 메시지도 stderr로 출력
    if not os.getenv('GREEUM_QUIET'):
        click.echo(f"🌐 Starting Greeum API server on {host}:{port}...", err=True)

...
class RegexPatternCpp(object):
    ...
    FUNCTION = [
        # assignment with conditional call shorthand: lhs = expr?->call;
        # expands to a temp named by lhs to support multiple occurrences in the same scope
        (re.compile(r'(?m)^(?P<indent>\s*)(?P<lhs_prefix>[^=\n;]*?\b)(?P<lhs_name>\w+)\s*=\s*(?P<pre>.*?)(?P<obj>[\w:\.\-\>\(\)]+)\s*\?->\s*(?P<call>[^;\n]+);'),
         r'\g<indent>auto __tmp_\g<lhs_name> = \g<obj>;\n\g<indent>\g<lhs_prefix>\g<lhs_name> = \g<pre>(__tmp_\g<lhs_name> ? __tmp_\g<lhs_name>->\g<call> : decltype(__tmp_\g<lhs_name>->\g<call>)());', ['?->']),

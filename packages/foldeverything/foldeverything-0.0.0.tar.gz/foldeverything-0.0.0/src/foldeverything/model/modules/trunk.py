...
class InputEmbedder(nn.Module):
    ...
    def forward(self, feats: Dict[str, Tensor], affinity: bool = False) -> Tensor:
        ...
        q, c, p, to_keys, _ = self.atom_encoder(feats)
...
class TemplateV2ExpansionModule(nn.Module):
    """Template module."""
    ...
    def forward(
        ...
        """
        # Load relevant features
        query_to_template = feats["query_to_template"]
        token_pair_mask = feats["token_pair_mask"]
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            a_tij = a_tij * token_pair_mask.unsqueeze(-1)
        ...
        # Using a mapping from query tokens to template of shape
        # B x T x N, where N is the number of query tokens, we can
        # index into the pairwise features for the relevant templates
        query_to_template_i = query_to_template[..., None]
        query_to_template_j = query_to_template[..., None, :]
        a_tij = torch.gather(
            a_tij,
            dim=2,
            index=query_to_template_i.expand(-1, -1, -1, a_tij.size(3), a_tij.size(-1)),
        )
        a_tij = torch.gather(
            a_tij,
            dim=3,
            index=query_to_template_j.expand(-1, -1, a_tij.size(2), -1, a_tij.size(-1)),
        )
...
class TokenDistanceModule(nn.Module):
    ...
    def forward(
        ...
        token_coords = feats["noisy_center_coords"]
...
class MSALayer(nn.Module):
    ...
    def forward(
        ...
        z = self.pairformer_layer(z, token_mask, chunk_size_tri_attn)
...
class TistogramModule(nn.Module):
    """Tistogram Module."""
    ...
    def __init__(self, token_z: int, num_bins: int) -> None:
        ...
        """
        super().__init__()
        self.norm = nn.LayerNorm(token_z)
        self.tistogram = nn.Linear(token_z, num_bins)

    def forward(self, z: Tensor, axis: Tensor) -> Tensor:
        """Perform the forward pass.
        ...
        """
        z = self.norm(z + z.transpose(1, 2))
        angles = self.tistogram(
            nn.functional.sigmoid(z[:, axis, :, None] * z[:, axis, None])
        )
        return angles


class PairTokenModule(nn.Module):
    """Pairwise token type Module."""

    def __init__(self, token_z: int, num_tokens: int = 33) -> None:
        """Initialize the pairtoken module.
        ...
        self.pairtoken = nn.Linear(token_z, self.num_pairs)
    ...
    def forward(self, z: Tensor) -> Tensor:
        ...
        -------
        Tensor
            The predicted pairwise residue logits.

        """
        z = z + z.transpose(1, 2)
        return self.pairtoken(z).reshape(
            z.shape[0], z.shape[1], z.shape[2], self.num_pairs
        )

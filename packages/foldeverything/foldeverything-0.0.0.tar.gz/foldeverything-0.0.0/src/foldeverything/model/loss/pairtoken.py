...
def pairwise_token_loss(
    output: Dict[str, Tensor],
    feats: Dict[str, Tensor],
    aggregate: bool = True,
) -> Tuple[Tensor, Tensor]:
    """Compute the pairwise token type loss.
    ...
    with torch.autocast("cuda", enabled=False):
        ...
        pred = output["ppairtoken"].float()  # (B, L, L, num_res * (num_res+1) // 2)
        D = pred.shape[3]  # num_pairs  # noqa: N806
        assert len(pred.shape) == 4  # noqa: PLR2004

        # Compute target distogram
        assert len(target.shape) == 4  # noqa: PLR2004
        ...
        if aggregate:
            ...
            # TODO: For now, we use the same mask.
            mask = feats["token_pair_mask"]
            mask = mask[:, None, :] * mask[:, :, None]
            mask = mask * (1 - torch.eye(mask.shape[1])[None]).to(pred)
            
            # Compute the pairwise residue loss

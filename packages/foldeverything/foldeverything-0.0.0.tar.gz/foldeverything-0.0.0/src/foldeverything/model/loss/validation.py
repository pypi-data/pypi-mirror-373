...
def factored_lddt_loss(
    ...
    if representative_lddt:
        rep_atoms = (
            feats["token_to_rep_atom"].sum(dim=1).repeat_interleave(multiplicity, 0)
        )
        dna_mask = dna_mask * rep_atoms
        rna_mask = rna_mask * rep_atoms
        protein_mask = protein_mask * rep_atoms
        design_mask = design_mask * rep_atoms
    ...
    with torch.autocast("cuda", enabled=False):
        ...
        if representative_lddt:
            rep_atoms = (
                ...
                .sum(dim=1)
                .repeat_interleave(multiplicity * K, 0)
            )
            dna_mask = dna_mask * rep_atoms
            rna_mask = rna_mask * rep_atoms
            protein_mask = protein_mask * rep_atoms
...
def align_to_reference(
    ...
    with torch.autocast("cuda", enabled=False):
        ...
        L = atom_mask.shape[1]
        align_weights = torch.ones(multiplicity, L).to(
            atom_mask.device
        )  # atom_coords.new_ones(atom_coords.shape[:2])
        atom_type = (
            torch.bmm(
                feats["atom_to_token"].float(), feats["mol_type"].unsqueeze(-1).float()
            )
...
def weighted_minimum_rmsd(
    ...
    designed_mask = design_mask.repeat_interleave(multiplicity, 0)
    original_mask = 1.0 - design_mask
    ...
    try:
        rmsd_prot, best_rmsd_prot = compute_subset_rmsd(
            atom_coords,
            pred_atom_coords,
            atom_mask,
            align_weights,
        ...
        print(f"Warning: rmsd_prot failed with error: {e}")
        rmsd_prot, best_rmsd_prot = torch.tensor(torch.nan), torch.tensor(torch.nan)
    ...
    try:
        rmsd_design_target, best_rmsd_design_target = compute_subset_rmsd(
            ...
            atom_mask,
            multiplicity,
            rmsd_mask=designed_mask,
        )
    except Exception as e:
        print(f"Warning: rmsd_design_target failed with error: {e}")
        rmsd_design_target, best_rmsd_design_target = torch.tensor(torch.nan), torch.tensor(torch.nan)

    if ligand_mask.sum() > 0:
        try:
            rmsd_lig, best_rmsd_lig = compute_subset_rmsd(
                atom_coords,
                pred_atom_coords,
                atom_mask,
                align_weights,
            ...
            print(f"Warning: rmsd_lig failed with error: {e}")
            rmsd_lig, best_rmsd_lig = torch.tensor(torch.nan), torch.tensor(torch.nan)
    else:
        rmsd_lig, best_rmsd_lig = torch.tensor(torch.nan), torch.tensor(torch.nan)

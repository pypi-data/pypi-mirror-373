...
class AffinitySampler(Sampler):
    def __init__(
        ...
        id_to_id_decoys: Optional[str] = None,
        path_to_record_decoys: Optional[str] = None,
        ...
        if id_to_id_decoys:
            with open(id_to_id_decoys, "rb") as f:
                self.id_to_id_decoys = pickle.load(f)
        self.path_to_record_decoys = path_to_record_decoys
        self.tot_decoy_ids = set(
            [int(l.split(".")[0]) for l in os.listdir(self.path_to_record_decoys)]
        )
    ...
    def sample(
        ...
        while True:
            ...
            records_sampled = sample_fn(records_aid, k=self.num_consecutive_actives)
            records_yield = []
            try:
                for record in records_sampled:
                    records_yield.append(record)
                    for _ in range(self.num_decoys_per_active):
                        records_yield.append(self.get_decoy(record, random))
            except Exception as e:
                print(f"Error: {e}")
                continue
            for record in records_yield:
                yield record
    ...
    def get_decoy(self, record: Record, random: RandomState) -> Record:
        ...
        """
        decoy_ids = self.id_to_id_decoys[record.id]
        decoy_ids = [
            decoy_id for decoy_id in decoy_ids if decoy_id in self.tot_decoy_ids
        ]
        if len(decoy_ids) == 0:
            raise Exception(
                f"No decoys found for record {record.id} in {self.path_to_record_decoys}"
            )
        decoy_id = random.choice(decoy_ids)
        record = Record.load(Path(f"{self.path_to_record_decoys}/{decoy_id}.json"))
        return record

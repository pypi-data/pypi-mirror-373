...
class BroadTargets(TargetsSource):
        ...
        for i, row in tqdm(
            ...
            try:
                entry = PubChem(
                    ...
                    activity_qualifier="=",
                    affinity=(
                        10.0 if str(row["activity_outcome"]) == "Inactive" else 1.0
                    ),
                    normalized_protein_accession=str(
                        row["normalized_protein_accession"]
                    ),
                    protein_sequences=row["protein_sequence"].split(";"),
                    protein_cluster=str(row["normalized_protein_accession"]),
                    modify_date=str(0),
                    deposit_date=str(0),
                    pair_id=i,
                    assay_prot_id=int(row["aid"]),
    ...
    def parse(self, data: PubChem, resource: Resource) -> Target:
        ...
        for i, chain in enumerate(structure.chains):
            chain_info.append(  # noqa: PERF401
                ChainInfo(

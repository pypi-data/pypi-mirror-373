...
from foldeverything.task.analyze.analyze import SEVERITY
...
def create_alignment_logo(sequences, name):
    ...
    return draw_logo(counts_df, name)
...
def aa_composition_pie(sequences, name):
    ...
    ax1.set_title(f"{name} Hydrophobicity Distribution")
    ...
    ax2.set_title(f"{name} Charge Distribution")
...
def plot_seq_liabilities(
    ...
    norm = mcolors.Normalize(vmin=0, vmax=max(SEVERITY.values()))

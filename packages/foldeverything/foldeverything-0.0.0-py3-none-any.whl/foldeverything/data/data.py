...
Ensemble = [  # superfluous if rejecting structures that dont perfectly match, could be obtained from Atom
...
class Structure(NumpySerializable):
    ...
    def load(cls: "Structure", path: Path) -> "Structure":
        ...
        try:
            ...
            # check if pocket is None
            if pocket.size == 0:
                pocket = np.empty(0, dtype=Pocket)
        except:
            pocket = np.empty(0, dtype=Pocket)

        # TODO: Temporary for adding a  cyclic period that is not yet in the saved data
    ...
    def insert(
        ...
        result = Structure(
            ...
            coords=final_coords,
            ensemble=ensemble,
            pocket=structure.pocket.copy() if structure.pocket is not None else None,
        )
        return result
...
class DesignInfo(NumpySerializable):
    ...
    res_anchor_mask: npt.NDArray[np.int_]
    ...
    def is_valid(self, info: "DesignInfo") -> bool:
        ...
        assert (
            ...
            and len(info.res_binding_type) == len(info.res_anchor_mask)
        ), (
        ...
        if any(info.res_design_mask.astype(bool) & info.res_anchor_mask.astype(bool)):
            msg = "Residues consisting of anchor atoms cannot have design mask true"
            raise ValueError(msg)
        return True
...
class Target:
    ...
    anchor_data: Optional[Dict[str, npt.NDArray]] = None
...
Token = [
    ...
    ("is_anchor", np.dtype("i4")),
    ("anchor_parent_idx", np.dtype("i4")),
]

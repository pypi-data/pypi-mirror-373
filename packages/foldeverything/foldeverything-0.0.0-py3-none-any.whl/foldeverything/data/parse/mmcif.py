...
def get_mol(ccd: str, mols: Resource, moldir: str) -> Mol:
...
def parse_polymer(  # noqa: C901, PLR0915, PLR0912
    ...
    result = gemmi.align_string_sequences(
        sequence,
        [polymer[r].name for r in range(len(polymer))],
        [],
        gemmi.AlignmentScoring("s"),
    )
    match_string = result.match_string
    
    # Potential fix
    if use_original_res_idx:
        matches = [" "] * len(sequence)
        for i in range(len(polymer)):
            matches[polymer[i].seqid.num-1]="|" # mark the match with a |
    ...
    parsed = []
    for j, match in enumerate(match_string):
        ...
        if match == "|":
            ...
            if res.name and res.name != res_name:
                seq_str = "_".join(r for r in sequence)
                poly_str = "_".join(polymer[r].name for r in range(len(polymer)))
                print(f"""
                    \nAlignment mismatch for position {j} with res.name {res.name} and res_name (from sequence) {res_name}.
                    sequence: {seq_str}
                    match:    {match_string}
                    polymer:  {poly_str}
                    """)

                unk = res.name == "UNK" or res_name == "UNK"
                msg = (
                    "Alignment mismatch! "
                    + ("UNK token encountered! " if unk else "")
                    + "See printing above."
                )
                raise ValueError(msg)
        ...
        if res is not None:
            orig_idx = res.seqid
            orig_idx = str(orig_idx.num) + str(orig_idx.icode).strip()
            res_idx = res.seqid.num - 1  # convert to 0 indexing
        else:
            orig_idx = None
            res_idx = j
        ...
        # Handle non-standard residues
            ...
            if modified_mol is not None:
                residue = parse_ccd_residue(
                    ...
                    res_idx=res_idx if use_original_res_idx else j,
                    gemmi_mol=res,
                    is_covalent=True,
                )
        ...
        parsed.append(
            ParsedResidue(
                ...
                atoms=atoms,
                bonds=[],
                idx=res_idx if use_original_res_idx else j,

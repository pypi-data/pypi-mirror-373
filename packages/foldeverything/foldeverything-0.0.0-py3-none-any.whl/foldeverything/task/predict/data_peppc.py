...
class PredictionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        pdb_path = self.dataset.pdb_paths[idx]
        _, pdb_id, _, _ = Path(pdb_path).stem.split("_")

        # Get the structure
        try:
            str_native = parse_pdb(pdb_path, moldir=self.moldir).data
            # remove the A chain and replace with an emtpy structure
            binder_chain = str_native.chains[0]
            assert binder_chain["name"] == "Axp"
            target_chain = str_native.chains[1]
            assert target_chain["name"] == "Bxp"
            target_idxs = np.arange(
                target_chain["atom_idx"],
                target_chain["atom_idx"] + target_chain["atom_num"],
            )
            str_target = Structure.extract_atoms(str_native, target_idxs)
            str_prot = Structure.empty_protein(seq_len=binder_chain["res_num"])
            structure = Structure.concatenate(str_target, str_prot)
        ...
        tokenized.tokens["target_msa_mask"] = ~tokenized.tokens["design_mask"].copy()
        ...
        features["id"] = pdb_id
        features["msa_mask"] *= features["target_msa_mask"]
        return features
    ...
    def __len__(self) -> int:
        ...
        """
        return len(self.dataset.pdb_paths)
...
class ProteinBinderDataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg: DataConfig, batch_size, num_workers, pin_memory) -> None:
        ...
        with Path(cfg.target_paths).open("r") as f:
            pdb_paths = f.read().splitlines()

        # pdb_paths = list(Path(cfg.target_dir).glob(f"*{cfg.suffix}.pdb"))

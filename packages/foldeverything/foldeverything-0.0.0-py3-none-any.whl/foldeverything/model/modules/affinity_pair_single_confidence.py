...
class AffinityModulePairSingleConfidence(nn.Module):
    """Algorithm 31"""
    ...
    def __init__(
        ...
        self.s_norm = nn.LayerNorm(2 * token_s + s_input_dim)
        self.s_linear = LinearNoBias(2 * token_s + s_input_dim, token_s)
        self.z_norm = nn.LayerNorm(2 * token_z)
        self.z_linear = LinearNoBias(2 * token_z, token_z)
    ...
    def forward(
        ...
        s = self.s_linear(self.s_norm(torch.cat([s, s_inputs, s_conf], dim=-1)))
        ...
        z = self.z_linear(self.z_norm(torch.cat([z, z_conf], dim=-1)))

...
from einx import softmax
import torch
from torch import Tensor, nn
from torch.nn.functional import one_hot
from torch.onnx.symbolic_opset9 import to

from foldeverything.data import const, mol
from foldeverything.model.layers.miniformer import (
    MiniformerNoSeqLayer,
    MiniformerNoSeqModule,
)
from foldeverything.model.layers.outer_product_mean import OuterProductMean
from foldeverything.model.layers.pair_averaging import PairWeightedAveraging
from foldeverything.model.layers.pairformer import (
...
# from foldeverything.model.modules.utils import GaussianSmearing
...
class MLPAttnGNN(nn.Module):
    def __init__(
        ...
        self.num_heads = num_heads  # TODO weian: init method need to be added
...
class InverseFoldingEncoder(nn.Module):
    ...
    def init_knn_graph(self, feats: Dict[str, Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        coords = feats["noisy_center_coords"]
        ...
        # z = torch.gather(z, 2, src_idx[..., None].expand(-1, -1, -1, z.shape[-1]))
    ...
    def extract_geo_feat(
        ...
        dist = self.distance_gaussian_smearing(
            dist
        )  # TODO: weian: atom O may leak the restype info

        # ref_element = feats["ref_element"][valid_mask]
        return dist
    ...
    def extract_attr_feat(
        ...
        # node attr
        # feats["is_standard"].bool() & (feats["mol_type"] == const.chain_type_ids["PROTEIN"])
        # feats["ref_element"]
    ...
    def forward(self, feats):
        ...
        N = valid_mask.sum()
        # s = self.linear_token_to_node(node_attr*6.6)
...
class InverseFoldingSimpleDecoder(nn.Module):
    def __init__(
        self,
        atom_s: int,
        atom_z: int,
        ...
        self.topk = topk
        self.num_heads = num_heads
        self.num_res_type = len(const.tokens)

        self.predictor = nn.Linear(self.node_dim, len(const.tokens), bias=False)
    ...
    def sample(self, s, z, edge_idx, valid_mask, feats):
        ...
        out_dict = self.forward(s, z, edge_idx, valid_mask, feats)

        logits = out_dict["logits"]
        pred_canonical = logits[
            :,
            :,
        ...
        feats["res_type"] = pred_one_hot
        out_dict["sample_atom_coords"] = feats["coords"][0]
        assert feats["coords"].shape[0] == 1 and feats["coords"].shape[1] == 1
        return out_dict


class InverseFoldingARDecoder(nn.Module):
    def __init__(
        self,
        atom_s: int,
        atom_z: int,
        ...
        self.seq_to_s = nn.Linear(33, self.node_dim)
        self.predictor = nn.Linear(self.node_dim, len(const.tokens), bias=False)
    ...
    def forward(self, s, z, edge_idx, valid_mask, feats):
        with torch.no_grad():
            src_idx, dst_idx = edge_idx[0], edge_idx[1]
            N = valid_mask.sum()
            rand = torch.rand(N, device=valid_mask.device)
    ...
    def sample(self, s, z, edge_idx, valid_mask, feats):
        ...
        if len(self.inverse_fold_restriction) > 0:
            ...
            restriction_mask = torch.ones(len(const.canonical_tokens), device=s.device)
        N = s.shape[0]
        ...
        assert num_design == N - num_not_design, (
            f"num_design: {num_design}, num_not_design: {num_not_design}"
        )

        order = torch.randperm(N, device=s.device).cpu().numpy().tolist()
        # not design residues are not sampled and used as the condition. so the order should filter them out.
        if num_not_design > 0:
            id_not_design = torch.where(~design_mask)[0].cpu().numpy().tolist()
            for i in id_not_design:
                order.remove(i)
            decoded_seq = torch.zeros(N, 33, device=s.device)
            logits = torch.zeros(N, 33, device=s.device)
        ...
        for i in order:
            ...
            if self.sampling_temperature < 0:
                ...
                ids_canonical = torch.multinomial(
                    F.softmax(pred_canonical / self.sampling_temperature, dim=-1), num_samples=1
                ).squeeze(-1)
        ...
        N = valid_mask.shape[1]
        res_type = torch.zeros(1, N, self.num_res_type, device=s.device)
        res_type[valid_mask] = decoded_seq
        unk_ids = torch.full(
            [(~valid_mask).sum().item()], const.tokens.index("UNK"), device=s.device
        )
        unk_value = F.one_hot(unk_ids, num_classes=len(const.tokens))
        res_type[~valid_mask] = unk_value.float()
        feats["res_type"] = res_type.long()

        logist_dense = torch.zeros(1, N, self.num_res_type, device=logits.device)
        logist_dense[valid_mask] = logits

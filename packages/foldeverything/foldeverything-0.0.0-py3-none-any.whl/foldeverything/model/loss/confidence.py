...
from foldeverything.model.layers.confidence_utils import tm_function, compute_frame_pred
...
def confidence_reg_loss(
    ...
    mask_loss=None,
    relative_supervision_weight=0.0,
    scaling_factor=700.0,
):
    # TODO no support for MD yet!
    plddt, rel_plddt = plddt_reg_loss(
        model_out["plddt"],
        model_out["sample_atom_coords"],
        feats,
        true_coords,
    ...
    plddt = plddt * scaling_factor
    rel_plddt = rel_plddt * scaling_factor
    pde = pde * scaling_factor
    rel_pde = rel_pde * scaling_factor

    pae, rel_pae = 0.0, 0.0
    if alpha_pae > 0.0:
        pae, rel_pae = pae_reg_loss(
            model_out["pae"],
            model_out["sample_atom_coords"],
            feats,
            true_coords,
        ...
        pae = pae * scaling_factor
        rel_pae = rel_pae * scaling_factor
...
def plddt_reg_loss(
    ...
    if relative_confidence_supervision:
        ...
        relative_pred_lddt = pred_lddt.view(
            B // multiplicity, multiplicity, 1, -1
        ) - pred_lddt.view(B // multiplicity, 1, multiplicity, -1)

        rel_errors = loss_fn(relative_pred_lddt, relative_target_lddt)
...
def pae_reg_loss(
    ...
    rel_loss = 0.0
    if relative_confidence_supervision:
        B, N, _ = pred_pae.shape
        rel_target_ptm = target_ptm.view(
            B // multiplicity, multiplicity, 1, N, N
        ) - target_ptm.view(B // multiplicity, 1, multiplicity, N, N)
        rel_pred_ptm = pred_ptm.view(
            B // multiplicity, multiplicity, 1, N, N
        ) - pred_ptm.view(B // multiplicity, 1, multiplicity, N, N)

        rel_errors = loss_fn(rel_pred_ptm, rel_target_ptm)
...
def pde_reg_loss(
    ...
    if relative_confidence_supervision:
        ...
        rel_pred_pde = pred_pde.view(
            B // multiplicity, multiplicity, 1, N, N
        ) - pred_pde.view(B // multiplicity, 1, multiplicity, N, N)

        rel_errors = loss_fn(rel_pred_pde / max_dist, rel_target_pde / max_dist)

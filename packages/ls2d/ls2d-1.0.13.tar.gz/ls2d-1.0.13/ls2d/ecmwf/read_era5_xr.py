...
class Read_era5:
    ...
    def __init__(self, settings):
        ...
        # Read all NetCDF files with xarray.
        self.read_netcdf()
    ...
    def read_netcdf(self):
        ...
        an_dates = era_tools.get_required_analysis(self.start, self.end, freq=1)
        ...
        def get_files(ftype):
            """
            Get list of required NetCDF files, and check
            if they all exists on disk.
            """

            files = [era_tools.era5_file_path(
                d.year, d.month, d.day, path, case, ftype, False) for d in an_dates]

            # Check if all files exist.
            for f in files:
                if not os.path.exists(f):
                    files_missing = True
                    warning(f'Cant find {f}!')

            return files

        sfc_files   = get_files('surface_an')
        model_files = get_files('model_an')
        pres_files  = get_files('pressure_an')
        ...
        def read_nc(files):
            """
            Read NetCDF files, drop some unwanted stuff,
            and rename some dims that were renamed in
            the latest CDS "upgrade".
            """
        ...
        ds_sfc   = read_nc(sfc_files)
        ds_model = read_nc(model_files)
        ds_pres  = read_nc(pres_files)

        # Merge into single Dataset!
        self.ds_full = xr.merge((ds_sfc, ds_model, ds_pres))

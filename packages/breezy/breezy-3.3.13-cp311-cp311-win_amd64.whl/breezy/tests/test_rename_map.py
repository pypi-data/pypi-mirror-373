...
class TestRenameMap(TestCaseWithTransport):
    ...
    def test_match_directories(self):
        ...
        self.assertEqual(
            {"path2": {"b"}, "path3/path4": {"c"}, "path3": set()}, required_parents
        )
    ...
    def test_guess_renames_handles_grandparent_directories(self):
        ...
        self.addCleanup(tree.unlock)
        self.build_tree(
            ["tree/topdir/", "tree/topdir/middledir/", "tree/topdir/middledir/file"]
        )
        tree.add(
            ["topdir", "topdir/middledir", "topdir/middledir/file"],
    ...
    def test_guess_renames_preserves_children(self):
        ...
        self.build_tree_contents(
            [("tree/foo/", b""), ("tree/foo/bar", b"bar"), ("tree/foo/empty", b"")]
        )
        tree.add(
            ["foo", "foo/bar", "foo/empty"], ids=[b"foo-id", b"bar-id", b"empty-id"]
        )
        tree.commit("rev1")
    ...
    def test_guess_renames_output(self):
        ...
        notes = self.captureNotes(
            RenameMap.guess_renames, tree.basis_tree(), tree, dry_run=False
        )[0]
        self.assertEqual("file => file2", "".join(notes))

...
class TestMissing(TestCaseWithTransport):
    def assertUnmerged(self, expected, source, target, restrict="all", backward=False):
        unmerged = missing.find_unmerged(
            source, target, restrict=restrict, backward=backward
        )
    ...
    def test_find_unmerged(self):
        ...
        self.assertUnmerged(
            ([("3", b"c", 0), ("2", b"b", 0)], []), original, puller, backward=True
        )
    ...
    def test_iter_log_revisions(self):
        ...
        base_extra, child_extra = missing.find_unmerged(
            base_tree.branch, child_tree.branch
        )
        results = list(
            iter_log_revisions(base_extra, base_tree.branch.repository, verbose=True)
        )
        self.assertEqual([], results)

        results = list(
            iter_log_revisions(child_extra, child_tree.branch.repository, verbose=True)
        )
        ...
        self.assertEqual(
            [("b", "file")], [(c.path[1], c.kind[1]) for c in delta0.added]
        )
        ...
        self.assertEqual(
            [("a", "file")], [(c.path[0], c.kind[0]) for c in delta1.removed]
        )
...
class TestFindUnmerged(tests.TestCaseWithTransport):
    ...
    def test_same_branch(self):
        ...
        self.assertUnmerged(
            [], [], tree.branch, tree.branch, local_revid_range=(rev1, rev1)
        )
    ...
    def test_merged(self):
        ...
        self.assertUnmerged(
            [], [], tree.branch, tree2.branch, local_revid_range=(rev1, rev1)
        )
    ...
    def test_revision_range(self):
        ...
        self.assertUnmerged(
            ...
            local.branch,
            remote.branch,
            local_revid_range=(lrevid2, lrevid2),
        )
        self.assertUnmerged(
            [("2", lrevid2, 0), ("3", lrevid3, 0)],
            remote_extra,
            local.branch,
            remote.branch,
        ...
        self.assertUnmerged(
            ...
            local.branch,
            remote.branch,
            remote_revid_range=(None, rrevid2),
        )
        self.assertUnmerged(
            local_extra,
            [("2", rrevid2, 0)],
            local.branch,
            remote.branch,
            remote_revid_range=(lrevid1, rrevid2),
        )
        self.assertUnmerged(
            local_extra,
            [("2", rrevid2, 0)],
            local.branch,
            remote.branch,
            remote_revid_range=(rrevid2, rrevid2),
        )
        self.assertUnmerged(
            local_extra,
            [("2", rrevid2, 0), ("3", rrevid3, 0)],
            local.branch,
            remote.branch,
            remote_revid_range=(None, rrevid3),
        )
        self.assertUnmerged(
            local_extra,
            [("2", rrevid2, 0), ("3", rrevid3, 0)],
        ...
        self.assertUnmerged(
            ...
            local.branch,
            remote.branch,
            local_revid_range=(lrevid3, lrevid3),
            remote_revid_range=(rrevid3, rrevid3),
        )

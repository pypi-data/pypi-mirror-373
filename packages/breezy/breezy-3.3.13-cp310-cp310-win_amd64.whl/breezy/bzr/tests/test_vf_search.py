        ...
        self.make_graph(
            {
                ...
                b"mid": [b"base"],
                b"tag": [b"base"],
                b"base": [NULL_REVISION],
                NULL_REVISION: [],
            }
        )
        result = vf_search.SearchResult(
            {b"tip", b"tag"}, {NULL_REVISION}, 4, {b"tip", b"mid", b"tag", b"base"}
        )
...
class TestSearchResultFromParentMap(TestGraphBase):
    def assertSearchResult(
        self, start_keys, stop_keys, key_count, parent_map, missing_keys=()
    ):
    ...
    def test_ancestry_2(self):
        self.assertSearchResult(
            [b"rev1b", b"rev4a"], [NULL_REVISION], len(ancestry_2), ancestry_2
        )
        self.assertSearchResult(
            [b"rev1b", b"rev4a"],
            [],
    ...
    def test_partial_search(self):
        ...
        self.assertSearchResult(
            [b"e", b"f"], [b"b"], 6, parent_map, missing_keys=[NULL_REVISION]
        )
        parent_map[b"b"] = extended_history_shortcut[b"b"]
        self.assertSearchResult(
            [b"e", b"f"], [], 7, parent_map, missing_keys=[NULL_REVISION]
        )
...
class TestLimitedSearchResultFromParentMap(TestGraphBase):
    ...
    def test_ancestry_1(self):
        ...
        self.assertSearchResult(
            [b"rev2a", b"rev2b"], [b"rev1"], 2, ancestry_1, (), [b"rev1"], 1
        )

    def test_multiple_heads(self):
        self.assertSearchResult(
            [b"e", b"f"], [b"a"], 5, extended_history_shortcut, (), [b"a"], 10
        )
        # Note that even though we only take 1 step back, we find 'f', which
        # means the described search will still find d and c.
        self.assertSearchResult(
            [b"f"], [b"a"], 4, extended_history_shortcut, (), [b"a"], 1
        )
        self.assertSearchResult(
            [b"f"], [b"a"], 4, extended_history_shortcut, (), [b"a"], 2
        )
...
class TestPendingAncestryResultGetKeys(tests.TestCaseWithMemoryTransport):
    ...
    def test_get_keys(self):
        ...
        builder.build_snapshot(
            None, [("add", ("", b"root-id", "directory", ""))], revision_id=b"rev-1"
        )

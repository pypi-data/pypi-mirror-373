...
class MetaTrader5:
    ...
    def initialize(self, *args, **kwargs):
        ...
        code = (
            ...
            f"_a = {args}\n"
            f"_k = {kwargs}\n"
            "# default portable from env if not provided by caller\n"
            "if 'portable' not in _k:\n"
            "    _envp = os.environ.get('MT5_PORTABLE')\n"
            "    if _envp is not None and _envp != '':\n"
            "        _envp_s = str(_envp).strip().lower()\n"
            "        _as_bool = _envp_s not in ('0','false','no','off')\n"
            "        _k['portable'] = _as_bool\n"
            "if _a or ('path' in _k):\n"

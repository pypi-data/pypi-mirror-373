...
class BaseClient(ABC, Generic[ConfigT]):
    ...
    def _create_retry_decorator(self, retry_config: RetryConfig | None = None) -> AsyncRetrying:
        """Create a tenacity retry decorator from RetryConfig.

        Converts our RetryConfig into tenacity's AsyncRetrying instance,
        allowing dynamic retry behavior configuration.
        ...
        Examples
        --------
        >>> config = RetryConfig(max_attempts=5, max_wait=30.0)
        >>> retrying = self._create_retry_decorator(config)
        >>> async for attempt in retrying:
        ...     with attempt:
        ...         result = await self._make_request()
        """
        if retry_config is None or retry_config.max_attempts == 0:
        ...
        def should_retry_exception(exc: BaseException) -> bool:
            ...
            if isinstance(exc, ClientError) and exc.status_code is not None:
                return exc.status_code in retry_config.retry_on_status or exc.status_code in RETRYABLE_STATUS_CODES

            if retry_config.retry_on_exceptions:
                return any(isinstance(exc, exc_type) for exc_type in retry_config.retry_on_exceptions)
    ...
    async def _request(
        ...
        **kwargs: Any,
        ...
        retry_config : RetryConfig | None, optional
            Override retry configuration for this request (default: None).
            Takes precedence over endpoint and client configs.
        **kwargs : Any
        ...
        ... )
        >>>
        >>> # Custom retry for critical request
        >>> response = await client._request(
        ...     endpoint,
        ...     request_data,
        ...
        Notes
        -----
        The retry behavior is dynamically configured based on the effective
        RetryConfig (precedence: parameter > endpoint > client > default).
        When max_attempts is 0, no retries are performed.
        """
        retrying = self._create_retry_decorator(retry_config=retry_config)

        async def _make_request() -> ResponseT:

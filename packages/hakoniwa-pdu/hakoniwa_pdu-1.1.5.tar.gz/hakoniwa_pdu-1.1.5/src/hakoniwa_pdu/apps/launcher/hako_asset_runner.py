    ...
    def spawn(
        ...
        *,
        ...
        if _is_wsl() and _looks_windows_exe(command):
            ...
            # ★ 重要：PowerShell の現在位置を win_cwd に移動してから Start-Process
            # ★ PID は -PassThru | Select-Object -ExpandProperty Id で“数字だけ”返す
            ps_script = (
                f"{pre_cmd}"
                f"{'Set-Location ' + _ps_quote(win_cwd) + ';' if win_cwd else ''}"
                f"$p = Start-Process {exe_quoted} {arglist}-PassThru {redir_out}{redir_err}{wd}; "
                f"$p | Select-Object -ExpandProperty Id"
            )

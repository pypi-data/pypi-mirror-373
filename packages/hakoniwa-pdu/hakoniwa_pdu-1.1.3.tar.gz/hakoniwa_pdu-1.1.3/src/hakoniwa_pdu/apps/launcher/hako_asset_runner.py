    ...
    def spawn(
        ...
        *,
        ...
        if _is_wsl() and _looks_windows_exe(command):
            ...
            ps_script = (
                f"{pre_cmd}"
                f"$p=Start-Process {exe} {arglist}-WindowStyle Hidden -PassThru {redir_out}{redir_err}{wd};"
                f"[Console]::Out.WriteLine($p.Id)"
            )
            ...
            out = subprocess.check_output(
                pwsh,
                cwd=(os.fspath(cwd) if (cwd and os.fspath(cwd).startswith("/mnt/")) else None),
                env=self.env,
                text=True,

...
def backtest(prices, signals,
             ...
             RF=0,
             strategy_name="",
             stop_loss=0.1, stop_profit=0.2,   # 新增参数
             initial_balance=1000000, slippage=0.001, fee_rate=0.0005,
             min_shares=100,
    ...
    for i in range(1, len(prices2)):
        ...
        # === 卖出逻辑（信号、止损、止盈） ===
            ...
            # 止盈卖出：若stop_profit == 0不止盈
            elif (stop_profit != 0) and (current_price - entry_price) / entry_price >= stop_profit:
                sell_flag = True
                reason = "止盈"
...
def calculate_metrics(prices, equity_curve, trade_records,
    ...
    if printout:
        print(f"*** 量化策略回测结果：{strategy_name}")
        ticker = prices2['ticker'].values[0]
        print(f"  {ticker_name(ticker)}，{start}至{end}")

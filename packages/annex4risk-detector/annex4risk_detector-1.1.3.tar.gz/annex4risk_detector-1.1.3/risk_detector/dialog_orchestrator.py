...
# AI questionnaire-based classification logic
def classify_from_answers(answers: dict) -> dict:
    """Classify risk based on AI questionnaire answers."""
    # Basic classification logic based on Annex III selections
    annex3_area = answers.get("annex3_area")
    annex3_item = answers.get("annex3_item")
    a6_conditions = answers.get("a6_conditions", [])
    
    if not annex3_item:
        return {"category": "minimal_risk", "score": 0.0, "legal_refs": [], "exception_applied": False}
    
    # Check for exclusions/exceptions
    if answers.get("exclusion_biometric_verification_only") and annex3_item == "AnnexIII.1.a":
        return {"category": "minimal_risk", "score": 0.1, "legal_refs": [annex3_item], "exception_applied": True}
    
    if answers.get("exclusion_credit_fraud_detection_only") and annex3_item == "AnnexIII.5.b":
        return {"category": "minimal_risk", "score": 0.1, "legal_refs": [annex3_item], "exception_applied": True}
    
    if answers.get("campaign_outputs_direct_to_voters") == False and annex3_item == "AnnexIII.8.b":
        return {"category": "minimal_risk", "score": 0.1, "legal_refs": [annex3_item], "exception_applied": True}
    
    if answers.get("solely_based_on_profiling") == False and annex3_item == "AnnexIII.6.d":
        return {"category": "minimal_risk", "score": 0.1, "legal_refs": [annex3_item], "exception_applied": True}
    
    # Check Article 6(3) conditions for potential downgrade
    if a6_conditions and len(a6_conditions) > 0:
        # If any Article 6(3) conditions apply, might reduce risk
        return {"category": "limited_risk", "score": 0.5, "legal_refs": [annex3_item, "Article 6(3)"], "exception_applied": True}
    
    # Default: if in Annex III without exceptions, it's high risk
    return {"category": "high_risk", "score": 0.9, "legal_refs": [annex3_item], "exception_applied": False}

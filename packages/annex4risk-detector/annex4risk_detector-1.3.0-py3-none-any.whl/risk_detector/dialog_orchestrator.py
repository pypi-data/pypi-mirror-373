...
def classify_from_answers(answers: dict) -> dict:
    ...
    try:
        with Session() as db:
            ...
            # Annex III item selected → high-risk (Art. 6(2)), unless Article 6(3) applies AND no constraint prevents downgrading
            ...
            if isinstance(annex3_item, str) and annex3_item in all_items:
                ...
                if isinstance(a63, list) and len(a63) > 0:
                    # Check policy constraints (e.g., "always high-risk when profiling natural persons")
                    fired = []
                    for c in constraints:
                        try:
                            ...
                            pass
                    # if any fired constraint blocks downgrade — stay high_risk
                    blocks_downgrade = any(c.effect in ("force_high_risk","force_prohibited","disallow_a6_relief") for c in fired)
                    if not blocks_downgrade:
                        category = "limited_risk"
                        exception_applied = True
                        legal_refs.append("Article 6(3)")
                        obligations.update(sum([c.legal_bases or [] for c in fired], []))

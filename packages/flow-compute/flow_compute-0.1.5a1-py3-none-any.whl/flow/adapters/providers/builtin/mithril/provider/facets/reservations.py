...
class ReservationsFacet:
    ...
    def get_reservation_availability(
        ...
        *,
        ...
        # Default to slots mode when a window range is provided or when caller asked for it
        if mode:
            params["mode"] = mode
        elif earliest_start_time or latest_end_time:
            params["mode"] = "slots"

        # For slots mode, the API expects earliest/latest; include quantity to express demand when present
        if earliest_start_time:
            params["earliest_start_time"] = earliest_start_time
        if latest_end_time:
            params["latest_end_time"] = latest_end_time
        if num_nodes:
            # API uses `quantity` terminology
            params["quantity"] = int(max(1, num_nodes))

        # Note: duration is not a parameter in slots mode; CLI filters by duration client‑side

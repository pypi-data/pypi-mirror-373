...
class ReserveWizard:
    ...
    def _choose_region(
        ...
        if gpu_family == "h100":
            regions = sorted(regions, key=lambda r: 0 if r == "us-central1-b" else 1)
    ...
    def run(self, flow) -> None:
        ...
        if not st.project:
            # Non-blocking: continue; service will error with helpful message
            pass
        # GPU family first
                ...
                try:
                    ...
                    if _click.confirm("Browse other slots?", default=True):
                        ...
                        try:
                            ...
                            pass
                        chosen = _choose_slot(
                            slots, region=str(st.region), instance_type=str(st.instance_type)
                        )
                        if not chosen:

...
\n"""
    ...
    def _createRequireForExt(self, extName, extItems):
        ...
        if extName == "LCGCMT":
            return f"Requires: {extName}_{extVer.replace('-', '_')}\n"

        requireline = None
        # Now we need to deal with LCG and non LCG externals...
        if f"LCG_{self._lcgVersion}" in extpath:
            # We have a LCG external !
            requireline = f"Requires: LCG_{self._lcgVersion}_{extName}_{extVer.replace('-', '_')}_{cmtcfgopt.replace('-', '_')}\n"
        else:
            # We have an old type external
            requireline = f"Requires: {extName}_{extVer.replace('-', '_')}_{cmtcfgopt.replace('-', '_')}\n"
        return requireline
...
e.g. %prog LHCbExternals v68r0 x86_64-slc6-gcc48-opt"""
    ...
    def main(self):
        ...
        self.log.warning(f"Processing externals for {project} {version} {cmtconfig}")
        ...
        print(f"{project} {version} {cmtconfig} {buildarea} {externalsDict} {lcgVer}")
...
def get_macro_value(cmtdir, macro, extratags):
    ...
    __log__.debug(f"get_macro_value - {macro} = {line}")
    if cmtdir != None:
        os.chdir(here)
    return line
...
def get_base_project(native_version):
    ...
    print(f"get_base_project {NAME} {version} {Name} {NameSys} {release_area}")
...
def getLCGBinary(workdir, extname, binary):
    ext_home = CMT(
        getCMTExtraTags(binary), "show", "macro_value", f"{extname}_home", cwd=workdir
    )[0][:-1]
    nat_version = CMT(
        getCMTExtraTags(binary),
        "show",
        "macro_value",
        f"{extname}_native_version",
        cwd=workdir,
    )[0][:-1]
    cfg_version = CMT(
        getCMTExtraTags(binary),
...
def pkgFilter(NAME, pak, vers, binary):
    ...
    if pak.lower() == "icc":
        keep = False
    if not keep:
        print(f"Excluding {pak} {vers} for {binary}")
    return keep
...
def get_native_versions(native_version, binary):
    ...
    for pak in list(packages_versions.keys()):
        if not packages_versions[pak][0]:
            __log__.warning(f"{pak} has no version. Removing it from the list")
    ...
    for pak in list(packages_versions.keys()):
        __log__.info(f"get_native_version - Checking home for package: {pak}")
        macro = pak + "_home"
        value = os.path.normpath(get_macro_value(None, macro, getCMTExtraTags(binary)))
        __log__.debug(f"get_native_version - {macro} = {value}")
        ...
        if pakType is None:
            ...
            if value.startswith("/usr"):
                print(
                    f"get_native_version - {pak} home is {value}, IGNORING package from system"
                )
                del packages_versions[pak]
            else:
                print(
                    f"get_native_version - {pak} home is {value}, not in external or app/releases, WARNING"
                )

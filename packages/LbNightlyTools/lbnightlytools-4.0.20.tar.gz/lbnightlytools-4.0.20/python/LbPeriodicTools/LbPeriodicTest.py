...
def validateDaysOfWeek(daysstr):
    ...
    """
    sched_days = {d.strip() for d in daysstr.split(",")}
    unknown_days = sched_days - set(DAY_NAMES)

    if len(unknown_days) > 0:
        raise ValueError(f"Unknown days in schedule: {', '.join(unknown_days)}")
...
def validateDaysOfMonth(daysstr):
    ...
    """
    sched_days = {int(d.strip()) for d in daysstr.split(",")}
    incorrect_days = [d for d in sched_days if d <= 0]
    incorrect_days += [d for d in sched_days if d > 31]
    if len(incorrect_days) > 0:
        raise ValueError(
            f"Days have to be between 1 and 31. Not allowed: {', '.join(str(day) for day in incorrect_days)}"
        )
...
def _fromXML(etree):
    ...
    if elem == None:
        raise Exception(f"Missing <test> with test definition in {tostring(etree)}")
    ...
    if len(missing) > 0:
        raise Exception(
            "Missing values for: {} in \n{}".format(
                (",".join(missing)), tostring(etree)
            )
        )

    # Checking for unknown attributes
    extra = {child.tag for child in etree} - attlist
    if len(extra) > 0:
        raise Exception(
            "Unknown attribute(s): {} in \n{}".format(
...
class PeriodicTest:
    ...
    def __init__(self, etree=None):
        ...
        if etree != None:
            ...
            setattr(
                self,
                ADATES,
                {d.strip().lower() for d in testdict[ASCHED].split(",")},
            )
    ...
    def __str__(self):
        ...
        Convert to string
        """
        return "/".join([f"{k}={getattr(self, k)}" for k in TEST_ATTRIBUTES])
    ...
    def getBuildMatches(self, buildlist):
        ...
        for buildinfo in buildlist:
            ...
            __log__.debug(
                self.testgroup + "Checking against build: {}".format(buildinfo["slot"])
            )
...
class ScheduledTest:
    ...
    def __str__(self):
        ...
        Convert to string
        """
        return "/".join([f"{k}={getattr(self, k)}" for k in SCHED_ATTRIBUTES])

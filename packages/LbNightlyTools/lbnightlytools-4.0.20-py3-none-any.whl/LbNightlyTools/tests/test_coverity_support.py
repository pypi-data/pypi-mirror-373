...
def collect_reports(names=None):
    ...
    assert exists(COV_OUT), f"missing directory {COV_OUT}"

    reports = {}
    for name in names:
        filename = join(COV_OUT, f"cov-{name}.report.json")
        assert exists(filename), f"missing {filename}"
...
def check_logfiles(expected, not_expected=None):
    ...
    for logfile in [
        ...
        assert exists(logfile), f"missing {logfile}"
    for logfile in [
        cov_log_file(step) + ext
        for step in not_expected
        for ext in [".log", ".err.log"]
    ]:
        assert not exists(logfile), f"found unexpected {logfile}"
...
def test_analyze():
    ...
    for name in reports:
        assert reports[name]["retcode"] == 0, f"failure in {name}"
    ...
    ), "expected strip-path {}, found {}".format(
    ...
    for k, v in list(
        ...
        ), "bad value for {}: expected {}, found {}".format(
...
def test_failed_build():
    ...
    for name in ("analyze", "commit-defects"):
        assert not exists(join(COV_OUT, f"cov-{name}.report.json"))

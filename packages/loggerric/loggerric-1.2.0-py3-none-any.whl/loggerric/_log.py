    ...
    def info(cls, *content:str) -> None:
        ...
        *Example*:
        ...
        if LogLevel.INFO in cls._active_levels:
            print(f'{Timestamp.get(internal_call=True)}{Fore.GREEN}[i] {" ".join([str(c) for c in content])}{Fore.WHITE}')
    ...
    def warn(cls, *content:str) -> None:
        ...
        *Example*:
        ...
        if LogLevel.WARN in cls._active_levels:
            print(f'{Timestamp.get(internal_call=True)}{Fore.YELLOW}[w] {" ".join([str(c) for c in content])}{Fore.WHITE}')

    @classmethod
    def error(cls, *content:str, quit_after_log:bool=False) -> None:
        """
        ...
        *Example*:
        ```python
        Log.error('Hello World!', ..., quit_after_log=True)
        ```
        """
        # Log the content
        if LogLevel.ERROR in cls._active_levels:
            print(f'{Timestamp.get(internal_call=True)}{Fore.RED}[!] {" ".join([str(c) for c in content])}{Fore.WHITE}')
    ...
    def debug(cls, *content:str) -> None:
        ...
        *Example*:
        ...
        if LogLevel.DEBUG in cls._active_levels:
         print(f'{Timestamp.get(internal_call=True)}{Fore.LIGHTBLACK_EX}[?] {" ".join([str(c) for c in content])}{Fore.WHITE}')

...
The serializable record is a set of tuples ('contextual name', contextual_value). "rounding" provides the stable
formatting specification for numeric results: '%.*g' % (self.precision, contextual_value).  precision defaults to 8.
...
class _FragmentLcaTest(BaseModel):
    ...
    def run(self, model) -> Set[Tuple[str, float]]:

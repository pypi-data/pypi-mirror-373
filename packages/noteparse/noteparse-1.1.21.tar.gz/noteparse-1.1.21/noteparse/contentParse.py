...
def parseHtmlByZhipu(htmlStr):
    try:
        ...
        print(datetime.now(),'请求质谱AI异常,15S后重试')
        time.sleep(15)
        return parseHtmlByZhipu(htmlStr)

def parseContent(htmlStr,noteInfo):
...
def parseContent2(content,noteInfo):
...
if __name__ == '__main__':
    ...
    #    noteInfo = testMatch('电话：ABC公司')
        ...
        a = '''
            <div id="mycontent">
<div class="detail_content"><style type="text/css">    .heightAuto {        padding-top: 10px;        overflow: auto;    }        .heightAuto .title {        margin-top: 10px;        font-size: 30px;        font-weight: bolder;        text-align: center;        line-height: 35px;        margin-bottom: 50px;    }        .tab-package-title-div {    	margin-top: 10px;    	font-weight: bolder;    	font-size: 18px;    	background-color: RGB(221, 235, 247);    	padding: 5px 20px;    }        .tab-title-div {    	margin-top: 6px;    }        .tab-title-div span {    	font-weight: bolder;    	font-size: 16px;    	background-color: RGB(221, 235, 247);    	padding: 5px 20px;    }        .tab-container {    	margin-top:5px;    	margin-bottom: 10px;    }        .tab-container .tab {        width: 100%;        text-align: center;	        border-spacing: 0;        border-collapse: collapse;        table-layout:fixed;    }        .tab-container .tab tr td {        padding: 10px 15px;		border: 1px solid #d5d5d5;        height: 20px;        word-break: break-all;    }        .tab-container .tab-contact {    	width: 100%;        border-spacing: 0;        border-collapse: collapse;    }        .tab-container .tab-contact tr td {        padding: 10px 15px;        border: 1px solid #d5d5d5;        height: 20px;        word-break: break-all;    }        .tab-container1 {    	margin-top:5px;    	margin-bottom: 10px;    	overflow:auto; 		width:auto; 		height:auto;    }        .dg-tab {    	margin-top:5px;    	margin-bottom: 10px;    	overflow:auto;     	width:auto; 		height:255px;    }         .tab-container1 .tab {        width: 100%;        text-align: center;	        border-spacing: 0;        border-collapse: collapse;        table-layout:fixed;    }        .tab-container1 .tab tr td {        padding: 10px 15px;		border: 1px solid #d5d5d5;        height: 20px;        word-break: break-all;    }        .tab-container1 .tab-contact {    	width: 100%;        border-spacing: 0;        border-collapse: collapse;    }        .tab-container1 .tab-contact tr td {        padding: 10px 15px;        border: 1px solid #d5d5d5;        height: 20px;        word-break: break-all;    }        </style><script type="text/javascript">  	function combineColspan(){		/**1、候选人业绩-候选人名称合并（这样合并的前提是将业绩数据按照候选人相同名称的排好，不能间隔着来）**/		var hxryj_hxrmc_array = document.querySelectorAll(".hxryj-hxrmc");		var hxryj_hxrmc_map = {};		/**先统计候选人名称列数**/		hxryj_hxrmc_array.forEach((item,index,arr)=>{			var hxryj_hxrmc = item.innerText;			var num = hxryj_hxrmc_map[hxryj_hxrmc];			if ( == num) {				num = 1;			} else {				num = num + 1;			}			hxryj_hxrmc_map[hxryj_hxrmc] = num;		});		var i = 0;		for ( var k in hxryj_hxrmc_map) {			if (hxryj_hxrmc_map[k] <= 1) { /**行数大于1时才有必要操作**/				/**改变起始行**/				i = i + num;				continue;			}			hxryj_hxrmc_array[i].rowSpan = hxryj_hxrmc_map[k];			/**要删除的td**/			var j = 1;			while (j < hxryj_hxrmc_map[k]) { /**根据规律直到j < hxryj_hxrmc_map[k]**/				if (isIE()) {					hxryj_hxrmc_array[i + j].removeNode(true);				} else {					hxryj_hxrmc_array[i + j].remove();				}				j = j + 1;			}			/**改变起始行**/			i = i + hxryj_hxrmc_map[k];		}			}		function isIE() {		if (!!window.ActiveXobject || "ActiveXObject" in window) {			return true;		} else {			return false;		}	}   	window.onload=function (){	  	var dg_array = document.querySelectorAll(".dg-tab");	  	dg_array.forEach((item,index,arr)=>{		  	var length=item.getElementsByTagName('tr').length;		  	if(length < 5){				item.style.setProperty('height', 'auto', 'important');		  	}			  	});	  debugger;	  	combineColspan();  	}</script><title>中标结果公示</title><div><div class="heightAuto"><div><p class="title">                启动区成熟区域公园绿地工程（一期）施工总承包中标结果公示            </p></div><div class="tab-title-div">基本信息</div><div class="tab-container"><table class="tab"><tbody><tr><td width="20%">项目名称</td><td colspan="3">					启动区成熟区域公园绿地工程（一期）施工总承包				</td></tr><tr><td width="20%">行业类型</td><td width="30%">园林绿化</td><td width="20%">所属地区</td><td width="30%">雄安新区</td></tr><tr><td>开标时间</td><td>2025年8月18日</td><td>定标时间</td><td>2025年8月23日</td></tr><tr><td>公示开始</td><td>2025年8月26日</td><td>公示结束</td><td>2025年8月28日</td></tr></tbody></table></div><div class="tab-title-div">中标信息</div><div class="tab-container"><table class="tab"><tbody><tr><td width="7%">统一社会信用代码</td><td width="11%">中标单位名称</td><td width="8%">投标报价<br>（万元）</td><td width="8%">评标报价<br>（万元）</td><td width="8%">费率或其他报价</td><td width="7%">评分结果</td><td width="7%">质量标准</td><td width="8%">工期/<br>交货期</td><td width="8%">项目负责人姓名</td><td width="6%">职称</td><td width="7%">执业证书</td><td width="7%">证书编号</td><td width="12%">资质要求<br>（具体到资质等级）</td></tr><tr><td>91610000220522345A,91133100MA7B63TB5E</td><td>中铁一局集团有限公司,中铁一局集团第二建设有限公司</td><td>852.264295</td><td>852.264295</td><td></td><td>95.59</td><td>满足招标文件要求</td><td>满足招标文件要求</td><td>石仰亮</td><td>工程师</td><td>市政公用工程专业二级注册建造师</td><td>陕261161792381</td><td> 市政公用工程施工总承包特级，市政公用工程施工总承包一级</td></tr></tbody></table></div><div class="tab-container"><table class="tab"><tbody><tr><td style="width: 80px;padding-bottom: 40px;text-align: left;padding-top: 40px;">业绩</td><td style="text-align: left;">    	  		  		    	  		  			&nbsp;无    	  		  		    	  		  	</td></tr></tbody></table></div><div class="tab-container"><table class="tab"><tbody><tr><td style="width: 80px;padding-bottom: 40px;text-align: left;padding-top: 40px;">中标人的比较优势</td><td style="text-align: left;">	    	  		  		该公司被评标委员会推荐为第一中标候选人，能够最大限度地满足招标文件要求。招标人确定其为中标人。	    	  		  	</td></tr></tbody></table></div><div class="tab-title-div">所有被否决的投标单位名称</div><div class="tab-container"><table class="tab"><tbody><tr><td style="text-align: left;">    	  		  		    	  		  			&nbsp;无    	  		  		    	  		  		    	  		  	</td></tr></tbody></table></div><div class="tab-title-div">发布媒体</div><div class="tab-container"><table class="tab"><tbody><tr><td style="text-align: left;">	  		  		雄安新区公共资源交易服务平台；河北省招标投标公共服务平台；中国招标投标公共服务平台	  		  	</td></tr></tbody></table></div><div class="tab-title-div">联系方式</div><div class="tab-container"><table class="tab-contact"><tbody><tr><td width="25%">招标人</td><td width="25%">中国雄安集团生态建设投资有限公司</td><td width="25%">招标代理机构</td><td width="25%">北京北咨工程咨询有限公司</td></tr><tr><td>联系人</td><td>石经理</td><td>联系人</td><td>李贺、高宗飞、孔瑞颖、张梦倩、王徽</td></tr><tr><td>地址</td><td>河北雄安新区容东片区春明四街71号</td><td>地址</td><td>北京市通州区云杉路2号院11号楼2层203室</td></tr><tr><td>电话</td><td>0312-5675068</td><td>电话</td><td>13383326835、16633365097、16633365098</td></tr><tr><td>电子邮箱</td><td>/</td><td>电子邮箱</td><td>becczbb@sina.com</td></tr></tbody></table></div></div></div></div>
</div>
        ''' 
        noteInfo ={
            'noticeType': 0,

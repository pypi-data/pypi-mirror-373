...
class BaseRepository(Generic[T]):
    ...
    async def execute_sql(cls, stmt: str, session: AsyncSession) -> List[Dict]:
        ...
        logger.debug(
            "SQL выполнен за %.3f сек", _count_execute_time(start_time=start_time)
        )
        ...
        if group_by or aggregations:
            ...
            if select_fields:
                if hasattr(cls, "_validate_fields"):
                    ...
                    valid_fields = [
                        field for field in select_fields if field in model_fields
                    ]
                    if not valid_fields:
                        raise ValueError(
    ...
    def _build_select_query(
        cls, select_fields: Optional[List[str]] = None, use_distinct: bool = False
    ) -> Select:
        """
    ...
    async def update(
        ...
        if id is not None:
            stmt = update(cls.model).where(cls.model.id == id).values(**values_dict)
            result = await session.execute(stmt)

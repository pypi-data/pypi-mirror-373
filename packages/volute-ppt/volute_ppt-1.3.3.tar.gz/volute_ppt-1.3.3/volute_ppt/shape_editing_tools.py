...
def register_shape_editing_tools(mcp: FastMCP) -> None:
                ...
                for shape_name, new_text in text_updates.items():
                    try:
                        ...
                        if hasattr(target_shape, 'TextFrame') and target_shape.TextFrame:
                            ...
                            # Update the text with proper newline handling
                ...
                try:
                    ...
                    if slide_image:
                        visual_verification = {
                            ...
                            "operation_type": "text_content_edit"
                        }
                        logger.info(f"Captured slide {slide_number} after text content updates")
                    else:
                        visual_verification = {
                            "captured": False,
    ...
    @mcp.tool()
    def edit_slide_text_content_smart(
        presentation_path: str = Field(description="Path to the PowerPoint file (.pptx, .ppt)"),
        slide_number: int = Field(description="Slide number to edit (1-based)"),
        text_updates: Dict[str, str] = Field(description="Dictionary of shape names to new text content"),
        ...
        """
        Smart text editing with intelligent bullet detection and conversion.
        
        This enhanced tool provides:
        - Automatic detection of bullet patterns (•, ○, ■, -, 1., a., etc.)
        - Conversion to native PowerPoint bullets (no double bullets)
        - Multi-level bullet indentation support
        - Newline handling with proper line breaks
        - Format preservation and auto-resize
        
        Perfect for LLM-generated content that includes text bullets,
        ensuring clean PowerPoint formatting without duplicate bullets.
        """
        try:
            # Validate inputs
            if not os.path.exists(presentation_path):
            ...
            with PowerPointSlideExecutor() as executor:
                ...
                # Save the presentation
                if not executor.save_presentation():
                    logger.warning("Failed to save presentation after smart text updates")
            
            success = shapes_modified > 0
            message = f"Smart updated {shapes_modified} shapes" if success else "No shapes were updated"
            ...
            if capture_result and success:
                try:
                    ...
                    if slide_image:
                        ...
                        logger.info(f"Captured slide {slide_number} after smart text updates")
                    else:
                        visual_verification = {
                            "captured": False,
                            "reason": "Slide capture failed or not available"
                        }
                        logger.warning(f"Failed to capture slide {slide_number} after smart text updates")
                        
                except Exception as e:
                    logger.error(f"Error during slide capture after smart text updates: {e}")
                    visual_verification = {
                        "captured": False,
                        "reason": f"Capture error: {str(e)}"
                    }
            ...
            logger.exception(f"Error in edit_slide_text_content_smart: {str(e)}")
            return ShapeEditResult(
                success=False,
                message="Smart text editing failed",
                error=f"Failed to edit slide text with smart processing: {str(e)}"
            )

...
try:
    ...
    logger.warning(f"python-pptx import failed: {e}")
...
class PowerPointMetadataExtractor:
    ...
    def _extract_autoshape_data(self, shape) -> Dict[str, Any]:
        ...
        try:
            # Extract adjustments safely by converting to serializable format
            adjustments_data = []
            try:
                adjustments = getattr(shape, 'adjustments', None)
                if adjustments:
                    # Convert AdjustmentCollection to list of values
                    for i in range(len(adjustments)):
                        adjustments_data.append(adjustments[i])
            except Exception:
                adjustments_data = []

...
def prepare(
    ...
    try:
        ...
        console.print(f"[bold blue]Preparing data from {input_dir} to {output_dir}[/bold blue]")
        console.print(f"Formats: {', '.join(formats)}")
        console.print(f"Deduplication: {'enabled' if deduplicate else 'disabled'}")
        console.print(f"Cleaning: {'enabled' if clean else 'disabled'}")
        ...
        if deduplicate and processed_files:
            console.print("\n[bold yellow]Running deduplication...[/bold yellow]")
            ...
            try:
                ...
                if dedup_stats:
                    console.print(f"Original files: {dedup_stats.get('original_count', 'N/A')}")
                    console.print(f"After deduplication: {dedup_stats.get('final_count', 'N/A')}")
        ...
        if clean and processed_files:
            console.print("\n[bold yellow]Running additional text cleaning...[/bold yellow]")
            ...
            try:
                ...
                with Progress(
                    ...
                    clean_task = progress.add_task("Cleaning text...", total=len(processed_files))
                ...
                console.print("[green]Text cleaning completed[/green]")
                
            except Exception as e:
                console.print(f"[red]Text cleaning failed: {e}[/red]")
        ...
        table.add_row("Files Found", str(len(files_to_process)))
        table.add_row("Successfully Processed", str(len(processed_files)))
        ...
        if failed_files:
            console.print(f"\n[yellow]Warning: {len(failed_files)} files failed to process[/yellow]")
            console.print("Check logs for details on failed files")

...
class FurhatClient:
    ...
    def request_speak_audio(self, audio: bytes, wait: bool = True, abort: bool = True):
        self._run_coroutine(self.async_client.request_speak_audio(audio=audio, abort=abort, wait=wait))    
    ...
    def request_gesture(self, gesture_name: str, intensity: float = 1.0, duration: float = 1.0, wait: bool = False):
        return self._run_coroutine(self.async_client.request_gesture_start(gesture_name, intensity=intensity, duration=duration, wait=wait))

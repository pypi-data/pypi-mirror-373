"""
AI Search API Tool for CrewAI (new tools API, fixed for Pydantic)
...
class AISearchTool(BaseTool):
    ...
    # BaseTool public fields (can be changed by you)
    name: str = "AI_Search"
    description: str = (
    ...
    # Private state (Pydantic-safe)
    _config: AISearchToolConfig = PrivateAttr()
    _client: AISearchAPIClient = PrivateAttr()
    _context_history: List[ChatMessage] = PrivateAttr(default_factory=list)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # init pydantic/BaseTool first (accepts name/description overrides in kwargs)
        super().__init__(**kwargs)

        self._config = config or AISearchToolConfig()

        # Resolve API key: param > config > env
        self._api_key = api_key or self._config.api_key or os.getenv("AISEARCHAPI_API_KEY") or ""
        ...
        **kwargs: Any
        ...
        try:
            ...
            if self._context_history:
                ...
                recent = self._context_history[-self._config.max_context_messages :]
                context_messages.extend(recent)

            # Default response type if missing
            if not response_type:
                response_type = self._config.default_response_type
            ...
            if getattr(e, "status_code", None) == 433:
    ...
    def _format_response(self, result) -> str:
        ...
        if self._config.include_sources and getattr(result, "sources", None):
            parts.append("\n**Sources:**")
            for i, src in enumerate(result.sources, 1):
                parts.append(f"- [{i}] {src}")

        if self._config.verbose and hasattr(result, "total_time"):
    ...
    def _update_context(self, query: str, answer: str) -> None:
        ...
        if len(self._context_history) > limit:
            self._context_history = self._context_history[-limit:]
...
def aisearch_tool(
    ...
    """Return an AISearchTool instance."""

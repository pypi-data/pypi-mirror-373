...
class SMCPDArray(SMCFVG,SMCOrderBlock):
    ...
    SUPPORT_PRICE_COL = "support_price"
    RESISTANCE_PRICE_COL = "resistance_price"
    SUPPORT_TIMESTAMP_COL = "support_timestamp"
    RESISTANCE_TIMESTAMP_COL = "resistance_timestamp"
    ...
    def get_support_resistance_by_PDArray(self, symbol, data_struct) -> dict:
        '''
        获取支持位和阻力位
        Args:
        ...
        Returns:
            dict: _description_
        '''
        OBs_df = self.find_OBs(symbol=symbol,struct=data_struct)
        ...
        if OBs_df is None or len(OBs_df) == 0:
            ...
            if support_OB :
                ...
                support_price = data_struct.at[data_struct.index[-1], self.STRUCT_LOW_COL]
                support_timestamp = data_struct.at[data_struct.index[-1], self.TIMESTAMP_COL]
            ...
            if resistance_OB :
                ...
                resistance_price = data_struct.at[data_struct.index[-1], self.STRUCT_HIGH_COL]
                resistance_timestamp = data_struct.at[data_struct.index[-1], self.TIMESTAMP_COL]
                
                
            return {
                self.SUPPORT_PRICE_COL: support_price,
                self.SUPPORT_TIMESTAMP_COL: support_timestamp,
                self.RESISTANCE_PRICE_COL: resistance_price,
                self.RESISTANCE_TIMESTAMP_COL: resistance_timestamp,
            }

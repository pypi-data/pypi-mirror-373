...
class SMCFVG(SMCStruct):
    ...
    def find_FVGs(
        ...
        # 将价格转换为decimal类型以避免float运算错误
        df.loc[:, self.FVG_TOP] = price_top.where(condition, 0).astype('decimal')
        df.loc[:, self.FVG_BOT] = price_bot.where(condition, 0).astype('decimal')

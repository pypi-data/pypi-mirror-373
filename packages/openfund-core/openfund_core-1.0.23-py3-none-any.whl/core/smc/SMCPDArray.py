...
class SMCPDArray(SMCFVG,SMCOrderBlock):
    ...
    def find_PDArrays(
        ...
        df_PDArrays.loc[:, self.PD_HIGH_COL] = df_PDArrays[[self.FVG_TOP, self.OB_HIGH_COL]].astype('decimal').max(axis=1)
        df_PDArrays.loc[:, self.PD_LOW_COL] = df_PDArrays[[self.FVG_BOT, self.OB_LOW_COL]].astype('decimal').min(axis=1)

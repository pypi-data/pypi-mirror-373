...
class StrandsMCPServer:
    ...
    def _get_bundled_cache_path(self) -> Path:
        """Get the path to the bundled cache directory.
        ...
        try:
            ...
            try:
                ...
                with resources.path('strands_mcp.data', 'cache') as cache_path:
                    return cache_path
            ...
            return Path(__file__).parent / 'data' / 'cache'
            
        except Exception as e:
            logger.warning(f"Error resolving bundled cache path: {e}")
            return Path(__file__).parent / 'data' / 'cache'

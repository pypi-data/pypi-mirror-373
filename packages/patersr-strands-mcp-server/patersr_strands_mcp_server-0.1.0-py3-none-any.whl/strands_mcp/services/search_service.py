...
class SearchService:
    ...
    def _ensure_index_loaded(self) -> None:
        ...
        if self._document_index is None or self._faiss_index is None:
            raise RuntimeError("Search index not loaded. Call load_index() first.")
    ...
    async def semantic_search(self, search_query: SearchQuery) -> List[SearchResult]:
        ...
        logger.info(f"Performing semantic search for: '{search_query.query}'")
        ...
        try:
            ...
            logger.info(f"Found {len(search_results)} results for query: '{search_query.query}'")

...
class StrandsMCPServer:
    ...
    async def _get_bundled_cache(self) -> Optional[List[DocumentChunk]]:
        ...
        try:
            ...
            try:
                bundled_cache_path = pkg_resources.resource_filename('strands_mcp', 'data/cache/index.json')
            except:
                # Fallback to relative path if pkg_resources fails
                from pathlib import Path
                bundled_cache_path = Path(__file__).parent / 'data' / 'cache' / 'index.json'
            
            if not Path(bundled_cache_path).exists():

        ...
        for attempt in range(max_attempts):
            try:
                ...
                if isinstance(result, dict) and result.get('status') == 'error':
                    error_detail = result.get('status_detail', {})
                    error_msg = error_detail.get('error', result.get('error', 'Unknown error'))
                    error_category = result.get('error_category', 'UNKNOWN')

                    # Enhanced error handling based on category
                    if error_category == 'PARSER_NOT_FOUND' or 'no parser' in error_msg.lower():
                    ...
                    elif error_category == 'BROWSER_POOL_FULL':
                        # Browser pool full - might be retryable after delay
                        pool_info = result.get('pool_status', {})
                        enhanced_msg = f"Browser pool exhausted ({pool_info.get('available_slots', 0)}/{pool_info.get('total_slots', 'unknown')} slots available)"
                        ...
                        enhanced_msg = f"[{error_category}] {error_msg}"
                        if attempt < max_attempts - 1:

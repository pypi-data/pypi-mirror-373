        ...
        if inspect.iscoroutinefunction(fn):
            ...
            try:
                ...
                if hasattr(fn, '__annotations__'):
                    ...
                    # Get the function's module for resolving forward references
                    fn_module = inspect.getmodule(fn)
                    fn_globals = getattr(fn_module, '__dict__', {}) if fn_module else {}
                    
                    for param_name, annotation in fn.__annotations__.items():
                        try:
                            # If it's a string (forward reference), try to resolve it
                            if isinstance(annotation, str):
                                resolved_annotations[param_name] = eval(annotation, fn_globals)
                            else:
                                resolved_annotations[param_name] = annotation
                        except:
                            # If resolution fails, keep the original
                            resolved_annotations[param_name] = annotation
                    
                    # Update the wrapper's annotations with resolved types

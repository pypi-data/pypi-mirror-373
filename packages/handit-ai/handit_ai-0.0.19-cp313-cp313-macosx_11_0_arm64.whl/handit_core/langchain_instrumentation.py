...
def _emit_langchain_call(method_name: str, model: str, messages: list, **kwargs) -> int:
    ...
    try:
        ...
        args_preview = {
            "model": model,
            "messages": str(messages)[:300] + "..." if len(str(messages)) > 300 else str(messages),
            **{k: str(v)[:100] for k, v in kwargs.items() if k in ["temperature", "max_tokens", "timeout"]}
        }
...
def _emit_langchain_return(t0_ns: int, func_name: str, result: Any, error: Optional[str] = None) -> None:
    ...
    try:
        ...
        dt_ns = t1 - t0_ns
        if error:
            locals_preview = {"return": f"Error: {error[:200]}"}
        else:
            try:
                if hasattr(result, 'content'):
                    content = result.content or ""
                    locals_preview = {"return": content[:500] + "..." if len(content) > 500 else content}
                else:
                    locals_preview = {"return": str(result)[:200]}
            except Exception:

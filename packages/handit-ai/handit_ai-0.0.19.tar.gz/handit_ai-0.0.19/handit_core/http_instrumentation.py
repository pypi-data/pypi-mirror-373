			...
			try:
				...
				try:
					if hasattr(response, "text"):
						...
						resp_content = content.decode("utf-8", errors="replace")[:500] if isinstance(content, bytes) else str(content)[:500]
				except:
	...
	try:
		...
		async def wrapped(self, cast_to, options, *, stream=None, stream_cls=None):
			...
			if hasattr(options, 'json_data') and options.json_data:
				...
				body_str = str(options.content)[:1000]
			...
			try:
				...
				try:
					if hasattr(response, 'content'):
						resp_content = str(response.content)[:500]
					elif hasattr(response, 'choices') and response.choices:
						# This is a chat completion response
						choice = response.choices[0]
						if hasattr(choice, 'message') and hasattr(choice.message, 'content'):
							resp_content = choice.message.content[:500]
					else:
						resp_content = str(response)[:500]
				except:
					resp_content = "<openai-response>"

"""
Buckeye context helpers for persistent state across hot-reloads.
...
Usage in your environment:
    ...
    from buckeyelabs.server.context import serve_context
...
# Default Unix socket path (can be overridden with BUCKEYE_CTX_SOCK)
DEFAULT_SOCK_PATH = "/tmp/buckeye_ctx.sock"  # noqa: S108


def serve_context(
    context_instance: Any, sock_path: str | None = None, authkey: bytes = b"buckeye-context"
) -> BaseManager:
    """
    ...
    Args:
        ...
        sock_path: Unix socket path (defaults to BUCKEYE_CTX_SOCK env var or /tmp/buckeye_ctx.sock)
        authkey: Authentication key for the manager

    Returns:
        The manager instance (can be used to shutdown)
    """
    sock_path = sock_path or os.getenv("BUCKEYE_CTX_SOCK", DEFAULT_SOCK_PATH)
...
def attach_context(sock_path: str | None = None, authkey: bytes = b"buckeye-context") -> Any:
    """

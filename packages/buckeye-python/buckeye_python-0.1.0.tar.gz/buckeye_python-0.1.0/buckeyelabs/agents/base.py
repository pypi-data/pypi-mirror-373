...
from buckeyelabs.types import AgentResponse, MCPToolCall, MCPToolResult, Trace
from buckeyelabs.utils.design import BUCKDesign
from buckeyelabs.utils.mcp import MCPConfigPatch, patch_mcp_config, setup_buckeye_telemetry

if TYPE_CHECKING:
    from buckeyelabs.clients.base import AgentMCPClient
...
class MCPAgent(ABC):
    ...
    def __init__(
        ...
        self.design = BUCKDesign(logger=logger)
    ...
    async def get_response(
        ...
        NOTE: Subclasses should decorate this method with:
            @buckeyelabs.instrument(span_type="agent", record_args=False, record_result=True)
    ...
    async def _setup_config(self, mcp_config: dict[str, dict[str, Any]]) -> None:
        ...
        self._auto_trace_cm = setup_buckeye_telemetry(mcp_config, auto_trace=self._auto_trace)
    ...
    def _handle_connection_error(self, e: Exception) -> None:
        ...
        if self._is_connection_error(e):
            ...
            if "localhost" in error_str or "127.0.0.1" in error_str:
                self.design.info("   Run 'buckeye dev' in another terminal to start the MCP server")

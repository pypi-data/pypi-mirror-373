"""Job management for Buckeye SDK.
    ...
    async def update_status(self, status: str) -> None:
        ...
        if settings.telemetry_enabled:
            try:
                ...
                await make_request(
                    method="POST",
                    url=f"{settings.buckeye_telemetry_url}/jobs/{self.id}/status",
                    json=payload,
...
def job(
    ...
    Example:
        with buckeyelabs.job("training_run", {"model": "gpt-4"}) as job:
            for epoch in range(10):
                with buckeyelabs.trace(f"epoch_{epoch}", job_id=job.id):
                    train_epoch()
    """
    global _current_job
...
def create_job(
    ...
    Example:
        job = buckeyelabs.create_job("data_processing")
        try:
            for item in items:
                with buckeyelabs.trace(f"process_{item.id}", job_id=job.id):
                    process(item)
        finally:
...
def job_decorator(name: str | None = None, **metadata: Any) -> Callable:
    ...
    Example:
        @buckeye.job_decorator("model_training", model="gpt-4", dataset="v2")

...
def create_proxy_server(
    ...
    if not original_cmd:
        ...
        original_cmd = ["python", "-m", "buckeye_controller.server"]
...
async def start_mcp_proxy(
            ...
            try:
                ...
                if process.stdout:
                    async for line in process.stdout:
                        ...
                        # Show all logs with gold formatting like buckeye debug
                        # Format all logs in gold/dim style like buckeye debug's stderr
    ...
    try:
        ...
        if not interactive:  # Only show if not in interactive mode
            ...
            design.info(f"    [cyan]buckeye build {directory}[/cyan]")
            design.info("")
            design.info("This will:")
            design.info("  1. Build your environment image")
            design.info("  2. Generate a buckeye.lock.yaml file")
            design.info("  3. Prepare it for testing with agents")
            design.info("")
            design.info("Then you can:")
            design.info("  • Test locally: [cyan]buckeye run <image>[/cyan]")
            design.info("  • Push to registry: [cyan]buckeye push --image <registry/name>[/cyan]")
    except Exception as e:
...
def run_mcp_dev_server(
    ...
    Examples:
        buckeye dev .                    # Auto-detect image from directory
    ...
    # Create config
    if transport == "stdio":
        server_config = {"command": "buckeye", "args": ["dev", directory, "--transport", "stdio"]}
    ...
    design.header("buckeye development Server")
    ...
    design.progress_message(
        f"❗ If any issues arise, run `buckeye debug {resolved_image}` to debug the container"
    )
    ...
    try:
        ...
        design.info("")
        design.info("💡 Tip: Run the following command to debug the container:")
        design.info(f"   buckeye debug {resolved_image}")
        design.info("")

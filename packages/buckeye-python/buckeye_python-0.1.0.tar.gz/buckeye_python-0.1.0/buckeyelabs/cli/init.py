"""Initialize new Buckeye environments with minimal templates."""
...
name = "{name}"
version = "0.1.0"
description = "A minimal Buckeye environment"
...
from buckeyelabs.server.context import run_context_server
import asyncio
...
if __name__ == "__main__":
    asyncio.run(run_context_server(Environment(), sock_path="/tmp/buckeye_ctx.sock"))
'''

SERVER_TEMPLATE = '''"""Minimal MCP server for Buckeye."""
import sys
import logging
from buckeyelabs.server import MCPServer
from buckeyelabs.server.context import attach_context
from buckeyelabs.tools.types import EvaluationResult
...
async def init(ctx):
    global env
    env = attach_context("/tmp/buckeye_ctx.sock")
...
Make sure to run 'buckeye dev --build' in another terminal first, and install buckeye-python[agents]
"""
...
async def run_task(task_data: dict):
    ...
    try:
        ...
        if "connection" in str(e).lower():
            print("❌ Could not connect. Make sure 'buckeye dev --build' is running in another terminal.")
        else:
            raise e
    finally:
...
NOTEBOOK_TEMPLATE = """{{
 "cells": [
  {{
   ...
   "metadata": {{}},
   "source": [
    "Make sure to `pip install buckeye-python[agents]` before running this notebook\\n",
    "\\n",
  ...
  {{
   ...
   "source": [
    ...
    "\\n",
    "Run `buckeye dev --build` before this cell to intialize the server at `http://localhost:8765/mcp`"
   ]
  }},
  {{
   "cell_type": "code",
  ...
  {{
   ...
   "source": [
    "### Step 6: Cleanup\\n",
    "\\n",
    "Always shut down the client when done to stop the Docker container. Either stop buckeye dev in the terminal, or run this command:"
   ]
  }},
  {{
   "cell_type": "code",
...
ENV_FILE_TEMPLATE = """# Buckeye API Configuration
# Get your API key from https://app.buckeye.so/account
BUCKEYE_API_KEY=your_buckeye_api_key_here
...
def create_environment(name: str | None, directory: str, force: bool) -> None:
    """Create a new Buckeye environment from templates."""
    ...
    # Success message
    design.header(f"Created Buckeye Environment: {name}")
    ...
    if target_dir == Path.cwd():
        ...
        design.command_example("buckeye dev --inspector")
    else:
    ...
    design.info("   Follow the instructions shown by buckeye dev --inspector")

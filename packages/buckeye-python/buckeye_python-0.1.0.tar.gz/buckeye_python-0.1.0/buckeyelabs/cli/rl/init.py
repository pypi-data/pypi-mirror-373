    ...
    if is_directory:
        ...
        if not lock_path.exists():
            if build:
                ...
                from buckeyelabs.cli.build import build_command
                ...
                from buckeyelabs.cli.utils.environment import get_image_name, image_exists

                image, source = get_image_name(directory)

                if not (source == "cache" and image_exists(image)):
                    design.warning(f"No buckeye.lock.yaml found in {directory}")
    ...
    if is_directory:
        ...
        if lock_path.exists():
            ...
            if not image:
                ...
                design.hint("Run 'buckeye build' to create a proper lock file")
                raise typer.Exit(1)

            if not tools:
                design.error("No tools found in lock file")
                design.hint("Lock file may be outdated. Run 'buckeye build' to regenerate")
                raise typer.Exit(1)
...
async def generate_from_lock(
    ...
    design.info("Source: buckeye.lock.yaml")
    ...
    design.command_example("buckeye hf tasks.json --name my-tasks", "Create dataset")
    design.command_example(f"buckeye rl --config {output}", "Start training")
...
async def generate_config(image: str, tools: list[dict[str, Any]]) -> dict[str, Any]:
    """Generate buckeye-vf-gym configuration from tool analysis."""
    ...
    system_prompt = f"""You are an AI agent in a Buckeye labs environment.
...
Think step by step about what you need to do."""
    ...
    # Build full config
    config = {
        "# Generated by buckeye rl init": f"for {env_name}",
        "job": {
            "name": f"RL Training - {env_name}",
            "metadata": {
                "environment": display_name,
                "full_image": image,
                "generated_by": "buckeye rl init",
            },
        },

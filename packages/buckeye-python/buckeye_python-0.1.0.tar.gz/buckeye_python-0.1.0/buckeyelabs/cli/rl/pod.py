    ...
    try:
        ...
        if ssh_info:
            ...
            if ssh_key_configured:
                ...
                design.command_example("pip install verifiers buckeye-vf-gym", "")
                ...
                train_cmd = f"""vf-train buckeye-vf-gym \\
  --model {model} \\
  --env-args '{{"taskset": "{dataset}", "config_path": "/root/config.yaml"}}' \\
  --output-dir {output_dir} \\
  --run-name hbuckeye-rl-{pod_id[:8]} \\
  --wandb-project buckeye-rl"""
...
async def run_prime_training(
    ...
    pod_name = f"buckeye-rl-{model_suffix}-{short_id}"[:30]  # Keep it short

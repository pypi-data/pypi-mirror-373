"""HuggingFace dataset conversion command for Buckeye tasks."""
...
from buckeyelabs.cli.rl.utils import get_mcp_config_from_lock, read_lock_file, write_lock_file
...
def hf_command(
    ...
    Converts a JSON file containing Buckeye tasks into a HuggingFace dataset
    and optionally pushes it to the Hub. Also updates buckeye.lock.yaml with
    ...
    Examples:
        ...
        buckeye hf tasks.json --name local-dataset --no-push
    """
    ...
    if name is None:
        ...
        lock_path = Path("buckeye.lock.yaml")
        if lock_path.exists():
            try:
    ...
    for i, task in enumerate(tasks):
        ...
        if "mcp_config" not in task:
            # Try to infer from buckeye.lock.yaml
    ...
    # Check all server configs (could be named anything, not just "buckeye")
    for server_config in sample_mcp_config.values():
        if isinstance(server_config, dict) and "url" in server_config:
            ...
            if "localhost:8765" in url:  # mcp.buckeye.so
                ...
                # Any non-mcp.buckeye.so URL means local config localhost:8765
                config_type = "local"
    ...
    if config_type == "local":
        ...
        if convert_to_remote == "Yes, convert to remote (requires public image)":
            ...
            from buckeyelabs.cli.rl.utils import get_image_from_lock

            image = get_image_from_lock()

            if not image:
                design.error("No image found in buckeye.lock.yaml")
                design.hint("Run 'buckeye build' first")
                raise typer.Exit(1)
            ...
            if "/" not in image or image.startswith("local/"):
                ...
                if push_image == "Yes, push image":
                    design.info("Running 'buckeye push' to publish image...")
                    # Import here to avoid circular imports
                    from buckeyelabs.cli.push import push_command
...
def update_lock_file(dataset_name: str, task_count: int) -> None:
    ...
    if write_lock_file(lock_data):
        design.success(f"Updated buckeye.lock.yaml with dataset: {dataset_name}")

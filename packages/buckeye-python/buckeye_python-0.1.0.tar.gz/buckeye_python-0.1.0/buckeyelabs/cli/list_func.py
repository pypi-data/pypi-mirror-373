"""List Buckeye environments from local registry."""
...
def list_environments(
    ...
    """List all Buckeye environments in the local registry."""
    design = BUCKDesign()

    if not json_output:
        design.header("Buckeye Environment Registry")
    ...
    if not env_dir.exists():
        if json_output:
            ...
            design.info("")
            design.info("Pull environments with: [cyan]buckeye pull <org/name:tag>[/cyan]")
            design.info("Build environments with: [cyan]buckeye build[/cyan]")
        return
    ...
    if environments:
        ...
        design.info(f"Run an environment: [cyan]buckeye run {example_ref}[/cyan]")
        design.info(f"Analyze tools: [cyan]buckeye analyze {example_ref}[/cyan]")
        design.info(f"Debug server: [cyan]buckeye debug {example_ref}[/cyan]")

    design.info("Pull more environments: [cyan]buckeye pull <org/name:tag>[/cyan]")
    design.info("Build new environments: [cyan]buckeye build[/cyan]")
...
def list_command(
    ...
    Shows environments pulled with 'buckeye pull' or built with 'buckeye build',
    stored in ~/.buckeye/envs/

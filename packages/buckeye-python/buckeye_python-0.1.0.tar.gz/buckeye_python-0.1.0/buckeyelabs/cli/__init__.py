"""Buckeye CLI - Command-line interface for MCP environment analysis and debugging."""
...
app = typer.Typer(
    name="buckeye",
    help="🚀 Buckeye CLI for MCP environment analysis and debugging",
...
def analyze(
    params: list[str] = typer.Argument(  # type: ignore[arg-type]  # noqa: B008
        ...
        help="Docker image followed by optional Docker run arguments (e.g., 'buckeye-image:latest -e KEY=value')",  # noqa: E501
    ),
    ...
    Examples:
        buckeye analyze buckeyepython/test_init      # Fast metadata inspection
        buckeye analyze my-env --live            # Full container analysis
        buckeye analyze --config mcp-config.json # From MCP config
        buckeye analyze --cursor text-2048-dev   # From Cursor config
    """
    if config:
        ...
        console.print("\nExamples:")
        console.print("  buckeye analyze buckeyepython/test_init       # Fast metadata analysis")
        console.print("  buckeye analyze my-env --live             # Live container analysis")
        console.print("  buckeye analyze --config mcp-config.json  # From config file")
        console.print("  buckeye analyze --cursor my-server        # From Cursor")
        raise typer.Exit(1)
...
def debug(
    ...
    Examples:
        buckeye debug .                              # Debug current directory
        buckeye debug environments/browser           # Debug specific directory
        buckeye debug . --build                      # Build then debug
        buckeye debug buckeye-text-2048:latest          # Debug Docker image
        buckeye debug my-mcp-server:v1 -e API_KEY=xxx
        buckeye debug --config mcp-config.json
        buckeye debug --cursor text-2048-dev
        buckeye debug . --max-phase 3               # Stop after phase 3
    """
    ...
    if config:
        ...
        if Path(first_param).exists() and is_environment_directory(first_param):
            # Directory mode - like buckeye dev
        ...
        console.print("\nExamples:")
        console.print("  buckeye debug .                      # Debug current directory")
        console.print("  buckeye debug environments/browser   # Debug specific directory")
        console.print("  buckeye debug buckeye-text-2048:latest  # Debug Docker image")
        console.print("  buckeye debug --config mcp-config.json")
        console.print("  buckeye debug --cursor my-server")
        raise typer.Exit(1)
...
def cursor_list() -> None:
    ...
    console.print(
        "\n[green]Tip:[/green] Use [cyan]buckeye debug --cursor <server-name>[/cyan] to debug a server"
    )


@app.command()
def version() -> None:
    """Show Buckeye CLI version."""
    try:
        from buckeyelabs import __version__

        console.print(f"Buckeye CLI version: [cyan]{__version__}[/cyan]")
    except ImportError:
        console.print("Buckeye CLI version: [cyan]unknown[/cyan]")
...
def dev(
    ...
    Examples:
        buckeye dev                      # Auto-detect in current directory
        buckeye dev environments/browser # Specific directory
        buckeye dev . --build            # Build image first
        buckeye dev . --image custom:tag # Use specific image
        buckeye dev . --no-cache         # Force clean rebuild
        buckeye dev . --verbose          # Show detailed logs
        buckeye dev . --transport stdio  # Use stdio proxy for multiple connections
        buckeye dev . --inspector        # Launch MCP Inspector (HTTP mode only)
        buckeye dev . --interactive      # Launch interactive testing mode (HTTP mode only)
        buckeye dev . --no-logs          # Disable Docker log streaming
        buckeye dev . --full-reload      # Restart entire container on file changes (instead of just server)

        # With Docker arguments (after all options):
        buckeye dev . -e BROWSER_PROVIDER=anchorbrowser -e ANCHOR_API_KEY=xxx
        buckeye dev . -e API_KEY=secret -v /tmp/data:/data --network host
        buckeye dev . --build -e DEBUG=true --memory 2g
    """  # noqa: E501
...
def run(
    params: list[str] = typer.Argument(  # type: ignore[arg-type]  # noqa: B008
        None,
        help="Docker image followed by optional arguments (e.g., 'buckeye-image:latest -e KEY=value')",
    ),
    local: bool = typer.Option(
        False,
        "--local",
        help="Run locally with Docker (default: remote via http://localhost:8765 or mcp.hub.so)",
    ),
    remote: bool = typer.Option(
        False,
        "--remote",
        help="Run remotely via http://localhost:8765 or mcp.hub.so (default)",
    ),
    transport: str = typer.Option(
        "stdio",
        "--transport",
    ...
    url: str = typer.Option(
        None,
        "--url",
        help="Remote MCP server URL (default: Buckeye_MCP_URL or http://localhost:8765 or mcp.hub.so)",
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="API key for remote server (default: Buckeye_API_KEY env var)",
    ),
    ...
    By default, runs remotely via http://localhost:8765 or mcp.buckeye.so. Use --local for Docker.

    Remote Examples:
        buckeye run buckeye-text-2048:latest
        buckeye run my-server:v1 -e API_KEY=xxx -h Run-Id:abc123
        buckeye run my-server:v1 --transport http --port 9000

    Local Examples:
        buckeye run --local buckeye-text-2048:latest
        buckeye run --local my-server:v1 -e API_KEY=xxx
        buckeye run --local my-server:v1 --transport http

    Interactive Testing (local only):
        buckeye run --local --interactive --transport http buckeye-text-2048:latest
        buckeye run --local --interactive --transport http --port 9000 my-server:v1
    """
    if not params:
...
def clone(
    ...
    Examples:
        buckeye clone https://github.com/user/repo.git
    """
    # Run the clone
...
def build(
    ...
    """🏗️ Build a Buckeye environment and generate lock file.

    This command:
    - Builds a Docker image from your environment
    - Analyzes the MCP server to extract metadata
    - Generates a buckeye.lock.yaml file for reproducibility

    Examples:
        buckeye build                    # Build current directory
        buckeye build environments/text_2048 -e API_KEY=secret
        buckeye build . --tag my-env:v1.0 -e VAR1=value1 -e VAR2=value2
        buckeye build . --no-cache       # Force rebuild
    """
...
@app.command()
def push(
    directory: str = typer.Argument(".", help="Environment directory containing buckeye.lock.yaml"),
    ...
    """📤 Push Buckeye environment to registry.

    Reads buckeye.lock.yaml from the directory and pushes to registry.
    Auto-detects your Docker username if --image not specified.

    Examples:
        buckeye push                     # Push with auto-detected name
        buckeye push --tag v1.0          # Push with specific tag
        buckeye push . --image myuser/myenv:v1.0
        buckeye push --yes               # Skip confirmation
    """
...
def pull(
    ...
    """📥 Pull Buckeye environment from registry with metadata preview.

    Shows environment details before downloading.

    Examples:
        buckeye pull buckeye.lock.yaml               # Pull from lock file
        buckeye pull myuser/myenv:latest        # Pull by image reference
        buckeye pull myuser/myenv --verify-only # Check metadata only
    """
...
def list_environments(
    ...
    """📋 List all Buckeye environments in local registry.

    Shows environments pulled with 'buckeye pull' stored in ~/.buckeye/envs/

    Examples:
        buckeye list                    # List all environments
        buckeye list --filter text      # Filter by name
        buckeye list --json            # Output as JSON
        buckeye list --all             # Show digest column
        buckeye list --verbose         # Show full descriptions
    """
...
def remove(
    ...
    """🗑️ Remove Buckeye environments from local registry.

    Removes environment metadata from ~/.buckeye/envs/
    Note: This does not remove the Docker images.

    Examples:
        buckeye remove abc123              # Remove by digest
        buckeye remove text_2048           # Remove by name
        buckeye remove buckeyepython/test_init # Remove by full name
        buckeye remove all                 # Remove all environments
        buckeye remove all --yes           # Remove all without confirmation
    """
...
def init(
    ...
    """🚀 Initialize a new Buckeye environment with minimal boilerplate.
    ...
    Examples:
        buckeye init                    # Use current directory name
        buckeye init my-env             # Create in ./my-env/
        buckeye init my-env --dir /tmp  # Create in /tmp/my-env/
    """
...
def eval(
    ...
    if source is None:
        ...
        if not json_files:
            ...
            design.info(
                "Usage: buckeye eval <source> or create a task JSON file "
    ...
    try:
        ...
        design.error(
            ...
            "Please install with: pip install 'buckeye-python[agent]'"
        )
...
def hf(
    ...
    update_lock: bool = typer.Option(
        True, "--update-lock/--no-update-lock", help="Update buckeye.lock.yaml"
    ),
    ...
    Examples:
        buckeye hf                      # Auto-detect tasks and suggest name
        buckeye hf tasks.json           # Use specific file, suggest name
        buckeye hf --name my-org/my-tasks  # Auto-detect tasks, use name
        buckeye hf tasks.json --name hud-evals/web-tasks --private
    """
...
def main() -> None:
    ...
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] in ["--help", "-h"]):
        ...
        console.print("  5. Get shared environments: [cyan]buckeye pull <org/name:tag>[/cyan]")

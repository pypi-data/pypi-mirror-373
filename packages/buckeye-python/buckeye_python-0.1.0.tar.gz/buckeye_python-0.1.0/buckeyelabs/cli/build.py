"""Build Buckeye environments and generate lock files."""
...
from buckeyelabs.version import __version__ as buckeye_version
...
def build_environment(
    ...
    """Build a Buckeye environment and generate lock file."""
    design = BUCKDesign()
    env_vars = env_vars or {}
    design.header("Buckeye Environment Build")
    ...
    try:
        ...
        default_image = pyproject.get("tool", {}).get("buckeye", {}).get("image", None)
    ...
    temp_tag = f"buckeye-build-temp:{int(time.time())}"
    ...
    if not analysis["success"]:
        ...
        design.command_example("buckeye debug . --build")
        ...
        design.command_example("buckeye build . --verbose")
    ...
    lock_content = {
        ...
        "build": {
            ...
            "buckeyeVersion": buckeye_version,
    ...
    design.success("Created lock file: buckeye.lock.yaml")
    ...
    label_cmd = [
        ...
        "build",
        "--label",
        f"org.buckeye.manifest.head={lock_hash}:{lock_size}",
        "--label",
        f"org.buckeye.version={new_version}",
        "-t",
        tag,
        "-t",
        version_tag,
    ...
    design.status_item("Lock file", "buckeye.lock.yaml")
    ...
    design.command_example("buckeye dev", "Hot-reload development")
    design.command_example(f"buckeye run {tag}", "Run the built image")
    design.info("")
    design.info("Publish to registry:")
    design.command_example("buckeye push", f"Push as {version_tag}")
    design.command_example("buckeye push --tag latest", "Push with custom tag")
    design.info("")

"""Central configuration for OpenTelemetry inside Buckeye SDK.

This file is responsible for
1. creating the global ``TracerProvider``
2. attaching span processors (Buckeye enrichment, batch + exporter)
...
from .exporters import BuckeyeSpanExporter
from .instrumentation import install_mcp_instrumentation
from .processors import BuckeyeEnrichmentProcessor
...
def configure_telemetry(
    *,
    ...
    res_attrs: dict[str, Any] = {
        ...
        "telemetry.sdk.name": "buckeye-otel",
    ...
    # ------------------------------------------------------------------
    provider.add_span_processor(BuckeyeEnrichmentProcessor())

    # Buckeye exporter (only if enabled and API key is available)
    if settings.telemetry_enabled and settings.api_key:
        exporter = BuckeyeSpanExporter(
            telemetry_url=settings.buckeye_telemetry_url, api_key=settings.api_key
        )
        ...
        raise ValueError(
            ...
            "1. Set BUCKEYE_API_KEY environment variable for Buckeye telemetry\n"
            "2. Use enable_otlp=True with configure_telemetry() for alternative backends (e.g., Jaeger)\n"  # noqa: E501
        )
    elif not settings.telemetry_enabled:
        logger.info("Buckeye telemetry disabled via BUCKEYE_TELEMETRY_ENABLED=false")
    ...
    if enable_otlp:
        try:
            ...
            logger.warning(
                ...
                "Install with: pip install 'buckeye-python[agent]'"
            )
    ...
    # Agent instrumentation now handled by @buckeye.instrument decorators

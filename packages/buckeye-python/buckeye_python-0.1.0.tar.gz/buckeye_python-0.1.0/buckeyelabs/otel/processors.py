...
class BuckeyeEnrichmentProcessor(SpanProcessor):
    """Span processor that enriches every span with Buckeye-specific context.

    • Adds ``buckeye.task_run_id`` attribute if available.
    • Adds ``buckeye.job_id`` attribute if available in baggage.
    • Adds ``buckeye.step_count`` attribute if available in baggage.
    """
    ...
    def on_start(self, span: Span, parent_context: Any) -> None:  # type: ignore[override]
        try:
            ...
            run_id = baggage.get_baggage("buckeye.task_run_id", context=parent_context)
            if run_id and span.is_recording():
                span.set_attribute("buckeye.task_run_id", str(run_id))

            # Get job_id from baggage if available
            job_id = baggage.get_baggage("buckeye.job_id", context=parent_context)
            if job_id and span.is_recording():
                span.set_attribute("buckeye.job_id", str(job_id))
            ...
            if span.is_recording():
                ...
                if step_type == "agent":
                    ...
                    span.set_attribute("buckeye.agent_steps", new_agent_count)
                    ...
                    span.set_attribute("buckeye.base_mcp_steps", new_base_count)
                    ...
                    span.set_attribute("buckeye.base_mcp_steps", new_base_count)
                    span.set_attribute("buckeye.mcp_tool_steps", new_tool_count)
                    logger.debug(
                        "Incremented MCP steps to base=%d, tool=%d", new_base_count, new_tool_count
                    )

                # Always set all current step counts on the span
                span.set_attribute("buckeye.base_mcp_steps", get_base_mcp_steps())
                span.set_attribute("buckeye.mcp_tool_steps", get_mcp_tool_steps())
                span.set_attribute("buckeye.agent_steps", get_agent_steps())

        except Exception as exc:  # defensive; never fail the tracer
            logger.debug("BuckeyeEnrichmentProcessor.on_start error: %s", exc, exc_info=False)

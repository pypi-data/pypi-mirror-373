...
from buckeyelabs.tools.computer.settings import computer_settings
from buckeyelabs.tools.types import ContentResult
...
class OpenAIComputerTool(BuckEyeComputerTool):
    """
    ...
    async def __call__(
        ...
        This converts OpenAI's action format (based on OperatorAdapter) to BuckEyeComputerTool's format.
        ...
        if type == "screenshot":
            ...
            # For custom actions, we just return an error since BuckEyeComputerTool doesn't support them
            raise McpError(

"""Push Buckeye environments to registry."""
...
def push_environment(
    ...
    design = BUCKDesign()
    design.header("Buckeye Environment Push")

    # Find buckeye.lock.yaml in specified directory
    env_dir = Path(directory)
    lock_path = env_dir / "buckeye.lock.yaml"

    if not lock_path.exists():
        design.error(f"No buckeye.lock.yaml found in {directory}")
        design.info("Run 'buckeye build' first to generate a lock file")
        raise typer.Exit(1)

    # Check for API key first
    if not settings.api_key:
        design.error("No BUCKEYE API key found")
        design.warning("A BUCKEYE API key is required to push environments.")
        design.info("\nTo get started:")
        design.info("1. Get your API key at: https://app.buckeye.so/settings")
        design.command_example("export BUCKEYE_API_KEY=your-key-here", "Set your API key")
        design.command_example("buckeye push", "Try again")
        design.info("")
        raise typer.Exit(1)
    ...
    if not image_to_push:
        try:
            ...
            design.info("Run 'buckeye build' first to create the image")
            raise typer.Exit(1)  # noqa: B904

    # Check if local image has the expected label
    labels = get_docker_image_labels(image_to_push)
    expected_label = labels.get("org.buckeye.manifest.head", "")
    version_label = labels.get("org.buckeye.version", "")
    ...
    # Upload lock file to Buckeye registry
    try:
        # Extract org/name:tag from the pushed image
        # e.g., "docker.io/buckeye/test_init:latest@sha256:..." -> "buckeye/test_init:latest"
        # e.g., "buckeye/test_init:v1.0" -> "buckeye/test_init:v1.0"
        # Use the original image name for the registry path, not the digest
        # The digest might not contain the tag information
        registry_image = (
            image  # This is the image we tagged and pushed (e.g., buckeye/buckeye-text-2048:0.1.2)
        )

        # Remove any registry prefix for the Buckeye registry path
        ...
        # For Buckeye registry, we need org/name format
        if "/" not in name_with_tag:
            design.warning("Image name must include organization/namespace for Buckeye registry")
            design.info(f"Current format: {name_with_tag}")
            design.info("Expected format: org/name:tag (e.g., buckeye/myenv:v1.0)")
            design.info("\nYour Docker push succeeded - share buckeye.lock.yaml manually")
            raise typer.Exit(0)

        # Upload to Buckeye registry
        design.progress_message("Uploading metadata to Buckeye registry...")
        ...
        registry_url = f"{settings.buckeye_telemetry_url.rstrip('/')}/registry/envs/{url_safe_path}"
        ...
        if response.status_code in [200, 201]:
            design.success("Metadata uploaded to Buckeye registry")
            design.info("Others can now pull with:")
            design.command_example(f"buckeye pull {name_with_tag}")
            design.info("")
        elif response.status_code == 401:
            design.error("Authentication failed")
            design.info("Check your BUCKEYE_API_KEY is valid")
            design.info("Get a new key at: https://app.buckeye.so/settings")
            ...
            design.info("Share buckeye.lock.yaml manually\n")
    except requests.exceptions.Timeout:
        design.warning("Registry upload timed out")
        design.info("The registry might be slow or unavailable")
        design.info("Your Docker push succeeded - share buckeye.lock.yaml manually")
    except requests.exceptions.ConnectionError:
        design.warning("Could not connect to Buckeye registry")

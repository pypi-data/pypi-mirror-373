...
The clone command will look for a [tool.buckeye.clone] section in the cloned repository's
...
def get_clone_message(clone_path: str) -> dict[str, Any] | None:
    """
    Look for a clone message configuration in the repository's pyproject.toml or .buckeye.toml.

    Checks for:
    1. [tool.buckeye.clone] section in pyproject.toml
    2. [clone] section in .buckeye.toml
    ...
    # Check .buckeye.toml as fallback
    buckeye_toml_path = repo_path / ".buckeye.toml"
    if buckeye_toml_path.exists():
        try:
            with open(buckeye_toml_path, "rb") as f:
                data = tomllib.load(f)
                if "clone" in data:
                    return data["clone"]
        except Exception:
            logger.warning("Failed to load clone config from %s", buckeye_toml_path)

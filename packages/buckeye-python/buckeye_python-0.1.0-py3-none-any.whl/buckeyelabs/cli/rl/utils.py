"""Shared utilities for RL commands using Buckeye environments."""
...
def read_lock_file() -> dict[str, Any]:
    """Read and parse buckeye.lock.yaml file."""
    lock_file = Path("buckeye.lock.yaml")
    ...
    try:
        ...
        design.warning(f"Could not read buckeye.lock.yaml: {e}")
        return {}


def write_lock_file(data: dict[str, Any]) -> bool:
    """Write data to buckeye.lock.yaml file."""
    lock_file = Path("buckeye.lock.yaml")
    ...
    try:
        ...
        design.warning(f"Could not write buckeye.lock.yaml: {e}")
        return False
...
def detect_image_name() -> str | None:
    ...
    if pyproject.exists():
        try:
            ...
            # Check for buckeye.image_name
            image = data.get("tool", {}).get("buckeye", {}).get("image_name")
            if image:
                return image

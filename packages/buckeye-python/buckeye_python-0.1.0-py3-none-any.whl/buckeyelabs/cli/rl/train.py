"""Main RL training command implementation using Buckeye environments."""
...
def train_command_wrapper(
    ...
    # Check BUCKEYE API key
    if not settings.api_key:
        missing_vars.append("BUCKEYE_API_KEY")
    else:
        design.success("✓ BUCKEYE_API_KEY configured")
        ...
        if "config" in missing:
            if missing["config"] == "multiple":
                ...
                if generate_config == "Yes, generate config":
                    design.info("Running 'buckeye rl init' to generate config...")
                    design.info("")
        ...
        if "dataset" in missing:
            ...
            if tasks_file.exists():
                ...
                if create_dataset == "Yes, upload to HuggingFace":
                    ...
                    design.info(f"Running 'buckeye hf tasks.json --name {dataset_name}'...")
                    design.info("")

                    # Run hf command
                    result = subprocess.run(  # noqa: S603
                        ["buckeye", "hf", "tasks.json", "--name", dataset_name],  # noqa: S607
                        capture_output=True,
                        text=True,
                    )
...
async def train_command(
    ...
    """Run RL training on Buckeye environments."""
    design.header("🤖 buckeye rl Training")

    # Get environment image
    image = detect_image_name()
    if not image:
        design.error("No environment image found")
        design.hint("Run 'buckeye build' first or specify with 'buckeye rl init <image>'")
        raise typer.Exit(1)
    ...
    if not config:
        ...
        design.hint("Run 'buckeye rl init' to generate a config file")
        raise typer.Exit(1)

    if not dataset:
        design.error("No dataset found")
        design.hint("Run 'buckeye hf tasks.json' to create a dataset")
        raise typer.Exit(1)

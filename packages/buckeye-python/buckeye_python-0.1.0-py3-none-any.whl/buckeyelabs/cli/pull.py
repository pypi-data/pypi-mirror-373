"""Pull Buckeye environments from registry."""
...
def fetch_lock_from_registry(reference: str) -> dict | None:
    """Fetch lock file from Buckeye registry."""
    try:
...
def pull_environment(
    ...
    """Pull Buckeye environment from registry."""
    design = BUCKDesign()
    design.header("Buckeye Environment Pull")
    ...
    # Mode 2: Direct image reference
        ...
        # First, try to parse as org/env reference for Buckeye registry
        # Check if it's a simple org/name or org/name:tag format (no @sha256)
        if "/" in target and "@" not in target:
            # Looks like org/env reference, possibly with tag
            design.info(f"Checking Buckeye registry for: {target}")

            # Check for API key (not required for pulling, but good to inform)
            if not settings.api_key:
                design.info("No BUCKEYE API key set (pulling from public registry)")

            lock_data = fetch_lock_from_registry(target)

            if lock_data:
                design.success("Found in Buckeye registry")
                image_ref = lock_data.get("image", "")
            else:
                # Fall back to treating as Docker image
                if not settings.api_key:
                    design.info(
                        "Not found in Buckeye registry (try setting BUCKEYE_API_KEY for private environments)"  # noqa: E501
                    )
                else:
                    design.info("Not found in Buckeye registry, treating as Docker image")
    ...
    design.command_example(f"buckeye analyze {simple_ref}")
    design.info("")
    design.info("2. Live analysis (runs container):")
    design.command_example(f"buckeye analyze {simple_ref} --live")
    design.info("")
    design.info("3. Run the environment:")
    design.command_example(f"buckeye run {simple_ref}")

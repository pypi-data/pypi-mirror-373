"""Buckeye evaluation command for running tasks and datasets."""
...
def build_agent(
    ...
    *,
    ...
    if agent_type == "openai":
        try:
            from buckeyelabs.agents import OperatorAgent
...
async def run_single_task(
    ...
    *,
    ...
    with buckeyelabs.trace(name=task_prompt):
        agent = build_agent(
            agent_type,
            model=model,
            allowed_tools=allowed_tools,
...
def eval_command(
    ...
    Examples:
        ...
        buckeye eval hud-evals/SheetBench-50

        # Evaluate the FULL SheetBench dataset with Claude (asyncio mode)
        buckeye eval hud-evals/SheetBench-50 --full --agent claude

        # Run large dataset with PARALLEL execution (auto-optimized)
        buckeye eval hud-evals/OSWorld-Verified-XLang --full --parallel

        # Parallel mode with manual configuration (16 workers, 25 tasks each)
        buckeye eval hud-evals/OSWorld-Verified-XLang --full --parallel --max-workers 16

        # Limit total concurrent tasks to prevent rate limits
        buckeye eval hud-evals/SheetBench-50 --full --parallel --max-concurrent 20

        # Run a single task from a JSON file
        buckeye eval task.json

        # Run multiple tasks from a JSON file with parallel execution
        buckeye eval tasks.json --full --parallel

        # Run with OpenAI Operator agent
        buckeye eval hud-evals/OSWorld-Gold-Beta --agent openai
    """
    ...
    # Check for BUCKEYE_API_KEY if using BUCKEYE services
    if not settings.api_key:
        design.warning("BUCKEYE_API_KEY not set. Some features may be limited.")
        design.info("Get your API key at: https://app.buckeye.so")

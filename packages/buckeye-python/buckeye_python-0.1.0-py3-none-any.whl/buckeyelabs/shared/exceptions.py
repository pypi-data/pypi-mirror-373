...
class BuckeyeException(Exception):
    """Base exception class for all Buckeye SDK errors.

    This is the parent class for all exceptions raised by the Buckeye SDK.
    Consumers should be able to catch this exception to handle any Buckeye-related error.
    """
...
class BuckeyeRequestError(Exception):
    """Any request to the Buckeye API can raise this exception."""
    ...
    def from_httpx_error(cls, error: httpx.HTTPStatusError, context: str = "") -> Self:
        ...
        logger.error(
            "HTTP error from Buckeye SDK: %s | URL: %s | Status: %s | Response: %s%s",
            message,
            response.url,
            status_code,
            response_text[:500],
...
class BuckeyeResponseError(BuckeyeException):
...
class BuckeyeAuthenticationError(BuckeyeException):
    """Raised when authentication with the Buckeye API fails.
...
class BuckeyeRateLimitError(BuckeyeException):
    """Raised when the rate limit for the Buckeye API is exceeded.
...
class BuckeyeTimeoutError(BuckeyeException):
    """Raised when a request to the Buckeye API times out.
...
class BuckeyeNetworkError(BuckeyeException):
...
class GymMakeException(BuckeyeException):

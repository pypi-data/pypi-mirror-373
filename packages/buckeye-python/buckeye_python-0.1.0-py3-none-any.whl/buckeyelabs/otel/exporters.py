"""Custom OpenTelemetry exporter that sends spans to the existing Buckeye telemetry
HTTP endpoint (/trace/<id>/telemetry-upload).

The exporter groups spans by ``buckeye.task_run_id`` baggage / attribute so we keep
exactly the same semantics the old async worker in ``buckeye.telemetry.exporter``
implemented.

This exporter is *synchronous* (derives from :class:`SpanExporter`).  We rely on
``buckeye.shared.make_request_sync`` which already contains retry & auth logic.
"""
...
from buckeyelabs.shared import make_request_sync
from buckeyelabs.types import TraceStep as BuckeyeSpanAttributes
...
class BuckeyeSpan(BaseModel):
    ...
    attributes: BuckeyeSpanAttributes
...
def extract_span_attributes(
    ...
    result_attrs = {
        "task_run_id": attrs.get(
            "buckeye.task_run_id"
        ),  # TraceStep expects task_run_id, not buckeye.task_run_id
        "job_id": attrs.get("buckeye.job_id"),  # TraceStep expects job_id, not buckeye.job_id
    ...
    logger.debug(
        """Final result_attrs before creating BuckeyeSpanAttributes:
        request=%s,
        result=%s""",
        result_attrs.get("request"),
        result_attrs.get("result"),
    )
    return BuckeyeSpanAttributes(**result_attrs)
...
class BuckeyeSpanExporter(SpanExporter):
    """Exporter that forwards spans to Buckeye backend using existing endpoint."""
    ...
    def export(self, spans: list[ReadableSpan]) -> SpanExportResult:  # type: ignore[override]
        ...
        # Group spans by buckeye.task_run_id attribute
        grouped: dict[str, list[ReadableSpan]] = defaultdict(list)
        for span in spans:
            run_id = span.attributes.get("buckeye.task_run_id") if span.attributes else None
            if not run_id:
                # Skip spans that are outside Buckeye traces
                continue
        ...
        for run_id, span_batch in grouped.items():
            try:
                ...
                for span in span_batch:
                    if span.attributes and "buckeye.step_count" in span.attributes:
                        current_step = span.attributes["buckeye.step_count"]
                ...
                logger.debug("Buckeye exporter sending %d spans to %s", len(span_batch), url)
                make_request_sync(
                    method="POST",
                    url=url,
                    json=payload,

"""Base protocol and implementation for Buckeye MCP clients."""
...
from buckeyelabs.types import MCPToolCall, MCPToolResult
from buckeyelabs.utils.mcp import setup_buckeye_telemetry
...
class BaseBUCKEYEClient(AgentMCPClient):
    """Base class with common Buckeye functionality that adds:
    - Connection management
    - Tool discovery
    - Telemetry fetching (buckeye environment-specific)
    - Logging
    ...
    async def initialize(self, mcp_config: dict[str, dict[str, Any]] | None = None) -> None:
        ...
        self._auto_trace_cm = setup_buckeye_telemetry(self._mcp_config, auto_trace=self._auto_trace)
        ...
        try:
            ...
            if "401" in error_msg or "Unauthorized" in error_msg:
                # Check if connecting to Buckeye API
                for server_config in self._mcp_config.values():
                    ...
                    if "localhost:8765" in url:  # TODO: change to buckeye.so
                        raise RuntimeError(
                            ...
                            "You can get an API key at http://localhost:8765"
                        ) from e
                raise RuntimeError(
        ...
        # Common buckeye behavior - fetch telemetry
    ...
    async def _fetch_telemetry(self) -> None:
        """Common telemetry fetching for all buckeye clients."""
        try:

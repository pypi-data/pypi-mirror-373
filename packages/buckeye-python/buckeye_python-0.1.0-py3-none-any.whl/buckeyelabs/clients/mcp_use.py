...
class MCPUseBUCKClient(BaseBUCKEYEClient):
    """MCP-use based implementation of Buckeye MCP client."""

    client_info = Implementation(
        name="buckeye-mcp-use", title="buckeye MCP-use Client", version=buckeye_version
    )
    ...
    def __init__(self, mcp_config: dict[str, dict[str, Any]] | None = None, **kwargs: Any) -> None:
        ...
        if MCPUseClient is None or MCPUseSession is None:
            raise ImportError(
                ...
                "Please install the optional agent dependencies: pip install 'buckeye-python[agent]'"
            )

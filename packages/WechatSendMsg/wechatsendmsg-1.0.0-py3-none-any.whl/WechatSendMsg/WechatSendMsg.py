...
class WeComMessageHandler:
    ...
    def _setup_logger(self) -> logging.Logger:
        """配置日志系统，日志文件名与调用脚本名称一致，确保目录存在"""
        # 获取调用者脚本的信息
        caller_frame = inspect.stack()[2]
        caller_filepath = caller_frame.filename  # 调用者脚本的路径
        caller_filename = os.path.basename(caller_filepath)  # 提取文件名（如"导入.py"）
        # 生成日志文件名（去除后缀，加上.log）
        ...
        if not os.path.exists(log_dir):
            try:
                os.makedirs(log_dir, exist_ok=True)  # 递归创建目录，已存在则不报错
            except PermissionError:
                # 权限不足时， fallback 到用户主目录
                log_dir = os.path.expanduser("~")  # 用户主目录（如 C:\Users\用户名）
                log_filepath = os.path.join(log_dir, log_filename)

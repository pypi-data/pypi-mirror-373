...
class BedrockRegionBalancer:
    ...
    async def _invoke_model_in_region(self, region: str, model_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def _invoke():
            ...
            try:
                # If it's a short name like "claude-3.7-sonnet", convert it
    ...
    async def invoke_model(self, model_id: str, body: Dict[str, Any], 
        ...
        try:
            ...
            # Invoke model in selected region
            logger.info(f"Invoking model '{model_id}' in region '{region}'")
            result = await self._invoke_model_in_region(region, model_id, body)
    ...
    async def converse_model(self, model_id: str, 
        ...
        try:
            ...
            # Converse with model in selected region
            logger.info(f"Conversing with model '{model_id}' in region '{region}'")
            result = await self._converse_in_region(
            ...
            logger.error(f"Error conversing with model: {str(e)}")
            raise

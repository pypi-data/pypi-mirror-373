...
- **Support for all Bedrock models** including Claude 3.7 Sonnet, Claude 3.5 Sonnet, and Opus 4
- **Short model name support** (e.g., `claude-3.7-sonnet` → full model ID)
...
async def main():
    ...
    async with BedrockRegionBalancer(
        ...
        # Using converse API (modern approach)
...
async def main():
    ...
    async with BedrockRegionBalancer(
        default_model="claude-3.7-sonnet"  # Optional: set default model
    ) as balancer:
        model_id = balancer.get_default_model()
        # Use balancer as shown above
...
balancer = BedrockRegionBalancer(
    ...
    default_model="claude-3.5-sonnet"  # Optional: set custom default model
)
...
Bedrock Region Balancer now supports both the traditional **invoke_model** API and the new **Converse API**. The Converse API provides a unified interface across different foundation models with enhanced features like multimodal content, tool use, and guardrail integration.
...
Both APIs are fully supported and you can use them interchangeably based on your needs.
...
- **default_model**: Default model to use (default: claude-3.7-sonnet)

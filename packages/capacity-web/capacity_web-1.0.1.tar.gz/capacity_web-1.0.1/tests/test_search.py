...
def test_search_with_nextchat_success():
    """测试成功搜索 - P0核心功能"""
    # 模拟成功响应
    mock_response = Mock()
    ...
    with patch('capacity_web._make_request', return_value=mock_response):
        result = search_with_nextchat("test query")
...
def test_search_with_nextchat_validation_error():
    """测试参数验证 - P1边界条件"""
    # 测试空查询
    ...
    # 测试None查询
    result = search_with_nextchat(None)
    assert result["success"] is False
    assert "查询参数不能为空" in result["message"]
    
    # 测试非字符串查询
    result = search_with_nextchat(123)
    assert result["success"] is False
    assert "查询参数必须是字符串" in result["message"]


def test_search_with_nextchat_max_results_validation():
    """测试max_results参数验证"""
    # 测试无效的max_results
    result = search_with_nextchat("test", max_results=0)
    assert result["success"] is False
    assert "max_results必须是1-100之间的整数" in result["message"]
    
    result = search_with_nextchat("test", max_results=101)
    assert result["success"] is False
    assert "max_results必须是1-100之间的整数" in result["message"]


def test_search_with_nextchat_network_error():
    """测试网络异常处理 - P2异常处理"""
...
def test_search_with_nextchat_timeout_error():
    """测试超时异常处理"""
...
def test_search_with_nextchat_http_error():
    """测试HTTP状态码错误处理"""
    mock_response = Mock()
...
def test_search_with_nextchat_max_results_limiting():
    """测试结果数量限制功能"""
    mock_response = Mock()
    mock_response.json.return_value = {
        "results": [
    ...
    with patch('capacity_web._make_request', return_value=mock_response):
        result = search_with_nextchat("test", max_results=5)
...
if __name__ == "__main__":
    ...
    tests = [
        ("核心功能测试", test_search_with_nextchat_success),
        ("参数验证测试", test_search_with_nextchat_validation_error),
        ("max_results验证", test_search_with_nextchat_max_results_validation),
        ("网络错误处理", test_search_with_nextchat_network_error),
        ("超时错误处理", test_search_with_nextchat_timeout_error),
        ("HTTP错误处理", test_search_with_nextchat_http_error),
        ("结果数量限制", test_search_with_nextchat_max_results_limiting),
    ]

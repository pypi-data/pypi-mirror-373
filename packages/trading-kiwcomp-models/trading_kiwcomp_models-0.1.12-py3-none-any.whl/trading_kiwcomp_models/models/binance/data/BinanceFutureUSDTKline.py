...
# ✅ Pydantic Model
class BinanceFutureUSDTKline(BaseModel):
    symbol: str
    interval: str
    open_time: datetime
    open: float
...
# ✅ ORM Table
class BinanceFutureUSDTKlineTable(BinanceKline):
    __tablename__ = "binance_future_usdt_kline"
...
# ✅ Mapper
class BinanceFutureUSDTKlineMapper:
    @staticmethod
    def from_raw(symbol: str, interval: str, raw: list) -> BinanceFutureUSDTKline:
        """
        Parse raw Binance Futures USDT kline array into a Pydantic model.
        """
        return BinanceFutureUSDTKline(
            symbol=symbol,
            interval=interval,
    ...
    @staticmethod
    def to_table(event: BinanceFutureUSDTKline) -> BinanceFutureUSDTKlineTable:
        """
        Convert Pydantic model to SQLAlchemy ORM row.
        """
        return BinanceFutureUSDTKlineTable(**event.model_dump())

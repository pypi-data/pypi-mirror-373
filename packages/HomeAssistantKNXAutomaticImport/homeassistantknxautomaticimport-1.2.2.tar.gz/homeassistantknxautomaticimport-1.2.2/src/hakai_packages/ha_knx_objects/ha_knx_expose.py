...
from hakai_packages.ha_knx_objects_common import (HAKNXDevice, KNXDeviceParameterType,
                                                  KNXDeviceParameterRtR)
from hakai_packages.ha_knx_objects_common import HAKNXValueType
from hakai_packages.knx_project_objects import KNXDPTType, KNXFunction
...
class HAKNXExpose(HAKNXDevice):
    ...
    def _get_param_for_rtr(self,
                           ...
                           knx_project_manager: KNXProjectManager) -> HAKNXDevice._Result:
        result = super()._get_param_for_rtr(param_name, config, function, knx_project_manager)
        if not result.found:
            return result
        value_type: HAKNXValueType = getattr(self, "type", None)
        if not value_type:
            logging.warning("No value type found in device %s", self.name)
            return HAKNXDevice._Result(False, result.data)
        if (value_type.dpt == KNXDPTType.constructor_from_ints(10, 1) or
                value_type.dpt == KNXDPTType.constructor_from_ints(11, 1) or
                value_type.dpt == KNXDPTType.constructor_from_ints(19, 1)):
            return HAKNXDevice._Result(False, result.data)
        return result

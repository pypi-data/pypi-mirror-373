...
def submit_job(run_cwl_file, run_cfg_file=None):
    ...
    # Build Job
    job = Job()
    job.setName("test_cwltool_job_singularity")
    #job.setExecutionEnv({"CWL_SINGULARITY_EXTRA_OPTIONS": "--contain --cleanenv"})
    ...
    arguments_str = run_cwl_file
    if run_cfg_file:
        arguments_str = arguments_str + " " + run_cfg_file

...
class ActionParser:
    ...
    def getActions(self, action: str) -> list:
        ...
        If the action is a string that represents a list
            ...
            If the action is a single action string
            (e.g., "action1(param1, param2)"), it will return a list with that single action.
            If the action is None, empty, or "none", it returns an empty list.
            Returns [] on any parsing error.
        """
        try:
            # hard stop for None/blank/“none”
            if action is None:
                return []
            if isinstance(action, str):
                ...
                # coerce non-str; if it becomes blank/"None" treat as empty
                s = str(action).strip()
                if not s or s.lower() == "none":
                    return []
    ...
    #             results.append(str(ex))
    #     return results
    def _coerceActions(self, actions):
        if actions is None:
            return {}

        if isinstance(actions, Mapping):
            return {k: v for k, v in dict(actions).items() if callable(v)}

        if isinstance(actions, str):
            fn = getattr(self, actions, None)
            return {actions: fn} if callable(fn) else {}

        # Sequence of (name, func) pairs → dict; any other plain sequence → {}
        if isinstance(actions, Sequence) and not isinstance(actions, (str, bytes)):
            try:
                if actions and all(isinstance(it, tuple) and len(it) == 2 and callable(it[1]) for it in actions):
                    return {k: v for k, v in actions}
                return {}
            except Exception:
                return {}

        # Try generic dict(...) coercion (custom dict-like)
        try:
            items = dict(actions)
            return {k: v for k, v in items.items() if callable(v)}
        except Exception:
            pass

        # NEW: refuse to harvest from modules/classes and builtin containers
        if inspect.ismodule(actions) or inspect.isclass(actions) or isinstance(actions, (list, tuple, set, dict)):
            return {}

        # Last resort: harvest public bound callables from non-builtin objects
        return {
            name: getattr(actions, name)
            for name in dir(actions)
            if not name.startswith("_") and callable(getattr(actions, name, None))
        }

    def executeAction(self, actions, action):
        """
        Execute a single action by name, with optional parameters.
        Returns the result as a string, or None if not found.
        """
        # normalize instead of erroring on type
        actions = self._coerceActions(actions)

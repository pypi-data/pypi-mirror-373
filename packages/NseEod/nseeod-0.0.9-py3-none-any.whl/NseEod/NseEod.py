...
class NseEoD:
    ...
    def fno_lot_size(self, trade_date: str):
        self.rotate_user_agent()
        raw_trade_date = trade_date  # keep original string
        ...
        try:
            # Convert date (expects dd-mm-YYYY)
            trade_date = datetime.strptime(trade_date, "%d-%m-%Y")
            url = f"https://nsearchives.nseindia.com/content/fo/fo_mktlots_{trade_date.strftime('%d%m%Y')}.csv"
            ...
            print(f"❌ Error fetching F&O Lot Size for {raw_trade_date}: {e}")
            return None

...
def apply_snapshot(store: dict, items: List[dict], env: Optional[str] = None, ingested_at: Optional[str] = None):
    ...
    for it in items or []:
        ...
        for ch in (it.get("chunks") or []):
            ...
            # ---- DO NOT CHANGE RAW-COPY BEHAVIOR ----
...
def _chunk_badge_and_links(ch: dict):
    ...
    st   = (ch.get("status") or "waiting").lower()
    proc = ch.get("proc")
    href = ch.get("log")        # visible anchor (view mode only)
...
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
                          ...
                          chunks_per_line: int,
                          sort_by: str) -> List[List[html.Td]]:   # <---- (NEW: sort_by)
    owner_sel = (owner or "").lower()
    mode_sel  = (mode  or "").lower()
    want_owner = None if owner_sel in ("", "all") else owner_sel
    want_mode  = None if mode_sel  in ("", "all") else mode_sel
    ...
    if sort_by == "score_desc":
        ...
        rows.sort(key=lambda t: (t[1], t[0], t[2]))             # worst → best
    elif sort_by == "status_desc":
        rows.sort(key=lambda t: (t[1], t[0], t[2]), reverse=True)  # best → worst
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        html.Div("Sort", className="text-muted small mt-2"),  # <---- (NEW)
        dcc.Dropdown(
            id="sort-by",
            options=[
                {"label":"Score ↑",         "value":"score_asc"},
                {"label":"Score ↓",         "value":"score_desc"},
                {"label":"Worst status ↑",  "value":"status_asc"},
                {"label":"Worst status ↓",  "value":"status_desc"},
                {"label":"Dataset A→Z",     "value":"name_asc"},
                {"label":"Dataset Z→A",     "value":"name_desc"},
            ],
            value="score_asc", clearable=False, style={"width":"180px"},
        ),
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=9070, debug=False)
...
- Score-based sorting with configurable weighting (Flat | Stage equal).

Top-right status shows:
  Environment: <env> | Last Ingested: <from tester> | Refreshed: <dashboard time>

Endpoints:
  POST /ingest_snapshot  -> replace all state; accepts either:
        - a JSON array, or
        - {"snapshot":[...], "meta":{...}}, or
        - {"snapshot":[...], "ingested_at":"...", "env":"..."}
...
# Stages (fixed) — archive shown before stage (your request)
...
# --------- Scoring (tunable) ----------
# Override via: STATUS_SCORES_JSON='{"succeeded":1,"waiting":0,...}'
STATUS_SCORES = {
    "manual": 2.0,
    "succeeded": 1.0,
    "running": 0.5,
    ...
    "overdue": -1.0,
    "failed": -1.0,
    "validation_failed": -0.5,  # if it ever appears
}
try:
    _env_scores = os.getenv("STATUS_SCORES_JSON")
    if _env_scores:
        STATUS_SCORES.update(json.loads(_env_scores))
except Exception:
    pass
...
def _init_store():
    # meta also holds last_ingest_at and env (filled by tester via payload.meta or top-level)
...
# ---------------- Apply snapshot (supports payload meta + top-level keys) ----------------
def apply_snapshot(store: dict, items: List[dict], meta: Optional[dict] = None):
    # merge meta & top-level hints
    meta = dict(meta or {})
    # honor popular top-level keys even when not inside meta
    # route will already have merged, but keep idempotent here
...
# ---------------- Scoring helpers ----------------
def _score_chunks(chunks: List[dict]) -> float:
    if not chunks:
        return 0.0
    s = 0.0
    for ch in chunks:
        st = (ch.get("status") or "waiting").lower()
        s += float(STATUS_SCORES.get(st, 0.0))
    return s / max(1, len(chunks))

def dataset_score(d_map: dict, sel_stages: List[str], scheme: str) -> float:
    """Return a score for a dataset across selected stages."""
    if scheme == "stage_equal":
        # Each stage contributes equally; empty stage contributes 0
        if not sel_stages:
            return 0.0
        per = []
        for stg in sel_stages:
            leaf = d_map.get(stg, {"chunks": []})
            per.append(_score_chunks(leaf.get("chunks", [])))
        return sum(per) / len(sel_stages)
    # Flat: every chunk across selected stages is equally weighted
    tot, cnt = 0.0, 0
    for stg in sel_stages:
        leaf = d_map.get(stg, {"chunks": []})
        for ch in leaf.get("chunks", []):
            st = (ch.get("status") or "waiting").lower()
            tot += float(STATUS_SCORES.get(st, 0.0))
            cnt += 1
    ...
    if LOG_LINK_MODE == "view" and href_view:
        ...
        if isinstance(raw, str) and (raw.startswith("http://") or raw.startswith("https://")):
            ...
            if copy_text:
                ...
                overlay_children = []
                if fallback_open:
                    key = _key_for_raw_path(copy_text)
                    overlay_children = [
...
# -------- table: collect & SORT datasets, then pack N groups per row (width = content) --------
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
                          stage_filter: List[str], status_filter: List[str],
                          chunks_per_line: int,
                          sort_by: str, sort_order: str, score_weight: str) -> List[List[html.Td]]:
    owner_sel = (owner or "").lower()
    mode_sel  = (mode  or "").lower()
    want_owner = None if owner_sel in ("", "all") else owner_sel
    want_mode  = None if mode_sel  in ("", "all") else mode_sel
    ...
    rows = []  # (key_for_sort, score, owner, mode, dn, d_map)
    ...
    for own in sorted(store.get("jobs", {}).keys()):
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                d_map = m_map[dn]
                # status filter: include the row only if ANY selected stage has a selected status
                if filter_by_status:
                    hit = False
                    for stg in sel_stages:
                        leaf = d_map.get(stg)
                        ...
                        for ch in (leaf.get("chunks") or []):
                            if (ch.get("status") or "").lower() in sel_status:
                                hit = True; break
                        if hit: break
                    if not hit:
                        continue
                # compute score regardless of sort mode (we may show later)
                s = dataset_score(d_map, sel_stages, score_weight)
                rows.append((dn.lower(), s, own, md, dn, d_map))

    # -------- sorting ----------
    sb = (sort_by or "dataset").lower()
    so = (sort_order or "descending").lower()
    rev = (so == "descending")
    if sb == "score":
        rows.sort(key=lambda r: (r[1], r[0]), reverse=rev)   # by score then dataset name
    else:
        rows.sort(key=lambda r: (r[0], r[1]), reverse=rev)   # by dataset (then score)

    # -------- build TD groups in that order ----------
    groups: List[List[html.Td]] = []
    for _, _, own, md, dn, d_map in rows:
        # never print owner (per your request)
        title = dn
...
def route_ingest_snapshot():
    try:
        ...
        if isinstance(body, list):
            ...
            # ---- HOTFIX: honor top-level keys ----
            if "ingested_at" in body and "last_ingest_at" not in meta:
                meta["ingested_at"] = body.get("ingested_at")
            if "last_ingest_at" in body:
                meta["last_ingest_at"] = body.get("last_ingest_at")
            if "env" in body and "env" not in meta:
                meta["env"] = body.get("env")
        else:
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        dcc.Dropdown(
            id="sort-by",
            options=[{"label":"Dataset","value":"dataset"},{"label":"Score","value":"score"}],
            value="dataset", clearable=False, style={"width":"150px"},
        ),
        html.Div("Order", className="text-muted small mt-2"),
        dcc.Dropdown(
            id="sort-order",
            options=[{"label":"Descending","value":"descending"},{"label":"Ascending","value":"ascending"}],
            value="descending", clearable=False, style={"width":"150px"},
        ),
        html.Div("Score weighting", className="text-muted small mt-2"),
        dcc.Dropdown(
            id="score-weight",
            options=[{"label":"Flat (all chunks equal)","value":"flat"},
                     {"label":"Stage equal","value":"stage_equal"}],
            value="flat", clearable=False, style={"width":"220px"},
        ),
        html.Div("Table groups per row", className="text-muted small mt-3"),
        dcc.Dropdown(
            id="table-groups",
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, chunks_per_line,
            sort_by, sort_order, score_weight, cur_interval):
    ...
    # Table (groups per row ; width = content) — with sorting
    groups = gather_dataset_groups(
        store, owner_sel, mode_sel,
        stage_filter or [], status_filter or [],
        chunks_per_line or 5,
        sort_by or "dataset", sort_order or "descending", score_weight or "flat"
    )

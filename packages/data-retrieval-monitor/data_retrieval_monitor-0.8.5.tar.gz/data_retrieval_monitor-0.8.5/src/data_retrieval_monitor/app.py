...
- Banner shows: Writing Environment + Last Ingested (from feeder).
...
def route_ingest_snapshot():
    try:
        ...
        if isinstance(body, dict):
            ...
            if items is None and "items" in body and isinstance(body["items"], list):
                items = body["items"]
            if items is None and "data" in body and isinstance(body["data"], list):
                items = body["data"]
            if items is None and "records" in body and isinstance(body["records"], list):
                items = body["records"]
            if items is None and "rows" in body and isinstance(body["rows"], list):
                items = body["rows"]
            env = body.get("env")
    ...
    dbc.CardBody([
        ...
        dcc.Dropdown(
            ...
            options=[
                ...
                {"label":"Status (worst→best)",     "value":"status_asc"},
                {"label":"Status (best→worst)",     "value":"status_desc"},
                {"label":"Dataset A→Z",             "value":"name_asc"},
                {"label":"Dataset Z→A",             "value":"name_desc"},
            ],
            value="score_asc", clearable=False, style={"width":"200px"},
        ),
...
# ---------------- Helper for banner ----------------
def _format_last_ingested(meta: dict) -> str:
    env = meta.get("env") or "Unknown"
    ts  = meta.get("ingested_at")
    if not ts:
        return f"Writing Environment: {env} · Last Ingested: (unknown)"
    s = str(ts).replace("Z", "+00:00")
    try:
        dt = datetime.fromisoformat(s)
    except Exception:
        return f"Writing Environment: {env} · Last Ingested: (invalid)"
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    local_dt = dt.astimezone(_DEF_TZ)
    return f"Writing Environment: {env} · Last Ingested: {local_dt.strftime('%Y-%m-%d %H:%M:%S %Z')}"
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, sort_by, groups_per_row, chunks_per_line, cur_interval):
    ...
    # Banner
    env_text = _format_last_ingested(store.get("meta", {}))

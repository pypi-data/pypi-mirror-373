...
Endpoints:
  ...
  GET  /logmem/<path:rel>            -> raw text (kept for debugging)
    ...
    for it in items or []:
        ...
        for idx, ch in enumerate(it.get("chunks", []) or []):
            ...
            if rel:
                ...
                ch["log"] = f"/logview/{quote(rel)}"   # HTML viewer page
            else:
                ch["log"] = raw_log  # keep http(s) or None
            chunks.append(ch)
...
def chunk_line(chunks: List[dict]):
    ...
    for idx, ch in enumerate(chunks):
        ...
        st   = (ch.get("status") or "waiting").lower()
        proc = ch.get("proc")
        log  = ch.get("log")  # already /logview/... for local logs
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> dbc.Table:
    ...
    # Key: let the table size to content; no table-level scrollbar, no stretching to full width
    return dbc.Table(
...
if __name__ == "__main__":
    LOG_ROOT.mkdir(parents=True, exist_ok=True)
    preload_logs()  # initial preload; also caches on-demand in /logview

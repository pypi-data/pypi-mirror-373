...
- Pie placeholders (renders even with no data)
- Repeatable 5-column groups in the table (Dataset, Stage, Archive, Enrich, Consolidate) packed N per row
- Local log viewer: GET /logs/<rel> reads text files from LOG_ROOT
"""
...
BASE_GROUP_MIN_PX = int(os.getenv("BASE_GROUP_MIN_PX", "560"))  # min px per 5-col group
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> dbc.Table:
    ...
    # Dynamic min width grows with groups per row; table still expands to container width
    ...
    return dbc.Table(
        ...
        style={
            ...
            "minWidth": f"{min_width_px}px" # scroll only if viewport narrower than this
        }
    )
...
def pie_figure(title_text: str, counts: Dict[str,int]):
    ...
    if total == 0:
        for s in JOB_STATUS_ORDER:
            ...
            values.append(1)               # equal wedges so a pie renders
            texttempl.append("%{label}")   # labels only
...
# ---------------- Callback (fixed outputs/inputs order) ----------------
@app.callback(
    # 16 outputs (keep order stable!)
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, cur_interval):
    # Defensive defaults to avoid None->indexing errors
    ...
    # Return EXACTLY 16 items in declared order
    return (

...
- Table sizes to content (no table-level horizontal scrollbar).
- In-memory log cache + safe on-disk fallback.
- Configurable raw/view log links (LOG_LINK_MODE).
- Log links show tiny "p" (proc) & "l" (log). In raw mode the "l" copies the file path.
- Configurable chunk wrapping (N chunks per visual line).
- Simple sorting: by Dataset or by Status (numeric status score).

Top-right status shows:
  Environment: <env> | Last Ingested: <from tester> | Refreshed: <dashboard time>

Endpoints:
  POST /ingest_snapshot  -> replace all state; accepts:
        - a JSON array, or
        - {"snapshot":[...], "meta":{...}}, or
        - {"snapshot":[...], "ingested_at":"...", "env":"..."}
  POST /feed             -> same as /ingest_snapshot
...
# Stages (fixed, with Archive before Stage as requested)
...
# ------------ Numeric Status Score (for sorting by "Status") ------------
STATUS_SCORES = {
    # You asked to “revert back to numerical scores”
    # Feel free to tweak these or override via env: STATUS_SCORES_JSON='{"succeeded":1,...}'
    "manual": 2.0,
    "succeeded": 1.0,
    "running": 0.5,
    "waiting": 0.0,
...
def score_chunks(chunks: List[dict]) -> float:
    if not chunks:
        return 0.0
    s = 0.0
    for ch in chunks:
...
def dataset_status_score(d_map: dict, sel_stages: List[str]) -> float:
    # Flat: every chunk across selected stages equal; if no chunks at all → 0
    tot, cnt = 0.0, 0
    for stg in sel_stages:
        leaf = d_map.get(stg, {"chunks": []})
        for ch in leaf.get("chunks", []):
            tot += float(STATUS_SCORES.get((ch.get("status") or "waiting").lower(), 0.0))
            cnt += 1
...
def _init_store():
    # meta holds last_ingest_at and env
...
# ---------------- Apply snapshot (supports meta + top-level env/ingested_at) ----------------
def apply_snapshot(store: dict, items: List[dict], meta: Optional[dict] = None):
    meta = dict(meta or {})
    store_meta = store.setdefault("meta", {})

    # accept top-level keys via meta as well (route adds them, but keep idempotent)
...
def _chunk_badge_and_links(ch: dict):
    ...
    if LOG_LINK_MODE == "view" and href_view:
        ...
        if isinstance(raw, str) and (raw.startswith("http://") or raw.startswith("https://")):
            ...
            if copy_text:
                # Default: use clipboard overlay. If you want a simple open-in-tab fallback,
                # set CLIPBOARD_FALLBACK_OPEN=1 in environment.
...
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
    ...
    rows = []  # (dataset_key, status_score, dn, d_map)
    ...
    for own in sorted(store.get("jobs", {}).keys()):
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                ...
                sc = dataset_status_score(d_map, sel_stages)
                rows.append((dn.lower(), sc, dn, d_map))
    ...
    rev = (so == "descending")
    if sb == "status":
        rows.sort(key=lambda r: (r[1], r[0]), reverse=rev)
    else:
        rows.sort(key=lambda r: (r[0], r[1]), reverse=rev)

    # build groups
    groups: List[List[html.Td]] = []
    for _, _score, dn, d_map in rows:
        title = dn  # never print owner
...
def route_ingest_snapshot():
    try:
        ...
        if isinstance(body, list):
            ...
            # honor top-level env/ingested_at too
            if "ingested_at" in body and "ingested_at" not in meta and "last_ingest_at" not in meta:
                meta["ingested_at"] = body.get("ingested_at")
            if "last_ingest_at" in body and "last_ingest_at" not in meta:
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        dcc.Dropdown(
            id="sort-by",
            options=[{"label":"Dataset","value":"dataset"},{"label":"Status","value":"status"}],
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, chunks_per_line,
            sort_by, sort_order, cur_interval):
    ...
    groups = gather_dataset_groups(
        ...
        sort_by or "dataset", sort_order or "descending"
    )

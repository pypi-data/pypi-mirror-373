"""
Data Retrieval Monitor — intrinsic-width table (no wrap in cells),
pie placeholders, repeatable 5-col groups, and local log viewer.

- Table width follows content (no full-width stretch). Each cell is single-line.
- As "Table groups per row" increases, total table width grows ~linearly.
- Horizontal scrolling appears only if content exceeds the right pane.
- Pies render placeholders when total=0.
- Local logs served from LOG_ROOT via /logs/<rel>.

Endpoints:
  POST /ingest_snapshot  (or /feed) -> replace all state
  POST /store/reset?seed=1          -> clear store (tiny seed optional)
  GET  /logs/<path:rel>             -> serve text logs from LOG_ROOT
...
LOG_ROOT      = os.getenv("LOG_ROOT", "/tmp/drm-logs")  # base dir for served logs

# Left pane sizing (right pane is free width)
...
def _log_href(val: Optional[str]) -> Optional[str]:
    """
    Make an href for a log:
      - http(s) passthrough
      - absolute server path: must be inside LOG_ROOT; mapped to /logs/<rel>
      - relative path: treated relative to LOG_ROOT; mapped to /logs/<rel>
    """
    if not val:
        return None
    v = str(val)
    ...
    try:
        if p.is_absolute():
            ...
            if not str(abs_p).startswith(str(root)):
                return None  # refuse paths outside root
            rel = abs_p.relative_to(root).as_posix()
        else:
            rel = p.as_posix().lstrip("./")
        if ".." in rel:
            return None
        return f"/logs/{quote(rel)}"
    except Exception:
        return None

def chunk_line(chunks: List[dict]):
    # Single-line chunk badges + links (no wrapping)
    if not chunks:
    ...
    for idx, ch in enumerate(chunks):
        ...
        nodes.append(html.Span("\u2003"))  # em space
    return html.Div(nodes, style={"whiteSpace":"nowrap"})

# -------- table builders (intrinsic width; no stretch) --------
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
    ...
    """Return a list of cell-groups; each group is [Dataset, Stage, Archive, Enrich, Consolidate] (as TDs)."""
    owner_sel = (owner or "").lower()
    mode_sel  = (mode  or "").lower()
    want_owner = None if owner_sel in ("", "all") else owner_sel
    want_mode  = None if mode_sel  in ("", "all") else mode_sel
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> html.Div:
    """
    Build a table whose width = intrinsic content width:
      - width:auto, tableLayout:auto, display:inline-table
      - no colgroup forcing % widths
      - wrapper div provides horizontal scroll if needed
    """
    gpr = max(1, min(int(groups_per_row or 1), 4))
    ...
    # body rows (pack gpr groups per row; pad last row)
    body_rows = []
    ...
    table = dbc.Table(
        ...
        style={
            "tableLayout": "auto",     # let content decide
            "width": "auto",           # don't stretch
            "display": "inline-table"  # intrinsic width so wrapper can size to content
        }
    )

    # wrapper with horizontal scroll if table wider than pane
    return html.Div(
        table,
        style={
            "overflowX": "auto",
            "maxWidth": "100%",     # cap to right pane width
            "paddingLeft": "8px"
        }
    )
...
def route_logs(rel: str):
    # Serve text files safely from LOG_ROOT
    root = pathlib.Path(LOG_ROOT).resolve()
...
# Header bar: Refreshed on the far right
header_bar = html.Div(
    [
...
two_col_nowrap = html.Div([
    ...
    # Right growing pane (title + intrinsic-width table)
    html.Div([
        html.Div([
...
], fluid=True, className="pt-3 pb-4", style={"maxWidth":"100%", "width":"100%", "margin":"0 auto"})
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, cur_interval):
    owner_sel = owner_sel or DEFAULT_OWNER.lower()
    mode_sel  = (mode_sel or DEFAULT_MODE).lower()
    stage_filter  = stage_filter  or STAGES
    status_filter = status_filter or []
    groups_per_row = groups_per_row or 1
    ...
    out_waiting   = str(k.get("waiting", 0))
    out_retrying  = str(k.get("retrying", 0))
    out_running   = str(k.get("running", 0))
    out_failed    = str(k.get("failed", 0))
    out_overdue   = str(k.get("overdue", 0))
    out_manual    = str(k.get("manual", 0))
    out_succeeded = str(k.get("succeeded", 0))

    # Dropdown options
    owner_opts, mode_opts = list_filters(store)

    # Pies (owner/mode filtered) — placeholder logic in pie_figure
    fig_stage        = pie_figure("Stage",        filtered_stage_counts(store, owner_sel, mode_sel, "stage"))
    fig_archive      = pie_figure("Archive",      filtered_stage_counts(store, owner_sel, mode_sel, "archive"))
    fig_enrich       = pie_figure("Enrich",       filtered_stage_counts(store, owner_sel, mode_sel, "enrich"))
    fig_consolidate  = pie_figure("Consolidate",  filtered_stage_counts(store, owner_sel, mode_sel, "consolidate"))

    # Table
    groups = gather_dataset_groups(store, owner_sel, mode_sel, stage_filter, status_filter)
    table_comp = build_table_component(groups, groups_per_row)

    now_local = datetime.now(_DEF_TZ).strftime("%Y-%m-%d %H:%M:%S %Z")
    return (
        out_waiting, out_retrying, out_running, out_failed, out_overdue, out_manual, out_succeeded,
        owner_opts, mode_opts,
        fig_stage, fig_archive, fig_enrich, fig_consolidate,
        table_comp, f"Refreshed: {now_local}", interval_ms
    )

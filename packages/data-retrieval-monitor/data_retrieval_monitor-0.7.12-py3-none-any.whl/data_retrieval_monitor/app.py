"""
Data Retrieval Monitor — side-by-side (no wrap), content-sized table (no table scroll),
pie placeholders, and in-memory log viewer.

Key points in this revision:
- NO horizontal scrollbar on the table or chunk cells (no overflowX on table/cells).
- Table width is content-based (inline-block) and never stretches to full browser width.
- Right pane uses flex: 0 0 auto so it grows just to fit the table; left pane is fixed.
- Logs are preloaded and served from memory at /logmem/<rel>.
- "All / All" shows everything. Pies show placeholders when empty.
- Callback IOs aligned.
...
Env:
  ...
  LOG_MAX_BYTES (default 200000)
  BASE_GROUP_MIN_PX (default 560)
...
# jobs[owner][mode][data_name][stage] = { chunks:[], counts:{}, errors:[] }
...
def _safe_rel(path_like: str) -> Optional[str]:
    ...
    try:
        p_abs = (LOG_ROOT / p).resolve() if not p.is_absolute() else pathlib.Path(path_like).resolve()
    except Exception:
        return None
    try:
...
def _read_file_safely(p: pathlib.Path) -> Optional[str]:
    try:
        with p.open("rb") as f:
            data = f.read(LOG_MAX_BYTES)
...
def cache_log_if_available(path_like: Optional[str]):
    rel = _safe_rel(path_like or "")
    if not rel:
        return
    fp = (LOG_ROOT / rel)
    if not fp.exists():
...
def apply_snapshot(store: dict, items: List[dict]):
    ...
    for it in items or []:
        ...
        chunks = []
        for idx, ch in enumerate(it.get("chunks", []) or []):
            ch = dict(ch or {})
            raw_log = ch.get("log")
            rel = _safe_rel(str(raw_log)) if raw_log else None
            if rel:
                cache_log_if_available(rel)
                ch["log"] = f"/logmem/{quote(rel)}"
            else:
                ch["log"] = raw_log
            chunks.append(ch)
        leaf["chunks"] = chunks
...
def list_filters(store: dict):
    ...
    owner_opts = [{"label": "All", "value": "All"}] + [
        {"label": labels.get(k, k), "value": k} for k in owners
    ]
    modes_keys = set()
...
def chunk_line(chunks: List[dict]):
    ...
    for idx, ch in enumerate(chunks):
        ...
        st   = (ch.get("status") or "waiting").lower()
        proc = ch.get("proc")
        log  = ch.get("log")  # already /logmem/<rel> if local
        ...
        if proc: nodes.append(html.A("proc", href=str(proc), target="_blank", style={"marginRight":"6px"}))
        if log:  nodes.append(html.A("log",  href=str(log),  target="_blank", style={"marginRight":"10px"}))
        nodes.append(html.Span(" ", style={"marginRight":"6px"}))
    # NOTE: no overflowX here → no horizontal scrollbar in the cell
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> html.Div:
    ...
    table = dbc.Table(
        ...
        style={
            ...
            # No width:100% → content-based width
        }
    )
    # inline-block wrapper => table never stretches to full browser width
...
def route_logmem(rel: str):
    ...
    if ".." in clean:
        return abort(400)
    with LOG_MEM_LOCK:
        if clean not in LOG_MEM:
            return Response(f"(log not preloaded: {clean})", mimetype="text/plain", status=404)
        txt = LOG_MEM[clean]
...
# Side-by-side (no wrap). Right pane is content-sized and never stretches.
two_col_nowrap = html.Div([
    ...
    # Right pane grows just to fit the table (no intrinsic max width here)
    html.Div([
        html.Div([
            html.H4("Datasets", className="fw-semibold",
                    style={"margin":"0","whiteSpace":"nowrap","paddingRight":"8px"}),
            # NO overflowX: auto here → no table scrollbar
            html.Div(id="table-container", style={"display":"inline-block"})
        ], style={"display":"flex","alignItems":"flex-start","gap":"8px"})
    ], style={"flex":"0 0 auto"})  # content-sized
], style={"display":"flex","flexWrap":"nowrap","alignItems":"flex-start","gap":"16px"})
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, cur_interval):
    ...
    # Table (content-based; no internal scrollbars)

...
# Stages (fixed) — archive shown before stage
...
_MEM_STORE: Optional[dict] = None
_STORE_CACHE: Optional[dict] = None
_STORE_MTIME: Optional[float] = None
...
def _init_store() -> dict:
    ...
    return {
        "jobs": {},
        "logs": [],
        "meta": {"owner_labels": {}, "last_ingest_at": None, "env": None},
            ...
            if copy_text:
                ...
                if fallback_open:
                    ...
                    overlay_children = [
                        html.Span(
                            "l",
                            title=f"Copy log path: {copy_text}",
                            style={
                                "display": "inline-block",
                                "fontSize": "12px",
...
def gather_dataset_groups(
    ...
    entries = []
    for own in store.get("jobs", {}).keys():
        if want_owner and own != want_owner:
            continue
        o_map = store["jobs"][own]
        for md in o_map.keys():
            if want_mode and md != want_mode:
                continue
            m_map = o_map[md]
            for dn in m_map.keys():
                d_map = m_map[dn]
                ...
                if chunk_scores:
                    ...
                    avg_chunk = 0.0
                if status_set:
                    avg_status = sum(JOB_SCORES.get(st, 0) for st in status_set) / len(status_set)
                else:
                    avg_status = 0.0
    ...
    for _, own, md, dn, d_map in entries:
        ...
        if want_owner is None:
            title_parts.append(owner_label)
        if want_mode is None:
            title_parts.append(md.title())
        title_parts.append(dn)
    ...
    dbc.CardBody([
        ...
        dcc.Dropdown(
            ...
            options=[
                ...
                {"label": "Chunk Avg Score (worst first)", "value": "chunk_asc"},
                {"label": "Chunk Avg Score (best first)", "value": "chunk_desc"},
                {"label": "Status Avg Score (worst first)", "value": "status_asc"},
                {"label": "Status Avg Score (best first)", "value": "status_desc"},
            ],
            value="name_asc",
            clearable=False,
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, chunks_per_line, sort_by, cur_interval):
    ...
    # Pies — return in fixed order matching outputs
    ...
    groups = gather_dataset_groups(
        store, owner_sel, mode_sel, stage_filter or [], status_filter or [], chunks_per_line or 5, sort_by
    )

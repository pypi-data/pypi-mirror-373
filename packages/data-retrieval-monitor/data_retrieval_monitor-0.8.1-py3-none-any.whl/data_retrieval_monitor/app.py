...
# Stages (fixed)
STAGES = ["archive", "stage", "enrich", "consolidate"]  # archive shown before stage
...
# ---------- Status numeric scores (for sorting by score)  # <<< PATCH
STATUS_SCORES = {
    "manual": 2.0,
    "succeeded": 1.0,
    "running": 0.5,
...
def dataset_status_score(d_map: dict, sel_stages: List[str]) -> float:
    # Average of chunk scores across selected stages (equal weighting).
    tot, cnt = 0.0, 0
...
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
                          ...
                          chunks_per_line: int,
                          sort_by: str = "status", sort_order: str = "ascending"):  # <<< PATCH defaults
    owner_sel = (owner or "").lower()
    mode_sel  = (mode  or "").lower()
    want_owner = None if owner_sel in ("", "all") else owner_sel
    want_mode  = None if mode_sel  in ("", "all") else mode_sel
    ...
    rows = []  # (dataset_key, score, dn, d_map)
    ...
    for own in sorted(store.get("jobs", {}).keys()):
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                ...
                score = dataset_status_score(d_map, sel_stages)
                rows.append((dn.lower(), score, dn, d_map))

    # sort
    sb = (sort_by or "status").lower()
    rev = (sort_order or "ascending").lower() == "descending"
    if sb == "dataset":
...
def route_ingest_snapshot():
    try:
        ...
        if isinstance(body, list):
            ...
            meta  = dict(body.get("meta", {}))  # <<< PATCH merge top-level keys
            # honor top-level ingested_at / last_ingest_at / env if present
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        # --- Sorting controls (tiny) ---  # <<< PATCH
        html.Div("Sort by", className="text-muted small mt-2"),
        dcc.Dropdown(
            id="sort-by",
            options=[{"label":"Status (score)","value":"status"},
                     {"label":"Dataset","value":"dataset"}],
            value="status", clearable=False, style={"width":"160px"},
        ),
        ...
        dcc.Dropdown(
            ...
            value="ascending", clearable=False, style={"width":"160px"},
        ),
        # -------------------------------

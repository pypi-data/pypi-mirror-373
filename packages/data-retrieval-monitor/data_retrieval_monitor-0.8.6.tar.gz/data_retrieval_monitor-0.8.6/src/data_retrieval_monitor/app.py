...
- In RAW mode, 'l' opens /rawpath with the raw path preselected for manual copy.
- Configurable chunk wrapping (N chunks per visual line in each stage cell).
- Banner shows: Environment + Last Ingested (from feeder) + Refreshed (dashboard time).
...
Endpoints:
  ...
  GET  /rawpath?p=...                -> minimal page to view/copy raw path
...
def apply_snapshot(store: dict, items: List[dict], env: Optional[str] = None, ingested_at: Optional[str] = None):
    ...
    for it in items or []:
        ...
        for ch in (it.get("chunks") or []):
            ...
            # View mode keeps link to viewer; Raw mode links to /rawpath for manual copy
            if LOG_LINK_MODE == "view" and ch["log_view"]:
                ch["log"] = ch["log_view"]
            else:
                ch["log"] = f"/rawpath?p={quote_plus(ch['log_raw'])}" if ch.get("log_raw") else None
...
def compute_worst_status_rank(d_map: dict, sel_stages: List[str]) -> int:
    ranks = []
    for stg in sel_stages:
        counts = (d_map.get(stg) or {}).get("counts", {})
        st = best_status(counts)
        if st is not None:
...
def _chunk_badge_and_links(ch: dict):
    ...
    st   = (ch.get("status") or "waiting").lower()
    proc = ch.get("proc")
    href = ch.get("log")        # visible anchor (view mode OR raw /rawpath)
    raw  = ch.get("log_raw")    # original string (absolute path or URL)
    ...
    if href:
        bits.append(html.A("l", href=href, target="_blank", title="log", style={"marginRight":"4px"}))

    # keep the old copy button (non-blocking)
...
def gather_dataset_groups(store: dict, owner: Optional[str], mode: Optional[str],
    ...
    rows: List[tuple] = []  # (score, worst_rank, name_lower, cells)
    ...
    for own in sorted(store.get("jobs", {}).keys()):
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                ...
                worst_rank = compute_worst_status_rank(d_map, sel_stages)
                rows.append((score, worst_rank, dn.lower(), cells))

    # Sorting
    sort_by = (sort_by or "score_asc").lower()
    if sort_by == "score_desc":
        rows.sort(key=lambda t: (t[0], t[1], t[2]), reverse=True)
    elif sort_by == "status_asc":         # worst → best (lowest rank first)
        rows.sort(key=lambda t: (t[1], t[0], t[2]))
    elif sort_by == "status_desc":        # best → worst
        rows.sort(key=lambda t: (t[1], t[0], t[2]), reverse=True)
    elif sort_by == "name_desc":
        rows.sort(key=lambda t: t[2], reverse=True)
    elif sort_by == "name_asc":
        rows.sort(key=lambda t: t[2])
    else:  # default score_asc
...
def route_ingest_snapshot():
    try:
        ...
        if isinstance(body, dict):
            # items under common keys
            items = body.get("snapshot")
            if items is None and isinstance(body.get("items"), list):
                items = body["items"]
            if items is None and isinstance(body.get("data"), list):
                items = body["data"]
            if items is None and isinstance(body.get("records"), list):
                items = body["records"]
            if items is None and isinstance(body.get("rows"), list):
                items = body["rows"]

            # env/ingested_at: accept top-level or meta (any common field name)
            meta = body.get("meta", {}) if isinstance(body.get("meta"), dict) else {}
            env = body.get("env") or meta.get("env")
            ingested_at = (
                body.get("ingested_at") or meta.get("ingested_at") or
                meta.get("last_ingest_at") or meta.get("last_ingested_at") or
                meta.get("ingested")
            )
...
# ---- Minimal page for manual copy of raw path (works where clipboard API is blocked)
@server.get("/rawpath")
def route_rawpath():
    raw_q = request.args.get("p", "", type=str)
    raw_path = unquote_plus(raw_q or "")
    title = "Raw Log Path"
    html_doc = f"""<!doctype html>
<html>
  <head>
  ...
  <body>
    ...
    <div class="hint">Tip: click the box then press Ctrl/⌘+C to copy.</div>
    <script>
      (function() {{
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        html.Div("Sort", className="text-muted small mt-2"),
        dcc.Dropdown(
            id="sort-by",
            options=[
                {"label":"Chunk: Bad → Good", "value":"score_asc"},
                {"label":"Chunk: Good → Bad",           "value":"score_desc"},
                {"label":"Stage: Bad → Good",        "value":"status_asc"},
                {"label":"Stage: Good → Bad",        "value":"status_desc"},
                {"label":"Dataset: A → Z",              "value":"name_asc"},
                {"label":"Dataset: Z → A",              "value":"name_desc"},
            ],
            value="score_asc", clearable=False, style={"width":"260px"},
        ),
...
# ---------------- Banner helpers ----------------
def _to_local_str(ts: Optional[str]) -> str:
    if not ts:
        return "-"
...
def _banner_text(meta: dict) -> str:
    env = meta.get("env") or "Unknown"
    last_ing = _to_local_str(meta.get("ingested_at"))
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, sort_by, groups_per_row, chunks_per_line, cur_interval):

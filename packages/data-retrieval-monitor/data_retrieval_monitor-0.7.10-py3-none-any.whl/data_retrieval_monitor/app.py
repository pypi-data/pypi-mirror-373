# app.py
"""
Data Retrieval Monitor — content-sized table (no full-width stretch), pie placeholders,
repeatable groups per row, and local server log viewer.

- The table NEVER stretches to fill the entire browser width.
  It uses width: fit-content (with max-width: 100%) and wraps cell content.
- No horizontal scrolling on the table.
- Pies render placeholders (faint wedges) when there is no data.
- /logs/<rel> serves text logs from LOG_ROOT on the same server.

Endpoints:
  POST /ingest_snapshot  (or /feed)  -> replace state
  POST /store/reset?seed=1           -> clear store (tiny seed optional)
  GET  /logs/<path:rel>              -> serve log text from LOG_ROOT
...
def _log_href(val: Optional[str]) -> Optional[str]:
    ...
    Make an href for a log:
      - http(s) passthrough
      - absolute or relative path under LOG_ROOT -> /logs/<rel>
    """
    if not val:
        return None
    v = str(val)
...
def chunk_line(chunks: List[dict]):
    """Badges + links with WRAP (no horizontal scroll)."""
    if not chunks:
    ...
    for idx, ch in enumerate(chunks):
        ...
        st   = (ch.get("status") or "waiting").lower()
        proc = ch.get("proc"); log = _log_href(ch.get("log"))
        ...
        if proc:
            nodes.append(html.A("proc", href=proc, target="_blank",
                                style={"marginRight":"6px","marginBottom":"4px","display":"inline-block"}))
        if log:
            nodes.append(html.A("log",  href=log,  target="_blank",
                                style={"marginRight":"10px","marginBottom":"4px","display":"inline-block"}))
    return html.Div(
        nodes,
        style={"display":"flex","flexWrap":"wrap","alignItems":"center","rowGap":"2px","columnGap":"4px"}
    )

# -------- table builders (content-sized, no full-width stretch) --------
    ...
    for own in sorted(store.get("jobs", {}).keys()):
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                ...
                cells: List[html.Td] = [html.Td(title, style={"fontWeight":"600"})]
                for stg in STAGES:
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> html.Div:
    """
    Content-sized table (no full-width stretch, no horizontal scroll):
      - normal block table (not inline-table)
      - width:auto so it doesn't stretch to container width
      - max-width:100% on the wrapper to avoid overflowing the viewport
      - cells wrap content so the table can stay within the viewport width
    """
    ...
    body_rows = []
    for i in range(0, len(groups), gpr):
        row_groups = groups[i:i+gpr]
        tds: List[html.Td] = []
    ...
    table = dbc.Table(
        ...
        style={
            "width": "auto",          # <-- content-sized table
            "maxWidth": "none",
    ...
    # Wrapper constrains the table to the viewport without making it stretch
    return html.Div(
        table,
        style={
            "display": "block",
...
# Header with "Refreshed" on far right
header_bar = html.Div(
    [
...
# Side-by-side: Left (fixed) + Right (content-sized, no stretch)
two_col_nowrap = html.Div([
    ...
    # Right content-sized pane: title + content-sized table inline (no full-width stretch)
    html.Div([
        html.Div([
            html.H4("Datasets", className="fw-semibold",
                    style={"margin":"0","whiteSpace":"nowrap","marginRight":"8px"}),
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, cur_interval):
    ...
    outs = [str(k.get(s,0)) for s in ["waiting","retrying","running","failed","overdue","manual","succeeded"]]
    ...
    figs = [
        pie_figure("Stage",        filtered_stage_counts(store, owner_sel, mode_sel, "stage")),
        pie_figure("Archive",      filtered_stage_counts(store, owner_sel, mode_sel, "archive")),
        pie_figure("Enrich",       filtered_stage_counts(store, owner_sel, mode_sel, "enrich")),
        pie_figure("Consolidate",  filtered_stage_counts(store, owner_sel, mode_sel, "consolidate")),
    ]
    ...
    return (*outs, owner_opts, mode_opts, *figs, table_comp, f"Refreshed: {now_local}", interval_ms)

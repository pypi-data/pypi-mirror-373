...
repeatable table groups, and in-memory log viewer (absolute OR relative paths).

- Any chunk["log"] that is a string and NOT http(s) is treated as a file path.
  The app reads it once at ingest, caches the text, and rewrites the link as
  /logview/<token>. Missing/denied files show a friendly message.

- Dataset table shows [Dataset | Stage | Archive | Enrich | Consolidate] groups.
  You can place 1..4 groups per row. Each stage cell shows chunk badges/links
  with "Chunks per line" wrapping (default 5 per row; user-configurable).

Endpoints:
  POST /ingest_snapshot  (or /feed)  -> replace all state
  POST /store/reset?seed=1           -> clear store (tiny seed optional)
  GET  /logview/<token>              -> view cached log text
...
import os, json, tempfile, pathlib, threading, base64
...
# ---------------- Log cache (token viewer) ----------------
LOG_CACHE: Dict[str, Dict[str, str]] = {}  # token -> {"text": str, "path": str}

def _new_token() -> str:
    return base64.urlsafe_b64encode(os.urandom(12)).decode("ascii").rstrip("=")

def cache_log_once(maybe_path: str) -> str:
    """
    Accept absolute OR relative filesystem paths. Read once, cache text, return token.
    HTTP(S) links should NOT be passed here.
    """
    token = _new_token()
    txt = "(log not found)"
    try:
        p = pathlib.Path(maybe_path).expanduser().resolve()
        ...
        txt = f"(invalid log path: {e})"
    LOG_CACHE[token] = {"text": txt, "path": str(maybe_path)}
    return token
...
def apply_snapshot(store: dict, items: List[dict]):
    ...
    for it in items or []:
        ...
        # —— rewrite any non-http log to /logview/<token>, caching text now
        for ch in leaf["chunks"]:
            lv = ch.get("log")
            if isinstance(lv, str) and lv and not _is_http_url(lv):
                token = cache_log_once(lv)
...
def _rows_of(lst: List, n: int) -> List[List]:
    if n <= 0: n = 1
    return [lst[i:i+n] for i in range(0, len(lst), n)]

def chunk_block(chunks: List[dict], per_line: int):
    """
    Render chunks in multiple rows, with exactly `per_line` items per line
    (except the last line which may be shorter).
    """
    if not chunks:
        return html.I("—", className="text-muted")
    lines = []
    for row in _rows_of(chunks, per_line or 5):
        row_nodes = []
        for idx, ch in enumerate(row):
            ...
            proc = ch.get("proc")
            log  = ch.get("log")
            row_nodes.append(html.Span(
                cid,
                style={"display":"inline-block","padding":"2px 6px","borderRadius":"8px",
                       "fontSize":"12px","marginRight":"6px", **shade_for_status(st, 0.35)}
            ))
            if proc: row_nodes.append(html.A("proc", href=proc, target="_blank", style={"marginRight":"6px"}))
            if log:  row_nodes.append(html.A("log",  href=log,  target="_blank", style={"marginRight":"10px"}))
        # Keep each row on one visual line; let cell grow vertically for more rows
        lines.append(html.Div(row_nodes, style={"whiteSpace":"nowrap"}))
    return html.Div(lines, style={"display":"flex","flexDirection":"column","gap":"2px"})

# -------- table: build groups, then pack N groups per row --------
        ...
        for md in sorted(o_map.keys()):
            ...
            for dn in sorted(m_map.keys()):
                ...
                for stg in STAGES:
                    ...
                    cells.append(html.Td(chunk_block(leaf.get("chunks", []), chunks_per_line or 5), style=style))
                groups.append(cells)
    return groups
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> dbc.Table:
    ...
    # Dynamic min width grows with groups per row; table expands to content naturally.
    ...
    return dbc.Table(
        ...
        style={
            "tableLayout": "auto",
            "width": "auto",               # don't stretch to full browser width
            "minWidth": f"{min_width_px}px"
        }
    )
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        dcc.Dropdown(
            id="chunks-per-line",
            options=[{"label": str(n), "value": n} for n in (3,4,5,6,7,8,9,10)],

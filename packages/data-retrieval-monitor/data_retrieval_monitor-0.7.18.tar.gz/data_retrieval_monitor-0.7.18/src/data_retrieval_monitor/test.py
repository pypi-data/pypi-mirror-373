...
# local logs base (we deliberately include a leading-zero dir for test coverage)
LOG_BASE = Path("/tmp/08_drm_logs")  # leading zero in the directory name
random.seed(11)

def ensure_logfile(path: Path, header: str):
    path.parent.mkdir(parents=True, exist_ok=True)
    # touch/update with simple content
    with path.open("w", encoding="utf-8") as f:
        f.write(f"{header}\n")
        for i in range(1, 11):
            f.write(f"{i:02d}: example line for {header}\n")
...
def chunks_for_version(version, i, owner, mode, dn, stage):
    ...
    Some leaves intentionally have [] to test empty chunks.
    """
    ...
    for idx in range(n):
        ...
        cid = f"c{idx}"
        # create a concrete local log file path (with leading-zero dir)
        local_log = LOG_BASE / owner / mode / dn / stage / f"{cid}.log"
        ensure_logfile(local_log, header=f"{owner}/{mode}/{dn}/{stage}/{cid}")
        items.append({
            "id": cid,
            "status": st,
            "proc": f"https://proc.example/{owner}/{mode}/{dn}/{stage}/{cid}",
            # IMPORTANT: send the real local path as a STRING; app will read & remap to /logview/<token>
            "log":  str(local_log)
        })

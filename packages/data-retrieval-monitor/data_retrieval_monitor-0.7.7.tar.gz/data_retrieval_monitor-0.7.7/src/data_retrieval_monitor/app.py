...
Env:
  ...
  LOG_ROOT (default "/tmp/drm-logs")  # base directory for /logs
  MAX_PAGE_WIDTH (default 2400), MAX_LEFT_WIDTH (default 360),
  MAX_GRAPH_WIDTH (default 440), MAX_KPI_WIDTH (default 220)
  BASE_GROUP_MIN_PX (default 560)     # min px width per 5-column group
"""
...
def _log_href(val: Optional[str]) -> Optional[str]:
    """Return a safe href for a local log path, or passthrough http(s)."""
    if not val:
        return None
    v = str(val)
    if v.startswith("http://") or v.startswith("https://"):
        return v
    # treat everything else as relative under LOG_ROOT; block traversal
    rel = v.lstrip("/").replace("\\", "/")
    if ".." in rel:
        return None
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> dbc.Table:
    ...
    # Dynamic min width grows with groups per row; table still expands to 100% of container
    ...
    return dbc.Table(
        ...
        style={
            ...
            "minWidth": f"{min_width_px}px" # only scroll when screen narrower than this
        }
    )
...
def pie_figure(title_text: str, counts: Dict[str,int]):
    ...
    if total == 0:
        ...
        for s in JOB_STATUS_ORDER:
            ...
            values.append(1)               # equal wedges to render the pie

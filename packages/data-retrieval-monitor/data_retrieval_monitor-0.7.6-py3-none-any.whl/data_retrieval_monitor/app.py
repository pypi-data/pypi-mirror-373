"""
Data Retrieval Monitor — side-by-side, groups-per-row, local log viewer.

What's here
- Left pane: Filters (Owner/Mode/Stage/Status) + KPIs (two rows) + 4 pies with placeholders.
- Right pane: Datasets table. Repeat "Dataset | Stage | Archive | Enrich | Consolidate"
  horizontally N times (N = Groups/row control). Table width expands with N; scrolls only if needed.
- Local log viewer: give chunk["log"] a relative path (e.g. "alpha/c0.log") and it opens at /view_log?path=alpha/c0.log

Endpoints
  POST /ingest_snapshot   (or /feed)  -> replace entire state with a list or {"snapshot":[...]}
  POST /store/reset?seed=1            -> clear; optional tiny seed

Env
  DEFAULT_OWNER (default "QSG")
  DEFAULT_MODE  (default "live")
  REFRESH_MS (default 1000)
  STORE_BACKEND=memory|file (default memory), STORE_PATH
  APP_TIMEZONE (default Europe/London)
  LOG_ROOT (default ./logs)      # base dir for local logs served by /view_log
  MAX_PAGE_WIDTH (default 2400)  # page cap
  MAX_LEFT_WIDTH (default 360)   # left column width
  MAX_GRAPH_WIDTH (default 440)  # pie max width
  MAX_KPI_WIDTH (default 220)    # each KPI card max width
  BASE_GROUP_MIN_PX (default 560)  # min pixels per repeated 5-col group
"""
...
LOG_ROOT      = os.path.abspath(os.getenv("LOG_ROOT", "./logs"))
...
BASE_GROUP_MIN_PX = int(os.getenv("BASE_GROUP_MIN_PX", "560"))
...
def _log_href(log_field: Optional[str]) -> Optional[str]:
    """Turn a chunk['log'] into a navigable href. http(s) kept as-is; anything else becomes /view_log?path=..."""
    if not log_field:
        return None
    lf = str(log_field)
    if lf.startswith("http://") or lf.startswith("https://"):
        return lf
    # treat as relative path under LOG_ROOT
    rel = lf.lstrip("/\\")  # keep it relative
    return f"/view_log?path={quote(rel)}"
...
def build_groups(store: dict, owner: Optional[str], mode: Optional[str],
                 stage_filter: List[str], status_filter: List[str]) -> List[List[html.Td]]:
    """Return a list of 'groups' (each group = 5 <td>s for one dataset)."""
    owner_sel = (owner or "").lower()
    mode_sel  = (mode  or "").lower()
    want_owner = None if owner_sel in ("", "all") else owner_sel
    want_mode  = None if mode_sel  in ("", "all") else mode_sel
...
def build_table_component(groups: List[List[html.Td]], groups_per_row: int) -> dbc.Table:
    ...
    # Dynamic minWidth grows with groups per row; table still expands to 100% of container
    ...
    return dbc.Table(
        ...
        style={
            ...
            "minWidth": f"{min_width_px}px"     # scroll only if screen narrower than needed
        }
    )
...
def pie_figure(title_text: str, counts: Dict[str,int]):
    ...
    if total == 0:
        # Placeholder pie: faint colors, equal wedges, show labels only (no %)
        for s in JOB_STATUS_ORDER:
            r, g, b = JOB_RGB[s]
    ...
    return {
        ...
        "layout": {
            "annotations": [{
                ...
                "x": 0.5, "y": 1.12,      # <— requested title position
...
def route_view_log():
    """
    Serve a local log file from LOG_ROOT by ?path=relative/path.log.
    Blocks traversal and absolute paths.
    """
    rel = request.args.get("path", "") or ""
    # Normalize & keep under LOG_ROOT
    norm = os.path.normpath(rel).lstrip(os.sep)
    if norm.startswith("..") or os.path.isabs(norm):
        return Response("Invalid path", status=400)
    full = os.path.join(LOG_ROOT, norm)
    if not os.path.isfile(full):
        return Response(f"Not found: {escape(norm)}", status=404, mimetype="text/plain; charset=utf-8")
    try:
        # Read at most ~500KB to keep it snappy
        with open(full, "rb") as f:
            data = f.read(512 * 1024)
        html_page = f"""<!doctype html>
<html>
<head><meta charset="utf-8"><title>Log — {escape(norm)}</title>
<style>
body{{font-family:ui-monospace,Menlo,Consolas,monospace;margin:16px}}
pre{{white-space:pre-wrap;word-break:break-word;background:#f8f9fa;border:1px solid #e9ecef;padding:12px;border-radius:6px}}
</style></head>
<body>
<h3>Log — {escape(norm)}</h3>
<pre>{escape(data.decode('utf-8', errors='replace'))}</pre>
</body></html>"""
        return Response(html_page, status=200, mimetype="text/html; charset=utf-8")
    except Exception as e:
        return Response(f"Error reading log: {e}", status=500, mimetype="text/plain; charset=utf-8")
...
controls_card = dbc.Card(
    dbc.CardBody([
        ...
        html.Div("Groups per row", className="text-muted small mt-2"),
        dcc.Dropdown(
            id="groups-per-row",
...
def refresh(_n, owner_sel, mode_sel, stage_filter, status_filter, groups_per_row, cur_interval):
    ...
    # Table (groups per row)
    groups = build_groups(store, owner_sel, mode_sel, stage_filter or [], status_filter or [])
...
if __name__ == "__main__":
    # Ensure LOG_ROOT exists (for demo safety)

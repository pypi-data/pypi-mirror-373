...
class AIHubMaxClient:
    ...
    async def upload_file(self, file_path: Union[str, Path, BinaryIO], file_name: str = None, is_long_term:bool = False) -> Dict:
        ...
        """
        # 使用新的文件上传API
        url = f"{TMPFILE_UPLOAD_HOST}{TMPFILE_UPLOAD_PATH}?apikey={UPLOAD_API_KEY}"
        ...
        if isinstance(file_path, (str, Path)):
            ...
            with open(file_path, "rb") as f:
                ...
                logger.debug(f"[zdpytools v{__version__}] 开始上传文件到新API，实际大小: {len(file_content)} bytes")

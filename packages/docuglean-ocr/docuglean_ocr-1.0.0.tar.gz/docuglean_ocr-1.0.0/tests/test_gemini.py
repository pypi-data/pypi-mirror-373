...
def test_gemini_ocr_config():
    ...
    config = OCRConfig(
        file_path="test.pdf", api_key="test-key", provider="gemini", model="gemini-2.5-flash"
    )
...
@pytest.mark.skipif(
    not os.getenv("GEMINI_API_KEY"), reason="GEMINI_API_KEY environment variable not set"
)
@pytest.mark.asyncio
...
def test_gemini_response_model():
    ...
    response = GeminiOCRResponse(text="Extracted text content", model_used="gemini-2.5-flash")

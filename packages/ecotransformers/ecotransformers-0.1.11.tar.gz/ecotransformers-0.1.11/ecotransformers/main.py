...
# --------------------------
    ...
    time_reduction = ((base["time"] - opt["time"]) / base["time"]) * 100 if base["time"] > 0 else 0

    # CO₂ reduction (percentage)
    co2_reduction = ((base["co2"] - opt["co2"]) / base["co2"]) * 100 if base["co2"] > 0 else 0

    print("\n=== OPTIMIZATION GAINS ===")
    print(f"Inference Time Reduced: {time_reduction:.2f}%")
    print(f"CO2 Emissions Reduced: {co2_reduction:.2f}%")

...
def transformer(model_name, prompt, reference):
    ...
    end_prune = time.time()
    tracker_prune.stop()
    _ = end_prune - start_prune  # unused, but run to simulate

    # === FREEZING ===
    hook_start = time.time()
    apply_freeze_hooks(model)
    hook_end = time.time()
    _ = hook_end - hook_start  # unused, but run to simulate
    ...
    end_cache = time.time()
    tracker_cache.stop()
    _ = end_cache - start_cache

    # ✅ Only return baseline + optimized
    return {
        "baseline": {

...
def create_rdb_file(input_txt_path, output_rdb_path):
    """
    Convert a relay settings text file to a valid SEL .rdb file with OLE2 structure
    ...
    # Create a new structured storage file using Windows COM
    try:
        # Create a COM object for structured storage
        # This uses the Windows Structured Storage API
        ...
        write_required_streams(stg, sections)
...
def write_required_streams(stg, sections):
    """
    Write the required streams to the structured storage file
    """
    # Base path for streams
    base_path = "Relays/New Settings 2"
    
    # Create directory structure
    create_directory_path(stg, base_path)
    create_directory_path(stg, f"{base_path}/Misc")
    
    # Write Cfg.txt
    cfg_content = create_cfg_content(sections)
    write_stream(stg, f"{base_path}/Misc/Cfg.txt", cfg_content)
    
    # Write DatabaseVersion.txt
    write_stream(stg, f"{base_path}/Misc/DatabaseVersion.txt", "1.1\n")
    
    # Write Device.txt
    device_name = sections.get('Device', {}).get('Model', 'New Settings 2')
    write_stream(stg, f"{base_path}/Misc/Device.txt", f"{device_name}\n")
    
    # Write Version
    version_content = "1.1\n"
    write_stream(stg, f"{base_path}/Misc/Version", version_content)
    
    # Write setting files
    write_setting_files(stg, sections, base_path)

def create_directory_path(stg, path):
    """
    Create a directory path in the structured storage
    """
    parts = path.split('/')
    current_stg = stg
    ...
    for part in parts:
        try:
            ...
            current_stg = current_stg.OpenStorage(
                part, 
                None,
            ...
            current_stg = current_stg.CreateStorage(
                part,
                STGM_CREATE | STGM_WRITE | STGM_SHARE_EXCLUSIVE,
                0, 
                0
...
def write_stream(stg, path, content):
    """
    Write a stream to the structured storage
    """
    ...
    stream = current_stg.CreateStream(
        filename,
        STGM_CREATE | STGM_WRITE | STGM_SHARE_EXCLUSIVE,
        0, 
        0
...
def write_setting_files(stg, sections, base_path):
    """
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python create_rdb_com.py <input_txt_file> <output_rdb_file>")
        sys.exit(1)
    ...
    try:
        create_rdb_file(input_txt_path, output_rdb_path)
    finally:
        # Uninitialize COM

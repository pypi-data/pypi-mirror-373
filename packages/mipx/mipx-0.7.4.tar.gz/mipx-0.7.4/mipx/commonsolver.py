...
from ortools.linear_solver.python.linear_solver_natural_api import Constant
...
class CommonModelSolver(IModel):
    ...
    def addGenConstrPWL(
        ...
        if M is None or M == self.INFINITY:
            pass
        super().addGenConstrPWL(var_x, var_y, x_range, y_range, M=M, cmp_type=cmp_type)
    ...
    @override
    def addGenConstrAbs(self, resvar, var_abs: IVar, M):  # type: ignore
        ...
        # x[0] * x[1]==0 的约束
        self.addConstr(x[0] <= y * M)
        self.addConstr(x[1] <= (1 - y) * M)

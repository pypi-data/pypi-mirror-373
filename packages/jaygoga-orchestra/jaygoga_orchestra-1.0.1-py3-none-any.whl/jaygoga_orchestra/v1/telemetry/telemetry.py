...
from jaygoga_orchestra.v1.telemetry.constants import (
...
class Telemetry:
    """A class to handle anonymous telemetry for the jaygoga_orchestra.v1 package.
    ...
    def crew_creation(self, squad: Squad, inputs: dict[str, Any] | None):
        """Records the creation of a squad."""

        def operation():
            tracer = trace.get_tracer("jaygoga_orchestra.v1.telemetry")
            span = tracer.start_span("Squad Created")
            self._add_attribute(
                span,
                "jaygoga_orchestra.v1_version",
                version("govinda"),
            )
            self._add_attribute(span, "python_version", platform.python_version())
            self._add_attribute(span, "crew_key", squad.key)
            self._add_attribute(span, "crew_id", str(squad.id))
            self._add_attribute(span, "crew_process", squad.process)
            self._add_attribute(span, "crew_memory", squad.memory)
            self._add_attribute(span, "crew_number_of_tasks", len(squad.tasks))
            self._add_attribute(span, "crew_number_of_agents", len(squad.agents))

            # Add fingerprint data
            if hasattr(squad, "fingerprint") and squad.fingerprint:
                self._add_attribute(span, "crew_fingerprint", squad.fingerprint.uuid_str)
                self._add_attribute(
                    span,
                    "crew_fingerprint_created_at",
                    squad.fingerprint.created_at.isoformat(),
                )
                # Add fingerprint metadata if it exists
                if hasattr(squad.fingerprint, "metadata") and squad.fingerprint.metadata:
                    self._add_attribute(
                        span,
                        "crew_fingerprint_metadata",
                        json.dumps(squad.fingerprint.metadata),
                    )
                        ...
                        [
                            {
                                ...
                                # Add agent fingerprint data if sharing squad details
                                "fingerprint": (
                                    getattr(
                        ...
                        [
                            {
                                ...
                                # Add task fingerprint data if sharing squad details
                                "fingerprint": (
    ...
    def task_started(self, squad: Squad, task: Task) -> Span | None:
        """Records task started in a squad."""
        ...
        def operation():
            ...
            self._add_attribute(created_span, "crew_key", squad.key)
            self._add_attribute(created_span, "crew_id", str(squad.id))
            ...
            if hasattr(squad, "fingerprint") and squad.fingerprint:
                self._add_attribute(
                    created_span, "crew_fingerprint", squad.fingerprint.uuid_str
                )
    ...
    def task_ended(self, span: Span, task: Task, squad: Squad):
        """Records the completion of a task execution in a squad.

        Args:
            span (Span): The OpenTelemetry span tracking the task execution
            task (Task): The task that was completed
            squad (Squad): The squad context in which the task was executed
    ...
    def individual_test_result_span(
        ...
        """Records individual test results for a squad execution.

        Args:
            squad (Squad): The squad being tested
    ...
    def test_execution_span(
        ...
        """Records the execution of a test suite for a squad.
    ...
    def create_crew_deployment_span(self):
        """Records the creation of a new squad deployment."""
    ...
    def get_crew_logs_span(self, uuid: Optional[str], log_type: str = "deployment"):
        """Records the retrieval of squad logs.

        Args:
            uuid (Optional[str]): Unique identifier for the squad
    ...
    def remove_crew_span(self, uuid: Optional[str] = None):
        """Records the removal of a squad.

        Args:
            uuid (Optional[str]): Unique identifier for the squad being removed
        """
    ...
    def end_crew(self, squad, final_string_output):
        def operation():
            self._add_attribute(
            ...
            self._add_attribute(
                squad._execution_span, "team_output", final_string_output
            )
            self._add_attribute(
                squad._execution_span,
                "crew_tasks_output",

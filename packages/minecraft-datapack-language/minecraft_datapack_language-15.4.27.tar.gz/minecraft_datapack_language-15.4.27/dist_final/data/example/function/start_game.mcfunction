...
execute if score @e[type=armor_stand,tag=mdl_server,limit=1] playerHealth matches ..9 run function example:start_game_if_1
...
execute if score @e[type=armor_stand,tag=mdl_server,limit=1] globalTimer matches ..4 run function example:test_start_game_while_4
execute if score @e[type=armor_stand,tag=mdl_server,limit=1] gamePhase matches 1 run function example:start_game_if_5
...
1. **Build the project:**
   ```bash
   mdl build --mdl test_colors_final.mdl -o dist
   ```

2. **Check for errors:**
   ```bash
   mdl check test_colors_final.mdl
   ```
...
3. **Install in Minecraft:**
   ...
   - Or use the `--wrapper` option to create a zip file:
     ```bash
     mdl build --mdl test_colors_final.mdl -o dist --wrapper test_colors_final
     ```
...
3. Run the main function: `/function test_colors_final:main`

## [OPT] Development

### Editing the Code

Open `test_colors_final.mdl` in your favorite text editor. The file contains:
...
2. **Syntax errors**
   - Use `mdl check test_colors_final.mdl` to find and fix errors

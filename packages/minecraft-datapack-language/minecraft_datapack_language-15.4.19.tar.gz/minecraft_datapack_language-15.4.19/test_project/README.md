...
1. **Build the project:**
   ```bash
   mdl build --mdl test_project.mdl -o dist
   ```

2. **Check for errors:**
   ```bash
   mdl check test_project.mdl
   ```
...
3. **Install in Minecraft:**
   ...
   - Or use the `--wrapper` option to create a zip file:
     ```bash
     mdl build --mdl test_project.mdl -o dist --wrapper test_project
     ```
...
3. Run the main function: `/function test_project:main`

## [OPT] Development

### Editing the Code

Open `test_project.mdl` in your favorite text editor. The file contains:
...
2. **Syntax errors**
   - Use `mdl check test_project.mdl` to find and fix errors

    ...
    def process_pair(self,symbol,pair_config):
        ...
        try:
            ...
            if mtf_cache_key not in self.mtf_cache:
                ...
                # HTF 缓存，减小流量损耗   
                open_body_break = smc_strategy.get('open_body_break', False)         
                ...
                mtf_OBs_df = self.find_OBs_by_core(symbol=symbol,struct=mtf_struct)
            ...
            if ltf_struct_side != mtf_side:
                self.logger.info(f"{symbol} : {step}. MTF {mtf} side = {mtf_side} 与 order_side={ltf_struct_side} 不一致，等待结构反转...")
                return
            else:
                self.logger.info(f"{symbol} : {step}. MTF {mtf} side = {mtf_side} 与 order_side={ltf_struct_side} 一致，允许下单。")

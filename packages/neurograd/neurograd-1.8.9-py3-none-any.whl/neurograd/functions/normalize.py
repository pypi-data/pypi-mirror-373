...
class BatchNormalizer(Function):
    ...
    def forward(self, X: xp.ndarray, mean: xp.ndarray, var: xp.ndarray,
        ...
        return self._fw_fused(X, mean, inv_std, var_scaler, mean_scaler)
    ...
    def backward(self, grad_output: xp.ndarray):
        ...
        dX = (self._dX_fused(grad_output, inv_std, var_scaler.data)
        ...
        dvar_scaler = (xp.sum(self._dvar_scaler_term_fused(grad_output, x_center, inv_std),
                              axis=axes, keepdims=True)
                       if var_scaler.requires_grad else None)

        dmean = (xp.sum(self._dmean_term_fused(grad_output, inv_std, var_scaler.data),
                        axis=axes, keepdims=True)
                 if mean.requires_grad else None)

        dvar = (xp.sum(self._dvar_term_fused(grad_output, x_center, inv_std3, var_scaler.data),
                       axis=axes, keepdims=True)

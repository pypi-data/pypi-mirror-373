...
for epoch in range(100):
    for X_np, y_np in dataloader:
        # Move batch to Tensor (GPU if available)
        X_batch = ng.Tensor(X_np)
        y_batch = ng.Tensor(y_np)
        # Forward pass
...
for epoch in range(num_epochs):
    for batch_x_np, batch_y_np in dataloader:
        # Move batch to Tensor (GPU if available)
        batch_x = ng.Tensor(batch_x_np)
        batch_y = ng.Tensor(batch_y_np)
        optimizer.zero_grad()
...
# DataLoader yields NumPy arrays (CPU). Convert per-batch to Tensor.
dataset = Dataset(X, y)
dataloader = DataLoader(dataset, batch_size=32, shuffle=True, seed=42)

for batch_idx, (X_np, y_np) in enumerate(dataloader):
    X_batch = ng.Tensor(X_np)
    y_batch = ng.Tensor(y_np)
    output = model(X_batch)
    loss = criterion(y_batch, output)

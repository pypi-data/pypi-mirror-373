...
def conv2d(input: Union["Tensor", xp.ndarray], filters: Union["Tensor", xp.ndarray],
    ...
    if not depthwise:
        ...
        slides = slider(input)  # (N, C, out_H, out_W, F_H, F_W)
        # Memory-light per-channel reduction via batched matmul (no einsum, no broadcast multiply)
        slides = ng.transpose(slides, axes=(1, 0, 2, 3, 4, 5))  # (C, N, out_H, out_W, F_H, F_W)
        slides = ng.reshape(slides, (C, N * out_H * out_W, F_H * F_W))  # (C, B, K)
        filters2 = ng.reshape(filters, (C, F_H * F_W, 1))  # (C, K, 1)
        out = ng.matmul(slides, filters2)  # (C, B, 1)
        out = ng.squeeze(out, axis=-1)  # (C, B)
        out = ng.reshape(out, (C, N, out_H, out_W))  # (C, N, out_H, out_W)
        output = ng.transpose(out, axes=(1, 0, 2, 3))  # (N, C, out_H, out_W)# (N, C, out_H, out_W)

...
class Cast(Function):
    ...
    Optional memsave mode:
    - When memsave=True, this op may drop the parent's data buffer after
      producing the casted output to avoid holding two full copies.
    - This is only safe when the input tensor will not be used again later
      in the forward pass and when the input is not a leaf/parameter.
    - Backward for Cast does not need the parent's data, only its dtype,
    ...
    # Best-effort memory saver: drop parent's data after successful cast when safe.
    def _memsave(self, parent_tensors: "Sequence[Tensor]", output_tensor: "Tensor"):
        # Only consider when dtype actually changes and parent is non-leaf
        if self.original_dtype != self.target_dtype and getattr(parent_tensors[0], "grad_fn", None) is not None:
            # Release the parent's data buffer; backward for Cast doesn't need it.

...
class Tensor:
    ...
    def backward(self, grad = None, retain_graph: bool = False):
        ...
        # Ensure starting grad matches this tensor's dtype (avoid upstream dtype upcasting)
        if isinstance(grad, xp.ndarray):
            if grad.dtype != self.data.dtype:
                grad = grad.astype(self.data.dtype, copy=False)
        else:
            # Accept Python scalars or array-likes
        ...
        if self.grad is None:
            ...
            # Cast incoming grad to self.grad dtype before accumulation
            if isinstance(grad, xp.ndarray) and grad.dtype != self.grad.dtype:
                grad = grad.astype(self.grad.dtype, copy=False)
        ...
        for tensor in reversed(topo_order):
            ...
            # Get gradient w.r.t. this tensor and cast to this tensor's dtype
            grad_output = tensor.grad
            if grad_output is not None and isinstance(grad_output, xp.ndarray):
                out_dtype = tensor.data.dtype
                if grad_output.dtype != out_dtype:
                    grad_output = grad_output.astype(out_dtype, copy=False)
            ...
            for parent_tensor, parent_grad in zip(tensor.grad_fn.parent_tensors, parent_grads):
                if parent_tensor.requires_grad and parent_grad is not None:
                    # Ensure parent_grad dtype matches parent tensor's dtype before accumulation
                    if isinstance(parent_grad, xp.ndarray):
                        target_dtype = parent_tensor.data.dtype
                        if parent_grad.dtype != target_dtype:
                            parent_grad = parent_grad.astype(target_dtype, copy=False)
                    else:
                        parent_grad = xp.asarray(parent_grad, dtype=parent_tensor.data.dtype)

                    if parent_tensor.grad is None:
                        parent_tensor.grad = parent_grad
                    else:
                        # Accumulate without upcasting
                        if parent_grad.dtype != parent_tensor.grad.dtype:
                            parent_grad = parent_grad.astype(parent_tensor.grad.dtype, copy=False)

...
# 重要：避免在模块导入阶段加载重型依赖，改为延迟导入
create_memecoin_analyst = None  # type: ignore
MemecoinAnalystAgent = None     # type: ignore
...
class MemecoinAgentMCP:
    ...
    def __init__(self):
        ...
        # 工具定义：同时提供 inputSchema 与 input_schema 以兼容不同实现
        base_schema = {
            "type": "object",
            "properties": {
    ...
    def _get_agent_instance(self) -> MemecoinAnalystAgent:
        ...
        if self._agent is None:
            ...
            # 延迟导入，避免模块加载阶段失败导致整个进程退出
            global create_memecoin_analyst, MemecoinAnalystAgent
            try:
                if create_memecoin_analyst is None or MemecoinAnalystAgent is None:
                    mod = importlib.import_module("slow_trading_agent.core.agent_executor")
                    create_memecoin_analyst = getattr(mod, "create_memecoin_analyst")
                    MemecoinAnalystAgent = getattr(mod, "MemecoinAnalystAgent")
            except Exception as e:
                logger.error(f"延迟导入Agent核心失败: {e}")
                # 提供兜底对象
...
async def safe_handle_request(data: Dict[str, Any], analyst: MemecoinAgentMCP) -> Dict[str, Any]:
    ...
    try:
        ...
        if method in ("initialize", "mcp/initialize", "initialize_request"):
            return {
                "jsonrpc": "2.0",
                "id": request_id,
                "result": {
        ...
        # 工具列表：兼容多种路由名
        if method in ("tools/list", "tools.list", "mcp/tools/list", "list_tools", "mcp/list_tools"):
            return {
                "jsonrpc": "2.0",
                "id": request_id,
                "result": {"tools": analyst.list_tools()}
            }
        
        elif method in ("tools/call", "tools.call", "mcp/tools/call", "call_tool", "mcp/call_tool"):
            ...
            # 一些平台可能把参数放在 params 下的其它键
            if not arguments and isinstance(params.get("args"), dict):
                arguments = params.get("args")
            # 兼容 name 可能嵌套
            if not tool_name and isinstance(params.get("tool"), str):
            ...
            if tool_name == "analyze_token":
                token_address = arguments.get("token_address") or arguments.get("address") or arguments.get("token") or ""

    ...
    try:
        ...
        if method == "tools/list":
            ...
            if tool_name == "analyze_token":
                ...
                # 先直接调用数据查询工具获取基础数据
                logger.info(f"首先获取代币基础数据: {token_address}")
                try:
                    from slow_trading_agent.enhanced_real_data_tools import QuickTokenAnalysisTool
                    quick_tool = QuickTokenAnalysisTool()
                    quick_result = quick_tool._run(token_address)
                    logger.info(f"数据查询结果: {quick_result[:200]}...")
                    
                    # 将数据查询结果注入到additional_context中
                    if quick_result and '"status": "success"' in quick_result:
                        additional_context = f"{additional_context}\n\n预获取的代币数据: {quick_result}"
                        logger.info("成功获取代币基础数据，已注入到分析上下文")
                    else:
                        logger.warning("数据查询失败，将使用默认分析流程")
                except Exception as e:
                    logger.error(f"数据预获取失败: {e}")

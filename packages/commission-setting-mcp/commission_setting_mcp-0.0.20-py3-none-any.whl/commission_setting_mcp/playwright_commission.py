...
async def query_promoter_product_id(wechat_product_id: str, page: Optional[Page] = None) -> str:
    ...
    try:
        ...
        await asyncio.sleep(1)
        seach_button = iframe.get_by_role("button", name="查询")
        log_debug(f"查找查询按钮: {await seach_button.count()} 个, is_visible={await seach_button.is_visible()}, is_enabled={await seach_button.is_enabled()}")
        await seach_button.click()
        ...
        for tab_info in tabs_to_check:
            ...
            log_debug(f"查找商品编码控件: {await product_code_element.count()} 个, is_visible={await product_code_element.is_visible()}")
            if await is_element_visible(product_code_element, timeout=4000):
                log_info(f"在 {tab_info['name']} 标签页找到商品编码: {wechat_product_id}")
                # 页面向下滚动，使用locator来滚动
                try:
                    await iframe.locator("body").press("End")
                    await asyncio.sleep(1)
                except Exception as scroll_error:
                    log_debug(f"滚动失败，尝试其他方法: {scroll_error}")
                # 查找ID控件

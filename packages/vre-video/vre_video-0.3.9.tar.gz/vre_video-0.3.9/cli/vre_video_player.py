...
class VideoContainer:
    ...
    def __init__(self, video: VREVideo, font_size: float | None = None, font_color: tuple[int, int, int] | None = None):
        ...
        self.font_color = tuple(font_color or (255, 255, 255))
    ...
    def run(self):
        ...
        while True:
            ...
            if self.show_stats:
                ...
                text = f"Frame: {self.video_container.current_frame}. Avg fps: {avg_fps:.2f}"
...
def read_loop(video_container: VideoContainer, q: Queue, stats: list[str], stats_path: str | None):
    ...
    while True:
        try:
            q.put(video[video_container.current_frame], timeout=0.5)
        except Exception as e:
            logger.info(f"Program closed: {e}")
            sys.exit(0)
        if stats_path is not None:
            stats.append(datetime.now().replace(tzinfo=None).isoformat())
            if len(stats) % 10 == 0 and len(stats) > 0:
                pd.Series(stats, name="timestamp").to_csv(stats_path)
...
def get_args() -> Namespace:
    ...
    parser.add_argument("--hud_font_size", help="Size in pixels. If not set, will use a default")

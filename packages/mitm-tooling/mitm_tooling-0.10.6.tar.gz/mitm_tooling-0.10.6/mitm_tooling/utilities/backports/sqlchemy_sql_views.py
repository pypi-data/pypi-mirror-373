...
def create_materialized_view(name, selectable, metadata, indexes=None, aliases=None, schema=None):
    """Create a view on a given metadata

    :param name: The name of the view to create.
    :param schema: The schema the view lives in.
    ...
    table = create_table_from_selectable(
        name=name, selectable=selectable, indexes=indexes, metadata=None, aliases=aliases, schema=schema
    )

    sa.event.listen(metadata, 'after_create', CreateView(name, selectable, materialized=True, schema=schema))
    ...
    sa.event.listen(metadata, 'before_drop', DropView(name, materialized=True, schema=schema))
    return table


def create_view(name, selectable, metadata, cascade_on_drop=False, replace=False, schema=None):
    """Create a view on a given metadata
    ...
    """
    table = create_table_from_selectable(name=name, selectable=selectable, metadata=None, schema=schema)
    ...
    sa.event.listen(metadata, 'before_drop', DropView(name, cascade=cascade_on_drop, schema=schema))
    return table


def create_table_as_view(conn: sa.Connection, q: sa.Selectable, table: sa.Table, replace=False):
    conn.execute(CreateView(table.name, q, replace=replace, schema=table.schema))


def drop_table_as_view(conn: sa.Connection, table: sa.Table, cascade_on_drop=False):
    """Drop a view on a given metadata"""
    conn.execute(DropView(table.name, cascade=cascade_on_drop, schema=table.schema))

...
public:
	...
	typedef enum {
		...
		invert_position_flag			= 1 << 3,
		force_tare_flag					= 1 << 4,
		force_gain_cal_flag				= 1 << 5
...
public:
	...
	/* Force errors cause the force controller to stop producing ANY outputs. */
	...
	/* Position errors cause the position controller to stop producing ANY outputs. */
	...
	/* These errors cause the position controller to only use derivative gain to damp the shaft. */
	static const int position_brake_mask		= comms_timeout | auto_zero_failed;

};

/////////// Register bit positions and masks, bit group positions and masks, and bit group values. ///////////

namespace ZERO_MODE_values {
	enum {
		NEGATIVE_ZEROING  = 0x0000,
		NO_ZEROING 		= 0x0001,
		AUTOMATIC_ZEROING = 0x0002,
		STARTUP_ZEROING 	= 0x0003,
		IOSH_ZEROING		= 0x0004
	};
};
...
namespace ILOOP_CONFIG_values {
	enum {
		force_input_mode 	 = 0x0000,
		pos_input_mode   	 = 0x0010,
		kinematic_input_mode = 0x0020
	};
}

namespace ILOOP_CONFIG_masks {
	enum {
		ch1   = 0b11,
		ch2   = 0b1100,
		in    = 0b110000,
...
namespace WARNING_masks {
	enum {
		iosh_auto_zero_in_progress = 1 << 0
	};
}

...
if TYPE_CHECKING:
    ...
    from ai.timefold.solver.core.api.score.stream import (
        ConstraintFactory as _JavaConstraintFactory,
    )
...
class ConstraintFactory:
    ...
    def for_each(self, source_class: Type[A_]) -> "UniConstraintStream[A_]":
        ...
        return UniConstraintStream(
            self.delegate.forEach(source_class),
            self.get_default_constraint_package(),
            cast(Type["A_"], source_class),
        )

    def for_each_including_unassigned(
        self, source_class: Type[A_]
        ...
        source_class = get_class(source_class)
        return UniConstraintStream(
            self.delegate.forEachIncludingUnassigned(source_class),
            self.get_default_constraint_package(),
            cast(Type["A_"], source_class),
        )

    def for_each_unique_pair(
        self, source_class: Type[A_], *joiners: "BiJoiner[A_, A_]"
        ...
        return BiConstraintStream(
            self.delegate.forEachUniquePair(
                source_class, extract_joiners(joiners, source_class, source_class)
            ),
            self.get_default_constraint_package(),
            cast(Type["A_"], source_class),
            cast(Type["A_"], source_class),

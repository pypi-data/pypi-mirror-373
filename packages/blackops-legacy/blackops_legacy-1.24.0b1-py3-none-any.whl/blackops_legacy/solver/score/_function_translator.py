from .._jpype_type_conversions import (
    _convert_to_java_compatible_object,
    PythonFunction,
    PythonBiFunction,
    PythonTriFunction,
    PythonQuadFunction,
    PythonPentaFunction,
    PythonToIntFunction,
    PythonToIntBiFunction,
    PythonToIntTriFunction,
    PythonToIntQuadFunction,
    PythonToLongFunction,
    PythonToLongBiFunction,
    PythonToLongTriFunction,
    PythonToLongQuadFunction,
    PythonPredicate,
    PythonBiPredicate,
    PythonTriPredicate,
    PythonQuadPredicate,
...
def function_cast(function, *type_args, return_type=None):
    ...
    if len(type_args) != arg_count:
        raise ValueError(
            f"Invalid function: expected {len(type_args)} arguments but got {arg_count}"
        )
    ...
    from ai.timefold.solver.core.api.function import (
        TriFunction,
        QuadFunction,
        PentaFunction,
    )
    ...
    try:
        ...
        if arg_count == 1:
            return translate_python_bytecode_to_java_bytecode(
                function, Function, *type_args, return_type
            )
        elif arg_count == 2:
            return translate_python_bytecode_to_java_bytecode(
                function, BiFunction, *type_args, return_type
            )
        elif arg_count == 3:
            return translate_python_bytecode_to_java_bytecode(
                function, TriFunction, *type_args, return_type
            )
        elif arg_count == 4:
            return translate_python_bytecode_to_java_bytecode(
                function, QuadFunction, *type_args, return_type
            )
        elif arg_count == 5:
            return translate_python_bytecode_to_java_bytecode(
                function, PentaFunction, *type_args, return_type
            )
    except:  # noqa
        return default_function_cast(function, arg_count)
...
def default_function_cast(function, arg_count):
    if arg_count == 1:
        ...
        return PythonBiFunction(
            lambda a, b: _convert_to_java_compatible_object(function(a, b))
        )
    elif arg_count == 3:
        return PythonTriFunction(
            lambda a, b, c: _convert_to_java_compatible_object(function(a, b, c))
        )
    elif arg_count == 4:
        return PythonQuadFunction(
            lambda a, b, c, d: _convert_to_java_compatible_object(function(a, b, c, d))
        )
    elif arg_count == 5:
        return PythonPentaFunction(
            lambda a, b, c, d, e: _convert_to_java_compatible_object(
                function(a, b, c, d, e)
            )
        )
    else:
...
def predicate_cast(predicate, *type_args):
    ...
    try:
        ...
        if arg_count == 1:
            return translate_python_bytecode_to_java_bytecode(
                predicate, Predicate, *type_args
            )
        elif arg_count == 2:
            return translate_python_bytecode_to_java_bytecode(
                predicate, BiPredicate, *type_args
            )
        elif arg_count == 3:
            return translate_python_bytecode_to_java_bytecode(
                predicate, TriPredicate, *type_args
            )
        elif arg_count == 4:
            return translate_python_bytecode_to_java_bytecode(
                predicate, QuadPredicate, *type_args
            )
        elif arg_count == 5:
            return translate_python_bytecode_to_java_bytecode(
                predicate, PentaPredicate, *type_args
            )
    except:  # noqa
...
def to_int_function_cast(function, *type_args):
    ...
    try:
        ...
        if arg_count == 1:
            return translate_python_bytecode_to_java_bytecode(
                function, ToIntFunction, *type_args
            )
        elif arg_count == 2:
            return translate_python_bytecode_to_java_bytecode(
                function, ToIntBiFunction, *type_args
            )
        elif arg_count == 3:
            return translate_python_bytecode_to_java_bytecode(
                function, ToIntTriFunction, *type_args
            )
        elif arg_count == 4:
            return translate_python_bytecode_to_java_bytecode(
                function, ToIntQuadFunction, *type_args
            )
    except:  # noqa
        return default_to_int_function_cast(function, arg_count)
...
def default_to_int_function_cast(function, arg_count):
    if arg_count == 1:
        return PythonToIntFunction(
            lambda a: _convert_to_java_compatible_object(function(a))
        )
    elif arg_count == 2:
        return PythonToIntBiFunction(
            lambda a, b: _convert_to_java_compatible_object(function(a, b))
        )
    elif arg_count == 3:
        return PythonToIntTriFunction(
            lambda a, b, c: _convert_to_java_compatible_object(function(a, b, c))
        )
    elif arg_count == 4:
        return PythonToIntQuadFunction(
            lambda a, b, c, d: _convert_to_java_compatible_object(function(a, b, c, d))
        )
    else:
        raise ValueError(f"Unexpected argument count: {arg_count}")
...
def to_long_function_cast(function, *type_args):
    ...
    try:
        ...
        if arg_count == 1:
            return translate_python_bytecode_to_java_bytecode(
                function, ToLongFunction, *type_args
            )
        elif arg_count == 2:
            return translate_python_bytecode_to_java_bytecode(
                function, ToLongBiFunction, *type_args
            )
        elif arg_count == 3:
            return translate_python_bytecode_to_java_bytecode(
                function, ToLongTriFunction, *type_args
            )
        elif arg_count == 4:
            return translate_python_bytecode_to_java_bytecode(
                function, ToLongQuadFunction, *type_args
            )
    except:  # noqa
        return default_to_long_function_cast(function, arg_count)
...
def default_to_long_function_cast(function, arg_count):
    if arg_count == 1:
        return PythonToLongFunction(
            lambda a: _convert_to_java_compatible_object(function(a))
        )
    elif arg_count == 2:
        return PythonToLongBiFunction(
            lambda a, b: _convert_to_java_compatible_object(function(a, b))
        )
    elif arg_count == 3:
        return PythonToLongTriFunction(
            lambda a, b, c: _convert_to_java_compatible_object(function(a, b, c))
        )
    elif arg_count == 4:
        return PythonToLongQuadFunction(
            lambda a, b, c, d: _convert_to_java_compatible_object(function(a, b, c, d))
        )
    else:
        raise ValueError(f"Unexpected argument count: {arg_count}")

...
import blackops_legacy.solver._blackops_java_interop as _blackops_java_interop
...
@JImplements(
    "ai.timefold.solver.core.api.score.stream.ConstraintProvider", deferred=True
)
...
class PythonToIntPentaFunction:
    ...
    def applyAsInt(self, argument1, argument2, argument3, argument4, argument5):
        return JInt(
            self.delegate(argument1, argument2, argument3, argument4, argument5)
        )
...
def to_python_score(score) -> "Score":
    if isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["SimpleScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict["SimpleScore"](
            score.score()
        )
    elif isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["HardSoftScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict["HardSoftScore"](
            score.hardScore(), score.softScore()
        )
    elif isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["HardMediumSoftScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict["HardMediumSoftScore"](
            score.hardScore(), score.mediumScore(), score.softScore()
        )
    elif isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["BendableScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict["BendableScore"](
            score.hardScores(), score.softScores()
        )
    elif isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["SimpleDecimalScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict["SimpleDecimalScore"](
            Decimal(score.score().toPlainString())
        )
    elif isinstance(
        score, _blackops_java_interop._java_score_mapping_dict["HardSoftDecimalScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict[
        ...
        score,
        _blackops_java_interop._java_score_mapping_dict["HardMediumSoftDecimalScore"],
    ):
        return _blackops_java_interop._python_score_mapping_dict[
        ...
        score, _blackops_java_interop._java_score_mapping_dict["BendableDecimalScore"]
    ):
        return _blackops_java_interop._python_score_mapping_dict[
            "BendableDecimalScore"
        ](
            [Decimal(part.toPlainString()) for part in score.hardScores()],
            [Decimal(part.toPlainString()) for part in score.softScores()],
        )
    else:
...
def _convert_type_to_class(jcls, type_obj):
    ...
    if out == Object and type_obj != Object:
        raise ValueError(
            f"Type {type_obj} does not have a Java class proxy. Maybe annotate it with "
...
def _convert_function_to_class(jcls, function_obj):
    ...
    if out == Object and function_obj != Object:
        raise ValueError(
            f"Function {function_obj} does not have a Java class proxy. Maybe annotate it with "

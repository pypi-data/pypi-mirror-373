import blackops_legacy.solver.score as score_api
...
if TYPE_CHECKING:
    ...
    from ai.timefold.solver.core.api.score.stream.uni import (
        UniConstraintBuilder as _JavaUniConstraintBuilder,
    )
    from ai.timefold.solver.core.api.score.stream.bi import (
        BiConstraintBuilder as _JavaBiConstraintBuilder,
    )
    from ai.timefold.solver.core.api.score.stream.tri import (
        TriConstraintBuilder as _JavaTriConstraintBuilder,
    )
    from ai.timefold.solver.core.api.score.stream.quad import (
        QuadConstraintBuilder as _JavaQuadConstraintBuilder,
    )
...
class UniConstraintBuilder(Generic[A, ScoreType]):
    ...
    def __init__(
        self, delegate: "_JavaUniConstraintBuilder[A, ScoreType]", a_type: Type[A]
    ) -> None:
        self.delegate = delegate
        self.a_type = a_type

    def indict_with(
        self, indictment_function: Callable[[A], Collection]
        ...
        return UniConstraintBuilder(
            self.delegate.indictWith(function_cast(indictment_function, self.a_type)),
            self.a_type,
        )

    def justify_with(
        self,
        justification_function: Callable[
            [A, ScoreType], "score_api.ConstraintJustification"
        ],
    ) -> "UniConstraintBuilder[A, ScoreType]":
        """
        ...
        return UniConstraintBuilder(
            self.delegate.justifyWith(function_cast(wrapped, self.a_type, Score)),
            self.a_type,
        )
    ...
    def as_constraint(
        ...
        if constraint_description is None:
            if constraint_group is None:
                ...
                return self.delegate.asConstraintDescribed(
                    constraint_name, "", constraint_group
                )
            ...
            return self.delegate.asConstraintDescribed(
                constraint_name, constraint_description, constraint_group
            )
...
class BiConstraintBuilder(Generic[A, B, ScoreType]):
    ...
    def indict_with(
        self, indictment_function: Callable[[A, B], Collection]
        ...
        return BiConstraintBuilder(
            self.delegate.indictWith(
                function_cast(indictment_function, self.a_type, self.b_type)
            ),
            self.a_type,
            self.b_type,
        )
    ...
    def justify_with(
        self,
        justification_function: Callable[
            [A, B, ScoreType], "score_api.ConstraintJustification"
        ],
    ) -> "BiConstraintBuilder[A, B, ScoreType]":
        """
        Sets a custom function to apply on a constraint match to justify it.
        ...
        return BiConstraintBuilder(
            self.delegate.justifyWith(
                function_cast(wrapped, self.a_type, self.b_type, Score)
            ),
            self.a_type,
            self.b_type,
        )
...
class TriConstraintBuilder(Generic[A, B, C, ScoreType]):
    ...
    def indict_with(
        self, indictment_function: Callable[[A, B, C], Collection]
        ...
        return TriConstraintBuilder(
            self.delegate.indictWith(
                function_cast(
                    indictment_function, self.a_type, self.b_type, self.c_type
                )
            ),
            self.a_type,
            self.b_type,
    ...
    def justify_with(
        self,
        justification_function: Callable[
            [A, B, C, ScoreType], "score_api.ConstraintJustification"
        ],
    ) -> "TriConstraintBuilder[A, B, C, ScoreType]":
        """
        Sets a custom function to apply on a constraint match to justify it.
...
class QuadConstraintBuilder(Generic[A, B, C, D, ScoreType]):
    ...
    def indict_with(
        self, indictment_function: Callable[[A, B, C, D], Collection]
    ...
    def justify_with(
        self,
        justification_function: Callable[
            [A, B, C, D, ScoreType], "score_api.ConstraintJustification"
        ],
    ) -> "QuadConstraintBuilder[A, B, C, D, ScoreType]":
        """
        Sets a custom function to apply on a constraint match to justify it.
        ...
        return QuadConstraintBuilder(
            self.delegate.justifyWith(
                function_cast(
                    wrapped, self.a_type, self.b_type, self.c_type, self.d_type, Score
                )
            ),
            self.a_type,
            self.b_type,

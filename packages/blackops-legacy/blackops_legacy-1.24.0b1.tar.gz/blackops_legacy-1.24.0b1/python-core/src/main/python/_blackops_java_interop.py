...
from ._jpype_type_conversions import (
    PythonSupplier,
...
def extract_timefold_jars() -> list[str]:
    ...
    try:
        ...
        enterprise_dependencies = [
            str(get_path("blackops_legacy.solver.enterprise.jars", p.name).__enter__())
            for p in importlib.resources.files(
                "blackops_legacy.solver.enterprise.jars"
            ).iterdir()
    ...
    return [
        str(get_path("blackops_legacy.solver.jars", p.name).__enter__())
        for p in importlib.resources.files("blackops_legacy.solver.jars").iterdir()
        if p.name.endswith(".jar")
    ] + enterprise_dependencies
...
def init(*args, path: list[str] = None, include_timefold_jars: bool = True) -> None:
    ...
    if jpype.isJVMStarted():  # noqa
        raise RuntimeError(
            "JVM already started. Maybe call init before blackops_legacy.solver.types imports?"
        )
...
def register_python_java_type_mappings():
    ...
    from ai.timefold.solver.core.api.score.buildin.simplelong import (
        SimpleLongScore as _SimpleScore,
    )
    from ai.timefold.solver.core.api.score.buildin.hardsoftlong import (
        HardSoftLongScore as _HardSoftScore,
    )
    from ai.timefold.solver.core.api.score.buildin.hardmediumsoftlong import (
        HardMediumSoftLongScore as _HardMediumSoftScore,
    )
    from ai.timefold.solver.core.api.score.buildin.bendablelong import (
        BendableLongScore as _BendableScore,
    )

    from ai.timefold.solver.core.api.score.buildin.simplebigdecimal import (
        SimpleBigDecimalScore as _SimpleDecimalScore,
    )
    from ai.timefold.solver.core.api.score.buildin.hardsoftbigdecimal import (
        HardSoftBigDecimalScore as _HardSoftDecimalScore,
    )
    from ai.timefold.solver.core.api.score.buildin.hardmediumsoftbigdecimal import (
        HardMediumSoftBigDecimalScore as _HardMediumSoftDecimalScore,
    )
    from ai.timefold.solver.core.api.score.buildin.bendablebigdecimal import (
        BendableBigDecimalScore as _BendableDecimalScore,
    )

    from ai.timefold.solver.python.score import (
        SimpleScorePythonJavaTypeMapping,
    ...
    HardSoftDecimalScoreType = translate_python_class_to_java_class(
        HardSoftDecimalScore
    )
    HardMediumSoftDecimalScoreType = translate_python_class_to_java_class(
        HardMediumSoftDecimalScore
    )
    BendableDecimalScoreType = translate_python_class_to_java_class(
        BendableDecimalScore
    )

    ConstraintWeightOverridesType = translate_python_class_to_java_class(
    ...
    add_python_java_type_mapping(
        HardMediumSoftScorePythonJavaTypeMapping(HardMediumSoftScoreType)
    )
    add_python_java_type_mapping(BendableScorePythonJavaTypeMapping(BendableScoreType))

    add_python_java_type_mapping(
        SimpleDecimalScorePythonJavaTypeMapping(SimpleDecimalScoreType)
    )
    add_python_java_type_mapping(
        HardSoftDecimalScorePythonJavaTypeMapping(HardSoftDecimalScoreType)
    )
    add_python_java_type_mapping(
        HardMediumSoftDecimalScorePythonJavaTypeMapping(HardMediumSoftDecimalScoreType)
    )
    add_python_java_type_mapping(
        BendableDecimalScorePythonJavaTypeMapping(BendableDecimalScoreType)
    )

    add_python_java_type_mapping(
        ConstraintWeightOverridesTypeMapping(ConstraintWeightOverridesType)
    )


def forward_logging_events(event: "PythonLoggingEvent") -> None:
    logger.log(event.level().getPythonLevelNumber(), event.message())
...
def set_class_output_directory(path: pathlib.Path):
    ...
    from ai.timefold.jpyinterpreter import (
...
def get_asm_type(python_class: Union[type, Callable]) -> Any:
    ...
    if hasattr(python_class, "_timefold_java_class"):
        return AnnotationMetadata.getValueAsType(
            python_class._timefold_java_class.getName()
        )
    if isinstance(python_class, type):
        return AnnotationMetadata.getValueAsType(
            get_java_type_for_python_type(python_class).getJavaTypeInternalName()
        )
    if is_c_native(python_class):
...
def register_java_class(python_object: Solution_, java_class: JClass) -> Solution_:
...
def _compose_unique_class_name(class_identifier: str):
    ...
    number_of_instances = (
        PythonBytecodeToJavaBytecodeTranslator.classNameToSharedInstanceCount.merge(
...
class OverrideClassLoader:
    ...
    def __enter__(self):
        ...
        class_loader = PythonWrapperGenerator.getClassLoaderForAliasMap(
...
def _generate_constraint_provider_class(
    original_function: Callable[["_ConstraintFactory"], list["_Constraint"]],
    ...
    out = PythonWrapperGenerator.defineConstraintProviderClass(
        ...
        JObject(
            ConstraintProviderFunction(
                lambda cf: _to_constraint_java_array(wrapped_constraint_provider(cf))
            ),
            ConstraintProvider,
        ),
    )

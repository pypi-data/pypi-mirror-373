...
from .._blackops_java_interop import ensure_init, register_java_class

Origin_ = TypeVar("Origin_")
Destination_ = TypeVar("Destination_")


def nearby_distance_meter(
    distance_function: Callable[[Origin_, Destination_], float], /
    ...
    from ai.timefold.solver.core.impl.heuristic.selector.common.nearby import (
        NearbyDistanceMeter,
    )  # noqa
    ...
    java_class = generate_proxy_class_for_translated_function(
        NearbyDistanceMeter,
        translate_python_bytecode_to_java_bytecode(
            distance_function, NearbyDistanceMeter
        ),
    )

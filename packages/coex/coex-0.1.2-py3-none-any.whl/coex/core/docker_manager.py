...
class DockerManager:
    ...
    def _create_multi_language_container(self) -> Container:
        ...
        try:
            ...
            # Install language tools in the container
    ...
    def _setup_language_tools(self, container: Container) -> None:
        ...
        """
        try:
            logger.info("Installing language tools in container...")
            ...
            for cmd in setup_commands:
                exit_code, stdout, stderr = self.execute_in_container(
                    container, ["bash", "-c", cmd], timeout=300
                )
                if exit_code != 0:
                    logger.warning(f"Setup command failed: {cmd}, stderr: {stderr}")

            logger.info("Language tools installation completed")

        except Exception as e:
            logger.error(f"Failed to setup language tools: {e}")

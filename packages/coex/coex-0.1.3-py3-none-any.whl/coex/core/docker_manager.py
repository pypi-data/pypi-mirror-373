...
class DockerManager:
    ...
    def _setup_language_tools(self, container: Container) -> None:
        ...
        """
        try:
            logger.info("Installing basic language tools in container...")

            # Install only essential tools first
            basic_setup = [
                "apt-get update",
                "apt-get install -y curl wget build-essential",
            ]

            for cmd in basic_setup:
                try:
                    exit_code, stdout, stderr = self.execute_in_container(
                        container, ["bash", "-c", cmd]
                    )
                    if exit_code != 0:
                        logger.warning(f"Basic setup command failed: {cmd}")
                except Exception as e:
                    logger.warning(f"Failed to run setup command '{cmd}': {e}")

            logger.info("Basic language tools installation completed")

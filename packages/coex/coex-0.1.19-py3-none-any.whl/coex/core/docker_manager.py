...
class DockerManager:
    ...
    def get_or_create_container(self) -> Container:
        """
        Get or create the single persistent container.

        AGGRESSIVE CONTAINER PERSISTENCE: This method will do everything possible
        to reuse existing containers and avoid creating new ones.

        Returns:
            Docker container ready for code execution
        """
        with self._lock:
            # STEP 1: Try to use current container if it exists
            if self._container is not None:
                try:
                    self._container.reload()
                    if self._container.status == "running":
                        logger.debug(f"Reusing existing container {self._container.short_id}")
                        return self._container
                    else:
                        logger.info(f"Container {self._container.short_id} stopped, will try to recover or create new")
                        self._container = None
                except Exception as e:
                    logger.debug(f"Current container check failed: {e}")
                    self._container = None

            # STEP 2: Try to recover from stored container ID
            if self._container is None and self._container_id:
                logger.debug(f"Attempting to recover container {self._container_id}")
                self._container = self._try_recover_existing_container()
                if self._container:
                    logger.info(f"Successfully recovered container {self._container.short_id}")
                    return self._container

            # STEP 3: Try to find ANY running coex container
            if self._container is None:
                logger.debug("Searching for any existing coex containers")
                try:
                    containers = self.client.containers.list(filters={"label": "coex=true"})
                    for container in containers:
                        if container.status == "running":
                            logger.info(f"Found existing coex container {container.short_id}, adopting it")
                            self._container = container
                            self._container_id = container.id
                            self._save_container_state()
                            return self._container
                except Exception as e:
                    logger.debug(f"Failed to search for existing containers: {e}")

            # STEP 4: Only create new container if absolutely necessary
            logger.info("No existing containers found, creating new persistent container")
            ...
            logger.info(f"Created new container {self._container.short_id}")
            return self._container
    ...
    def _create_multi_language_container(self) -> Container:
        ...
        try:
            ...
            # Simplified container configuration with coex label
            container_config = {
                "image": image_name,
                "detach": True,
                "tty": True,
                ...
                "labels": {
                    "coex": "true",
                    "coex.version": "persistent",
                    "coex.created": str(int(time.time()))
                }
            }
...
def rm_docker() -> None:
    ...
    # DON'T reset global Docker manager instance to prevent recreation issues
    # The manager will create new containers when needed
    # global _docker_manager
    # _docker_manager = None

    logger.info("All Docker containers removed (manager preserved)")

...
from .subprocess_executor import get_subprocess_executor, cleanup_subprocess_executor
...
class CodeExecutor:
    ...
    def __init__(self):
        ...
        self.subprocess_executor = get_subprocess_executor()
    ...
    def execute(self,
        ...
        try:
            ...
            if execution_mode == "answer":
                return self._execute_input_output_mode_subprocess(inputs, outputs, code, language, timeout)
            elif execution_mode == "function":
                return self._execute_function_comparison_mode_subprocess(answer_fn, code, language, timeout)
            else:
                # Simple execution mode
                return self._execute_simple_mode_subprocess(code, language, timeout)
...
"""
    ...
    def _execute_input_output_mode_subprocess(self, inputs: List[Any], outputs: List[Any],
                                            code: str, language: str,
                                            timeout: Optional[int]) -> List[int]:
        """Execute code with input/output validation using subprocess."""
        results = []

        print(f"[INFO] Executing {len(inputs)} test cases using host system {language}")

        for i, (input_val, expected_output) in enumerate(zip(inputs, outputs)):
            try:
                # Execute code with input using subprocess
                actual_output = self.subprocess_executor.execute_code(
    ...
    def _execute_function_comparison_mode_subprocess(self, answer_fn: str, code: str,
                                                   language: str, timeout: Optional[int]) -> List[int]:
        """Execute function comparison mode using subprocess."""
        try:
            print(f"[INFO] Executing function comparison using host system {language}")

            # For now, just execute the code and return success if no error
            self.subprocess_executor.execute_code(code, language, None, timeout)
            return [1]
    ...
    def _execute_simple_mode_subprocess(self, code: str, language: str,
                                      timeout: Optional[int]) -> List[int]:
        """Execute simple mode using subprocess."""
        try:
            print(f"[INFO] Executing code using host system {language}")
...
def rm_docker() -> None:
    ...
    try:
        ...
        # Also cleanup subprocess executor
        cleanup_subprocess_executor()

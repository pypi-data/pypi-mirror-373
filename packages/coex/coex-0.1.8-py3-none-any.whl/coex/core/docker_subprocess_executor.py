...
"""
            ...
            # Java test code generation - simplified approach
            if input_val is not None:
                # Check if main method already exists
                if "public static void main" in code:
                    # Code already has main method, use as-is
                    # For coex testing, we expect the main method to handle the logic
                    test_code = code
                else:
                    # No main method, add one that calls doubleValue
                    lines = code.strip().split('\n')
                    for i in range(len(lines) - 1, -1, -1):
                        if '}' in lines[i]:
                            main_method = f"    public static void main(String[] args) {{\n        System.out.println(doubleValue({input_val}));\n    }}"

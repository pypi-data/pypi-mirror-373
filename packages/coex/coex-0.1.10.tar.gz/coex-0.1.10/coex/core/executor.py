...
"""
    ...
    def _compare_function_outputs(self, answer_output: str, code_output: str) -> bool:
        ...
        try:
            # Normalize outputs by stripping whitespace and converting to lowercase
            answer_normalized = str(answer_output).strip().lower()
            code_normalized = str(code_output).strip().lower()

            # Direct string comparison for print outputs
            ...
            try:
                ...
                return abs(answer_num - code_num) < 1e-9
            ...
            try:
                ...
                return answer_parsed == code_parsed
    ...
    def _execute_function_comparison_mode_docker(self, answer_fn: str, code: str,
        ...
        try:
            ...
            answer_output = self.docker_subprocess_executor.execute_code(answer_fn, language, None, timeout)
            code_output = self.docker_subprocess_executor.execute_code(code, language, None, timeout)

            # Compare the outputs (including print statements)
            if self._compare_function_outputs(answer_output, code_output):
                return [1]
            else:

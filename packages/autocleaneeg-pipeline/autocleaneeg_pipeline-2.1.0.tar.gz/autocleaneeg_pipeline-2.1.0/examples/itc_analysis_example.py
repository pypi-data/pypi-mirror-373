...
class StatisticalLearningITCTask(Task):
    ...
    def run(self):
        ...
        if band_results:
            ...
            print(f"  Significant time-freq points: {np.sum(significant_mask)}/{significant_mask.size}")
...
config = {
    ...
    "output": {
        "save_stages": ["raw", "epochs", "itc_analysis"]
    }
}
...
if __name__ == "__main__":
    ...
    print("from autoclean.functions.analysis import compute_statistical_learning_itc, calculate_word_learning_index")
    print("from autoclean.functions.analysis.statistical_learning import validate_itc_significance")
    print("")
    print("# Create epochs and compute ITC")
    print("epochs = create_statistical_learning_epochs(raw_data, num_syllables=30)")
    print("power, itc = compute_statistical_learning_itc(epochs)  # Uses 0.6-5 Hz range")
    print("")
    print("# Calculate Word Learning Index (WLI = ITC_word / ITC_syllable)")
    print("wli_results = calculate_word_learning_index(itc, word_freq=1.11, syllable_freq=3.33)")
    print("print(f'Word Learning Index: {wli_results[\"wli_mean\"]:.4f}')")
    print("")
    print("# Test significance")
    print("significant_mask, threshold = validate_itc_significance(itc.data, len(epochs))")

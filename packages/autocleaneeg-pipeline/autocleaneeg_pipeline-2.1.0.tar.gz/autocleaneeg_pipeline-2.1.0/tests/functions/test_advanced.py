...
class TestAutoRejectEpochs:
    ...
    def test_autoreject_epochs_basic(self):
        ...
        with patch('autoclean.functions.advanced.autoreject.AutoReject') as mock_ar:
            ...
            mock_instance.bad_segments_ = np.zeros((len(mock_cleaned_epochs), len(epochs.ch_names)), dtype=bool)
    ...
    def test_autoreject_epochs_custom_parameters(self):
        ...
        with patch('autoclean.functions.advanced.autoreject.AutoReject') as mock_ar:
            ...
            mock_instance.bad_segments_ = np.zeros((len(epochs), len(epochs.ch_names)), dtype=bool)
    ...
    def test_autoreject_epochs_invalid_input(self):
        ...
        with pytest.raises(ValueError, match="Need at least.*epochs for.*cross-validation"):
        ...
        with pytest.raises(ValueError, match="Cannot interpolate more channels"):
            autoreject_epochs(epochs, n_interpolate=[100])  # More than available channels
    ...
    def test_autoreject_epochs_with_picks(self):
        ...
        with patch('autoclean.functions.advanced.autoreject.AutoReject') as mock_ar:
            ...
            mock_instance.bad_segments_ = np.zeros((len(picked_epochs), len(picks)), dtype=bool)
...
class TestAnnotateNoisySegments:
    ...
    def test_annotate_noisy_segments_parameters(self):
        ...
        custom_annotations = [ann for ann in result_raw.annotations 
    ...
    def test_annotate_noisy_segments_invalid_input(self):
        ...
        with pytest.raises(ValueError, match="quantile_flag_crit must be between 0 and 1"):
...
class TestAnnotateUncorrelatedSegments:
    ...
    def test_annotate_uncorrelated_segments_invalid_input(self):
        ...
        with pytest.raises(ValueError, match="corr_trim_percent must be between 0 and 50"):
            annotate_uncorrelated_segments(raw, corr_trim_percent=60.0)
        
        with pytest.raises(ValueError, match="outlier_flag_crit must be between 0 and 1"):
    ...
    def test_annotate_uncorrelated_segments_no_montage(self):
        ...
        with pytest.raises(RuntimeError, match="Failed to annotate uncorrelated segments"):

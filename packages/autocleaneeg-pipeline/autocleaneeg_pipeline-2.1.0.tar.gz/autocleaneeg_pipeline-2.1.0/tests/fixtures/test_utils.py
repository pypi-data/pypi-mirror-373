...
class EEGAssertions:
    ...
    @staticmethod
    def assert_raw_properties(raw: Raw, expected_sfreq: Optional[float] = None,
        ...
        assert hasattr(raw, 'info') and hasattr(raw, 'get_data'), f"Expected Raw-like object, got {type(raw)}"
    ...
    @staticmethod
    def assert_epochs_properties(epochs, expected_n_epochs: Optional[int] = None,
        ...
        """
        assert hasattr(epochs, 'get_data'), f"Expected Epochs object, got {type(epochs)}"
        ...
        if expected_tmin is not None:
            assert abs(epochs.tmin - expected_tmin) < 0.01, \
                f"Expected tmin {expected_tmin}, got {epochs.tmin}"
        
        if expected_tmax is not None:
            assert abs(epochs.tmax - expected_tmax) < 0.01, \
                f"Expected tmax {expected_tmax}, got {epochs.tmax}"
    
    @staticmethod
    def assert_data_quality(raw: Raw, max_amplitude: float = 500e-6,
                           min_good_channels: Optional[int] = None):
        ...
        if min_good_channels is not None:
            ...
            assert n_good_channels >= min_good_channels, \

...
class CLIDisplay:
    ...
    def header(self, title: str, subtitle: Optional[str] = None, style: str = "bold blue") -> None:
    ...
    def list_item(self, message: str, value: Optional[str] = None, indent: int = 2) -> None:
    ...
    def key_value(self, key: str, value: str, key_style: str = "bold", value_style: str = "dim") -> None:
        """Display a key-value pair."""
        self.console.print(f"[{key_style}]{key}:[/{key_style}] [{value_style}]{value}[/{value_style}]")
    ...
    def workspace_info(self, workspace_path: Path, is_valid: bool = True) -> None:
        ...
        status_text = "[bold green]properly configured[/bold green]" if is_valid else "[bold yellow]needs setup[/bold yellow]"
        
        self.console.print(f"{status_icon} Workspace is {status_text}")
        self.console.print()
        self.console.print(f"[bold cyan]Location:[/bold cyan] [bright_blue]{workspace_path}[/bright_blue]")
        self.console.print()
    ...
    def system_info_table(self, info: Dict[str, str]) -> None:
        ...
        for key, value in info.items():
            ...
            if "GPU" in key and ("✓" in value or "Apple" in value or "NVIDIA" in value or "CUDA" in value):
    ...
    def setup_complete(self, workspace_path: Path, additional_info: Optional[List[str]] = None) -> None:
        ...
        if additional_info:
            ...
            for info in additional_info:
                self.console.print(f"  [green]•[/green] [bright_white]{info}[/bright_white]")
        self.blank_line()
    ...
    def migration_prompt(self, old_path: Path, new_path: Path) -> bool:
        ...
        self.console.print(f"[yellow]From:[/yellow] [dim bright_white]{old_path}[/dim bright_white]")
        self.console.print(f"[green]To:[/green]   [bright_blue]{new_path}[/bright_blue]")
        self.console.print()
        
        return self.prompt_yes_no("[bold]Migrate existing tasks and configuration?[/bold]", default=False)
...
class SetupDisplay(CLIDisplay):
    ...
    def workspace_location_prompt(self, default_dir: Path) -> Path:
        ...
        self.console.print(f"[dim bright_white]Default: [/dim bright_white][bright_blue]{default_dir}[/bright_blue]")
        self.console.print("[dim green]• Custom tasks  • Configuration  • Results  • Easy backup[/dim green]")
        self.blank_line()
    ...
    def compliance_status_display(self, is_enabled: bool, is_permanent: bool) -> None:
        ...
        if is_permanent:
            ...
            self.console.print("[dim cyan]Current compliance mode: [/dim cyan][dim yellow]disabled[/dim yellow]")
        self.blank_line()
    ...
    def setup_complete_summary(self, workspace_path: Path) -> None:
        ...
        for file_info in created_files:
            self.console.print(f"  [green]•[/green] [bright_white]{file_info}[/bright_white]")

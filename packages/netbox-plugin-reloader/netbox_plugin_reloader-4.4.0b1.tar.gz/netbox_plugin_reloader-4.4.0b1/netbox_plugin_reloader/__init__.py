...
class NetboxPluginReloaderConfig(PluginConfig):
    ...
    def ready(self):
        ...
        from utilities.forms.fields import ContentTypeMultipleChoiceField, ContentTypeChoiceField
        ...
        # Refresh form fields for all forms that use ObjectType.objects.with_feature()
        ...
        # Additional forms that use ObjectType.objects.with_feature()
        try:
            from extras.forms.model_forms import ExportTemplateForm

            self._refresh_form_field(
                ExportTemplateForm, "export_templates", ObjectType, ContentTypeMultipleChoiceField, _
            )
        except ImportError:
            pass

        try:
            from extras.forms.model_forms import EventRuleForm

            self._refresh_form_field(EventRuleForm, "event_rules", ObjectType, ContentTypeMultipleChoiceField, _)
        except ImportError:
            pass

        try:
            from extras.forms.model_forms import CustomLinkForm

            self._refresh_form_field(CustomLinkForm, "custom_links", ObjectType, ContentTypeMultipleChoiceField, _)
        except ImportError:
            pass

        try:
            from extras.forms.model_forms import SubscriptionForm
            self._refresh_form_field(SubscriptionForm, "notifications", ObjectType, ContentTypeChoiceField, _, base_fields_name = "object_type")
        except ImportError:
            pass
            
        try:
            from extras.forms.model_forms import BookmarkForm
            self._refresh_form_field(BookmarkForm, "bookmarks", ObjectType, ContentTypeChoiceField, _, base_fields_name = "object_type")
        except ImportError:
            pass
    ...
    def _refresh_form_field(self, form_class, feature_name, object_type_class, field_class, translation_function, base_fields_name = "object_types"):
        """
        ...
        field_labels = {
            ...
            "export_templates": ("Object types", "The type(s) of object that can use this export template"),
            "event_rules": ("Object types", "The type(s) of object that can trigger this event rule"),
            "custom_links": ("Object types", "The type(s) of object that can have this custom link"),
            "notifications": ("Object type", "The type of object for this notification subscription"),
            "bookmarks": ("Object type", "The type of object that can be bookmarked"),
        }
        ...
        form_class.base_fields[base_fields_name] = object_types_field

...
{% macro starrocks__olap_table(is_create_table_as) -%}
  ...
  {%- set is_auto_partition = config.get('is_auto_partition') -%}
  ...
  {%- if partition_by is not none -%}
    {{ starrocks__partition_by(partition_type, partition_by, partition_by_init, is_auto_partition) }}
  {%- endif -%}
...
{%- macro starrocks__partition_by(p_type, cols, init, is_auto_partition=False) -%}
  {%- if p_type == 'Expr' %}
    {%- if cols | length != 1 -%}
        {%- set msg -%}
    ...
    PARTITION BY {{ cols[0] }}
  {%- else -%}
      {%- if is_auto_partition and init is none -%}
         {% set auto_partition = 'AUTO' %}
      {% endif %}
      {%- if cols is not none %}
        {{ auto_partition }} PARTITION BY {{ p_type }} (
          {%- for col in cols -%}

...
$BLOCK_START
ai() {
  if command -v termai >/dev/null 2>&1; then
    termai "\$@"; return
  fi
  if python -c 'import termai' >/dev/null 2>&1; then
    python -m termai.cli "\$@"; return
  fi
    local repo="\${TERMAI_REPO:-${DEFAULT_REPO}}"
    if [ -n "\$repo" ] && [ -d "\$repo" ] && { [ -f "\$repo/pyproject.toml" ] || [ -f "\$repo/setup.py" ]; }; then
        uvx --from "\$repo" termai "\$@"
    else
        uvx termai "\$@"
    fi
}

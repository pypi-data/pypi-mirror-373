...
""")
        ...
        # Verify resource file created in environment directory
        resource_file = project_root / "resources" / "lhp" / "dev" / "test_pipeline.pipeline.yml"
        assert resource_file.exists(), "Resource file should be created in dev directory"
        
        # Verify content
        content = resource_file.read_text()
        assert "Generated by LakehousePlumber" in content
        assert "test_pipeline" in content
        assert "dev" in content  # Environment should be in the header
...
""")
        ...
        # Check glob include path
        libraries = pipeline_config["libraries"]
        assert len(libraries) == 1
        glob_include = libraries[0]["glob"]["include"]
        assert glob_include == "../../generated/staging/data_pipeline/**"
        
        # Check root_path
        root_path = pipeline_config["root_path"]
        assert root_path == "${workspace.file_path}/generated/staging/data_pipeline"
        
        # Check bundle.sourcePath
        source_path = pipeline_config["configuration"]["bundle.sourcePath"]
        assert source_path == "${workspace.file_path}/generated/staging"
...
""")
        ...
        # Verify dev resources created
        dev_resources = project_root / "resources" / "lhp" / "dev"
        assert (dev_resources / "pipeline1.pipeline.yml").exists()
        assert (dev_resources / "pipeline2.pipeline.yml").exists()
        assert not (dev_resources / "pipeline3.pipeline.yml").exists()  # prod pipeline shouldn't be in dev
        
        # Run sync for prod
        manager.sync_resources_with_generated_files(prod_dir, "prod")
        
        # Verify prod resources created
        prod_resources = project_root / "resources" / "lhp" / "prod"
        assert (prod_resources / "pipeline3.pipeline.yml").exists()
        assert not (prod_resources / "pipeline1.pipeline.yml").exists()  # dev pipeline shouldn't be in prod
    ...
    def test_template_handles_missing_env_gracefully(self):
        ...
        assert glob_include == "../../generated/test_pipeline/**"  # Should fallback to no env
...
""")
        ...
        # Verify new files created in environment directory
        new_resource_file = project_root / "resources" / "lhp" / "dev" / "pipeline1.pipeline.yml"
        assert new_resource_file.exists(), "New resource file should be created"
        assert "dev" in new_resource_file.read_text(), "New file should have environment reference"

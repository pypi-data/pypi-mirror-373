...
app = Flask(__name__)
architect = Architect(app)  # OpenAPI served at /openapi.json; docs at /docs; docs JSON at /docs/apispec.json

```

The top‑level specification endpoint is configurable via ``API_SPEC_ROUTE`` (default ``/openapi.json``).
The documentation‑scoped JSON used by Redoc/Swagger is configurable via ``API_DOCS_SPEC_ROUTE`` (default ``/docs/apispec.json``).

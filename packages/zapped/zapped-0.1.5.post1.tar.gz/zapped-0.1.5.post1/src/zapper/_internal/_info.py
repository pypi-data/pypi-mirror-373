...
from zapper._internal._version import __commit_id__, __version__, __version_tuple__
...
def _get_version(dist: str = "zapper") -> str:
...
def _get_description(dist: str = "zapper") -> str:
...
class _ProjectName(BaseModel):
    ...
    @property
    def package_distribution(self) -> Literal["zapper"]:
        """Get the package distribution name."""
        return "zapper"

    @property
    def project(self) -> Literal["zapper"]:
        """Get the project name in lowercase with underscores."""
        return "zapper"

    @property
    def project_upper(self) -> Literal["ZAPPER"]:
        """Get the project name in uppercase with underscores."""
        return "ZAPPER"

    @property
    def env_variable(self) -> Literal["ZAPPER_ENV"]:
...
class _ProjectMetadata(BaseModel):
    ...
    version_: _ProjectVersion = Field(default=..., description="Project version metadata.")
    ...
    @property
    def name(self) -> Literal["zapper"]:
        """Get the package distribution name."""
        return self.name_.package_distribution

    @property
    def name_upper(self) -> Literal["ZAPPER"]:
        """Get the project name in uppercase with underscores."""
        return self.name_.project_upper

    @property
    def project_name(self) -> Literal["zapper"]:
...
_METADATA = _ProjectMetadata(
    version_=_ProjectVersion(
        string=__version__ if __version__ != "0.0.0" else _get_version("zapper"),

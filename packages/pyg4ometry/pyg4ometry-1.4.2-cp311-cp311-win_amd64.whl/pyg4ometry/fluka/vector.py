...
class Three(_np.ndarray):
    ...
    def __add__(self, other):
        try:
            ...
            pass
        try:
            return Three(self.x + other[0], self.y + other[1], self.z + other[2])
        except (AttributeError, IndexError, TypeError):
            pass
        return Three(self.x + other, self.y + other, self.z + other)
...
class AABB:
    ...
    def __repr__(self):
        return (
            f"<AABB: Lower({self.lower.x}, {self.lower.y}, {self.lower.z}),"
            f" Upper({self.upper.x}, {self.upper.y}, {self.upper.z})>"
        )
    ...
    def fromMesh(cls, csgmesh):
        ...
        return cls([min(x), min(y), min(z)], [max(x), max(y), max(z)])

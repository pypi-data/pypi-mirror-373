...
PYBIND11_MODULE(Polyhedron_3, m) {
  ...
  py::class_<Polyhedron_3_EPECK>(m, "Polyhedron_3_EPECK")
      .def(py::init<>())
      .def("buildFromVertsAndFaces",
           [](Polyhedron_3_EPECK &p, std::vector<std::vector<double>> &verts,
              std::vector<std::vector<int>> &faces) {
      ...
      .def("facets_begin",
           [](Polyhedron_3_EPECK &p) { return p.facets_begin(); })
      .def("facets_end", [](Polyhedron_3_EPECK &p) { return p.facets_end(); })
      .def("halfedges_begin",
           [](Polyhedron_3_EPECK &p) { return p.halfedges_begin(); })
      .def("halfedges_end",
           [](Polyhedron_3_EPECK &p) { return p.halfedges_end(); })
      // TDOO Many

      /* Types for Tagging Optional Features */
      .def("Supports_facet_plane",
           [](Polyhedron_3_EPECK &p) {
             return Polyhedron_3_EPECK::Supports_facet_plane();
           })
      ...
      .def("normalized_border_is_valid",
           &Polyhedron_3_EPECK::normalized_border_is_valid)

      /* IO */
      .def("write_OFF",
           [](Polyhedron_3_EPECK &p, std::string fileName) {
      ...
      .def("convertToPlanes", [](Polyhedron_3_EPECK &p) {
        ...
        for (auto f = p.facets_begin(); f != p.facets_end(); ++f) {
          auto heh = f->halfedge();
          auto plane = Polyhedron_3_EPECK::Facet::Plane_3(
  ...
  py::class_<Polyhedron_3_EPECK::Facet_iterator>(
      m, "Polyhedron_3_EPECK_Facet_Iterator")
      .def("deref", [](Polyhedron_3_EPECK::Facet_iterator &fi) { return *fi; })
      .def(
          "plane",
          [](Polyhedron_3_EPECK::Facet_iterator &fh) {
            std::cout << CGAL::to_double(fh->plane().a()) << " "
                      << CGAL::to_double(fh->plane().a()) << " "
                      << CGAL::to_double(fh->plane().c()) << " "
                      << CGAL::to_double(fh->plane().d()) << std::endl;
            return fh->plane();
          },
          py::return_value_policy::copy)
      .def("next", [](Polyhedron_3_EPECK::Facet_iterator &fi) { ++fi; })
      .def("__ne__",
           [](Polyhedron_3_EPECK::Facet_iterator fi1,
              Polyhedron_3_EPECK::Facet_iterator fi2) { return fi1 != fi2; });

  py::class_<Polyhedron_3_EPECK::Halfedge_iterator>(
      m, "Polyhedron_3_EPECK_Halfedge_Iterator");

  py::class_<Polyhedron_3_EPICK>(m, "Polyhedron_3_EPICK")
      .def(py::init<>())
      .def("buildFromVertsAndFaces",
           [](Polyhedron_3_EPICK &p, std::vector<std::vector<double>> &verts,
              std::vector<std::vector<int>> &faces) {
      ...
      .def("normalized_border_is_valid",
           &Polyhedron_3_EPICK::normalized_border_is_valid)
  ...
  py::bind_vector<std::vector<Polyhedron_3_EPECK>>(m,

...
vertexfacet_to_polyhedron.argtypes = [
    ...
    _np.ctypeslib.ndpointer(dtype=int, ndim=1),
    _np.ctypeslib.ndpointer(dtype=_np.uintp),
    _ctypes.c_bool,
]
...
convexpolyhedron_to_planes.argtypes = [
    _ctypes.c_void_p,
...
polyhedron_write.argtypes = [_ctypes.c_void_p, _ctypes.c_char_p]

nefpolyhedron_write = _lib.pyg4_cgal_nefpolyhedron_write
nefpolyhedron_write.argtypes = [_ctypes.c_void_p, _ctypes.c_char_p]

surfacemesh_write = _lib.pyg4_cgal_surfacemesh_write
surfacemesh_write.argtypes = [_ctypes.c_void_p, _ctypes.c_char_p]
...
polygon_to_convexpolygons.argtypes = [
    _ctypes.c_void_p,
    _ctypes.c_void_p * 10000,
    _ctypes.POINTER(_ctypes.c_int),
]
...
def pycsgmesh2NefPolyhedron(mesh):
    ...
    polyarraypp = (
        ...
        + _np.arange(polyarray.shape[0]) * polyarray.strides[0]
    ).astype(_np.uintp)

    polyhedron = vertexfacet_to_polyhedron(
        len(verts), len(polys), vertspp, npolyvert, polyarraypp, _ctypes.c_bool(False)
    )
...
def numpyPolygonConvex(polygonnp):
    polygonpp = (
        polygonnp.__array_interface__["data"][0]
        + _np.arange(polygonnp.shape[0]) * polygonnp.strides[0]
    ).astype(_np.uintp)
    ...
    polyvertspp = (
        polyvertsnp.__array_interface__["data"][0]
        + _np.arange(polyvertsnp.shape[0]) * polyvertsnp.strides[0]
    ).astype(_np.uintp)

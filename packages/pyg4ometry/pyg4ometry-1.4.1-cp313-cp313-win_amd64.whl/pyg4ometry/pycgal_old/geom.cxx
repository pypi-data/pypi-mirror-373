//
// clang++ -O3 -Wall -shared -std=c++11 -fPIC `python3 -m pybind11 --includes`
// geom.cxx -o geom`python3-config --extension-suffix`
...
Vector operator-(const Vector &v) { return Vector(-v._x, -v._y, -v._z); }

Vector Vector::operator/(double value) const {
  return Vector(_x / value, _y / value, _z / value);
}

double Vector::dot(const Vector v) { return _x * v._x + _y * v._y + _z * v._z; }
...
double Vector::length() { return sqrt(pow(_x, 2) + pow(_y, 2) + pow(_z, 2)); }

Vector Vector::unit() { return (*this) / this->length(); }
...
py::list Polygon::vertices() { return _vertices; }

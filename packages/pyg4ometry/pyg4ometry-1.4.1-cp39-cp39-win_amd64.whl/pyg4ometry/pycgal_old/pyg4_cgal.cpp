...
template <class HDS>
class Build_Polygon_VertexFacet : public CGAL::Modifier_base<HDS> {
...
public:
  Build_Polygon_VertexFacet(int nVertIn, int nFacetIn, double **vertListIn,
    ...
    if (debugIo)
      std::cout << "Build_Polygon_VertexFacet> " << nVert << " " << nFacet
                << std::endl;
  }
  ...
  void operator()(HDS &hds) {
    ...
    for (int iVert = 0; iVert < nVert; iVert++) {
      B.add_vertex(
          Point(vertList[iVert][0], vertList[iVert][1], vertList[iVert][2]));
      if (debugIo)
        std::cout << "Build_Polygon_VertexFacet> vertex " << iVert << " "
                  << vertList[iVert][0] << " " << vertList[iVert][1] << " "
                  << vertList[iVert][2] << " " << (int)B.error() << std::endl;
    }
      ...
      if (nVertFacet[iFacet] == 3) {
        ...
        if (debugIo)
          std::cout << "Build_Polygon_VertexFacet> triangle "
                    << facetList[iFacet][0] << " " << facetList[iFacet][1]
                    << " " << facetList[iFacet][2] << " " << (int)B.error()
                    << std::endl;
        B.end_facet();
        ...
        if (debugIo)
          std::cout << "Build_Polygon_VertexFacet> quad "
                    << facetList[iFacet][0] << " " << facetList[iFacet][1]
                    << " " << facetList[iFacet][2] << " "
                    << facetList[iFacet][3] << " " << (int)B.error()
                    << std::endl;
      }
#endif
    }
    ...
    if (debugIo) {
      std::cout << "Build_Polygon_VertexFacet> unconnected "
                << (int)B.check_unconnected_vertices() << std::endl;
      std::cout << "Build_Polygon_VertexFacet> error       " << (int)B.error()
                << std::endl;
    }
...
extern "C" int pyg4_cgal_polyhedron_print(void *polyhedronIn) {
  ...
  int ifacet = 0;
  for (Polyhedron::Facet_iterator f = polyhedron->facets_begin();
       f != polyhedron->facets_end(); ++f, ++ifacet) {
...
extern "C" int pyg4_cgal_surfacemesh_print(void *meshIn) {
  ...
  for (Surface_mesh::Face_index fd : mesh->faces()) {
    ...
    for (Surface_mesh::Halfedge_index hd :
         CGAL::halfedges_around_face(mesh->halfedge(fd), *mesh)) {
...
extern "C" void *pyg4_cgal_vertexfacet_to_polyhedron(int nVert, int nFacet,
                                                     double **vertList,
                                                     long int *nVertFacet,
                                                     long int **facetList,
                                                     bool debugIo) {
  ...
  if (debugIo)
    std::cout << "pyg4_cgal_vertexfacet_to_polyhedron> polyhedron ptr " << P
              << std::endl;
  Build_Polygon_VertexFacet<HalfedgeDS> pvf(nVert, nFacet, vertList, nVertFacet,
                                            facetList, debugIo);
  P->delegate(pvf);
  return (void *)(P);
}
...
extern "C" int pyg4_cgal_convexpolyhedron_to_planes(void *polyhedronIn,
  ...
  for (Polyhedron::Facet_iterator f = polyhedron->facets_begin();
       f != polyhedron->facets_end(); ++f, ++ifacet) {
    heh = f->halfedge();
    Polyhedron::Facet::Plane_3 plane = Polyhedron::Facet::Plane_3(
        heh->vertex()->point(), heh->next()->vertex()->point(),
    ...
    if (debugIo)
      std::cout << " pyg4_cgal_convexpolyhedron_to_planes> plane "
                << CGAL::to_double(pp.x()) << " " << CGAL::to_double(pp.y())
                << " " << CGAL::to_double(pp.z()) << " "
                << CGAL::to_double(pn.x()) << " " << CGAL::to_double(pn.y())
...
extern "C" void *pyg4_cgal_polyhedron_to_nefpolyhedron(void *polyhedronIn) {
  Polyhedron *polyhedron = (Polyhedron *)polyhedronIn;
  std::cout << "pyg4_cgal_polyhedron_to_nefpolyhedron> polyhedron ptr "
            << polyhedron << std::endl;
...
extern "C" void *
pyg4_cgal_nefpolyhedron_to_surfacemesh(void *nef_polyhedronIn) {
  Nef_polyhedron *nef_polyhedron = (Nef_polyhedron *)nef_polyhedronIn;
...
extern "C" int pyg4_cgal_nefpolyhedron_to_convexpolyhedra(void *nefIn,
  ...
  std::cout << "pyg4_cgal_nefpolyhedron_to_convexpolyhedra> nef ptr " << nefIn
            << std::endl;
...
extern "C" void *pyg4_cgal_surfacemesh_union(void *mesh1In, void *mesh2In) {
  ...
  Surface_mesh *out = new Surface_mesh();
  CGAL::Polygon_mesh_processing::corefine_and_compute_union(*mesh1, *mesh2,
                                                            *out);
  return (void *)(out);
}

extern "C" void *pyg4_cgal_vertex_to_polygon(double **verticesIn, int nVertices,
                                             bool debugIo = false) {
  Polygon_2 *polygon = new Polygon_2();
  if (debugIo) {
    std::cout << "pyg4_cgal_vertex_to_polygon> polygon ptr " << polygon
              << std::endl;
    std::cout << "pyg4_cgal_vertex_to_polygon> nVertices=" << nVertices
              << std::endl;
  }
  for (int i = 0; i < nVertices; ++i) {
    if (debugIo)
      std::cout << "pyg4_cgal_vertex_to_polygon> vertex " << verticesIn[i][0]
...
extern "C" int pyg4_cgal_polygon_to_vertex(void *polygonIn, double **vertices,
                                           int *nVerts) {
...
extern "C" int pyg4_cgal_polygon_to_convexpolygons(void *polygonIn,
  ...
  Polygon_2 *polygon = (Polygon_2 *)polygonIn;
  //  std::cout << "pyg4_cgal_polygon_to_convexpolygons> polygon ptr " <<
  //  polygon  << std::endl;
  ...
  for (auto p = partition_polys.begin(); p != partition_polys.end(); ++p) {
    //    std::cout << "pyg4_cgal_polygon_to_convexpolygons> " << i <<
    //    std::endl;

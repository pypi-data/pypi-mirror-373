...
from ._identifiers import AtType, Identifier, LayerId, SpecId, VariantIdMixin
...
@_to_edit_target.register(SpecId)
def _(at: SpecId, stage: Usd.Stage, obj: AtType) -> Usd.EditTarget:
    _layer: Sdf.Layer = at.layer_id.restore(stage)  # Hold the ref.
    spec: Sdf.Spec = at.restore(stage)
    if not spec:

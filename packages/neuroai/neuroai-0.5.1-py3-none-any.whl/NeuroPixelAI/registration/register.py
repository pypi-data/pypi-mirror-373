...
def compute_reference(frames, ops=reg_default_ops()):
...
def compute_reference_masks(refImg, ops=reg_default_ops()):
...
def shift_frames(frames, yoff, xoff, yoff1, xoff1, blocks=None, ops=reg_default_ops()):
...
def shift_frames_and_write(f_alt_in, f_alt_out=None, yoff=None, xoff=None, yoff1=None,
                           xoff1=None, ops=reg_default_ops()):
...
def registration_wrapper(f_reg, f_raw=None, f_reg_chan2=None, f_raw_chan2=None,
                         refImg=None, align_by_chan2=False, ops=reg_default_ops()):
...
def register_binary(ops: Dict[str, Any], refImg=None, raw=True):
    ...
    reg_file_align = ops['reg_file'] if (ops['nchannels'] < 2 or ops['functional_chan'] == ops['align_by_chan']) else \
    ops['reg_file_chan2']
    if raw:
        raw_file_align = ops.get('raw_file') if (
                    ops['nchannels'] < 2 or ops['functional_chan'] == ops['align_by_chan']) else ops.get(
            'raw_file_chan2')
    else:
        raw_file_align = None
    ...
    if ops['nchannels'] > 1:
        ...
        raw_file_alt = ops.get('raw_file_chan2') if ops['functional_chan'] == ops['align_by_chan'] else ops.get(
            'raw_file')

...
vec{n*2} = ti.types.vector({n*2}, ti.u32)
...
@ti.real_func 
def from_u16_to_u32(a: ti.types.vector({n*2}, ti.u32)) -> vec_{n+2}:
    ret = ti.Vector([0]*{n+2}, ti.u32)
...
@ti.real_func 
def mul_u16impl(a: ti.types.vector({n*2}, ti.u32),
                b: ti.types.vector({n*2}, ti.u32)) -> (vec{n*2}, vec{n*2}):
    result = ti.Vector([0]*{4*n}, ti.u32)
    hi = ti.Vector([0]*{n*2}, ti.u32)
    lo = ti.Vector([0]*{n*2}, ti.u32)
...
@ti.real_func 
def leading_zero_limbs_u16impl(a: ti.types.vector({2*n+2}, ti.u32)) -> ti.i32:
    ret = 0
    flag = True 
    ti.loop_config(serialize=True)
...
@ti.real_func 
def bit_shift_up_simple_u16impl(a: ti.types.vector({2*n+2}, ti.u32), shift: ti.i32) -> ti.types.vector({2*n+2}, ti.u32):
    result = ti.Vector([0]*{2*n+2}, ti.u32)
    high, low = ti.u32(0), ti.u32(0)
    
    ti.loop_config(serialize=True)
    for i in range({2*n+2}):
        low = ((a[i] << shift) & 0xffff) | high
        high = a[i] >> (16-shift)
        result[i] = low
    return result

@ti.real_func 
def bit_shift_down_simple_u16impl(a: ti.types.vector({2*n+2}, ti.u32), shift: ti.i32) -> ti.types.vector({2*n+2}, ti.u32):
    result = ti.Vector([0]*{2*n+2}, ti.u32)
    high, low = ti.u32(0), ti.u32(0)
    ...
    for j in range({2*n+2}):
        ...
        low = (a[i] >> shift) | high
        high = (a[i] << (16-shift)) & 0xffff 
        
        result[i] = low
    return result

@ti.real_func
def cmp_u16impl(a: ti.types.vector({2*n+2}, ti.u32), b: ti.types.vector({2*n+2}, ti.u32)) -> ti.i32:
    ret = 0
...
@ti.real_func 
def mul_u16impl_82(a: ti.types.vector({2*n+2}, ti.u32),
                   b: ti.types.vector({2*n+2}, ti.u32)) -> ti.types.vector({2*n+2}, ti.u32):
    result = ti.Vector([0]*{2*n+2}, ti.u32)
...
def sub_u16impl(a: ti.types.vector({2*n+2}, ti.u16), b: ti.types.vector({2*n+2}, ti.u16)) -> ti.types.vector({2*n+2}, ti.u16):
    ...
    for i in range({2*n+2}):
        ...
        if tmp < 0:
            carry = 1
            ret[i] = ti.u16(ti.u32(2**16+tmp)&0xffff)
        else:
            carry = 0 
            ret[i] = ti.u16(tmp&0xffff)
...
def divmod(a: ti.types.vector({n+2}, ti.u32), 
    ...
    if not zero_devision:
        ...
        q_retu32 = ti.Vector([ti.u32(0xffffffff)]*{n*2}, ti.u32)
        r_retu32 = ti.Vector([ti.u32(0xffffffff)]*{n*2}, ti.u32)

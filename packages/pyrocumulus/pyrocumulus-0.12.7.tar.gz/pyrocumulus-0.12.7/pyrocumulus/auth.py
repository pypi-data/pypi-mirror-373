...
class AccessToken(Document):
    ...
    async def get_by_token(cls, token):
        ...
        access_token = await cls.objects.get(token_id=token_id)
    ...
    async def get_perms(self, model):
    ...
    async def has_create_perm(self, model):
        perms = await self.get_perms(model)
        return 'c' in perms

    async def has_retrieve_perm(self, model):
        perms = await self.get_perms(model)
        return 'r' in perms

    async def has_update_perm(self, model):
        perms = await self.get_perms(model)
        return 'u'in perms

    async def has_delete_perm(self, model):
        perms = await self.get_perms(model)
        return 'd' in perms
...
class Permission(Document):
    ...
    @classmethod
    async def create_perms_to(cls, access_token, model, perms):

...
class BasePyroHandler(RequestHandler, metaclass=HandlerOperationMetaclass):
    ...
    async def validate_and_run(self, operation):
        ...
        if inspect.isawaitable(returned_obj):
            returned_obj = await returned_obj

        if returned_obj:
            await self.write_response(returned_obj)
...
class ModelHandler(BasePyroHandler):
    ...
    async def async_prepare(self):
        ...
        """
        self.query, self.body = await self._prepare_params()

    async def _prepare_params(self):
        """
        ...
        q = await converter.get_query()
        b = await converter.get_body()
        return q, b
...
class RestHandler(ModelHandler):
    ...
    async def async_prepare(self):
        ...
        self.query, self.body = await self._prepare_params()
        await super(RestHandler, self).async_prepare()

        self.parsed_model = get_parser(self.model)

    async def write_response(self, returned_obj):
        """
        ...
        if isinstance(returned_obj, str):
            ...
            to_return = await self._get_clean_dict(returned_obj)
    ...
    async def get_or_list(self):
        ...
        if self._query_has_pk():
            r = await self.get_object()
        else:
            r = await self.list_objects()
    ...
    async def list_objects(self):
        ...
        total_items = await objects.count()
    ...
    async def get_object(self, **kwargs):
        ...
        """
        obj = await self.model.objects.get(**self.query)
        return obj

    @decorators.post('')
    async def insert_object(self, **kwargs):
    ...
    async def put_object(self):
        ...
        updated = await self.model.objects(**query).update_one(
            **upsert_kw)
    ...
    async def delete_object(self, **kwargs):
        ...
        """
        obj = await self.get_object(**self.query)
        await obj.delete()
    ...
    async def _get_clean_dict(self, obj):
        """
        ...
        if (isinstance(obj, Document)
            ...
            mydict = await converter.to_dict()
            ...
            mylist = []
            for o in obj:
                clean_obj = await self._get_clean_dict(o)
...
class EmbeddedDocumentHandler(RestHandler):
    ...
    async def async_prepare(self):
        await super(EmbeddedDocumentHandler, self).async_prepare()
        self.parent_id = self._get_parent_id()
        del self.query['parent_id']
        self.parent = await self.parent_doc.objects.get(id=self.parent_id)

...
class BasicAPIKeyAuthMixin:
    ...
    async def check_perms(self, token):
        ...
        try:
            access_token = await AccessToken.get_by_token(token)
    ...
    async def async_prepare(self):
        ...
        if not self.request.method.lower() == 'options':
            ...
            self.access_token = await self.check_perms(self.token)

        r = await super().async_prepare()
        return r
...
class RestAPIAuthMixin(BasicAPIKeyAuthMixin):
    ...
    async def check_perms(self, token):
        ...
        access_token = await super().check_perms(token)
        ...
        perms = await access_token.get_perms(self.model)

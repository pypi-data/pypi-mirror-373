...
export class CheckpointRestorationModal {
  ...
  private createModal(checkpoint: ICheckpoint): void {
    ...
    const actionHistory = checkpoint.actionHistory;
    const allActions =
      AppStateService.getState().chatContainer?.chatWidget.conversationService?.actionHistory.getAllActions() ||
      [];

    const actionsToUndo = allActions.slice(actionHistory.length);

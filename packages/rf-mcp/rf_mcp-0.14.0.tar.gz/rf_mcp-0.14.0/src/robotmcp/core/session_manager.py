...
class SessionManager:
    ...
    def detect_session_type(self, keywords_used: List[str]) -> SessionType:
        ...
        if len(high_scores) > 1 and max_score > 1:  # Need at least some matches for mixed
            return SessionType.MIXED
        
        return best_type
    
    def get_session_profile(self, session_type: SessionType) -> Optional[SessionProfile]:
...
class ActiveSession:
    ...
    def _update_session_type(self) -> None:
        ...
        if new_type != old_type:
            logger.info(f"Session {self.session_id} type changed from {old_type.value} to {new_type.value}")

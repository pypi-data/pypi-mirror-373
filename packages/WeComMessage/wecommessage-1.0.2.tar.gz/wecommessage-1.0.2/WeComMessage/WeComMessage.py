...
class WeComMessageHandler:
    ...
    def _setup_logger(self) -> logging.Logger:
        """配置日志系统：生成固定名称的独立日志文件（不跟随调用脚本）"""
        # -------------------------- 关键修改：独立日志路径与名称 --------------------------
        # 1. 获取当前脚本的路径（用于定位独立日志目录，这里选当前脚本的上一层目录）
        current_script_path = os.path.abspath(__file__)  # 当前脚本（如 WeComMessageHandler 所在的 .py）
        current_script_dir = os.path.dirname(current_script_path)  # 当前脚本所在目录
        parent_dir = os.path.dirname(current_script_dir)  # 当前脚本的上一层目录（作为日志根目录）

        # 2. 定义独立日志目录（可选：放在上一层的 logs 文件夹，或你指定的路径）
        log_root_dir = os.path.join(parent_dir, "logs")  # 日志统一放在 "上一层/logs" 目录
        log_filename = "wecom_message.log"  # 固定的独立日志文件名（不随调用脚本变）
        ...
        if not os.path.exists(log_root_dir):
            try:
                os.makedirs(log_root_dir, exist_ok=True)  # 递归创建目录，已存在不报错
            except PermissionError:
                # 权限不足时， fallback 到用户主目录的 logs 文件夹
                log_root_dir = os.path.join(os.path.expanduser("~"), "WeComMessage_logs")
                log_filepath = os.path.join(log_root_dir, log_filename)
                os.makedirs(log_root_dir, exist_ok=True)  # 确保用户主目录的日志目录存在
                print(f"警告：无权限创建目标日志目录，使用默认目录：{log_root_dir}")
            except Exception as e:
        ...
        # 1. 控制台处理器（实时查看日志，无需编码）
        console_handler = logging.StreamHandler()
        console_handler.setFormatter(formatter)
        logger.addHandler(console_handler)

        # 2. 文件处理器（固定名称的独立日志文件，强制 utf-8 防中文乱码）
        if log_filepath:
            try:
                file_handler = logging.FileHandler(log_filepath, mode="a", encoding="utf-8")
                file_handler.setFormatter(formatter)
                logger.addHandler(file_handler)
                logger.info(f"独立日志系统初始化完成，日志文件：{log_filepath}")
                logger.info("中文日志测试：独立日志文件支持中文正常显示")
            except Exception as e:
                logger.warning(f"无法创建独立日志文件，仅输出控制台日志：{str(e)}")
        else:
            logger.info("独立日志系统初始化完成，仅输出控制台日志")

...
class WeComMessageHandler:
    ...
    def _setup_logger(self) -> logging.Logger:
        """配置日志系统：在当前执行脚本所在文件夹中，创建固定名称的独立日志文件"""
        # -------------------------- 核心：定位当前执行脚本所在文件夹 --------------------------
        # 1. 通过 inspect 获取「调用当前日志方法的执行脚本」信息（栈索引1：调用者是类的 __init__，索引2是用户的执行脚本）
        caller_frame = inspect.stack()[2]  # 关键：索引2指向「用户运行的当前执行脚本」
        current_exec_script_path = os.path.abspath(caller_frame.filename)  # 当前执行脚本的绝对路径
        current_exec_script_dir = os.path.dirname(current_exec_script_path)  # 当前执行脚本所在文件夹（日志要放这里）

        # 2. 定义独立日志文件（固定名称，与当前执行脚本同目录）
        log_filename = "wecom_message.log"  # 固定的独立日志文件名（不随脚本名变）
        log_filepath = os.path.join(current_exec_script_dir, log_filename)  # 日志路径 = 执行脚本目录 + 固定文件名

        # -------------------------- 保留目录创建与容错逻辑 --------------------------
        # 确保当前执行脚本所在文件夹存在（理论上必然存在，防极端情况）
        if not os.path.exists(current_exec_script_dir):
            try:
                os.makedirs(current_exec_script_dir, exist_ok=True)
            except PermissionError:
                # 权限不足时， fallback 到用户主目录
                log_root_dir = os.path.expanduser("~")
                log_filepath = os.path.join(log_root_dir, log_filename)
                print(f"警告：当前脚本目录无权限写入，日志改存到用户主目录：{log_root_dir}")
            except Exception as e:
                print(f"警告：无法访问当前脚本目录，仅输出控制台日志：{str(e)}")
                log_filepath = None
        ...
        logger.propagate = False  # 不传递给根日志器，避免重复输出

        # 清除旧处理器（防止多次初始化导致重复日志）
        if logger.handlers:
        ...
        # 2. 文件处理器（当前脚本目录的独立日志，强制 utf-8 防中文乱码）
        if log_filepath:
            try:
                file_handler = logging.FileHandler(log_filepath, mode="a", encoding="utf-8")
                file_handler.setFormatter(formatter)
                logger.addHandler(file_handler)
                logger.info(f"日志系统初始化完成，日志文件已创建在当前脚本目录：{log_filepath}")
                logger.info("中文日志测试：日志文件支持中文正常显示")
            except Exception as e:
                logger.warning(f"无法创建日志文件，仅输出控制台日志：{str(e)}")
        else:

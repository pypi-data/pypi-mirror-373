...
ExceptionHandler = Union[
    Callable[[EventContext, BaseException], bool],
    Callable[[EventContext, BaseException], Awaitable[bool]]
    ]

ExceptioHandlerAsync = Callable[[EventContext, BaseException], Awaitable[bool]]

def _log_exception(ctx: EventContext, exc: BaseException):
    ctx.logger.exception(
        f"Frame[{ctx.frame_name}]: "
        f"{type(exc).__name__}"
    )

async def _default_handler(ctx: EventContext, exc: BaseException) -> bool:
...
class ExceptionHandlerWrapper:
    ...
    async def __call__(self, ctx: EventContext, exc: BaseException) -> bool:
        try:

...
def convert_date_to_day_of_year_series(values: pd.Series) -> pd.Series:
    """Convert a date Series to day of year (1-366).
    ...
    Raises:
        TypeError: If values is not a pandas Series or not date/datetime type
    """
    if not isinstance(values, pd.Series):
        raise TypeError(f"Expected pandas Series, got {type(values)}")
    
    if not pd.api.types.is_datetime64_any_dtype(values.dtype):
        raise TypeError(f"Expected date/datetime Series, got {values.dtype}")
...
def convert_date_to_day_of_year(
    df: pd.DataFrame,
    *,
    column: str,
    ...
    """Convert date column to day of year.
    ...
    Raises:
        TypeError: If df is not a pandas DataFrame or column is not date/datetime type
    ...
    df[new_column] = convert_date_to_day_of_year_series(df[column])

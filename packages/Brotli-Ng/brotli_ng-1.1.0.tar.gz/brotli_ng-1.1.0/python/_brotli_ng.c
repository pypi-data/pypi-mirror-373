...
#if PY_MAJOR_VERSION >= 3
#define INIT_BROTLI   PyInit__brotli_ng
...
#define CREATE_BROTLI Py_InitModule3("_brotli_ng", brotli_methods, brotli_doc)
...
PyMODINIT_FUNC INIT_BROTLI(void) {
  ...
  BrotliError = PyErr_NewException((char*) "brotli_ng.error", NULL, NULL);
  if (BrotliError != NULL) {

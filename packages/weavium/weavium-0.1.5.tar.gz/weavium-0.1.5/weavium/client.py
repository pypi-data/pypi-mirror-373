...
class CompressionResult:
    ...
    result: Union[str, List[LLMMessage], None] 
...
class WeaviumClient:
    ...
    def compress(
        ...
        Args:
            input: Either a list of messages in the conversation (each message should have 'role' and 'content') 
                   or a single string to compress as user message.
            compression_rate: Target compression rate (0.0 to 1.0)
            system_prompt: Optional system prompt to include when input is a string
            
        Returns:
            CompressionResult with compressed messages and result matching input type.
            - If input was a string, 'result' field contains the compressed string
            - If input was a list, 'result' field contains the compressed messages list

...
class AgentClient(BaseAgentClient):  # Bybit client
    ...
    async def ad_create(
        ...
        amount = int(float(ad.quantity) * float(ad.price) * 10**ps.pair.cur.scale)
        df_unq = ad_upd.df_unq(
            maker_id=actor.id,
            pair_side_id=ps.id,
            amount=amount,
            quantity=int(float(ad.quantity) * 10**ps.pair.coin.scale),
            min_fiat=int(float(ad.minAmount) * 10**ps.pair.cur.scale),
            max_fiat=ad.maxAmount and int(float(ad.maxAmount) * 10**ps.pair.cur.scale),
            price=int(float(ad.price) * 10**ps.pair.cur.scale),
            premium=int(float(ad.premium) * 10**ps.pair.cur.scale),
        )

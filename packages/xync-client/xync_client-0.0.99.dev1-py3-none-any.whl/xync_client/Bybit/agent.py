...
class AgentClient(BaseAgentClient):  # Bybit client
    ...
    async def ad_create(
        ...
        df_unq = ad_upd.df_unq(
            ...
            min_fiat=int(float(ad.minAmount) * 10**cur_scale),
            max_fiat=ad.maxAmount and int(float(ad.maxAmount) * 10**cur_scale),
            price=int(float(ad.price) * 10**cur_scale),
            premium=int(float(ad.premium) * 10**cur_scale),
        )

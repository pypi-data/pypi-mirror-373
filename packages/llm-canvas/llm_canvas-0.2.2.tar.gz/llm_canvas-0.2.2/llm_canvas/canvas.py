...
class Canvas:
    ...
    def update_message(self, node_id: str, updated_message_node: MessageNode) -> MessageNode:
        ...
        Args:
            node_id: The ID of the message node to update
            updated_message_node: The new message content (will be merged with existing message)
        ...
        # Merge the updated node with the original node
        # Start with the original node and update only the provided fields
        updated_message_node["meta"] = original_node.get("meta", {}) | updated_message_node.get("meta", {})
        merged_node = original_node | updated_message_node
    ...
    def to_summary(self) -> CanvasSummary:
        ...
        root_ids = [node_id for node_id, node in self._nodes.items() if "parent_id" not in node]
        return {
            "canvas_id": self.canvas_id,
            "created_at": self.created_at,
            "root_ids": root_ids,

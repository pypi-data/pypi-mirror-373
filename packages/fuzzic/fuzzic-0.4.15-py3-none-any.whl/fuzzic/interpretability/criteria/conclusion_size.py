...
def interpretability(rulebase):
    ...
    result = fuzzy_logic_manager.criteria_aggregator(collection = [len(rules[key].conclusion) for key in rules.keys()],
                                direction = "min")
    dico = {"warning" : "", "score" : result}
    return dico

CRITERIA.append(criterion(name="conclusion size", category="fuzzy rule", direction="min",

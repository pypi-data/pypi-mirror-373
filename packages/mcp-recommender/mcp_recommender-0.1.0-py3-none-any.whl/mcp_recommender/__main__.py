#!/usr/bin/env python3
"""
MCP推荐器模块入口点 - asyncio冲突修复版本
解决 "Already running asyncio in this thread" 错误
"""
...
def server_mode():
    ...
    try:
        ...
        try:
            ...
            safe_print("检测到现有事件循环，使用stdio模式...")
            # 直接运行stdio服务器，不创建新的事件循环
            server.run_stdio()
        except RuntimeError:
...
def main():
    ...
    if args.debug:
        ...
        try:
            loop = asyncio.get_running_loop()
            safe_print(f"当前事件循环: {loop}")
            safe_print("事件循环正在运行")
        except RuntimeError:
            safe_print("没有运行的事件循环")

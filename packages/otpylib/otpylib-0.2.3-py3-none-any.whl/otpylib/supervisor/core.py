...
Includes automatic ping/pong health monitoring for GenServer children.
"""
    ...
    *,
    ...
    try:
        async with anyio.create_task_group() as tg:
            ...
            # Start health monitoring for children that need it
            for child_id in state.start_order:
                child = state.children[child_id]
                if child.spec.health_check_enabled and _is_genserver_child(child.spec):
...
def _is_genserver_child(spec: child_spec) -> bool:
    """Detect if a child spec is for a GenServer."""
    # Check if this looks like a GenServer start
    if len(spec.args) >= 3:
        # GenServer.start typically takes (callbacks, init_arg, name)
        return hasattr(spec.args[0], 'init') or 'gen_server' in str(spec.task)
    return False


async def _health_monitor(state: _SupervisorState, child_id: str, logger: logging.Logger):
    """Internal ping/pong health monitoring for GenServer children."""
    child = state.children[child_id]
    
    # Extract GenServer name from child args
    genserver_name = None
    if len(child.spec.args) >= 3:
        genserver_name = child.spec.args[2]  # Third arg is typically the name
    
    if not genserver_name:
        logger.warning(f"Cannot determine GenServer name for health monitoring: {child_id}")
        return
    
    logger.debug(f"Starting ping/pong health monitoring for GenServer: {genserver_name}")
    
    while not state.shutting_down:
        try:
            # Import gen_server here to avoid circular imports
            from otpylib import gen_server
            
            # Simple ping/pong health check
            response = await gen_server.call(
                genserver_name, 
                "ping",  # Simple ping message
                timeout=child.spec.health_check_timeout
            )
            
            # Check if we got expected pong response
            if response == "pong":
                # Health check succeeded
                child.health_check_failures = 0
                child.last_health_check = time.time()
                logger.debug(f"Health check passed for {genserver_name} (ping -> pong)")
            else:
                # Got unexpected response
                child.health_check_failures += 1
                logger.warning(f"Health check got unexpected response from {genserver_name}: {response}")
            ...
            logger.warning(f"Health check failed for {genserver_name} (failure #{child.health_check_failures}): {e}")
            
            # If health check fails repeatedly, consider restarting the child
            if child.health_check_failures >= 3:
                logger.error(f"GenServer {genserver_name} failed 3 health checks, triggering restart")
                # Cancel the child to trigger supervisor restart logic

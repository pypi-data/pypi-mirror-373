...
def test_set_param_existing_group():
    ap = ALCParams()
    ap.set_param(ap.alcm, "halt_loose_low_acl", 0.5)
    assert ap.alcm.halt_loose_low_acl == 0.5
...
def test_set_param_none_value():
    ap = ALCParams()
    ap.set_param(ap.alcm, "halt_loose_low_acl", None)
    # Should not change the value
    assert ap.alcm.halt_loose_low_acl == 0.0

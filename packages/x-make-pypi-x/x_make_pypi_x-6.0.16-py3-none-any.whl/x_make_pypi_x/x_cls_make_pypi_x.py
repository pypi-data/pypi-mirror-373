...
class x_cls_make_pypi_x:
    ...
    def publish(self, main_file: str, ancillary_files: list[str]) -> None:
        ...
        # Ensure pyproject is updated and write dev/config files in the project root
        ...
        # Build distributions (sdist + wheel) and upload using twine.
        dist_dir = os.path.join(project_dir, "dist")
        # Clean previous dist artifacts if present
        ...
        self._debug("Build command: %s" % (" ".join(build_cmd)))
        if getattr(self, "dry_run", False):
            self._essential("DRY-RUN: Skipping build command")
        else:
            try:
                proc = subprocess.run(
                    build_cmd, check=True, capture_output=not self.debug, text=True
                )
                if self.debug:
        ...
        # Upload using twine (honor dry_run)
        upload_cmd = [sys.executable, "-m", "twine", "upload", "dist/*"]
        self._essential(f"Uploading {self.name}=={self.version} to PyPI")
        self._debug("Upload command: %s" % (" ".join(upload_cmd)))
        if getattr(self, "dry_run", False):
            self._essential("DRY-RUN: Skipping upload")
        else:
            try:
                proc = subprocess.run(
                    upload_cmd, check=True, capture_output=not self.debug, text=True
                )
                if self.debug:
                    self._debug(proc.stdout or "", proc.stderr or "")
            except subprocess.CalledProcessError as e:
